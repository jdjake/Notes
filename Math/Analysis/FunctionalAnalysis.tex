...
On finite dimensional spaces $X$, there is essentially a unique way to define the size of a function. We say two norms $\| \cdot \|_1$ and $\| \cdot \|_2$ on a vector space $X$ are {\it comparable} if for each $x \in X$, $\| x \|_1 \sim \| x \|_2$.

\begin{theorem}
    If $X$ is a finite dimensional vector space, then all norms on $X$ are comparable.
\end{theorem}
\begin{proof}
    If $X$ is finite dimensional, with basis $\{ e_1,\dots,e_n \}$, then for any norm $\| \cdot \|$ on $X$, then by Cauchy Schwartz
    %
...
\begin{example}
    Let $X$ be a measure space. For each $1 \leq p < \infty$, let $\mathcal{L}^p(X)$ denote the space of all measurable functions $f: X \to \mathbf{K}$ such that
    %
...
\begin{example}
    The most important norm spaces for intuition are the sequence spaces $l^p$, defined for $1 \leq p < \infty$, and consisting of all sequences $a = \{ a_n : n \geq 1 \}$ such that the norm
    %
...
\end{theorem}
\begin{proof}
    We deal for $1 \leq p < \infty$, the case $p = \infty$ being left to the reader. Let $\{ f_k : k \geq 1 \}$ be a Cauchy sequence in $L^p(X)$. Then we can find a subsequence $\{ f_{k_i} : i \geq 1 \}$ such that for each $i$,
    %
...
\begin{example}
    ...
    %
    This norm makes $\text{Lip}_p(K)$ into a Banach space. To see this, consider a Cauchy sequence $\{ f_n : n \geq 1 \}$ in $\text{Lip}_p(K)$. Then the completeness of $L^\infty(K)$ shows there exists $f \in L^\infty(K)$ such that $f_n \to f$ in the $L^\infty$ norm, and it is not too difficult to argue from this that $f \in \text{Lip}_p(K)$, and thus $f_n \to f$ in the $\text{Lip}_p(K)$ norm.
...
\begin{example}
    ...
    %
    \[ H^p(\DD) = \left\{ f \in \mathcal{O}(\DD^\circ) : \left( \sup_{0 < r < 1} \int_{|z| = r} |f(w)|^p\; dw \right)^{1/p} < \infty \right\}, \]
    %
...
\begin{theorem}
    A sequence $\{ e_n \}$ in a Banach space $X$ is a Schauder basis if it's span is dense in $X$, and for any $m_1 \leq m_2$, and any scalars $\{ a_n \}$, there is $M > 0$ such that
    %
    \[ \| \sum_{n = 1}^{m_1} a_n e_n \| \leq M \| \sum_{n = 1}^{m_2} a_n e_n \|. \]
\end{theorem}
\begin{proof}
...
\begin{proof}
    ...
    %
    \[ g_\varepsilon(x) = \frac{1}{f(x)} \cdot \mathbf{I}_{S_\varepsilon^c}(x). \]
    %
