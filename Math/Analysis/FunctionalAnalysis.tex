...
The core here was a family of smooth functions. Given any generator $A$ of a semigroup on any Banach space, we can abstractly identify a family of \emph{smooth functions} for these purposes, i.e. the space $\mathcal{D}(A^\infty) = \bigcap_n \mathcal{D}(A^n)$. For any generator $A$, $\mathcal{D}(A^\infty)$ is a core for $A$, since it is invariant under $A$. To show $\mathcal{D}(A^\infty)$ is dense, we apply a mollification using integrals, i.e. for $x \in X$, considering
%
...
Iterating the bound above, one might expect that for $\text{Re}(\lambda) > \omega$, a good bound is that
%
...
\begin{theorem} [Hille-Yosida Theorem]
    If $A$ is an unbounded, densely defined operator, such that there exists a number $\omega \in \RR$ such that
    %
