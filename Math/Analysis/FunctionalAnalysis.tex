...
\end{corollary}
\begin{proof}
    The proof is immediate if $X$ is a real norm space. If $X$ is a norm space over the complex numbers, then we may view $X$ as a real norm space. If we let $\gamma = \Ree(\lambda)$, then $\gamma$ is a real linear functional with $\| \gamma \| = \| \lambda \|$. Thus the real case gives us an extension of $\gamma$ to $X$ preserving the operator norm. But we may then define
    %
...
%
\[ \Im (v,w) = \Ree(-i (v,w)) = \Ree (v,iw) = \langle v, iw \rangle \]
%
...
\begin{proof}
    ...
    %
    \[ \| \alpha w + v - x \|^2 = \| v - x \|^2 + 2 \alpha \Ree \langle w, v - x \rangle + |\alpha|^2 \| w \|^2  \]
    %
...
%
Thus $iT_2 = -iT_2$, which means $T_1 = T_2 = 0$. We denote $T$ by $\Ree(M)$ and $S = \text{Im}(M)$. An operator $M: H \to H$ is \emph{normal} if $M^* M = MM^*$, or equivalently, if $\Ree(M) \text{Im}(M) = \text{Im}(M) \Ree(M)$.
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= \langle Tx, x \rangle \pm 2 \Ree \langle Tx, y \rangle + \langle Ty, y \rangle
    \end{align*}
    %
    Then, subtracting, we find
    %
    \[ 4\Ree \langle Tx, y \rangle = \langle T(x+y),x+y \rangle - \langle T(x - y), x - y \rangle \]
    %
    Thus
    %
    \[ 4 \Ree \langle Tx, y \rangle \leq M(\|x + y\|^2 + \|x - y\|^2) = 2M(\|x\|^2 + \|y\|^2) \leq 4M \]
    %
...
\begin{theorem}
    ...
    \begin{itemize}
        ...
        \item $\sigma(T)$ is an at most countable, compact set whose only limit point is $0$.
        \item For any $\lambda \neq 0$, the eigenspace of $T$ and $T^*$ for $\lambda$ have the same dimension, and these spaces are both finite dimensional.
    \end{itemize}
\end{theorem}
\begin{proof}
...
We can also summarize some of our results in an equational form, commmonly known as the \emph{Fredholm alternative}.
...
\begin{proof}
    ...
    so $y \neq 0$. But
    %
    \[ \| Tx_i - \lambda x_i \|^2 = \| Tx_i \|^2 + \lambda^2 - 2 \lambda \Ree( \langle Tx_i, x_i \rangle ) \to 0. \]
    %
...
\begin{remark}
    We see from this result that the inductive limit of locally convex spaces has open sets highly localized around any particular set. One can see this quite explicitly if we look at the kinds of continuous seminorms that are defined on these spaces. Let us restrict ourselves to the LF space $C_c^\infty(\Omega)$. If $\{ U_i \}$ are a strictly increasing family of precompact open sets in $\Omega$ such that $\Omega = \bigcup U_i$, then for any increasing sequence $\{ \alpha_i \}$ and any increasing sequence $\{ k_i \}$ of positive integers, the norm
    %
...
\begin{lemma}
    ...
    \[ X_1 \to X_2 \to \dots, \]
    %
    where each $X_i$ is a locally convex $T1$ space, and such that each map
    %
    \[ f_i: X_i \to X_{i+1} \]
    %
    is injective. Then the direct limit $X$ is $T1$, and thus Hausdorff and Tychonoff.
\end{lemma}
\begin{proof}
    Consider $x_1,x_2 \in X$. We define an increasing family of open sets $\{ U_i \}$ in the sets $\{ X_i \}$ inductively. Suppose $i_0$ is the smallest index such that $x_1,x_2 \in X_i$. Define $U_i = \emptyset$ for $i < i_0$. Starting with $i = i_0$, define a convex set $U_i$, which is open in $X_i$, such that $U_{i-1} \subset U_i$, $x_1 \in U_i$, but $x_2$ is not an element of $\overline{U_i}$. To do this, we start by finding a convex, balanced open neighborhood of the origin $V$ in $X_i$ such that $\overline{x_2 + V} = x_2 + \overline{V}$ is disjoint from $\overline{U_{i-1}}$. Then $U_i = U_{i-1} + V$ is convex, open, contains $x_1$, and it's closure is disjoint from $x_2$. Then the set $\bigcup_i U_i$ is a convex, open neighborhood of $x_1$ in $X$, not containing $x_2$, so $X$ is $T1$.
\end{proof}
...
\begin{lemma}
    Let $E$ be a bounded subset of an inductive limit $X$ of an increasing sequence of locally convex spaces $\{ X_i \}$. Then $E \subset X_i$ for some $i$.
\end{lemma}
\begin{proof}
...
The last result is sufficient to show that every inductive limit of complete locally convex spaces is \emph{quasicomplete}, i.e. every bounded Cauchy net converges. An LF space is actually complete, though this requires some more work.
...
Since a map $T: X \to Y$ is continuous from an LF space if and only if $T_i: X_i \to Y$ is continuous for all $i$, and the domain of these maps are Fr\'{e}chet spaces, it follows that a linear map is continuous on an LF space if and only if it is bounded. Thus LF spaces are Bornological.
...
Let us now briefly dwell on duality. If $X$ is the inductive limit of a family of locally convex spaces $\{ X_\alpha \}$, then $X^*$ is canonically \emph{algebraically} isomorphic to the algebraic projective limit of the spaces $\{ X_\alpha^* \}$. Indeed, by the universal property of the inductive limit, any continuous function $\phi$ on $X$ is induced by a compatible family of continuous functionals $\phi_\alpha$ on $X_\alpha$. Similarily, if $X$ is the projective limit of a family of spaces $\{ X_\alpha \}$ then $X^*$ is algebraically isomorphic to the inductive limit of the spaces $\{ X_\alpha \}$.

If $X$ is an inductive limit of a sequence $\{ X_\alpha \}$, then the map $X^* \to (\lim (X_\alpha^*)_b)^*$ is continuous. But the reverse need not be. However, if $X$ is the \emph{strict} inductive limit of a sequence of locally convex spaces $\{ X_i \}$, i.e. so that $X_i$ is embedded as a closed subspace of $X$ for each $i$, one can obtain such a result.

\begin{lemma}
    Let $X$ be the strict inductive limit of a sequence of locally convex spaces $\{ X_i \}$. Then the canonical identification of $X^*_b$ with $\lim (X_i^*)_b$ is an isomorphism.
\end{lemma}
\begin{proof}
    The map $\lim (X_i^*)_b \to X^*$ is certainly continuous, since if a net $\{ \phi^\alpha \}$ converges to $\phi$ in $X^*_b$, then $\phi^\alpha$ converges uniformly to $\phi$ on any bounded set $B \subset X$, which immediately implies $\phi^\alpha_i$ converges uniformly to $\phi$ on any bounded set $B \subset X_i$, since this same $B$ is a bounded subset of $X$. Conversely, any bounded subset of $X$ lies in $X_i$ for some $B$, which implies uniform convergence on bounded subsets of $X$ is equivalent to uniform convergence on the union of bounded subsets of $X_i$, for each $i$.
\end{proof}

\begin{example}
    Let $X$ be a $\sigma$ compact, locally compact space, and consider a sheaf of locally convex spaces $\{ F(U) \}$. Then $F_c(X)^*$ can algebraically be identified with $F^*_{\text{loc}}(X)$, and $F_{\text{loc}}(X)^*$ can naturally be identified with $F^*_c(X)$. In particular, if $X$ is equipped with a Radon measure, then for $1 \leq p < \infty$, $L^p_c(X)$ can naturally be identified with $L^{p^*}_{\text{loc}}(X)$, and $L^p_{\text{loc}}(X)$ can naturally be identified with $L^{p^*}_c(X)$.
\end{example}
...
\begin{example}
    A convex set is norm closed if and only if it is weakly closed, since a linear functional is weakly continuous if and only if it is norm continuous. This is no longer true for the weak $*$ topology on a non semireflexive space $X$, since this means precisely that there is a linear functional on $X^*$ which is continuous on $X^*_b$, but not on $X^*_{\sigma^*}$. The kernel of this functional is a convex set, closed in $X^*_b$, but not closed in $X^*_{\sigma^*}$.
\end{example}
...
\begin{proof}
    ...
    %
    \[ \Ree \{ x^*(x_0) \} < \inf_{x \in B} \Ree \{ x^*(x) \} = - \sup_{x \in B} |x^*(x)|. \]
    %
...
\begin{example}
    ...
    %
    \[ \| x_n - x \|_H^2 = \| x_n \|^2_H + \| x \|^2_H - 2 \Ree( (x_n \cdot x) ). \]
    %
...
Suppose we wish to solve the equation $(1 - M) R = 1$ for $R$. Expanding out, we have $R = 1 + MR$. Substituting the equation into itself on the right hand side, we have $R = 1 + M(1 + MR) = 1 + M + M^2 R$. Iterating this process, we have
%
\[ R = 1 + M + M^2 + \cdots + M^n R. \]
%
Provided that $M^n \to 0$ suitably rapidly, we have
%
\[ R = \sum_{n = 0}^\infty M^n, \]
%
an extension of the power series formula $(1 - z)^{-1} = \sum_{k = 0}^\infty z^k$.
...
In algebra, a semigroup is a set equipped with an associative operation. In analysis, when we talk about semigroups, we are normally talking about \emph{one parameter semigroups}, i.e. semigroups of linear operators isomorphic to $[0,\infty)$. More precisely, a \emph{one parameter semigroup} on a vector space $X$ is a family of linear maps $S(t): X \to X$ for each $t \geq 0$, such that $S(0)$ is the identity map, and $S(t_1) \circ S(t_2) = S(t_1 + t_2)$ for any $t_1,t_2 \geq 0$. One can also consider non-linear semigroups on a set $A$, i.e. a family of maps $\{ S(t) \}$ on $A$ such that $S(t) \circ S(s) = S(t + s)$, but often it is to consider the resulting linear semigroup on the vector space $X$ of \emph{functions} on $A$ by the resulting linear pushforward operators induced by the nonlinear semigroup.

If $X$ is a Banach space, then a function $S: [0,\infty) \to B(X)$ is called \emph{strongly continuous} if it is continuous from $[0,\infty)$ to the strong operator topology on $B(X)$. A \emph{strongly continuous} semigroup is a strongly continuous homomorphism $S: [0,\infty) \to B(X)$, and a \emph{strongly continuous group} is a strongly continuous homomorphism $S: (-\infty,\infty) \to B(X)$. This distinction only really appears for infinite dimensional Banach space, since, given a differentiable homomorphism $S: [0,\infty)$, if we let $A = S'(0)$, then setting $S(t) = e^{tA}$ for \emph{all} $t \in \RR$ gives an extension of the semigroup.

We note that a function $F: [0,\infty) \to B(X)$ is continuous in the strong operator topology of $B(X)$ if and only if for any $R > 0$, the family of operators $\{ F(t) : 0 \leq t \leq R \}$ is uniformly bounded, and for a dense subset $D \subset X$, and any $x \in D$, the map $t \mapsto F(t) x$ is continuous from $[0,R]$ to $X$. Indeed, the former condition certainly implies the latter. And conversely, if the latter condition holds, then any sequence $\{ t_n \}$ in $[0,\infty)$ converging to some $t \in [0,\infty)$ is contained in $[0,R]$ for some $R$. It thus follows that the set $\{ F(t_n) \}$ is uniformly bounded, and the continuity condition on $[0,R]$ implies that for all $x \in D$, $F(t_n)x \to F(t) x$. But then uniform boundedness and a density argument implies that the same is true for all $x \in X$, so that $F(t_n) \to F(t)$ in the strong operator topology.

\begin{theorem}
    Let $\{ S(t) \}$ be a semigroup on a Banach space $X$. Then the following are equivalent:
    %
    \begin{itemize}
        \item $\{ S(t) \}$ is a strongly continuous semigroup.

        \item For all $x \in X$,
        %
        \[ \lim_{t \to 0^+} T(t) x = x. \]

        \item There exists $\delta > 0$ and $M \geq 1$, and a dense set $D \subset X$ such that $\| T(t) \| \leq M$ for all $t \in [0,\delta]$, and
        %
        \[ \lim_{t \to 0^+} T(t) x = x \]
        %
        for all $x \in D$.
...
\end{theorem}
\begin{proof}
    The first condition easily implies the second and third conditions. Conversely, the techniques involved in the last paragraph show how the third condition implies the second. Finally, if the second condition holds, then for any $t_0 \in (0,\infty)$, as $h \to 0^+$, we have $S(t+h) = S(t) S(h)$ converging to $S(t) S(0)$ in the strong operator topology, since composition is continuous here since the left hand operator is fixed. This implies that for any $R > 0$, the family of operators $\{ S(t) : t \in [0,R] \}$ is uniformly boudned. Otherwise, we can find a decreasing sequence $\{ t_n \}$ such that $\sup_n \| S(t_n) \| = \infty$, which is impossible by the uniform boundedness theorem since, if $t_n \to t$, then $S(t_n)$ converges in the strong operator topology to $S(t)$. Now as $h \to 0^-$, we have $S(t + h) - S(t) = S(t + h) [ 1 - S(-h) ]$, and this converges in the strong operator topology to $0$, since $\{ S(t + h) : h \in [-t,0] \}$ is uniformly bounded.
\end{proof}

Now iterating the bound from the third condition in the theorem, if $\{ S(t) \}$ is a strongly continuous semigroup, then there exists $\omega > 0$, and $M \geq 1$, such that for all $t \geq 0$,
%
\[ \| S(t) \| \leq M e^{\omega t}. \]
%
If we can choose $\omega \leq 0$, then the semigroup is called \emph{bounded}, and if in addition, $M = 1$, we say the semigroup is \emph{contractive}.

Let us set $\omega_0$ to be the infinum of the choices of $\omega$ for which a semigroup $\{ S(t) \}$ has a bound of the form above. Then a bound of the form
%
\[ \| S(t) \| \lesssim e^{\omega_0 t} \]
%
might \emph{not} exist, since the constant $M$, which depends on $\omega$, might explode as $\omega$ converges to $\omega_0$. And we might have to take $M > 1$ regardless of what value of $\omega$ we pick.

\begin{theorem}
    A semigroup $\{ T(t) \}$ on a Banach space $X$ is strongly continuous if and only if it is weakly continuous.
\end{theorem}
\begin{proof}
    Suppose a semigroup $\{ T(t) \}$ is weakly continuous. TODO
\end{proof}

Operator semigroups tend to arise in the study of the abstract Cauchy problem $\partial_t u(t) = Au(t)$, for a densely defined, unbounded linear operator $A$ on a Banach space $X$, subject to the constraint that $u(0) = x_0$ for some $x_0 \in X$. If solutions to the abstract Cauchy problem exist and are unique, we can define a family of operators $\{ S(t) \}$ which map $u(0)$ to $u(t)$ for any solution $u$ to the abstract Cauchy problem.

A natural question, therefore, is that given a linear operator $A$, when can one find a semigroup which solves the abstract Cauchy problem, and conversely, when does a semigroup $\{ S(t) \}$ correspond to a solution operator for some unbounded operator $A$. In finite dimensions, this is easy; just take the exponential of the operator $A$. That is, for a given $A$, we define $S(t) = e^{tA}$. Conversely, if $\{ S(t) \}$ is \emph{differentiable} semigroup, then setting $A = S'(0)$ gives an operator $A$ with $S(t) = e^{tA}$.

As a first try, we might do the obvious and apply the holomorphic functional calculus. Given a bounded operator $A$ on a Banach space $X$, we can define a uniformly strongly continous group
%
\[ S(t) = e^{tA} = \sum_{k = 0}^\infty \frac{t^k A^k}{k!} \]
%
defined for all $t \in \RR$. Conversely, any \emph{uniformly continuous} semigroup $\{ S(t) \}$ is differentiable in $t$, and $A = S'(0)$ (TODO).

However, there are many more general semigroups induced by \emph{unbounded operators}, especially emerging from the solutions to differential equations. So the preceding paragraph does not give a general enough theory for our purposes. As an example of this, consider the translation operators $\{ T_t \}$ on $L^p(\RR)$ given by \emph{translation}, i.e. $T_t f(x) = f(x + t)$. Then this semigroup is strongly continuous. If $f$ is, for instance, smooth with compact support, then the map $t \mapsto T_t f$ is \emph{differentiable} as a map into $L^p(\RR)$, and the derivative at the origin is precisely $f'$. We shall find that one can obtain such an unbounded operator from any strongly continuous semigroup.

\begin{lemma}
    Let $\{ T_t \}$ be a strongly continuous semigroup on a Banach space $X$, and let $x \in X$. Then the following are equivalent:
    %
    \begin{itemize}
        \item The map $t \mapsto T_t x$ is differentiable for all $t \geq 0$.

        \item The map $t \mapsto T_t x$ is right differentiable at $t = 0$.
    \end{itemize}
\end{lemma}
\begin{proof}
    Certainly if $t \mapsto T_t x$ is right differentiable at $t = 0$, with derivative $x'$, then the function is right differentiable at any time $t \geq 0$, simply by using the semigroup property, and this right derivative at a time $t$ is equal to $T_t x'$. To get left differentiability, fix $t_0$, and consider $0 < h < t_0$. Then we claim that
    %
    \[ \lim_{h \to 0} \frac{T_{t-h} x - T_t x}{h} = T_t x'. \]
    %
    We can write
    %
    \[ \frac{T_{t-h} x - T_t x}{h} = T_{t - h} \left( \frac{x - T_h x}{h} - x' \right) + T_{t+h} x' - T_t x', \]
    %
    and since the operators $\{ T_t : t \in K \}$ are uniformly bounded for any compact set $K$, taking $h \to 0$ yields the required result.
\end{proof}

Given a continuous semigroup $\{ T_t \}$ on a Banach space $X$, we therefore define the \emph{generator} of the group is the unbounded operator $A$, whose domain $\mathcal{D}(A)$ is precisely the set of all $x \in X$ such that $t \mapsto T_t x$ is differentiable, and the derivative at the origin is then precisely $Ax$. The domain of $A$ is then an invariant subspace of all of the operators $\{ T_t \}$, and we find that $T_t A = A T_t$.

\begin{lemma}
    For all $t \geq 0$, and $x \in X$,
    %
    \[ \int_0^t T_s x\; ds \in \mathcal{D}(A), \]
    %
    and moreover,
    %
    \[ A \int_0^t T_s x\; ds = T_t x - x. \]
    %
    Moreover if $x \in \mathcal{D}(A)$, then we have
    %
    \[ A \int_0^t T_s x\; ds = \int_0^t T_s Ax\; ds. \]
\end{lemma}
\begin{proof}
    To check the integral is in the domain, we have to check that
    %
    \[ \lim_{h \to 0^+} \frac{T_h \int_0^t T_s\; ds - \int_0^t T_s x\; ds}{h} \]
    %
    is well defined. Now by continuity, we can calculate explicitly
    %
    \begin{align*}
        \frac{T_h \int_0^t T_s x\; ds - \int_0^t T_s x\; ds}{h} &= \frac{1}{h} \int_0^t T_{s+h} x\; ds - \frac{1}{h} \int_0^t T_s x\; ds\\
        &= \frac{1}{h} \left( \int_t^{t+h} T_s x\; ds - \int_0^h T_s x\; ds \right)\\
        &\to T_t x - x.
    \end{align*}
    %
    This gives the required result. Now if $x \in \mathcal{D}(A)$, TODO.
\end{proof}

We therefore have a way to \emph{smooth} elements of $X$, to get elements of $\mathcal{D}(A)$.

\begin{theorem}
    The generator $A$ is closed and densely defined, and no two semigroups share the same generator.
\end{theorem}
\begin{proof}
    By strong continuity of the semigroup, the elements $h^{-1} \int_0^h T_t x\; dt$, which are elements of $\mathcal{D}(A)$, converge to $x$ for any $x \in X$. Thus $A$ is densely defined.

    To verify closure, suppose that $\{ x_n \}$ are in $\mathcal{D}(A)$, and converge to some $x \in X$, and also, $\{ Ax_n \}$ converge to some $y \in X$. Then
    %
    \[ \frac{T_h x_n - x_n}{h} = \frac{1}{h} \int_0^h T_s Ax_n\; ds, \]
    %
    and as $n \to \infty$, the right hand side converges to
    %
    \[ \frac{1}{h} \int_0^h T_s y\; ds. \]
    %
    The left hand side converges to $h^{-1}(T_h x - x)$. Thus we conclude that
    %
    \[ \frac{T_h x - x}{h} = \frac{1}{h} \int_0^h T_s y\; ds. \]
    %
    Taking $h \to 0$ gives differentiability, and that $Ax = y$. Thus $A$ is closed.

    Suppose that $\{ T_t \}$ and $\{ T_t' \}$ are two strongly continuous semigroups with the same generator $A$. Set $S_t = T_t - T_t'$. Then for a dense subset of $X$, i.e. for $x \in \mathcal{D}(A)$, the map $t \mapsto S_t x$ is differentiable, and has derivative zero. Sicne $S_0 x = 0$, we conclude that $S_t x = 0$ for all $t$. By boundedness, we conclude $S_t = 0$ for all $t$, which yields the required uniqueness statement.
\end{proof}

We can read off many properties of the semigroup $\{ T_t \}$ from it's generator $A$.
...
\begin{theorem}
    ...
    \begin{itemize}
        ...
        \item $\mathcal{D}(A) = X$.
        \item $\mathcal{D}(A)$ is closed in $X$.
        \item $\{ T_t \}$ is uniformly continuous.
    \end{itemize}
\end{theorem}
\begin{proof}
    The first three properties are easily verified to be equivalent by the closed graph theorem. If these three properties hold, then for any $x \in X$, the map $f(t) = T_t x$ is differentiable, and we have $f'(t) = T_t A x$. Pick $M$ and $\omega$ such that
    %
    \[ \| T_t x \| \leq M e^{\omega t} \| x \|, \]
    %
    ...
    %
    \[ f(t) = x + \int_0^t f'(s)\; ds. \]
    %
    Thus TODO
    %
%    \[ \| T_t x \| \leq \| x \| + C \left( \int_0^t M e^{\omega s}\; ds \right) \| x \| \leq \left( 1 + (CM / \omega) ( e^{\omega t} - 1 ) \right) \| x \|_X \]

%    \[ A \int_0^t T_s x\; ds = \int_0^t T_s Ax\; ds. \]
\end{proof}

It is often hard to determine the domain of a generator of a semigroup. But we have tools that mean we do not necessarily have to work with the entire domain. Let $A$ be a generator of a semigroup $\{ T_t \}$. A set $\mathcal{D} \subset \mathcal{D}(A)$ is called a \emph{core} for $A$ if the graph of $A$ restricted to $\mathcal{D}$ is dense in the graph of $A$ on $\mathcal{D}(A)$, i.e. if $\mathcal{D}$ is dense in $\mathcal{D}(A)$ with respect to the norm $\| \cdot \|_A$ on $\mathcal{D}(A)$ given by
%
\[ \| x \|_A = \| x \|_X + \| Ax \|_X. \]
%
The next Lemma allows us to easily identify cores.

\begin{lemma}
    If $\mathcal{D}$ is a dense subspace of $X$ contained in $\mathcal{D}(A)$, and is an invariant subspace of $\{ T_t \}$, then $\mathcal{D}$ is a core of $A$.
\end{lemma}
\begin{proof}
    Fix $x \in \mathcal{D}(A)$. Find a sequence $\{ x_n \}$ in $\mathcal{D}$ converging to $x$. For each $t \geq 0$, the element
    %
    \[ x_n(t) = \frac{1}{t} \int_0^t T_s x_n\; ds \]
    %
    lies in the closure of $\mathcal{D}$ with respect to the graph norm. Indeed, for each $n$, the function
    %
    \[ f_n(s) = T_s x_n \]
    %
    is continuous, so that the integral above can be approximated by a Riemann sum, and each Riemann sum lies in $\mathcal{D}$. Moreover, the derivative
    %
    \[ f_n'(s) = A T_s x_n = T_s A x_n \]
    %
    is continuous, so that the term
    %
    \[ \frac{A}{t} \int_0^t T_s x_n = \frac{1}{t} \int_0^t T_s A x_n\; ds \]
    %
    is approximated by the same Riemann sum. This verifies that the term lies in the closure of $\mathcal{D}$ with respect to the graph norm.
\end{proof}

As an example, in the semigroup $\{ T_t \}$ on $L^1(\RR)$ given by translation, the functions $C_c^\infty(\RR)$ form a dense subset of $L^1(\RR)$, and are contained in the domain of the generator $A$ of $\{ T_t \}$. Moreover, $\{ T_t \}$ maps $C_c^\infty(\RR)$ to itself. So the result above implies $C_c^\infty(\RR)$ is a core of the domain of the generator of $A$.

The core here was a family of smooth functions. Given any generator $A$ of a semigroup on any Banach space, we can abstractly identify a core consisting of abstract \emph{`smooth functions'} purposes, i.e. the space
%
\[ \mathcal{D}(A^\infty) = \bigcap_n \mathcal{D}(A^n). \]
%
For any generator $A$, $\mathcal{D}(A^\infty)$ is a core for $A$, since it is invariant under $A$. To show $\mathcal{D}(A^\infty)$ is dense, we apply a mollification using integrals, i.e. for $x \in X$, considering
%
\[ x_\phi = \int_0^\infty \phi(s) T_s x\; ds. \]
%
To verify that $x_\phi \in \mathcal{D}(A)$, we write
%
\begin{align*}
    \frac{T_h - I}{h} x_\phi &= \frac{1}{h} \int_0^\infty \phi(s) ( T_{s + h} - T_s ) x\; ds\\
    &= \frac{1}{h} \int_0^\infty ( \phi(s - h) - \phi(s)  ) T_s x\; ds.
\end{align*}
%
We can write $\phi(s - h) - \phi(s) = h \eta(s,h)$, where $\eta$ is smooth, compactly supported in $s$ uniformly for any bounded subset of values of $h$, and $\eta(s,0) = - \phi'(s)$. But this is sufficient to justify that
%
\[ \frac{T_h - I}{h} x_\phi = \int_0^\infty \eta(s,h) T_s x\; ds \to - \int_0^\infty \phi'(s) T_s x\; ds. \]
%
Thus $x_\phi \in \mathcal{D}(A)$, and $A x_\phi$ is given by an integral of the same form as $x_\phi$ was given. But this mean by induction that $x_\phi \in \mathcal{D}(A^\infty)$. We now use Hahn-Banach to show that $\mathcal{D}$ is dense. If $\lambda \in X^*$ vanishes on $\mathcal{D}(A^\infty)$, then by linearity, for any smooth compactly supported $\phi$, and any $x \in X$, we have
%
\[ \int_0^\infty \phi(s) \lambda(T_s x)\; ds = \lambda (x_\phi) = 0. \]
%
But this is only possible for all $\lambda$ if $\lambda(T_s x) = 0$ for all $s$. But then setting $s = 0$ gives that $\lambda(x) = 0$, and so $\lambda = 0$. This is sufficient to conclude that $\mathcal{D}(A^\infty)$ is dense.

We now want to study the \emph{resolvent} of $A$ in terms of the semigroup $\{ T_t \}$. Given that, heuristically, we think of $\{ T_t \}$ as being identified with the exponential $\{ e^{tA} \}$, the identity
%
\[ (\lambda - z)^{-1} = \int_0^\infty e^{-\lambda s} e^{s z}\; ds \]
%
suggests that we have an equation for the resolvant of $A$ of the form
%
\[ R(\lambda, T) = \int_0^\infty e^{-\lambda s} T_s\; ds. \]
%
Indeed, we calculate that for any fixed $t$, and any $x \in \mathcal{D}(A)$, the function $f(x) = e^{-\lambda t} T_t x$ is differentiable, with derivative $f'(x) = (A - \lambda) f(x)$, which leads us to the formula
%
\[ e^{-\lambda t} T_t x - x = (A - \lambda) \int_0^\infty e^{-\lambda s} T_s x\; ds. \]
%
Suppose that $M$ and $\omega$ exist such that $\| T_t \| \leq M e^{\omega t}$. If $\text{Re}(\lambda) > \omega$, then we can take $t \to \infty$ to conclude that
%
\[ x = (\lambda - A) \int_0^\infty e^{-\lambda s} T_s x\; ds. \]
% Ask Brian if this formula is true for all x in X.
Thus we have a formula for the resolvant for $x \in \mathcal{D}(A)$. Summarizing, we have proved most of the following result.

\begin{theorem}
    Suppose that $\| T_t \| \leq M e^{\omega t}$ for $t \geq 0$. Then:
    %
    \begin{itemize}
        \item If $\lambda \in \CC$, and the integral
        %
        \[ \int_0^\infty e^{-\lambda s} T_s x\; ds \]
        %
        exists for all $x \in X$, then $\lambda \in \rho(A)$, and
        %
        \[ R(\lambda, A) = \int_0^\infty e^{-\lambda s} T_s x\; ds. \]

        \item If $\text{Re}(\lambda) > \omega$, then $\lambda \in \rho(A)$, and
        %
        \[ R(\lambda, A) x = \int_0^\infty e^{-\lambda s} T_s\; ds. \]

        \item For $\text{Re}(\lambda) > \omega$,
        %
        \[ \| R(\lambda,A) \| \leq \frac{M}{\text{Re}(\lambda) - \omega}. \]
    \end{itemize}
\end{theorem}
\begin{proof}
    TODO: Justify.
...
\begin{remark}
    We have proved here a useful property separating generators of semigroups from general unbounded operators. In particular, we see here that the generator of a semigroup \emph{must} have it's spectrum contained in a half plane.
\end{remark}

Iteration of the bound above leads us to obtain a bound of the form
%
\[ \| R(\lambda,A)^n \| \leq \left( \frac{M}{\text{Re}(\lambda) - \omega} \right)^n. \]
%
We can do better. To see this, for a general unbounded operator $A$, if $\lambda \in \rho(A)$, then the function $\lambda \mapsto R(\lambda,A)$ is analytic in a neighborhood of $\lambda$, and we have
%
\[ R(\lambda,A)^n = \frac{(-1)^{n-1}}{(n-1)!} \left( \frac{d}{d\lambda} \right)^n \{ R(\lambda,A) \}. \]
%
But since we have an integral formula for $R(\lambda,A)$, we actually can write
%
\[ R(\lambda,A)^n = \frac{1}{(n-1)!} \int_0^\infty s^{n-1} e^{-\lambda s} T_s x\; ds. \]
%
In particular, we get the better operator norm bound
%
\begin{align*}
    \| R(\lambda,A)^n \| &\leq \frac{1}{(n-1)!} \int_0^\infty s^{n-1} e^{- \text{Re}(s)} M e^{\omega s}\; ds\\
    &\leq \frac{M}{(\text{Re}(\lambda) - \omega)^n}.
\end{align*}
%
Thus, for a fixed $\lambda$, the operators $\| R(\lambda,A)^n \|$ are actually \emph{uniformly bounded} in $n$. This is another property that characterizes generators of semigroups. In fact, this is essentially a \emph{characterization} of those unbounded operators that induce semigroups of operators.

To see this, we have to essentially remind ourselves of the different ways we can represent the exponential function, since each gives insight into the way we can generate a semigroup from it's generator:
%
\begin{itemize}
    \item If $A$ is \emph{bounded}, we can define
    %
    \[ T_t = \sum_{n = 0}^\infty \frac{(tA)^n}{n!} \]

    \item For any holomorphic function of an operator, a good strategy is often ot use the Cauchy integral formula, e.g. writing
    %
    \[ T_t = \frac{1}{2 \pi i} \int_{\gamma} e^{\lambda t} R(\lambda,A)\; d\lambda, \]
    %
    where $\gamma$ is a closed loop around $\sigma(A)$. The problem of this approach is that to apply this formula, we often need more information about the distribution of $\sigma(A)$.

    \item The formula
    %
    \[ e^{tA} = \lim_{n \to \infty} \left( 1 + \frac{tA}{n} \right)^n \]
    %
    is pretty bad, because high powers of unbounded operators behave terribly. Hille's idea was to instead consider the slight modification
    %
    \[ e^{tA} = \lim_{n \to \infty} (1 - tA/n)^{-n} = \lim_{n \to \infty} (n/t)^n R(n/t, A)^n. \]
    %
    High powers of a resolvent are \emph{not} necessarily bad.

    \item Yosida's method is to approximate $A$ by a family of bounded operators $\{ A_n \}$, in an appropriate sense. Then hopefully $e^{tA}$ can be approximated by $e^{tA_n}$.
\end{itemize}
%
Applying either Hille or Yosida's method will yield the following theorem. The next lemma indicates a possible choice for the operators $\{ A_n \}$.

\begin{lemma}
    Let $A$ be a closed, densely defined operator on a Banach space $X$, and suppose there is $\omega \in \RR$ and $M > 0$ such that
    %
    \[ [\omega, \infty) \subset \rho(A) \]
    %
    and such that $\| R(\lambda,A) \| \leq M / \lambda$ for all $\lambda \geq \omega$. Then for all $x \in X$,
    %
    \[ \lim_{x \to \infty} \lambda R(\lambda, A) x = x. \]
\end{lemma}
\begin{proof}
    We note that
    %
    \[ \lambda R(\lambda,A) = 1 + A R(\lambda,A), \]
    %
    and so it suffices to show that for any $x \in X$, as $\lambda \to \infty$, $A R(\lambda,A) x \to 0$. We have a uniform bound
    %
    \[ \| A R(\lambda,A) \| = \| \lambda R(\lambda,A) - 1 \| \leq 1 + M \]
    %
    by assumption, so it suffices to prove this result on a dense subclass of $X$. For $x \in \mathcal{D}(A)$, we have
    %
    \[ A R(\lambda,A) = R(\lambda,A) A, \]
    %
    and so it follows that
    %
    \[ \| A R(\lambda,A) x \| = \| R(\lambda, A) A x \| \leq \frac{M}{\lambda} \| A x \|, \]
    %
    which converges to zero as $\lambda \to \infty$.
\end{proof}

It follows that on $\mathcal{D}(A)$, the bounded operators $A_\lambda = \lambda A R(\lambda,A)$ converge to $A$ in the strong operator topology as $\lambda \to \infty$. Let us use Yosida's method to prove the Hille-Yosida theorem for contaction semigroups. Recall a semigroup $\{ T_t \}$ is a contraction semigroup if it satisfies bounds of the form $\| T_t \| \leq 1$ for all $t$.

\begin{theorem}
    Let $A$ be an unbounded, densely defined closed operator on a Banach space $X$. Then $A$ generates a strongly continuous contraction semigroup if and only if for all $\lambda > 0$, $\lambda \in \rho(A)$, and $\| \lambda R(\lambda,A) \| \leq 1$.
\end{theorem}
\begin{proof}
    We have seen the necessity of the resolvent estimates already, which gives one side of the result. So assume the resolvent estimates. Define $A_\lambda = \lambda A R(\lambda,A)$. Then on $\mathcal{D}(A)$, $\{ A_\lambda \}$ converges to $A$ in the strong operator topology. Write
    %
    \[ A_\lambda = \lambda (A - \lambda) R(\lambda,A) + \lambda^2 R(\lambda,A) = \lambda^2 R(\lambda,A) - \lambda. \]
    %
    Set $T_t(\lambda) = e^{t A_\lambda}$. We claim that $T_t(\lambda)$ converges in the strong operator topology for each fixed $t$ to an operator $T_t$, and the operators $\{ T_t \}$ will be a contraction semigroup. To start with, we have the bound
    %
    \[ \| T_t(\lambda) \| = \| e^{t A_\lambda} \| = \| e^{-t \lambda} e^{t \lambda^2 R(\lambda,A)} \| \leq e^{-t \lambda} e^{t \lambda^2 \| R(\lambda,A) \|} \leq e^{-t \lambda} e^{t \lambda} \leq 1. \]
    %
    Thus the operators are a contraction semigroup. By uniform boundedness, to show $\{ T_t(\lambda) \}$ has a limit in $\lambda$, it suffices to work on a dense subspace. For $x \in \mathcal{D}(A)$,
    %
    \begin{align*}
        T_t(\lambda)x - T_t(\lambda')x &= \int_0^t \frac{d}{ds} T_{t-s}(\lambda') T_s(\lambda) x\\
        &= \int_0^t T_{t-s}(\lambda') T_s(\lambda) ( A_\lambda x - A_{\lambda'} x ).
    \end{align*}
    %
    Thus we conclude that
    %
    \[ \| T_t(\lambda) x - T_t(\lambda') x \| \leq t \| A_\lambda x - A_{\lambda'} x \|. \]
    %
    We know the right hand side converges to zero, Thus the left hand side converges, and actually converges uniformly on compact subsets of $[0,\infty)$, \emph{for a fixed} $x$. But now we can define $T_t x$ as this limit. To prove the result is a semigroup, but this follows by taking uniform limits (and using the fact multiplication is continuous in the strong operator topology on \emph{bounded} families of operators). Finally, we must prove that $A$ is actually the generator of the semigroup $\{ T_t \}$. Let $B$ denote the generator of $\{ T_t \}$. Fi $x \in \mathcal{D}(A)$, and let
    %
    \[ \xi_\lambda(t) = T_t(\lambda) x. \]
    %
    Then $\xi_\lambda$ is differentiable, and $\xi_\lambda'(t) = T_t(\lambda) A_\lambda x$. The functions $\{ \xi_\lambda \}$ converge locally uniformly to the function
    %
    \[ \xi(t) = T_t x. \]
    %
    Th functions $\xi_n'$ converge locally uniformly to $\eta(t) = T_t A x$. But we also have
    %
    \[ \xi_n(t) = \int_0^t \xi_n'(s)\; ds \]
    %
    the right hand side converges locally uniformly to
    %
    \[ \int_0^t \eta(s)\; ds \]
    %
    Thus we find that
    %
    \[ \xi(t) = \int_0^t \eta(s)\; ds. \]
    %
    But this means $\xi$ is differentiable, and $\xi'(t) = \eta(t)$. Thus we find that $\mathcal{D}(A) \subset \mathcal{D}(B)$, and for $x \in \mathcal{D}(A)$, $Bx = Ax$. The bounds on $\{ T_t \}$ imply that $(0,\infty) \in \rho(B)$. But this means that $\lambda - A$ and $\lambda - B$ are surjections. But it is impossible for $\lambda - B$ to be surjective, \emph{except} if the domains of $A$ and $B$ are equal. Thus $A = B$.
\end{proof}

If $\{ T_t \}$ is a \emph{quasi-contractive} semigroup ,i.e. it satisfies bounds of the form
%
\[ \| T_t \| \leq e^{\omega t} \]
%
for some $\omega \in \RR$, then $e^{-\omega t} T_t$ is a contractive semigroup. Thus we can apply the result above to give an equivalence between these semigroups, and generators $A$ satisfying bounds of the form
%
\[ \| (\lambda - \omega) R(\lambda,A) \| \leq 1 \]
%
for all $\lambda > \omega$.

A contractive group $\{ T_t \}$ is a group satisfying
%
\[ \| T_t \| \leq 1 \]
%
for all $t \in \RR$. All the operators involved must be isometries, since if $\| T_t x \| < \| x \|$ for some $x \in X$, then
%
\[ \| x \| = \| T_{-t} T_t x \| \leq \| T_t x \| < \| x \|, \]
%
which gives a contradiction. Following through the proof above, a closed, unbounded, densely defined operator $A$ generates a contractive group if and only if for all $\RR - \{ 0 \} \subset \rho(A)$, 
%
\[ \| \lambda R(\lambda,A) \| \leq 1. \]
%
Indeed, one just applies that result to $A$ and to $-A$ separately.

\begin{theorem} [Hille-Yosida Theorem]
    Let $A$ be an unbounded, densely defined operator, such that there exists a number $\omega \in \RR$ such that
    %
    \[ \sigma(A) \subset \{ \lambda : \text{Re}(\lambda) \leq \omega \}, \]
    %
    and such that we have estimates of the form
    %
    \[ \| R(\lambda,A)^n \| \lesssim \frac{1}{(\text{Re}(\lambda) - \omega)^n}. \]
    %
    Then there exists a semigroup $\{ T_t \}$ with $A$ as it's generator.
\end{theorem}
\begin{proof}
    s
\end{proof}
...
A useful example of semigroups for our purposes, given a function $q: \Omega \to \CC$ such that $\sup \Ree(q) < \infty$, is the family of bounded multiplier operators $\{ M_{e^{tq}} : t > 0 \}$ on $C_0(\Omega)$. This semigroup is continuous in the strong operator topology on $C_0(\Omega)$. Moreover, if $f \in C_0(\Omega)$, and $qf \in C_0(\Omega)$, then the function $t \mapsto e^{tq} f$ is differentiable as a map into $C_0(\Omega)$, and the derivative at the origin is precisely $qf$.

\begin{remark}
    Let $H = L^2(\RR^d)$. If $L = P(D)$ is any constant coefficient differential operator, then let $\mathcal{F}: L^2(\RR^d) \to L^2(\RR^d)$ be the Fourier transform, which is a unitary isometry. Then there exists a polynomial $P$ such that
    %
    \[ L = \mathcal{F}^{-1} \circ M_P \circ \mathcal{F}, \]
    %
    and so, provided that $\Ree(P)$ is upper bounded on $\RR^d$, we can define a semigroup $e^{tL}$, e.g. by setting
    %
    \[ e^{tL} = \mathcal{F}^{-1} \circ M_{e^{tL}} \circ \mathcal{F}. \]
\end{remark}

Let $\{ T_t \}$ be the semigroup on $L^p(\RR)$


And indeed, we will see that the operator $Af = f'$ is then an unbounded operator on $L^p(\RR)$ (TODO: What is the domain), and $T_t = e^{tA}$, in a suitable sense.
...
We begin with a purely algebraic theory, and mainly follow the development in Peter Lax's \emph{Functional Analysis}, Chapter 1. For two vector space $V$ and $W$, define the class $\text{Deg}(V,W)$ of \emph{degenerate maps} to be all linear maps $T: V \to W$ with finite dimensional range. Then this family forms a `two-sided ideal', in the sense that if $S: W \to B$ and $U: A \to V$ are linear maps, then $S \circ T$ and $T \circ U$ are degenerate. In particular, $\text{Deg}(V)$ is a genuine two-sided ideal of $\text{End}(V)$. A \emph{pseudoinverse} for a linear map $T \in \text{End}(V,W)$ is a map $S \in \text{End}(W,V)$ such that $T \circ S$ and $S \circ T$ differ from the identity on $V$ and $W$ by a degenerate map. A linear map has a pseudoinverse if and only if it has finite dimensional kernel and cokernel.

\begin{example}
    The right and left shift operators on a sequence space are pseudoinverses for one another.
\end{example}

If $T: V \to W$ has a pseudoinverse $T^\dagger: W \to V$ and $S: W \to U$ has a pseudoinverse $S^\dagger: U \to W$, then $T^\dagger \circ S^\dagger$ is a pseudo-inverse to $S \circ T$.

Given a pseudo-invertible linear map $T: V \to W$, we define it's \emph{index} to be
%
\[ \text{Ind}(T) = \dim(\text{Ker}(T)) - \dim(\text{Coker}(T)). \]
%
If $V$ and $W$ are finite dimensional, then $\text{Ind}(T) = \dim(V) - \dim(W)$ is independent of the pseudo-invertible operator $T$. But the definition can still be applied in the infinite dimensional case, and we will see it still has some stability properties. Consider a short exact sequence of finite-dimensional vector spaces
%
\[ 0 \to V \to W \to U \to 0. \]
%
Then $\dim(V) + \dim(U) = \dim(W)$. This is essentially just the rank nullity theorem in diguise. Extending this gives that for a short exact sequence
%
\[ 0 \to V_1 \to \dots \to V_n \to 0, \]
%
then the Euler characteristic of the sequence is zero, i.e.
%
\[ \sum (-1)^j \dim(V_j) = 0. \]
%
If $T: V \to W$ and $S: W \to U$ are pseudo-invertible, then
%
\[ \text{Ind}(S \circ T) = \text{Ind}(S) + \text{Ind}(T). \]
%
To prove this, we consider the exact sequence
%
\[ 0 \to \text{Ker}(T) \to \text{Ker}(S \circ T) \to \text{Ker}(S) \to \text{Coker}(T) \to \text{Coker}(S \circ T) \to \text{Coker}(S) \to 0. \]
%
The Euler characteristic of this sequence is then precisely $\text{Ind}(S) - \text{Ind}(S \circ T) + \text{Ind}(T)$.

The importance of the index is that it is remarkable stable under pertubations.

\begin{lemma}
    Let $T: V \to W$ be a pseudo-invertible linear map. If $V_0 \subset V$ has finite codimension, then the restriction $T_0: V_0 \to W$ is pseudo-invertible, and
    %
    \[ \text{Ind}(T_0) = \text{Ind}(T) - \text{Codim}(X_0). \]
\end{lemma}
\begin{proof}
    The inclusion map $A: V_0 \to V$ is pseudo-invertible, and $T_0 = T \circ A_0$, so
    %
    \[ \text{Ind}(T_0) = \text{Ind}(T) + \text{Ind}(A_0). \]
    %
    But $\text{Ind}(A_0) = - \text{Codim}(X_0)$, which yields the result required.
\end{proof}

\begin{theorem}
    Let $T: V \to W$ be a pseudo-invertible linear map. If $\varepsilon: V \to W$ is degenerate, then
    %
    \[ \text{Ind}(T + \varepsilon) = \text{Ind}(T). \]
\end{theorem}
\begin{proof}
    Let us first prove the theorem for the identity map $I: V \to V$, i.e. that for any degenerate $\varepsilon: V \to V$,
    %
    \[ \text{Ind}(I + \varepsilon) = 0. \]
    %
    Let $S = I + \varepsilon$. Let $V_0 \subset V$ be the kernel of $\varepsilon$. Then $S_0: V_0 \to V$ is just the composition of the identity map $V_0 \to V_0$ with the inclusion map $V_0 \to V$, so
    %
    \[ \text{Ind}(S_0) = - \text{Codim}(S_0). \]
    %
    But the last lemma also implies that
    %
    \[ \text{Ind}(S_0) = \text{Ind}(S) - \text{Codim}(S_0) \]
    %
    and thus $\text{Ind}(S) = 0$.

    Now if $T: V \to W$ is an arbitrary pseudo-invertible map, and $\varepsilon: V \to W$ is degenerate, and $S$ is a pseudo-inverse for $T$, then $S \circ T = I + \delta$ for a degenerate map $\delta$, and $S \circ (T + \varepsilon) = I + \delta + S \circ \varepsilon$. Since $S \circ \varepsilon$ is degnerate, we conclude that
    %
    \[ \text{Ind}(S \circ (T + \varepsilon)) = \text{Ind}(S \circ T), \]
    %
    and by the product formula, this can only be true if $\text{Ind}(T + \varepsilon) = \text{Ind}(T)$.
\end{proof}

\section{Noether Index Theory}

We now present an analytic variant of the index theory of the last section, following Chapter 27 of Lax's \emph{Functional Analysis}. We work with bounded linear maps $T: X \to Y$ between two Banach spaces $X$ and $Y$. In this context, we say $S$ is a pseudoinverse of $T$ if there exist compact operators $K_X: X \to X$ and $K_Y: Y \to Y$ such that $S \circ T = I + K_X$ and $T \circ S = I + K_Y$. Then $T$ is called pseudoinvertible, or a \emph{Fredholm operator}. It turns out that $T$ is Fredholm if and only if it has finite dimensional kernel and cokernel. Applying the following lemma shows that we can write $T: \text{Ker}(T) \oplus X_0 \to \text{Coker}(T) \oplus Y_0$, where $T$ restricts to an isomorphism from $X_0$ to $Y_0$. But the inverse map from $Y_0$ to $X_0$ then extends to a pseudoinverse for $T$.

\begin{lemma}
    If $T: X \to Y$ is a bounded map between Banach spaces with finite dimensional cokernel, then $T$ has closed range.
\end{lemma}
\begin{proof}
    Define a bounded operator
    %
    \[ T': X \oplus \text{Coker}(T) \to Y \]
    %
    by setting $T'(x + y) = Tx + y$. Then $T'$ is bijective, and thus an isomorphism by the open mapping theorem. But this means that the range of $T$ is closed in $Y$.
\end{proof}

Since a map between Banach spaces is Fredholm if and only if it has a pseudoinverse, we can define the index and apply the theorems of the last section. But here we can get even more results, by following much of the same proof.

\begin{lemma}
    If $\varepsilon: X \to X$ is compact, and $T$ is Fredholm, then $T + \varepsilon$ is Fredholm and $\text{Ind}(T + \varepsilon) = \text{Ind}(T)$
\end{lemma}
\begin{proof}
    For $T = I$, this is a result of the spectral theory of compact operators, and one can find this result in that section of these notes. In the general case, if $S$ is a pseudoinverse to $T$, then
    %
    \[ S \circ (T + \varepsilon) = I + \delta, \]
    %
    where $\delta: X \to X$ is degenerate, and thus compact, and so
    %
    \[ \text{Ind}(S) + \text{Ind}(T + \varepsilon) = \text{Ind}(I + \delta) = 0. \]
    %
    Since $\text{Ind}(S) = - \text{Ind}(T)$, the result follows.
\end{proof}

Every linear map $T: X \to Y$ between Banach spaces has an adjoint $T^*: Y^* \to X^*$. If $T$ is Fredholm, then it's kernel and cokernel are finite dimensional, and the image of $T$ is closed, which implies that $T^*$ also has finite dimensional kernel and cokernel, and thus is also Fredholm. Using the fact that
%
\[ \dim(\text{Ker}(T^*)) = \dim(\text{Coker}(T)) \quad\text{and}\quad \dim(\text{Coker}(T^*)) = \dim(\text{Ker}(T)), \]
%
we conclude that $\text{Ind}(T^*) = - \text{Ind}(T)$.

The next result, due to Dieudonn\'{e} shows that the index is a continuous function.

\begin{theorem}
    If $T: X \to Y$ has a pseudoinverse, then there is $\varepsilon > 0$ such that if $\delta: X \to Y$ is bounded and $\| \delta \| < \varepsilon$, then $T + \delta$ has a pseudoinverse and
    %
    \[ \text{Ind}(T + \delta) = \text{Ind}(T). \]
    %
    Thus if $\{ T_n \}$ is a sequence of bounded operators converging in operator norm to a pseudo-invertible map $T$, then $T_n$ is pseudoinvertible for large enough $n$, and $\text{Ind}(T_n) = \text{Ind}(T)$.
\end{theorem}
\begin{proof}
    Let $S \circ T = I + K$, where $K$ is compact. Then
    %
    \[ S \circ (T + \delta) = I + K + S \circ \delta. \]
    %
    If $\varepsilon$ is suitably small, $I + S \circ \delta$ is invertible. If $R$ is an inverse, then
    %
    \[ R \circ S \circ (T + \delta) = 1 + R \circ K, \]
    %
    and $R \circ K$ is compact, so $T + \delta$ has a pseudoinverse $R \circ S$. Now the product formula implies that
    %
    \[ \text{Ind}(R) + \text{Ind}(S) + \text{Ind}(T + \delta) = 0. \]
    %
    Since $\text{Ind}(S) = - \text{Ind}(T)$, and since $R$ is invertible, so $\text{Ind}(R) = 0$, we conclude that $\text{Ind}(T + \delta) = \text{Ind}(T)$.
\end{proof}

Another consequence of the above result is the \emph{homotopy invariance} of the index, i.e. if $\{ T(t) : t \in [0,1] \}$ is a continuous family of operators, and each operator has finite index, then
%
\[ \text{Ind}(T(1)) = \text{Ind}(T(0)). \]
%
On the other hand, the index is not continuous in the strong operator topology, e.g. define $T_n: l^2 \to l^2$ by setting $T_n x = \{ x_1,x_2,\dots,x_n,0,x_{n+1},\dots \}$. Then $\text{Ind}(T_n) = -1$, but the strong operator limit of $T_n$ is the identity, which has index $0$. On the other hand, it is a result of H\"{o}rmander that if $T_n: X \to Y$ are sequences of pseudoinvertible maps, and we can find pseudo-inverses $S_n: Y \to X$ such that $S_n \circ T_n = I + K_n$ and $T_n \circ S_n = I + H_n$ for a uniformly compact sequence of operators $\{ K_n \}$ and $\{ H_n \}$, in the sense that the union of $K_n(B)$ and $H_n(B)$ is compact, where $B$ is the unit ball in $X$, then the strong operator limit of $\{ T_n \}$, if it exists, has index equal to the limit of the indexes of the operators $\{ T_n \}$ (see H\"{o}rmander, Volume 3, Theorem 19.1.10).

Using these results, let us compute some indices of a family of operators. Let $H_+$ be the subspace of functions in $L^2(\TT)$ with Fourier support on $\{ 0, 1, 2, \dots \}$, and let $H_-$ be the subspace of functions in $L^2(\TT)$ with Fourier support on $\{ 0, -1, -2, \dots \}$. Let $P_+: L^2(\TT) \to H_+$ be the projection of $L^2(\TT)$ onto $H_+$. For any continuous function $s: \TT \to \CC$, we define a bounded operator $T_s: H_+ \to H_+$ given by $T_s f = P_+(s u)$. Then $T_{s_1 + s_2} = T_{s_1} + T_{s_2}$. In Fourier analysis terms, we can write
%
\[ T_s f(t) = \sum_{n = 0}^\infty \sum_{m \in \ZZ} \widehat{f}(m) \widehat{s}(n - m) (\widehat{f} * \widehat{s})(n) e^{2 \pi i t n}. \]
%
We can then calculate the index of these operators.

\begin{theorem}
    If $s$ is nowhere vanishing on $\TT$, then $T_s$ is pseudoinvertible, and then $\text{Ind}(T_s)$ is the \emph{winding number} of $T_s$ about the origin.
\end{theorem}
\begin{proof}
    If $s$ is nowhere vanishing on $\TT$, and we let $a = 1/s$, then we claim $T_a$ is a pseudoinverse for $T_s$. First, we note that if $s$ is continuous, then
    %
    \[ Kf = P_+[sf] - sf \]
    %
    is a compact map of $H_+$ into $L^2(\TT)$. This is because if $s$ is a trigonometric polynomial, then $K$ is a finite rank operator on $H_+$, and all continuous functions $s$ are the limit of trigonometric polynomials in $L^\infty(\TT)$. But
    %
    \[ (T_a \circ T_s) f = P_+(a P_+ (sf)) = P_+(a [sf + [ P_+ (sf) - sf ] ) = f + P_+[ P_+(sf) - sf ]. \]
    %
    The second term is compact, which shows $T_a \circ T_s - 1$ is compact.

    Now to calculate the index of $T_s$. But this is simple. By applying a homotopy map as above, if $k$ is the winding number of $s$, we can deform $T_s$ continuously into $T_k$, where
    %
    \[ T_k f = P_+[ e^{2 \pi i k t} f ]. \]
    %
    Since $T_k$ is injective, and has cokernel which we can identify with trigonometric polynomials with degree at most $N-1$, it follows that the index of $T_k$ is $-N$, and thus the index of $T_s$ is $-N$.
\end{proof}

See Lax for more results and references about Toeplitz operators.
