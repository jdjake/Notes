...
As is expected in the name, functional analysis mostly applies to functions between topological and algebraic structures, because there is a natural notion of scaling and addition, and we may obtain topological structure based on the various methods of convergence a function can have. The power of functional analysis rests on how strongly the algebraic structure of a space connects to the topology of the space.

The most common analytic operations are scaling and rotation. As a result, the most common vector spaces which occur in functional analysis are over the real and complex numbers. More specifically, real vector spaces $X$ are those equipped with natural scaling operations $x \mapsto \lambda x$ for $\lambda > 0$, combined with `reflection' operators $x \mapsto -x$. If $X$ is a real vector space with a specified `counterclockwise twisting', i.e. a linear map $J: X \to X$ with $J^2 = -1$, then there is a natural way to turn $X$ into a complex vector space by defining $(a + ib)x = ax + b(Jx)$. The geometry of the complex numbers then enables us to define a counterclockwise twist by any angle $\theta$, by the operation $x \mapsto e^{i\theta} x$. Thus we have a natural operation of the complex numbers over the space. 

Moreover, if we work with vector spaces over a field of characteristic zero equipped with a `complete' topological structure, then we should expect the underlying field of the vector space to be of characteristic zero as well. In particular, working over the real numbers {\it at least} isn't too particular a criterion. In these notes, we let $\mathbf{K}$ denote either the real or complex numbers.
...
\begin{example}
    ...
    %
    Then Minkowski's inequality tells us that the quantity $\| \cdot \|_p$ is a prenorm on $\mathcal{L}^p(X)$. We notice that if $u$ is a function with $u(x) = 0$ for almost every $x$, then $\| f \|_p = \| f + u \|_p$. In particular, this means that $\| \cdot \|_p$ is a well defined operation on the quotient space of $\mathcal{L}^p(X)$ where functions are identified almost everywhere. Since $\| f \|_p = 0$ if and only if $f(x) = 0$ for almost every $x$, we obtain a norm space, denoted by $L^p(X)$. This is the space where most theorems of functional analysis prove useful.
\end{example}

\begin{example}
    Given a measurable function $f$ on a measure space $X$, we can define the $L^\infty$ norm
    %
    \[ \| f \|_\infty = \inf \{ t: |f(x)| \leq t\ \text{for almost every $x$} \}. \]
    %
    The space of functions with finite $L^\infty$ prenorm is denoted by $\mathcal{L}^\infty(X)$. Considering the quotient space of functions identified almost everywhere, we obtain a norm space $L^\infty(X)$.
\end{example}
...
\begin{example}
    ...
    %
    gives $C(K)$ the structure of a norm space.
\end{example}

For the purpose of the general theory, one need not know too much measure theory to get by. The most important examples of the $L^p$ spaces for intuition and forming counterexamples are the spaces $l^p$, which is the collection of all sequences $\{ a_n \}$ for which, if $p < \infty$,
%
\[ \sum_{n = 1}^\infty a_n^p < \infty, \]
%
and for $p = \infty$, $l_\infty$ is the collection of all bounded sequences.

\begin{remark}
    We will focus on norms in these notes, but prenorms are really no more general. If $X$ is a space with a prenorm $\| \cdot \|$, then the space $X_0 = \{ x \in X: \| x \| = 0 \}$ forms a subspace of $X$. We have $\| x \| = \| x + x_0 \|$ for each $x_0 \in X_0$, so if we form the quotient space $X' = X/X_0$, then the quantity $\| x + X_0 \|$ is well defined, and this is a {\it norm} on $X'$. So results about norms automatically extend to results about prenorms where we identify vectors which differ by an element of $X_0$. For instance, if we
\end{remark}

For each norm on a space $X$, we can associate a metric given by $d(x,y) = \| x - y \|$. The most powerful theorems of functional analysis deal with the study of norm spaces where the induced metric gives the space $X$ a {\it complete} metric space structure. These spaces are known as {\it Banach spaces}. It is a basic fact of measure theory that the spaces $L^p(X)$ are complete, as is the space $C(K)$.
...
\begin{proof}
    ...
    %
    then Markov's inequality implies $|E_i| \leq 1/2^{ip}$, and so $\sum_{i = 1}^\infty |E_i| < \infty$. Thus we can define a measurable function $f$ defined almost everywhere by the infinite series
    %
    ...
    \end{align*}
    %
    Thus $f_{k_i} \to f$ in the $L^p$ norm. Since $\{ f_k \}$ is a Cauchy sequence, it is easy to justify that $f_k \to f$ in the $L^p$ norm. Thus Cauchy sequences converge.
\end{proof}

\begin{remark}
    Again, studying Banach spaces is not too much more general than studying norm spaces. Given any norm space $X$, we can always find a Banach space $X'$ containing $X$, such that $X$ is dense in $X'$. One can view the elements of $X' - X$ as `artifical' elements of $X$ that nonetheless are key to understanding the geometric structure of $X$. Provided we are willing to deal with these artifical elements, it suffices without loss of generality to assume in the theory that we are working in a Banach space.
\end{remark}

The three examples above provide most of the intuition we will need for the basic Banach space theory. But it is useful to know other examples for applications in other areas of analysis.

\begin{example}
    For each $0 < p \leq 1$, we say $f: [0,1] \to \mathbf{K}$ is {\it Lipschitz of order $p$} if $|f(x) - f(y)| \lesssim |x - y|^p$ for each $x,y \in [0,1]$. The set of such functions form a vector space $\text{Lip}_p [0,1]$, which is a subspace of $C[0,1]$, but not a closed subspace. For each $f \in \text{Lip}_p [0,1]$, we define
    %
    \[ \| f \|_{\text{Lip}_p[0,1]} = \| f \|_\infty + \sup \frac{|f(x) - f(y)|}{|x - y|^p}. \]
    %
    This gives $\text{Lip}_p[0,1]$ a Banach space structure.
\end{example}

We can develop the theory of series in a norm space. Given a sequence $\{ x_n \}$. We say the sequence is absolutely summable if $\sum \| x_n \| < \infty$. The same proof as for $\RR$ shows that the sequence is {\it unconditionally summable}, i.e. that for each permutation $\pi: \mathbf{N} \to \mathbf{N}$,
%
\[ \sum_{n = 1}^\infty x_{\pi(n)} = \sum_{n = 1}^\infty x_n. \]
%
A space $X$ is a Banach space if and only if every absolutely summable series converges. However, in infinite dimensional spaces, there are always examples of unconditionally summable sequences which are not absolutely summable. This is a result of A. Dvoretzky and C.A. Rogers.

\begin{example}
    Consider the space $c_0$.
\end{example}
...
\begin{example}
    ...
    %
    which converges for $|z| < 1$ since $\{ \widehat{f}(n) \}$ is a bounded sequence. The $L^p$ norm also implies $u$ satisfies a suitably growth condition as $z$ approaches the boundary of the unit disk, but we do not dwell on this fact.
\end{example}

\section{Convexity}

A critical notion in functional analysis is {\it convexity}. A set $E \subset X$ is {\it convex} if for each $x,y \in E$ and $\lambda \in (0,1)$, $\lambda x + (1 - \lambda) y \in E$. A set $E$ is {\it balanced} if $\alpha E \subset E$ for each $|\alpha| \leq 1$, and {\it symmetric} if $-E = E$. As an example, the triangle inequality says every ball is convex, and  these balls are also balanced and symmetric. In the future, we will make heavy use of the following result.

\begin{theorem}
    Every closed, convex, absorbing set contains a neighbourhood of the origin.
\end{theorem}
