...
%
We will later see this remains true for the class of normal operators, and moreover, we can refine this orthogonal sum when $T$ is compact to give a complete eigenfunction expansion.
...
\begin{theorem}
    If $P$ is a projection with image $W$, then the following are equivalent:
    %
    \begin{enumerate}
...
If $H$ is a Hilbert space, then two orthogonal projections $P_1$ and $P_2$ might not necessarily commute. As a basic example, let us consider only projections onto a line. Fix $\| v_1 \| = \| v_2 \| = 1$, and consider $P_i x = \langle x, v_i \rangle v_i$. Then
%
...
\begin{remark}
    It is a theorem of Calkin that $K(H)$ is the only closed, two-sided ideal of $B(H)$ for any Hilbert space $H$. Thus we are lead to consider the \emph{Calkin Algebra} $B(H)/K(H)$, which is simple, and quite useful in $K$-theory.
\end{remark}
...
\begin{theorem}
    Let $H$ be a Hilbert space. Then $T: H \to H$ is compact if and only if $T$ is the limit in the operator norm of a sequence of finite rank operators $\{ T_n \}$.
\end{theorem}
\begin{proof}
...
\begin{remark}
    Finding a Banach space \emph{without} the approximation property (i.e. finding a compact operator which cannot be approximated by finite rank operators) proved to be an incredibly difficult problem in 20th century research on functional analysis. Such an operator was only found in 1972.
\end{remark}
...
\begin{example}
    The simplest operators on a Hilbert space are the diagonal operators, i.e. those operators $T: H \to H$ given in terms of an orthogonal basis $\{ e_n \}$ and $\{ \lambda_n \}$ such that $Te_n = \lambda_n$. Then
    %
    \[ \| T \| = \sup_n |\lambda_n|, \]
    %
...
TODO: If $T$ is compact, $T^*$ is compact.
...
\begin{example}
    Let $T: L^2[0,1] \to L^2[0,1]$ be the multiplication operator given by $Tf(x) = x \cdot f(x)$. Then $T$ has \emph{no} eigenvectors, i.e. $Tf = \lambda f$ precisely when $f = 0$. On the other hand, for any $\lambda \in [0,1]$, we can find a sequence of `almost eigenvectors for $\lambda$', i.e. a sequence of functions $f_n \in L^2[0,1]$ such that $\| f_n \|_{L^2[0,1]} = 1$, and
    %
    \[ \lim_{n \to \infty} \| Tf_n - \lambda f_n \|_{L^2[0,1]}. \]
\end{example}

There are several alternate generalizations which \emph{do} work for an arbitrary self-adjoint operator on a Hilbert space (in fact, for any normal operator), which are best treated in the more advanced theory of operators. For now, we limit ourselves to studying operators on a Hilbert space which are `almost finite dimensional', i.e. they are compact. It is simple to see why compactness is useful in this regime: if a compact operator $T: H \to H$ has an sequence $\{ x_n \}$ such that $\| x_n \| = 1$ and $\lim_{n \to \infty} \| Tx_n - \lambda x_n \|$ for some $\lambda \neq 0$, then the compactness of $T$ shows that $Tx_n / \lambda$ must have a convergent subsequence, converging to some $y \in H$ with $\| y \| = 1$, and then continuity shows that
%
\[ \| Ty - \lambda y \| = \lim_{n \to \infty} \| T^2 x_n / \lambda - T x_n \| \leq \lambda^{-1} \| T \| \lim_{n \to \infty} \| Tx_n - \lambda x_n \| = 0. \]
%
Thus $Ty = \lambda y$, a sequence of almost eigenvectors implies the existence of an \emph{exact eigenvector}. For a normal operator $T$, we let $\sigma(T)$ be the set of \emph{almost eigenvalues} for $T$, and $\sigma_p(T)$ the set of \emph{exact eigenvalues}. We have shown that if $T$ is compact and normal, then $\sigma(T) = \sigma_p(T)$.

To begin our study, let us introduce the basic notation. If $X$ is a Banach space, and $T: X \to X$ is bounded, for each $\lambda$, we let $X_\lambda = \{ x : Tx = \lambda x \}$. Then $X_\lambda$ is a closed subspace of $X$. The scalar $\lambda$ is called an \emph{eigenvalue} if $X_\lambda \neq 0$, and any nonzero $x \in X_\lambda$ is an eigenvector with eigenvalue $\lambda$. It turns out that in infinite dimensions, one not only has the phenomena of eigenvalues, but also \emph{almost} eigenvalues, which makes the family of eigenvalues not so powerful a description of operators. Thus we replace this description with the \emph{spectrum} of a bounded operator $T: X \to X$, which is the set $\sigma(T)$ of all scalars $\lambda$ such that $T - \lambda$ is \emph{not} an isomorphism between $X$ and itself. If $T$ is normal, then $T - \lambda$ is an isomorphism if and only if an inequality of the form
%
\[ \| (T - \lambda) x \| \gtrsim \| x \| \]
%
holds for all $x \in H$. In particular, in this case $\sigma(T)$ consists of all $\lambda$ such that for any $\varepsilon > 0$, we can find $x \in H$ such that $\| Tx - \lambda x \| \leq \varepsilon \| x \|$, i.e. such that we can find $x$ such that $Tx \approx \lambda x$, up to arbitrary precision. Thus $\sigma(T)$ detects not only eigenvalues, but also \emph{almost eigenvalues}.

\begin{example}
    A self-adjoint operator need not have any eigenvalues. For instance, if $T: L^2[0,1] \to L^2[0,1]$ is given by $Tf(x) = x f(x)$, then $T$ is self-adjoint, but has no eigenvalues, for if $Tf = \lambda f$, for any $\lambda$, then $x f(x) = \lambda f(x)$, which implies $f = 0$. On the other hand, $T$ any $\lambda \in [0,1]$ is \emph{almost} an eigenvalue, in the following sense. If $f(x) = \mathbf{I}(x \in I)$, where $I$ is an interval of length $\delta$ containing $\lambda$, then $|Tf(x) - \lambda f(x)| \leq \delta \mathbf{I}(x \in I)$, and so
    %
...
\begin{example}
    As another example of this phenomenon, let $\{ \lambda_n \}$ be an enumeration of all rational numbers in $[0,1]$. Thus the bounded operator $T: l^2(\NN) \to l^2(\NN)$ given by
    %
    \[ (Ta)(n) = \lambda_n a(n) \]
    %
    Then $Te_n = \lambda_n e_n$, so $T$ has eigenvectors corresponding to eigenvalues for all rational numbers in $[0,1]$. On the other hand, $T - \lambda$ is not invertible for any $\lambda \in [0,1]$, so $\sigma(T - \lambda) = [0,1]$.
\end{example}

The main result of this section is that a compact operator on a Hilbert space $H$ can always be diagonalized, with a sequence of eigenvalues $\lambda_i$ converging to zero, and such that $H$ can be decomposed into the orthogonal sum of the eigenspaces $H_{\lambda_i}$, and for $i \neq 0$, $H_{\lambda_i}$ is finite dimensional.

\begin{theorem}
    If $X$ is a Banach space, $Y$ is a closed subspace, $T$ is a compact operator on $X$, and $\lambda \neq 0$ is a scalar such that
    %
    \[ \inf_{\substack{y \in Y\\\| y \| = 1}} (T - \lambda)y = 0, \]
    %
    then $Y \cap X_\lambda \neq 0$.
\end{theorem}
\begin{proof}
    Let $y_i$ be a sequence in $Y$ with $\| y_i \| = 1$ such that $(T - \lambda)y_i \to 0$. Since $T$ is a compact operator, we may assume without loss of generality that $Ty_i$ converges to some $z$. But then, by continuity,
    %
    \[ \lim_i \lambda y_i = \lim_i \lambda y_i - T y_i + T y_i = \lim_i y_i - T y_i + \lim_i T y_i = 0 + z \]
    %
    Since $\lambda \neq 0$, this means $y_i \to \lambda^{-1} z$, which means $\| z \| = \lambda$, and in particular, $z \neq 0$. But then $z \in Y$, and $z = Mz$, which completes the proof since $z \in Y \cap X_\lambda$.
\end{proof}

TODO: IF $\lambda$ is an eigenvalue for $T$, it is an eigenvalue for $T^*$.
...
\end{corollary}
\begin{proof}
    First note that $T = \lambda$ restricted to $X_\lambda$. Since $X_\lambda$ is closed, this implies the identity operator is compact on $X_\lambda$, hence $X_\lambda$ is finite dimensional. Since $X_\lambda$ is finite dimensional, we can find a subspace $Y$ of $X$ such that $X = X_\lambda \oplus Y$. $T - \lambda$ restricted to $Y$ is bounded and injective, and the last theorem implies, since $Y \cap X_\lambda = (0)$, that there is $C > 0$ such that
    %
    \[ \| (T - \lambda) y \| \geq C \| y \| \]
    %
    for $y \in Y$. Thus $T - \lambda$ is an isomorphism from $Y$ to a subspace $Z$ of $X$, which is therefore complete and thus closed. Note that
    %
    \[ Z^\perp = [(T - \lambda)X]^\perp = \{ \phi \in X^*: \phi \circ (T - \lambda) = 0 \} = \ker( T^* - \lambda). \]
    %
    Since $T$ is compact, so is $T^*$. Because $\lambda$ must also be an eigenvalue of $T^*$, the previous part of this argument therefore implies that
    %
    \[ \codim(T - \lambda) = \text{dim}[Z^\perp] < \infty. \]
    %
    TODO
\end{proof}
...
\begin{lemma}
    If $H$ is a Hilbert space, $T: H \to H$ is bounded and self-adjoint, and $\lambda_1 \neq \lambda_2$, then $H_{\lambda_1}$ and $H_{\lambda_2}$ are orthogonal.
\end{lemma}
\begin{proof}
    We begin by assuming without loss of generality that $\lambda_1$ and $\lambda_2$ are real, since a self-adjoint operator can only have real eigenvalues. If $x_1 \in H_{\lambda_1}$ and $x_2 \in H_{\lambda_2}$, then
    %
...
\[ \| M^* \|^2 = \| M M^* \| \leq \| M^* \| \| M \| \]
%
implies $\| M^* \| \leq \| M \|$. The next result shows that any involution on a commutative, semisimple Banach algebra is continuous (an algebra $A$ is semisimple if the intersection of all maximal ideals is trivial).
...
The next result is clearly not true in an arbitrary Banach algebra (e.g. by considering the trivial Banach algebra structure on any Banach space $X$).
...
%
\begin{itemize}
    \item $A$ is generated as a unital algebra by $M$.
...
\end{itemize}
%
Then $\widehat{M}: \Phi_A \to \sigma(M)$ is a homeomorphism. Since $\Phi_A$ is compact, it suffices to show that $\widehat{M}$ is injective. But this follows because, in each of the three cases above, if two characters agree at $M$, they agree on the entirety of $M$.
...
Now suppose that $A$ is a commutative unital $C^*$ algebra, either generated by $M$, or generated by $M$ and $M^{-1}$ for some invertible $M$ (this is obtained in practice, by starting with a $C^*$ algebra, and then considering the smallest $C^*$ algebra generated by a normal element). Then $\widehat{M}: \Phi_A \to \sigma(M)$ is a homeomorphism. We thus have an isometric $*$ isomorphism
%
...
We denote the unique positive square root of $M \in A_+$ by $\sqrt{M}$. If self-adjoint operators behave somewhat like real numbers, positive operators behave like positive real numbers.
...
\begin{lemma}
    Let $A$ be a $C*$ algebra. If $M$ is self-adjoint, $t \geq 0$, and $\| M - t \| \leq t$, then $M$ is positive. If $M$ is positive, and $\| M \| \leq t$, then $\| M - t \| \leq t$.
\end{lemma}
\begin{proof}
...
For $M$ lying in a $C*$ algebra $A$, we are inclined to define $|M| = \sqrt{MM^*}$, for any $M \in A$, which is a positive element.
...
\begin{example}
    ...
    Thus in an infinite dimensional, separable Hilbert space $H$, the \emph{Calkin Algebra} $C(H) = B(H)/K(H)$ has no nontrivial closed ideals.
\end{example}
