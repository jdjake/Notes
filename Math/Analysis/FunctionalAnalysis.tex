...
%
\begin{align*}
    |(E(\omega_1) x_1, x_2)| + \dots + |(E(\omega_N) x_1, x_2)| &\leq \| E(\omega_1) x_1 \| \| E(\omega_1) x_2 \| + \dots + \| E(\omega_N) x_1 \| \| E(\omega_N) x_2 \| \\
        &\leq \| x_1 \| \| x_2 \|.
\end{align*}
%
...
%
and taking $\varepsilon \to 0$ gives equality. Thus $I: L^\infty(E) \to B(H)$ is a $*$ isometry between the two spaces.
...
\begin{theorem}
    Let $A$ be a closed unital commutative $C^*$ subalgebra of $B(H)$. If $\Phi_A$ is the Gelfand space of $A$, then there is a unique Borel measurable spectral resolution $E$ on $\Phi_A$ such that for any $T \in A$,
    %
    \[ T = \int \widehat{T} dE, \]
    %
...
It is very desirable to have a spectral theory of operators that are not necessarily bounded from a space to itself, especially in the theory of partial differential equations, where the natural spaces.
