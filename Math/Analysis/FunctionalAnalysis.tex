...
\section{Dual Spaces, Spaces of Linear Transforms, and Equicontinuity}
...
A family of functions $\mathcal{S} \subset L(X,Y)$ will be called \emph{equicontinuous} if, for any open neighborhood $W$ of the origin in $Y$, there is a neighborhood $V$ of the origin in $X$ such that $T(V) \subset W$ for all $T \in S$.
...
\begin{proof}
    ...
    %
    \[ T_\alpha(x) \in T_\alpha(x_i) + T_\alpha(U) \subset T(x_i) + W + W \subset T(x) + T(U) + W + W \subset T(x) + W + W + W \subset T(x) + V. \]
    %
...
The uniform boundedness theorem then follows from the following result.
...
TODO: Chapter 34, 36, 37, 38 of Treves.
...
As proved in a remark in the part of these notes on locally convex spaces, since the weak $*$ topology is defined on $X^{**}$ in terms of a family of linear functionals, the only weak $*$ continuous linear functionals on $X^*$ are induced from elements of $X$. Thus the weak topology and weak $*$ topology agree only when $X$ is reflexive.
...
In this chapter, we develop the analytic theory of the tensor product. Let us recall the algebraic theory. Given two vector spaces $V$ and $W$, we can consider their algebraic tensor product $V \otimes W$, which is the universal vector space such that any bilinear map $V \times W \to U$ factors through the inclusion map $V \times W \to V \otimes W$. Elements of $V \otimes W$ can be written as finite sums of the form
%
...
The main example we wish to generalize to more analytical contexts is the following; given two sets $X$ and $Y$, and two functions $f: X \to \CC$ and $g: Y \to \CC$, we define the tensor product $f \otimes g: X \times Y \to \CC$ by setting $(f \otimes g)(x,y) = f(x) g(y)$. This gives a bilinear map $\CC^X \times \CC^Y \to \CC^{X \times Y}$, and thus induces a homomorphism $\CC^X \otimes \CC^Y \to \CC^{X \times Y}$, which is easily seen to be an isomorphism if $X$ and $Y$ are finite. We wish to define an analytic definitions of the tensor product generalizing this property to infinite dimensional setting, for instance, so that the tensor product of $L^p(X)$ and $L^p(Y)$ is isomorphic to $L^p(X \times Y)$, and the tensor product of $C_c^\infty(\Omega_1)$ with $C_c^\infty(\Omega_2)$ is isomorphic to $C_c^\infty(\Omega_1 \times \Omega_2)$.

\section{Bilinear Maps}

Let $X$ and $Y$ be topological vector space. We say a bilinear map $B$ on $X \times Y$ is \emph{separately continuous} if for each fixed $x_0 \in X$ and $y_0 \in Y$, the two linear functionals $y \mapsto B(x_0,y)$ and $x \mapsto B(x,y_0)$ are continuous. The map is \emph{continuous} if it is continuous in the product topology, which if $X$ and $Y$ are locally convex, is equivalent to the existence of two continuous seminorms $\rho$ and $\sigma$ on each space such that for all $x \in X$ and $y \in Y$,
%
\[ |B(x,y)| \lesssim \rho(x) \sigma(y). \]
%
For Fr\'{e}chet spaces, separately continuous maps are automatically continuous.

\begin{theorem}
    Let $X$ be a Fr\'{e}chet space, and $Y$ a metrizable topological vector space. Then every separately continuous bilinear map is continuous.
\end{theorem}
\begin{proof}
    Since $X$ and $Y$ are metrizable, it suffices
    For any $x \in X$, let 
    ...
    Suppose $\{ (x_\alpha, y_\alpha) \}$ is a net in $X \times Y$ converging to $(x,y) \in X \times Y$.
    For each $y \in Y$,
    The product space
    Suppose $B$ is a separately continuous bilinear map. 
\end{proof}

We will restrict ourselves to the study of locally convex topological vector spaces. Given two such spaces $V$ and $W$, there are many ways we could give $V \otimes W$ a locally convex structure so that the natural map $i: V \times W \to V \otimes W$ is continuous. But abstractly, there are two main choices:
%
\begin{itemize} % B(V^*,W)
    \item The \emph{coarsest} locally convex topology such that $i: V \times W \to V \otimes W$ is continous, so that $U \subset V \otimes W$ is open if and only if $i^{-1}(U)$ is open in $V \times W$. This is the \emph{equicontinuous topology} on $V \otimes W$, and the completion of the tensor product with respect to this topology is denoted by $V \otimes_\varepsilon W$.

    \item The \emph{finest} locally convex topology such that $i: V \times W \to V \otimes W$ is continuous, so that a convex set $U \subset V \otimes W$ is open if and only if $i^{-1}(U)$ is open. This is the \emph{projective topology} on $V \otimes W$, and the closure with respect to this topology is called the \emph{projective tensor product}, denoted $V \otimes_\pi W$.
\end{itemize}

More abstractly, there are two important identifications



We have a natural bilinear map $V^* \times W^* \to (V \otimes W)^*$ given by $(\phi,\psi) \mapsto \phi \otimes \psi$, where $\phi \otimes \psi$ is induced by the bilinear map
%
\[ (\phi \otimes \psi)(v,w) = \phi(v) \psi(w). \]
%
Thus we get a natural homomorphism $V^* \otimes W^* \to (V \otimes W)^*$.

Given a pair of linear functionals $\phi \in V^*$ and $\psi \in W^*$, we have a natural linear functional $\phi \otimes \psi \in (V \otimes W)^*$ induced by the bilinear map $(\phi \otimes \psi)(v,w) = \phi(v) \psi(w)$. Given a family of linear functionals $A \subset V^*$ and $B \subset W^*$ which separate points, the family $A \otimes B \subset (V \otimes W)^*$ separates $V \otimes W$. There are two important identifications of the tensor product which will be useful to treat the tensor product from an analytic perspective:
%
\begin{itemize}
    \item There is a natural embedding $i: V \otimes W \to B(V^*,W)$ given for $v \in V$, $w \in W$, and $v^* \in V^*$ by the map
    %
    \[ i(v \otimes w)(v^*) = v^*(v) w. \]
    %
    If $W$ is finite dimensional, then $i$ is an isomorphism.

    \item Because of the universal property of tensor products, the algebraic dual of $V \otimes W$ can be identified with the vector space of all bilinear, scalar-valued maps with domain $V \times W$.
\end{itemize}
%
We use these identifications to make the algebraic tensor product of two Banach spaces into a norm space. Let us begin with the first identification. In fact, we can characterize the image of $i$ when taking the tensor product of Banach spaces.

\begin{theorem}
    If $X$ and $Y$ are Banach spaces, then the algebraic tensor product $X \otimes Y$ is naturally isomorphic to the space of all weak $*$ to weak continuous finite rank operators from $X^*$ to $Y$. The tensor product $X^* \otimes Y$ is naturally isomorphic to the family of all bounded, finite rank operators from $X$ to $Y$
\end{theorem}
\begin{proof}
    Let $B$ be the space of all weak $*$ to weak continuous finite rank operators described above. The isomorphism $i: X \otimes Y \to B$ is the unique one such that for $x \in X$, $y \in Y$, and $\lambda \in X^*$,
    %
    \[ i(x \otimes y)(\lambda) = \lambda(x) y. \]
    %
    For any element $T \in B$, there are linear functionals $\gamma_1,\dots,\gamma_n$ on $X^*$, continuous in the weak $*$ topology, and $y_1,\dots,y_n \in Y$, such that for $\lambda \in X^*$,
    %
    \[ T\lambda = \gamma_1(\lambda) y_1 + \dots + \gamma_n(\lambda) y_n. \]
    %
    Since these linear functionals are weak $*$ topology, there are $x_1,\dots,x_n \in X$ such that $\gamma_i(\lambda) = \lambda(x_i)$. But this means that $T = i(x_1 \otimes y_1 + \dots + x_n \otimes y_n)$. Thus $i$ is surjective. To show injectivity, if $i(x_1 \otimes y_1 + \dots + x_n \otimes y_n) = 0$, then $\lambda(x_1) y_1 + \dots + \lambda(x_n) y_n = 0$ for all $\lambda \in X^*$, which means that for any $\gamma \in X^*$,
    %
    \[ \lambda(x_1) \gamma(y_1) + \dots + \lambda(x_n) \gamma(y_n) = (\lambda \otimes \gamma)(x_1 \otimes y_1 + \dots + x_n \otimes y_n) = 0. \]
    %
    But since $X^* \otimes Y^*$ is separating, this means that $x_1 \otimes y_1 + \dots + x_n \otimes y_n = 0$. Thus $i$ is injective.

    Now given $X$ and $Y$, we know that $X^* \otimes Y$ is naturally isomorphic to the family of all weak $*$ to weak continuous finite rank operators from $X^{**}$ to $Y$. Any such operator restricts to a continuous, finite rank operator from $X$ to $Y$. Thus we have an injective mapping $j: X^* \otimes Y \to F(X,Y)$ such that for $\lambda \in X^*$, $x \in X$, and $y \in Y$,
    %
    \[ j(\lambda \otimes y)(x) = \lambda(x) y. \]
    %
    But any continuous finite rank operator $T: X \to Y$ can be written as
    %
    \[ \sum \lambda_i(x) y_i \]
    %
    for some $\lambda_i \in X^*$ and $y_i \in Y$, and thus $T = j(\lambda_1 \otimes y_1 + \dots + \lambda_n \otimes y_n)$.
\end{proof}

The \emph{injective norm} $\varepsilon$ on the algebraic tensor product $X \otimes Y$ is given by
%
\[ \varepsilon \left( a \right) = \sup_{\| x^* \|, \| y^* \| \leq 1} (x^* \otimes y^*)(a). \]
%
This is precisely the operator norm of $i(a)$ as a bounded operator from $X^*$ to $Y$. The completion of $X \otimes Y$ is called the \emph{equicontinuous tensor product}, and denoted by $X \otimes_\varepsilon Y$. It can be identified with the closure of the finite rank, weak $*$ to weak continuous finite rank operators from $X^*$ to $Y$

Now we obtain a second norm. Because any algebraic linear functional on $X \otimes Y$ is naturally equivalent to a bilinear map on $X \times Y$, we obtain a natural pairing of the two vector spaces, given for a bilinear map $B$ and $a = \sum_i x_i \otimes y_i$ by setting
%
\[ \langle B, a \rangle = \sum_i B(x_i,y_i). \]
%
Similarily, for a linear map $T: X \to Y^*$, we consider
%
\[ \langle T, a \rangle = \sum_i \langle T(x_i), y_i \rangle. \]
%
These are really the same action, because linear maps from $X$ to $Y^*$ are in one to one correspondence with linear maps. We might be inspired by this correspondence to introduce a norm structure on $X \otimes Y$. This gives the \emph{projective tensor product} of $X$ and $Y$, whose completion we denote by $X \otimes_\pi Y$. The dual of this space is naturally isomorphic to the vector space $\text{Bil}(X,Y)$ of all \emph{continuous linear maps}, i.e. the space of all scalar-valued bilinear maps $B$ such that
%
\[ |B(x,y)| \lesssim \| x \| \| y \|. \]
%
for all $x \in X$ and $y \in Y$. We let $\| B \|$ denote the optimal constant in this inequality, which makes $\text{Bil}(X,Y)$ into a Banach space. The \emph{projective norm} on $X \otimes Y$ is defined by setting
%
\[ \pi \left( a \right) = \sup_{\| B \| \leq 1} |\langle B, a \rangle|, \]
%
and $X \otimes_\pi Y$ denotes the completion of this norm. Moreover, we note that $\pi(x_0 \otimes y_0) = \| x_0 \| \| y_0 \|$ for any $x_0 \in X$ and $y_0 \in Y$.

It is simple to see that $\varepsilon(a) \leq \pi(a)$ for all $a \in X \otimes_\pi Y$, since $\varepsilon$ is taken over a special subset of continuous bilinear functionals (those of the form $x^* \otimes y^*$). Thus we have a natural continuous inclusion $X \otimes_\pi Y \to X \otimes_\varepsilon Y$.

Given any continuous linear functional $\lambda$ on $X \otimes_\pi Y$, we get a bilinear map $B$ by setting $B(x,y) = \lambda(x \otimes y)$ for $x \in X$ and $y \in Y$, and
%
\[ |B(x,y)| = |\lambda(x \otimes y)| \leq \| \lambda \| \| x \otimes y \|_\pi = \| \lambda \| \| x \| \| y \|. \]
%
Thus $B$ is a continuous bilinear map. Conversely, if $B$ is a continuous bilinear map, then setting $\lambda(a) = \langle B, a \rangle$ gives a linear functional such that
%
\[ |\lambda(a)| = |\langle B, a \rangle| \leq \| B \| \| a \|_\pi. \]
%
Thus $\text{Bil}(X,Y)$, or equivalently, $B(X,Y^*)$, is isometric to $(X \otimes_\pi Y)^*$.

\begin{lemma}
    For any $a \in X \otimes Y$,
    %
    \[ \pi(a) = \inf \left\{ \sum_{i = 1}^n \| x_i \| \| y_i \| : a = \sum x_i \otimes y_i \right\}. \]
\end{lemma}
\begin{proof}
    Temporarily, let $\lambda(a)$ denote the right hand side of the equation above. Given any bilinear map $B$, let $\| B \|_\pi$ and $\| B \|_\lambda$ denote the operator norms with respect to the two norms $\pi$ and $\lambda$, viewing $B$ as a functional on $X \otimes Y$. We claim that for any bilinear map $B$, we have $\| B \|_\pi = \| B \|_\lambda$. Given a continuous bilinear map $B$, and any $a \in X \otimes Y$, if $a = \sum x_i \otimes y_i$, then
    %
    \[ |\langle B, a \rangle| \leq \sum_i |B(x_i,y_i)| \leq \| B \|_\pi \sum_i \| x_i \| \| y_i \|. \]
    %
    Taking infima over the right hand side, we find that
    %
    \[ |\langle B, a \rangle| \leq \lambda(a) \| B \|_\pi. \]
    %
    Taking suprema over all $a \in X \otimes Y$, we conclude that $\| B \|_\lambda \leq \| B \|_\pi$. This is equivalent to proving that $\pi \leq \lambda$, since by Hahn-Banach, for any $a \in X \otimes Y$, we can find a continuous bilinear map $B$ with $\| B \|_\pi = 1$ such that $\langle B, a \rangle = \pi(a)$, and then
    %
    \[ \pi(a) = \langle B, a \rangle \leq \| B \|_\lambda \lambda(a) \leq \| B \|_\pi \lambda(a) \leq \lambda(a). \]
    %
    For any $x \in X$ and $y \in Y$, it follows that
    %
    \[ \| x \| \| y \| = \pi(x \otimes y) \leq \lambda(x \otimes y) \leq \| x \| \| y \|. \]
    %
    Thus we have $\lambda(x \otimes y) = \| x \| \| y \|$ for any $x \in X$ and $y \in Y$. But this means that for any bilinear map $B$,
    %
    \[ \| B \|_\pi = \sup_{\| x \| \| y \| \leq 1} |B(x,y)| \leq \sup_{\| x \| \| y \| \leq 1} \| B \|_\lambda \lambda(x \otimes y) = \sup_{\| x \| \| y \| \leq 1} \| B \|_\lambda \| x \| \| y \| = \| B \|_\lambda. \]
    %
    A similar Hahn-Banach argument thus shows that $\lambda \leq \pi$, which gives equality.
\end{proof}

Because $X \otimes_\pi Y$ is the completion of $X \otimes Y$, every element $a \in X \otimes_\pi Y$ can be written as an absolutely convergent infinite series
%
...
\begin{remark}
    Since $X \otimes_\pi Y$ is isometric to $Y \otimes_\pi X$, it follows that the two duals of these spaces are isometric, namely, $B(X,Y^*)$ is isometric to $B(Y,X^*)$. This is of course just the \emph{transpose} map, since if $T: X \to Y^*$ is givn, we get $S: Y \to X^*$ by composing the inclusion $i: Y \to Y^{**}$ with the adjoint of $T$, i.e. $T^*: Y^{**} \to X^*$.
\end{remark}
