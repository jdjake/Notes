...
converge?
\begin{solution}
	If $y = e^x$, then $dy = e^x dx = y dx$, and so
	%
	...
	%
	\[ C_n = \left| \int_{\pi n}^{\pi (n + 1)} \frac{\sin y}{y} \frac{\log y}{\log y + \sin y} \right| \]
	%
	...
	%
	\begin{align*}
		\frac{1}{y} \frac{\log y}{\log y + \sin y} &= \frac{1}{y} \frac{1}{1 + \sin y / \log y}\\
		&= \left( \frac{1}{\pi n} + O(1/n^2) \right) \left(1 - \frac{\sin y}{\log y} + O \left( \frac{1}{(\log n)^2} \right) \right)\\
		&= \left( \frac{1}{\pi n} + O(1/n^2) \right) \left(1 - \frac{\sin y}{\log(\pi n)} + O \left( \frac{1}{(\log n)^2} \right) \right)\\
		&= \left( \frac{1}{\pi n} - \frac{\sin y}{\pi n \log \pi n} + O \left( \frac{1}{n (\log n)^2} \right) \right).
	\end{align*}
	%
	Thus
	%
	\begin{align*}
		C_n &= \int_0^\pi \frac{\sin y}{\pi n} - \frac{(\sin y)^2}{\pi n \log \pi n}\; dy + O \left( \frac{1}{n (\log n)^2} \right)\\
		&= \frac{2}{\pi n} - \frac{1}{2 n \log(\pi n)} + O \left( \frac{1}{n (\log n)^2} \right)\\
		&= C_n' + O \left( \frac{1}{n (\log n)^2} \right).
	\end{align*}
	%
	The quantities $\{ C_n' \}$ are decreasing for suitably large $n$. Thus by the \emph{Leibnitz test}, the alternating series
	%
	\[ \sum_{n = 1}^\infty (-1)^n C_n' \]
	%
	converges. On the other hand, the series
	%
	\[ \sum_{n = 1}^\infty O \left( \frac{1}{n (\log n)^2} \right) \]
	%
	converges \emph{absolutely}, i.e. by applying the Cauchy condensation theorem. But this means that the alternating series
	%
	\[ \sum_{n = 1}^\infty (-1)^n C_n \]
	%
	converges.
...
\question (Fall 2015) Identify all $x\in \R$ such that $\lim_{n\to\infty} \sin(nx)$ exists.
