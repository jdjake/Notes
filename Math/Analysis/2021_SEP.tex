...
\begin{solution}
	Let $V_n$ be the vector space of degree $\leq n$ polynomials. The theory of polynomial interpolation shows that the operator $T: V_n \to \RR^n$ given by
	%
	\[ TP = ( p(x_1), \dots, p(x_{n+1}) ) \]
	%
	is a bijection. Thus $T^{-1}: \RR^n \to V_n$ is well defined. As a linear map between two finite dimensional vector spaces, $T^{-1}$ is continuous from any norm on $\RR^n$, to any norm on $V_n$. If we equip $\RR^n$ with the $l^\infty$ norm, and $V_n$ with the $L^\infty[0,1]$ norm, then the result immediately follows.
\end{solution}
