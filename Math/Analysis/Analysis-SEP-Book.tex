...
\begin{solution}
		Our goal, for each $g \in H^1(\RR^+)$, is to find a function $h \in H^1(\RR)$ so that for any $f \in H^1(\RR)$,
		%
		\[ \langle f,h \rangle-{H^1(\RR)} = \langle Tf, g \rangle_{H^1(\RR^+)}. \]
		%
		Such a $h$ will be unique by general properties of the adjoint, and we can then set $T^* g = h$. We can expand this identity as
		%
		\[ \int_{-\infty}^\infty f(x) h(x) + f'(x) h'(x) = \int_0^\infty f(x) g(x) + f'(x) g'(x). \]
		%
		We will assume that $g \in C^\infty(\RR^+)$, the general case following from a density argument. Now if $f \in C_c^\infty(\RR)$, we may integrate by parts, and write the identity as
		%
		\[ \int_{-\infty}^\infty f(x) [ h(x) - h''(x) ] = \int_0^\infty f(x) [ g(x) - g''(x) ]\; dx - f(0) g'(0). \]
		%
		Since $f$ is an arbitrary compactly supported function, this means that, viewed as distributions,
		%
		\[ h - h'' = I(g - g'') - g'(0) \delta_0, \]
		%
		where $I$ is the operator that takes a function on $[0,\infty)$, and gives the distribution such that
		%
		\[ \langle I(g - g''), f \rangle = \int_0^\infty f(x) [ g(x) - g''(x) ]\; dx. \]
		%
		Thus our goal really turns out to be solving an ordinary differential equation, where the forcing function is a distribution.

		A natural guess is to set $h = I(g)$. For a function $g \in H^1(\RR^+)$, let $I(g)$ be the distribution on $\RR$ given by
		%
		\[ \langle I(g), f \rangle = \int_0^\infty f(x) g(x)\; dx. \]
		%
		Since $g$ is smooth away from $0$, and has a jump discontinuity at $0$ if we define $g(x) = 0$ for $x < 0$, the distributional derivatives of $I(g)$ are simple to compute. If $h_0 = I(g)$, then $h_0' = I(g') + g(0) \delta_0$, and $h_0'' = I(g'') + g'(0) \delta_0 + g(0) \delta_0'$. Thus
		%
		\[ h_0 - h_0'' = I(g - g'') - g'(0) \delta_0 - g(0) \delta_0'. \]
		%
		Thus if $h$ solves the differential equation, and we define $h_1 = h - h_0$, then
		%
		\[ h_1 - h_1'' = [ I(g - g'') - g'(0) \delta_0 ] - [ I(g - g'') - g'(0) \delta_0 - g(0) \delta_0' ] = - g(0) \delta_0'. \]
		%
		So now we only have to solve the differential equation $y - y'' = \delta_0'$, which is comparatively simpler.
...
\begin{parts}
    ...
    \begin{solution}
    	Write
    	% |x - y|
    	\[ \frac{g(x) - g(0)}{x} = \int_{-\infty}^\infty \frac{f(x + y) - f(y)}{x} |y|^{-\alpha}\; dy. \]
    	%
    	If $f$ is Lipschitz, the integrand converges pointwise almost everywhere as $x \to 0$ to $f'(y) / |y|^\alpha$. If we can justify that the dominated convergence theorem holds, then integrating by parts, we have
    	%
    	\begin{align*}
    		g'(0) &= \int_{-\infty}^\infty \frac{f'(y)}{|y|^\alpha}\; dy\\
    		&= \alpha \int_{-\infty}^\infty f(y) \frac{\text{sgn}(y)}{|y|^{\alpha + 1}},
    	\end{align*}
    	%
    	which gives a form of the derivative that might continue to hold when $f$ is not Lipschitz. To justify the dominated convergence theorem, we have that $|f(x + y) - f(y)| \lesssim |x|$ uniformly in $x$, which justifies that the integrand is pointwise bounded by a multiple of $|y|^{-\alpha}$, which is integrable.

    	Now we must prove the general case. Define
    	%
    	\[ T_x f = \int_{-\infty}^\infty \frac{f(x + y) - f(y)}{x |y|^\alpha}\; dy \]
    	%
    	If we can justify that the functions $\{ T_x \}$ are uniformly bounded on some Banach space $X$ containing all compactly supported H\"{o}lder continuous functions of order $\beta$, such that smooth, compactly supported functions are dense in $X$, then an approximation argument will justify that $T_x f \to \alpha \int f(y) \text{sgn}(y) / |y|^{\alpha + 1}$ for all $f \in X$.

    	We will let $X$ be the closed subspace of the Sobolev space $W^{\gamma,p}(\RR)$ of all functions supported in $[-N,N]$ for some $N > 0$, where $\alpha < \gamma < \beta$, and where $p$ is to be fixed later. Since $\gamma < \beta$, all compactly supported, H\"{o}lder continuous functions are in $W^{\gamma,p}(\RR)$.
...
\question (Fall 2004) Let $f_n: [0,1] \to \RR$ be a sequence of continuous functions whose distributional derivatives $f'_n$ belong to $L^2[0,1]$, and $f_n(0) = 0$ for each $n$.
\begin{parts}
	\part Assume that
	%
	\[ \lim_{n \to \infty} \int_0^1 f_n' g \]
	%
	exists for all $g \in L^2[0,1]$. Show that the sequence $\{ f_n \}$ converges uniformly as $n \to \infty$.
	\begin{solution}
	Define linear operators $T_n: L^2[0,1] \to L^2[0,1]$ by setting
	%
	\[ T_n g = \int_0^1 f_n' g \]
	%
	for $g \in L^2[0,1]$. Then $T_n$ is linear and bounded. The assumption implies that $\sup_n |T_n g| < \infty$ for each $g \in L^2[0,1]$. By the uniform boundedness principle, this implies that $\sup_n \| T_n \| < \infty$. But $\| T_n \| = \| f_n' \|_{L^2[0,1]}$, so the derivatives $f_n'$ are uniformly bounded in $L^\infty[0,1]$.

	Using this fact, we can now verify that the sequence $\{ f_n \}$ is bounded and equicontinuous in $C[0,1]$, so that we can apply Arzela-Ascoli. Indeed, we have
	%
	\[ |f_n(x) - f_n(y)| = \left| \int_x^y f_n'(x)\; dx \right| \leq | y - x| \| f_n' \|_{L^\infty[0,1]} \lesssim |y - x|. \]
	%
	Similarily,
	%
	\[ |f_n(x)| = |f_n(x) - f_n(0)| \lesssim |x|, \]
	%
	so we have $\| f_n \|_{L^\infty[0,1]} \lesssim 1$ uniformly in $n$. Thus $\{ f_n \}$ is precompact.

	To prove the sequence is uniformly converging to some value, it thus suffices to prove that $\{ f_n \}$ has only a single limit point. But this follows from the fact that if $f$ is a limit point of $\{ f_n \}$, then for each $\phi \in C_c^\infty((0,1))$,
	%
	\[ \int_0^1 f_n' \phi = - \int_0^1 f_n \phi' \]
	%
	converges as $n \to \infty$ to $\int_0^1 f \phi'$, and this can only be possible of a single function $f \in C[0,1]$ with $f(0) = 0$, since the span of the functions $\{ a + \phi' : \phi \in C_c^\infty(0,1), a \in \RR \}$ is dense in $L^1[0,1]$.
	\end{solution}
	...
	\[ \lim_{n \to \infty} \int_0^1 f_n' g \]
	%
	exists for all $g \in C[0,1]$. Does the sequence $\{ f_n \}$ still necessarily converge uniformly?
	\begin{solution}
		Fix $\phi \in C_c^\infty((0,1))$, and let $f_n(x) = \phi(n x)$. Then $f_n'(x) = n \phi'(n x)$, and $f_n(0) = 0$. If we define $T_n g = \int_0^1 f_n' g$ then $|T_n g| \lesssim \| g \|_{L^\infty[0,1]}$, by virtue of the fact that $\| f_n' \|_{L^1[0,1]} \lesssim \| \phi' \|_{L^\infty[0,1]} \lesssim 1$. Thus the operators $\{ T_n \}$ are uniformly bounded on $C[0,1]$. To prove that $T_n g$ converges for each $g \in C[0,1]$, it thus suffices to prove that $T_n g$ converges for each $g \in C^\infty[0,1]$ by the first fundamental principle of functional analysis. But then
		%
		\[ \int n \phi'(n x) g(x) = - \int \phi(nx) g'(x), \]
		%
		and $\int \phi(nx) g'(x)$ converges to zero by the Lebesgue dominated convergence theorem (since $\phi(nx) \to 0$ pointwise).

		So we have verified $T_n g \to 0$ for each $g \in C[0,1]$. But $\{ f_n \}$ \emph{do not} converge uniformly, because the functions converge pointwise to zero, but $\| f_n \|_{L^\infty[0,1]} = \| \phi \|_{L^\infty} > 0$ for each $n$.
	\end{solution}
\end{parts}
