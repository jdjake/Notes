...
\[ F(x) = \int_a^x f(t)\ dt \]
%
is differentiable on $[a,b]$, and $F'(x) = f(x)$. In this chapter, we attempt to generalize this property to more general classes of functions. The formula above allows us to define a function $F$ for any function $f \in L^1[a,b]$, and we ask under what conditions $F$ is differentiable, and where does $F'(x) = f(x)$. In analyzing this problem, we shall use a fundamental tool in modern analysis, the technique of maximal functions. This technique enables one to generalize convergence results for families of operators which have already been established for a dense family of functions. We summarize the idea of the technique in the next theorem.

\begin{theorem}
  If $T_t$ is a family of linear operators on $L^p(X)$, then we can define the pointwise maximal operator $(T^* f)(x) = \sup_t |T_t f(x)|$. If there exists $q < \infty$ such that $T^*$ is weak $(p,q)$, then the set $\{ f \in L^p(X) : \lim_t T_t f(x) = f(x)\; \text{a.e.} \}$ is closed in $L^p(X)$.
\end{theorem}
\begin{proof}
  Let $f_n$ be a sequence of functions such that $\lim_t (T_t f_n)(x) = f_n(x)$ almost everywhere, which converge in $L^p(X)$ to some function $f$. Then for each $\lambda > 0$, if $\| T^* g \|_{(q,\infty)} \leq C \| g \|_p$, for all $g$, then
  %
  \begin{align*}
    |\{ x: &\limsup_t |T_t f(x) - f(x)| > \lambda \}|\\
    &\leq |\{ x: \limsup_t |T_t(f - f_n)(x) - (f - f_n)(x)| > \lambda \}|\\
    &\leq |\{ x : |T^*(f - f_n)(x)| > \lambda/2 \}| + | \{ x: |(f - f_n)(x)| > \lambda/2 \} |\\
    &\leq \left( \frac{2 C \| f - f_n \|_p}{\lambda} \right)^q + \left( \frac{2 \| f - f_n \|_p}{\lambda} \right)^p
  \end{align*}
  %
  as $n \to \infty$, this quantity tends to zero. Thus for all $\lambda > 0$,
  %
  \[ |\{ x: \limsup_t |T_t f(x) - f(x)| > \lambda \}| = 0 \]
  %
  Taking $\lambda \to 0$ gives that $\limsup_t |T_t f(x) - f(x)| = 0$ almost everywhere, and this was the result we were required to prove.
\end{proof}

The main scenario in which this theorem can be applied is when we can easily establish a pointwise convergence result for suitably regular functions, which are norm dense in the class of all functions we want to obtain pointwise convergence for. While we might not be able to get pointwise convergence everywhere for all such functions, one can use the theorem above to get almost everywhere pointwise convergence. All this requires is to understand the properties of the maximal operator $T^*$, which is often the most difficult part of solving the problem.
...
In order to obtain results for non-continuous functions $f \in L^1[a,b]$, we employ maximal function techniques. Thus we introduce the {\bf Hardy-Littlewood maximal function}
%
\[ Mf(x) = \sup_{x \in B} \frac{1}{|B|} \int_B |f(y)|\ dy \]
%
This function provides an upper bound on the averages of a function $f$. We would like to conclude that $Mf$ is an integrable function, so we can apply dominated convergence-type results, but in fact, $Mf$ is almost never integrable.
...
\begin{example}
  ...
  %
  \[ Mf(x) \geq \fint_{B_{|x|+R}(x)} |f(y)|\; dy \gtrsim \frac{\|f \|_1 - \varepsilon}{(|x| + R)^d} \gtrsim \frac{1}{|x|^d} \]
  %
  But this means, $Mf$ is non-integrable. Note, however, that it appears to be on the `border' of integrability. This is why we introduce the {\it weak $L^1$ inequalities} below. In certain cases, $Mf$ isn't even locally integrable. If $f(x) = 1/|x| \log|x|^2$, then the fact that for $x \geq 0$
  %
  \begin{align*}
  ...
  %
  and so $Mf$ isn't integrable about the origin.
\end{example}

It is easy to see that $M$ is $(\infty,\infty)$ bounded, since the average of a function on a ball is bounded by it's maximum on that ball. It is not $(1,1)$ bounded, but we will soon show it is weak $(1,1)$, which is sufficient to apply maximal function techniques. Furthermore, by the Marcinkiewitz interpolation theorem, this implies $M$ is $(p,p)$ bounded for all $1 < p \leq \infty$. Proving $M$ is weak $(1,1)$ relies on a combinatorial result about coverings of sets, known as the Vitali covering lemma.
...
\begin{theorem}
    ...
    %
    Thus $Mf$ has a weak $(1,1)$ bound $\| Mf \|_{1,\infty} \leq 3^d \| f \|_1$.
\end{theorem}
\begin{proof}
...
%
In particular, $M_U \lesssim M$, which implies $M_U$ is weak $(1,1)$. We therefore conclude that for any locally integrable $f$,
%
