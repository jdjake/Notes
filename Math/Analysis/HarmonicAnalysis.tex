...
\[ f(t) = A + \sum_{n = 1}^\infty B_n \cos(nt) + C_n \sin(kt) \]
%
for $2\pi$ periodic functions on the real line, originated in the theory of waves. Physical considerations lead Fourier and the mathematicians of his time to believe that the partial differential equation defining wave propogation was given by the {\bf wave equation}
%
...
%
Daniel Bernoulli had discovered this before Fourier, but it was Fourier who suggested that these were {\it all} such solutions, provided we take $n \to \infty$, and consider an infinite series. Now given initial conditions $u(0,x) = f(x)$, we find
%
...
Suppose we have a two-dimensional region $D$, with a heat distribution fixed on the buondary, upon which temperature fluctuates in the interior. The equation modelling the evolution of the heat distribution over time is the {\bf heat equation}
%
...
\begin{theorem}
    ...
    \begin{itemize}
        ...
        \item If $f$ is absolutely continuous, then $\widehat{f'}(n) = in \widehat{f}(n)$. In particular, this is true if $f$ is continuously differentiable.

        \item If $f \in L^1(\mathbf{T})$, then $\widehat{f} \in l^\infty(\mathbf{Z})$, with $\| \widehat{f} \|_\infty \leq \| f \|_1$ for all $n$, and vanishes at $\infty$ (the fact that the coefficients vanish at $\infty$ is known as the Riemann Lebesgue lemma).
    \end{itemize}
    %
    All but the last two relations are all easy exercises in manipulating integrals over $\mathbf{T}$. The second last statement can be reworded using the differentiation theorem by proving that if $F(x) = \int_0^x f(x)\; dx$, then $\widehat{f}(n) = in \widehat{F}(n)$. To prove the last statement, we note that
    %
    \[ |\widehat{f}(n)| = \left| \fint_0^{2\pi} f(x) e^{-nix}\; dx \right| \leq \| f \|_\infty  \]
    %
    To prove the last statement, we claim it is true for the characteristic function of an interval $\chi_I$. If $I = [a,b]$, then
    %
    \[ \widehat{\chi_I}(n) = \frac{1}{2\pi} \int_a^b e^{-nit} = \frac{i(e^{-nib} - e^{-nia})}{2 \pi n} = O(1/n) \]
    %
    This implies by finite linearity that the Fourier transform of any step function vanishes at $\infty$. But the sequence of maps $\smash{T_n: f \mapsto \widehat{f}(n)}$ is a family of uniformly bounded linear functionals, and $\lim_{|n| \to \infty} T_nf$ exists for the dense set of step functions, thus it exists for every integrable function and is a continuous linear operator. Since it is equal to zero on the step functions, it equals zero everywhere.
\end{theorem}
...
\begin{example}
    ...
    %
    then it is easy to see that $\widehat{f}(n) = a_n$ for each $n \leq N$, and $\widehat{f}(n) = 0$ otherwise. One very important trigonometric polynomial to harmonic analysis is the {\bf Dirichlet kernel}
    %
    \[ D_N(t) = \sum_{n = -N}^N e^{nit} \]
    %
    which assigns a unit mass to each integer frequency between $-N$ and $N$. By the geometric series summation formula, we may write
    %
    \begin{align*}
        D_N(t) &= 1 + \sum_{n = 1}^N e^{nit} + e^{-nit} = 1 + e^{it} \frac{e^{Nit} - 1}{e^{it} - 1} + e^{-it} \frac{e^{-Nit} - 1}{e^{-it} - 1}\\
...
%
In some sense, $f$ should be able to be approximated by the trigonometric polynomials obtained by truncating this series. However, at this point, we haven't deduced any reason for these sums to converge to $f$ analytically. To understand the convergence, we define the partial sums
%
\[ (S_mf)(x) = \sum_{n = -m}^m \hat{f}(n) e^{inx} \]
%
The reason we sum from $-m$ to $m$ is that if $f$ is real valued, then $\smash{\hat{f}(n) = \overline{\hat{f}(n)}}$, so the complex parts of the equation will cancel out and we find $S_m(f)$ will just be a sum of cosines and sines, as in classical Fourier analysis. An initial hope is that the Fourier series of a function converges pointwise, i.e. that for every $x$,
%
\[ \lim_{m \to \infty} S_m(f)(x) = f(x) \]
%
Perhaps if we're lucky, we'll get uniform convergence as well. Unfortunately, we will show that there are even examples of continuous periodic functions whose partial sums diverge pointwise. However, there are more exotic forms of convergence which are satisfied for all integrable functions.
...
\begin{proof}
    ...
    %
    where $\varepsilon$ is small enough that $P(x) > A > 1$ for $|x| < \delta/2$, $P(x) > 0$ for $\delta/2 \leq |x| < \delta$, and $P(x) < B < 1$ for $|x| \geq \delta$. The series of trigonometric polynomials $P_n(x) = (\varepsilon + \cos x)^n$ satisfy
    %
    \begin{align*}
    ...
    %
    Regardless of the values of $\| f \|_1, f(0)$, and $\delta$, eventually, we find that the right hand side of the equation is positive, which is impossible, completing the proof by contradiction. In general, if $f$ is complex valued, then we may write $f = u + iv$, where
    %
...
%
which we can think of smoothing $g$ by taking it's averages in a neighbourhood of a point, according to a distribution with density $f$, and this is indeed the case if $f$ is positive and integrates to $2\pi$.

\begin{lemma}
	If $f \in L^p(\mathbf{T})$, then $T_hf \to f$ in the $L^p$ norm as $h \to 0$.
\end{lemma}
\begin{proof}
	If $f$ was continuous, then $f_h$ would tend to $f$ in $L^\infty(\mathbf{T})$, and the bound $\| f_h - f \|_p \leq \| f_h - f \|_\infty$ completes the argument in this case. But the family of operators $f \mapsto f_h$ is bounded in $L^p$, implying that the map $f \mapsto \lim_{h \to 0} f_h$ exists for each $h$, and is bounded. Since it is equal to the identity on a dense set, it is equal to the identity everywhere.
\end{proof}
...
\begin{theorem}
    ...
    %
    \begin{itemize}
    	\item If $f \in L^p(\mathbf{T})$ and $g \in L^{p^*}(\mathbf{T})$, then $f * g$ is uniformly continuous.

    	\item If $f \in L^p(\mathbf{T})$ and $g \in L^q(\mathbf{T})$, and if $r$ is defined so that $1/r = 1/p + 1/q - 1$, then $f * g$ is defined almost everywhere, is an element of $L^r(\mathbf{T})$, and satisfies the inequality $\| f * g \|_r \leq \| f \|_p \| g \|_q$. This is known as {\it Young's inequality} for convolutions.
        ...
        \item If $f$ is supported on $E$, and $g$ supported on $F$, then $f * g$ is supported on $E + F$.
    \end{itemize}
\end{theorem}
\begin{proof}
	Suppose $f \in L^p(\mathbf{T})$, and $g \in L^{p^*}(\mathbf{T})$, then
	%
	\begin{align*}
	...
	\end{align*}
	%
	The right hand side is a bound independant of $x$ and converges to zero as $h \to 0$, completing the proof that $f * g$ is uniformly continuous. Applying H\"{o}lder's inequality again gives that $\| f * g \|_\infty \leq \| f \|_p \| g \|_{p^*}$. If $f \in L^p(\mathbf{T})$, and $g \in L^1(\mathbf{T})$, we calculate that
	%
	\begin{align*}
		\| f * g \|_p &= \left( \int \left| \int f(x-y)g(y)\; dy \right|^p\; dx \right)^{1/p} \leq \int \left( \int |f(x-y)g(y)|^p\; dx \right)^{1/p}\; dy\\
		&= \int g(y) \| f \|_p\; dy = \| f \|_p \| g \|_1
	\end{align*}
	%
	Thus $f * g$ is finite almost everywhere. The same is true if $f \in L^1(\mathbf{T})$, and $g \in L^p(\mathbf{T})$. But now consider the operator $T: f \mapsto f * g$, where $g in L^q(\mathbf{T})$. We have proved that
	%
	\[ \| T \|_{(1,q)}, \| T \|_{(q^*,\infty)} \leq \| g \|_q \]
	%
	Therefore, if $q \leq r$, and $\alpha \in [0,1]$ is chosen such that $\alpha/q = 1/r$, then
	%
	\[ \alpha + \frac{1 - \alpha}{q^*} = 1 - \frac{1 - \alpha}{q} = 1 - \frac{1}{q} + \frac{1}{r} \]
	%
	and so, setting $1/p$ to be the value on the right side of the equation, we find that $1/p + 1/q - 1 = 1/r$, and the Riesz interpolation theorem implies that
	%
	\[ \| T \|_{p,r} \leq \| g \|_q \]
	%
	This is precisely Young's inequality. Elementary applications of change of coordinates and Fubini's theorem establish the commutativity and associativity of convolution for functions $f, g \in L^1(\mathbf{T})$. But in general, if we pick sequences $f_1, f_2, \dots$ and $g_1, g_2, \dots$ of integrable functions approaching $f$ and $g$ in the $L^p$ and $L^q$ norms, then $f_n * g_n = g_n * f_n$ approaches both $f * g$ and $g * f$ in the $L^r$ norm, hence $f * g = g * f$. Similarily, if $h_1, h_2, \dots$ is an integrable sequence, with norms chosen appropriately so the integral is well defined, then $f_n * (g_n * h_n) = (f_n * g_n) * h_n$ converges to $f * (g * h) = (f * g) * h$. To obtain the product identity for the Fourier series, we can apply Fubini's theorem to write
    %
    \begin{align*}
    ...
    \end{align*}
    %
    and this is exactly the identity required. The fact that $f * g$ is differentiable, and that the derivative is $f' * g$ when $f$ is differentiable, follows because, using the mean value theorem, that the partial sums
    %
    \[ \frac{f(x + h) - f(x)}{h} \]
    %
    ...
    %
    and this is the required property. If $f = 0$ a.e outside $E$, and $g = 0$ a.e. outside $F$, then $(f * g)(x)$ can be nonzero only when there is a set $G$ of positive measure such that for any $y \in G$, $f(y) \neq 0$ and $g(x-y) \neq 0$. But this means that $E \cap G \cap (x-F)$ has positive measure, so that there is $y \in E$ such that $x-y \in F$, meaning that $x \in E + F$.
\end{proof}

We know that suitably smooth functions have convergent Fourier series. If we want to establish the convergence of the series corresponding to a function $f$, we might want to `smooth' $f$ by convolving it with a function $g$. Provided that $\widehat{g}$ is `close' to 1, the above theorem says that $\widehat{f * g}$ will be close to $\widehat{g}$. If we can establish the convergence properties on the convolution $f * g$, then we can probably obtain results about $f$. The family of functions $g$ we will consider for which $\widehat{g}$ approximates 1 are called {\bf good kernels}. In particular, a good kernel is a sequence of integrable functions $K_n$ on $\mathbf{T}$ bounded in $L^1$ norm, for which
%
\[ \fint_{-\pi}^\pi K_n(t) = 1 \]
%
and for any $\delta > 0$, as $n \to \infty$,
%
\[ \int_{\delta < |t|} |K_n(t)| \to 0 \]
%
so the functions $K_n$ become concentrated at the origin. The $L^1$ boundedness follows from the first condition if we assume the $K_n$ are positive. Basic analysis shows that if $f$ is integrable, then $(f * K_n)(x)$ converges to $f(x)$ at each point $x$ where $f$ is continuous, and converges uniformly to $f$ if $f$ is in $C(\mathbf{T})$. But since the map $f \mapsto f * K_n$ is a bounded family of continuous linear functionals from $L^p(\mathbf{R}^n)$ to itself, and $\lim (f * K_n) = f$ on the dense set of continuous functionals, hence we conclude $\lim (f * K_n) = f$ everywhere. As $n \to \infty$, we find that
%
\[ \widehat{K_n}(m) = \frac{1}{2\pi} \int K_n(t) e^{-mit} \to 1 \]
%
so if $f$ is any function, then the Fourier series of $f * K_n$ converges to the Fourier series of $f$ pointwise.

\section{Countercultural Methods of Summation}

Consider the following manipulation of the truncation of a Fourier series:
%
\[ (S_Nf)(t) = \sum_{n = -N}^N \widehat{f}(n) e^{nit} = \fint f(x) \left( \sum_{n = -N}^N e^{ni(t-x)} \right)\; dx  \]
%
The bracketed part of the final term in the equation is independant of the function $f$, and is therefore key to understanding the behaviour of the sums $S_m$. We call it the Dirichlet kernel, and denote it by $D_N(t-x)$, so that
%
\[ D_N(t) = \sum_{n = -N}^N e^{nit} \]
%
The properties of convolution guarantee that $S_m f = f * D_m$, so that convolutions of the Dirichlet kernel give results about the sums of Fourier series. If $D_n$ was a good kernel, then we would obtain that the partial sums of $S_n$ converge uniformly. This initially seems a good strategy, because $\fint D_N(t) = 1$. However, we find
%
\begin{align*}
...
\end{align*}
%
So the $L^1$ norm of $D_n$ grows, albeit slowly, to $\infty$. This reflects the fact that $D_n$ oscillates very frequently, and also that the pointwise convergence of the Fourier series is much more subtle than that provided by good kernels. One way around this is to consider only functions $f$ which provide a suitable dampening condition on the oscillation of $D_n$. It gives the surprising fact that the local behaviour of a function at a point corresponds to a convergence property on the Fourier series, which is defined in terms of the integral of the function on it's entire domain.
...
\begin{proof}
	...
	%
	\begin{align*}
		(S_N f)(x) - f(x) &= (f * D_N)(x) - f(x) = \fint D_N(y) [f(x-y) - f(x)]\\
		&= \frac{1}{2\pi} \int_{|x| \leq \varepsilon} D_N(y) [f(x-y) - f(x)]\\
		&\ \ \ \ + \frac{1}{2\pi} \int_{|x| > \varepsilon} D_N(y) [f(x-y) - f(x)]
	\end{align*}
	%
	We have
	%
	\[ D_N(y) = \frac{\sin((N + 1/2)y)}{\sin(y/2)} = \frac{\sin(Ny) \cos(y/2) + \cos(Ny) \sin(y/2)}{\sin(y/2)} \]
	%
	The function
	%
	\[ g(y) = \frac{f(x-y) - f(x)}{y} \]
	%
	is integrable because it is bounded around the origin, and bounded by the sum of two integrable functions away from the origin. Thus
	%
	\[ \int D_N(y)[f(x-y) - f(x)] = \sin(Ny) \frac{yg(y) \cos(y/2)}{\sin(y/2)} + \cos(Ny) y g(y) \]
	%
	The functions $y g(y)$ and $yg(y)\cos(y/2)/\sin(y/2)$ are integrable, so the Riemann Lebesgue lemma implies that this integral vanishes as $N \to \infty$.
\end{proof}

We now interpret our convergence of series according to a different kernel, we do get a family of good kernels, and therefore we obtain pointwise convergence for suitable reinterpretations of partial sums. One reason why the Dirichlet kernel fails to be a good kernel is that the Fourier coefficients of the kernel have a sharp drop -- the coefficients are either equal to one or to zero. We find if we mollify the coefficients of this kernel to have a slightly smoother drop, then we will obtain a family of good kernels.

First, we introduce the summation methods which correspond to a sufficiently smooth kernel. The standard method of summation suffices for much of analysis. Given a sequence $a_0, a_1, \dots$, we define the infinite sum as the limit of partial sums.
%
...
\begin{theorem}
    A Cesaro summable sequence is Abel summable to the same value.
\end{theorem}
\begin{proof}
...
%
and therefore links the study of trigonometric series and the theory of analytic functions. Furthermore, it is very important to solving the problem of heat distribution on a disk. We will explore both these facts later in these notes.
...
\section{Application: Tchebychev Polynomials}
...
%
\[ \left| f(u) - \sum_{n = 1}^N a_n T_n(u) \right| < \varepsilon \]
%
and so we have uniformly approximated $f$ by a polynomial.
\end{proof}

\section{Application: Harmonic Functions on $\mathbf{D}$}
...
Before we end our discussion of the basic theory of Fourier series, we briefly discuss how the genesis of inner product spaces fits into harmonic analysis. Using the theory of inner product spaces, we can show that the Fourier series of a {\it square integrable} function converges in a very pleasant manner. It is the closest we can get to showing the correspondence between functions and their Fourier series. Recall that for any measure space $X$, $L^2(X)$ forms a Hilbert space under the inner product
%
\[ (f,g) = \int f(x) \overline{g(x)}\; dx \]
%
The theory of Hilbert spaces essentially gives us the entire convergence theory in the $L^2$ norm for free. Since the trigonometric polynomials are dense in $L^2(\mathbf{T})$, and the exponentials $e_n(x) = e^{nix}$ are orthonormal, in the sense that $(e_n,e_m) = \delta_{nm}$, we conclude that the exponentials are actually an orthonormal basis for the Hilbert space $L^2(\mathbf{T})$. The finite sums $S_N f$ are actually projections of $f$ onto the closed subspace of $L^2(\mathbf{T})$ consisting of the trigonometric polynomials of degree bounded by $N$. Thus we obtain that if $a_n$ is any family of scalars, for $|n| \leq N$, then $\| f - \sum a_n e_n \|_2 \geq \| f - S_N f \|_2$.
%
We also get Parseval's identity
%
\[ \| f \|_2^2 = \sum_{n \in \mathbf{Z}} \left| \widehat{f}(n) \right|^2 \]
%
Thus taking the Fourier series of a function establishes an isometry between $L^2(\mathbf{T})$ and $l^2(\mathbf{Z})$. The $L^2$ norm is where the best correspondence between a function and its series occurs.
...
\begin{example}
	...
	%
	\[ |A_r f| = \left| \sum r^{|n|} \widehat{f}(n) e^{nit} \right| \leq \| f \|_1 \sum r^{|n|} = \| f \|_1 \left( 1 + \frac{2}{1 - r} \right) \]
	%
...
\begin{theorem}
	If $f \in C^1_c(\mathbf{R}^n)$, then $f$ has a partial derivative $D_k f$ in $L^1$.
\end{theorem}
\begin{proof}
...
In real analysis, it is proved that if $\{ K_\delta \}$ is a good kernel, i.e. elements of $L^1(\mathbf{R}^n)$ satisfying $\int K_\delta(x)\; dx = 1$ and such that for any $\varepsilon$,
%
\[ \lim_{\delta \to 0} \int_{|x| > \varepsilon} |K_\delta(x)| = 0 \]
%
then $f * K_\delta$ converges to $f$ in the $L^1$ norm as $\delta \to 0$. We have this result even for more general norms. We say a subspace $X$ of $L^1(\mathbf{T})$ is a homogenous space of functions if it is closed under translation of the domain, and is equipped with a translation invariant norm $\| \cdot \|$ which is lower bounded up to a constant by the $L^1$ norm.
...
\begin{example}
	...
	%
	so this functional is a tempered distribution, denoted by $\text{p.v.}(1/x)$. It is intimately connected to the theory of the Hilbert transform. 
\end{example}
...
\begin{theorem}
	If $\Lambda$ is tempered, then
	%
	\[ (P(D) \Lambda)^\ft = P(-2 \pi i \xi) \widehat{\Lambda}\ \ \ \ \ (P(-2 \pi i \xi) \Lambda)^\ft = P(D) \widehat{f} \]
\end{theorem}
\begin{proof}
	We calculate
	%
...
\begin{example}
	...
	%
	as $h \to 0$, the values of the function in the integral converge pointwise to $e(-x \cdot \xi)$, and so the continuity follows by the dominated convergence theorem. 
\end{example}
...
\begin{example}
	It is often useful to know the Fourier transform of the radial functions $f(x) = 1/|x|^\alpha$ on $\mathbf{R}^d$, for $\alpha < d$, so that the function is locally integrable. Since $f$ satisfies $f(tx) = t^{-\alpha} f(x)$, the multiplicative character, and the Haar measure $dt/t$ become very useful in the analysis of this function. We calculate that the multiplicative convolution of this character against a Gaussian gives, by a change of variables, for $\alpha > 0$,
    %
...
\begin{example}
	...
	%
	But this means that $f \in L^\infty(\mathbf{R}^n)$, for if there is a set $E$ of positive measure where $|f| \geq M$, we can find $\widehat{\phi}$ with $\widehat{\phi} = 1$ on $E$ and with $\| \widehat{\phi} \|_2 = |E| + \varepsilon$, and then $\| f \widehat{\phi} \|_2 \geq M \| \widehat{\phi} \|_2$. Note that over $L^2(\mathbf{T})$, the only convolution operators are given by the distributions given by a Fourier series with bounded coefficients.
\end{example}
...
The study of a function $f$ defined on the real line can often be understood by extending it's definition holomorphically to the complex plane. Here we will extend this tool, establishing that a large family of functions $f$ defined on $\mathbf{R}^n$ can be understood by looking at a {\it harmonic} function on the upper half plane $\mathbf{H}^{n+1}$, which approximates $f$ at it's boundary. This is a form of the Dirichlet problem, which asks, given a domain and a function on the domain's boundary, to find a function harmonic on the interior of the domain which `agrees' with the function on the boundary, in one of several senses. As we saw in our study of harmonic functions on the disk in the study of Fourier series, we can study such harmonic functions by convolving $f$ with an appropriate approximation to the identity which makes the function harmonic in the plane. In this case, we shall use the Poisson kernel for the upper half plane.

\begin{theorem}
	If $f \in L^p(\mathbf{R}^n)$, for $1 \leq p \leq \infty$, and $u(x,y) = (f * P_y)(x)$, where
	%
	\[ P_y(x) = \frac{\Gamma((n+1)/2)}{\pi^{(n+1)/2}} \frac{1}{(1 + |x|^2)^{(n+1)/2}} \]
	%
	then $u$ is harmonic in the upper half plane, $u(x,y) \to f(x)$ for almost every $x$, and $u(\cdot,y)$ converges to $f$ in $L^p$ as $y \to 0$, with $\| u(\cdot,y) \|_p \leq \| f \|_p$. If, instead, $f$ is a continuous and bounded function, then $u(\cdot,y)$ converges to $f$ locally uniformly as $y \to 0$.
\end{theorem}
\begin{proof}
	...
	%
	\[ \frac{\partial^2 u}{\partial x^2}(x,y) = (f * P_y'')(x)\ \ \ \ \ \frac{\partial^2 u}{\partial y^2} = (f * ) \]
\end{proof}
...
The most basic way to interpolate is using the notion of convexity. Given two inequalities $A_0 \leq B_0$ and $A_1 \leq B_1$, for any parameter $0 \leq \theta \leq 1$, if we define the additive weighted averages
%
...
\begin{example}
    If $U$ is merely closed, then (ii) need not hold. For instance, in $\mathbf{R}$, take $U = \alpha \mathbf{Z}$, and $V = \mathbf{Z}$, where $\alpha$ is an irrational number. Then $U + V = \alpha \mathbf{Z} + \mathbf{Z}$ is dense in $\mathbf{R}$, and is hence not closed.
\end{example}
