...
Consider the Hermite operator $L = \partial_x^2 - x^2$. The $L$ is diagonalized by the Hermite operators. Define $H_n = e^{x^2} (-\partial_x)^n \{ e^{-x^2} \}$, the Hermite polynomial of degree $n$. These polynomials have coefficient expansions of the form

%
\[ H_n(x) = n! \sum_{k \leq n/2} \frac{(-1)^k}{(n - 2k)! k!} (2x)^{n - 2k}. \]
%
If we define $\tilde{H}_n = e^{-x^2} H_n$, then $\{ \tilde{H}_n \}$ are an orthogonal basis for $L^2(\RR)$, with
%
\[ \| \tilde{H}_n \|_{L^2(\RR)} = 2^{n/2} (n!)^{1/2} \pi^{1/4}, \]
%
i.e. so that the $L^2$ norm of $\tilde{H}_n$ grows on the order of $n^{n/2}$. We then have
%
%\[ (0.51 n)^n \lesssim \| H_n e^{-x^2/2} \|_{L^2(\RR)} \lesssim ( 0.86 )^n n^{n/2 + O(1)} \]
% log(A) = (n/2) log(2) + (1/2) [ n log n - n + O(ln n) ] + C
% A = n^{n/2} e^{-0.35 n}
%
\[ L \tilde{H}_n = (2 n) \tilde{H}_n. \]
%
Having diagonalized the operator, to study the heat equation $\partial_t + L = 0$, we can use symbolic calculus to define the heat propogators $e^{-tL}$ with $e^{-tL} \tilde{H}_n = e^{-2tn} \tilde{H}_n$. The kernel can be written (formally at least), as
%
\[ e^{-tL}(x,y) = \sum_{n = 0}^\infty \frac{e^{-2tn}}{2^{n/2} (n!)^{1/2} \pi^{1/4}} H_n(x) H_n(y) \exp \left( - \frac{x^2 + y^2}{2} \right). \]
%
Using the Taylor's formula for $e^{-(x-z)^2}$, one can prove that
%
\[ \sum_{n = 0}^\infty \frac{H_n(x)}{n!} z^n = e^{2xz - z^2} \]
% nth derivative of e^{- (x - z)^2} in the z variable
%   If the nth derivative in the z variable is F_n(x,z) e^{-x^2} e^{2xz - z^2}
% then F_0 = 1
% and F_{n+1} = d_z F_n(x,z) - 2 (x - z) F_n
%             = d_z [ d_z F_{n-1} - 2 (x - z) F_{n-1}  ]
%             = d_z^2 F_{n-1} - 2 (x - z) d_z F_{n-1} + 2 F_{n-1}
%             = 




% Thus F_{n+1}(x,0) = d_z F_n(x,0) - 2x F_n(x,0)
% F_1(x,z) = 2z - 2x
%   F_1(x,0) = - 2x
%
% F_2(x,z) = 2 - 4xz + 4x^2
...
% then F_{n+1} = d_z F_n - 2 (x - z) F_n
% e^{2xz - z^2 - x^2}

then $L$ is diagonalized by the functions $\{ H_n e^{-x^2/2} \}$, i.e. $L\{ H_n e^{-x^2/2} \} = 2n H_n e^{-x^2/2}$.


Mehler's identity specifies that if we define an operator
%
\[ T_tf(x) = \int (1 - t^2)^{-1/2} \exp \left( - \frac{t^2 (x^2 + y^2) - 2 t xy}{1 - t^2} \right) f(y)\; dy, \]
%
then, , we have
%
\[ T_t H_n \]
...
%
\[ K(x,y,t) = (1 - t^2)^{-1/2} \exp \left( - \frac{t^2(x^2 + y^2) - 2 t x y}{1 - t^2} \right), \]
%
then $K$ has an expansion in Hermite polynomials, i.e.
%
\[ K(x,y,t) = \sum_{n = 0}^\infty \frac{(t / 2)^n}{n!} H_n(x) H_n(y). \]
%
