...
Let $T$ be a Fourier integral operator from an open set $U \subset \RR^n$ to $V \subset \RR^m$ by an equation of the form
%
\[ Tf (x) = \int a(x,y,\theta) e^{2 \pi i \phi(x,y,\theta)} f(y)\; dy\; d\theta, \]
%
where $\phi$ is a phase function with the property that for any $(x,y,\theta) \in \msupp(a)$ such that
...
. In other words, the canonical relation $\mathcal{C}$ of $T$ has the property that
%
\[ \mathcal{C} \subset  \]

 from a domain $X$ to a domain $Y$ with canonical relation
%
\[ \mathcal{C} \subset (T^* X - \{ 0 \}) \times (T^* Y - \{ 0 \}). \]
%
...
\end{itemize}
%
Thus $T$ preserves smoothness, in a certain sense.

Let's see a more quantitative version of this phenomenon, namely that these are precisely the family of FIOs to which we can apply Littlewood Paley theory. It will be easiest to see this locally, and assume that $X$ and $Y$ are open subsets of $\RR^n$ and $\RR^m$, and that $T$ can be expressed in the form
%
\[ Tf(x) = \int_Y \int_{\RR^p} a(x,y,\theta) e^{2 \pi i \phi(x,y,\theta)} f(y)\; d\xi\; dy, \]
%
where $\text{supp}_x(a)$ and $\text{supp}_y(a)$ are compact, and $a$ is a symbol of order $\mu$ in the $\theta$ variable. For each integer $k$, we consider the Littlewood-Paley projection operators
%
\[ L_k f(x) = \int \eta( \xi / 2^k ) \widehat{f}(\xi) e^{2 \pi i \xi \cdot x}\; d\xi. \]
%
We claim that, fuzzily speaking, $T$ acts separately on each Littlewood-Paley projection operator.

\begin{proof}
	Assume $k_1 > k_2$. We calculate that the operator $L_{k_1} \circ T \circ L_{k_2}$ has a kernel $K_{k_1,k_2}$ of the form
	%
	\begin{align*}
...
Let $T$ be a Fourier integral operator from a manifold $M$ to a manifold $N$, with a canonical relation $\mathcal{C} \subset$
