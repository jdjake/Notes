...
%
For each $m \in C^\infty(\RR^d)$ such that $m$ and all of it's derivatives are slowly increasing, we can define a bounded operator $m(X): \mathcal{S}(\RR^d) \to \mathcal{S}(\RR^d)$ by setting
%
\[ m(X) f(x) = m(x) f(x). \]
%
...
\begin{proof}
    ...
    %
    Since $\chi \in \DD(\RR^d)$, it follows by the Paley-Wiener theorem, taking the inverse Fourier transform that $L \Phi_0 = \delta - w$, where $w$ is an entire analytic function of at most polynomial increase. The Cauchy-Kovalevskaya theorem (i.e. solving the equation by expanding out power series), we can find an entire analytic function $u$ of at most exponential increase such that $Lu = w$. Then $\Phi = \Phi_0 + u$ is a fundamental solution for $L$.
\end{proof}

The theory of pseudodifferential operators was mainly used to generalize the kinds of construction that occured in this proof. A \emph{(left) parametrix} for a linear differential operator $L$ with smooth coefficients on a domain $\Omega$ is an operator $S: \DD(\Omega) \to \DD(\Omega)^*$ such that $1 - S \circ L$ is a \emph{smoothing operator}. We think of $S$ as given an `approximate inverse' for the operator $T$. The existence of a regular parametrix, which will be justified by the theory of pseudodifferential operators, is quite important in the theory of differential equations. In particular, it proves that certain differential operators are \emph{hypoelliptic}, i.e. that if $u \in \DD(\Omega)$, then $\singsupp(Lu) = \singsupp(u)$; it suffices to show $\singsupp(u) \subset \singsupp(Lu)$, since the opposite inclusion is trivial.

\begin{theorem}
    Let $L = \sum c_\alpha D^\alpha$ is a linear partial differential operator with smooth coefficients. If $L$ has a very regular left parametrix $S$, then $L$ is hypoelliptic.
\end{theorem}
\begin{proof}
...
The pseudodifferential operators we will study are \emph{microlocal}, i.e. they do not expand wavefront sets when applied to distributions. Thus it follows that if an operator $L$ has a left parametrix $S$ which is a pseudodifferential operator, then for any distribution $u$, we will have the stronger equation
%
\[ \text{WF}(Lu) = \text{WF}(u). \]
%
We shall find that it is precisely the \emph{elliptic} differential operators that have pseudodifferential parametrices, and thus elliptic differential operators preserve wavefront sets.

From a more general perspective, the process of constructing functions of a family of linear operators is a special case of the \emph{spectral calculus}, whose goal, for a suitable family of normal operators $A$, produces an isomorphism of $A$ with an algebra of functions on some space $X$, called the \emph{spectrum} of $A$. A natural question is whether we can obtain such a calculus for the family of all operators formed from combinations of the multiplication \emph{and} differentiation operators. This would, in particular, enable us to analyze arbitrary \emph{non constant coefficient} linear differential operators. However, we quickly see that such a theory would not quite work in as standard a way as the spectral calculus provides, because the families of operators $\{ X^\alpha \}$ and $\{ D^\alpha \}$ do \emph{not} commute with one another, i.e. the chain rule implies that
%
\[ [D^i,X^i] = D^i X^i - X^i D^i = 1. \]
%
The key thing to notice, that will enable us to obtain a partial spectral theory of such operators, is that these operators commute 'up to lower order terms', i.e. if we think of $X^\alpha$ and $D^\alpha$ as being operators of \emph{order $|\alpha|$}, then $[D^\alpha,X^\beta]$ is equal to zero, \emph{modulo terms of order $|\alpha| + |\beta| - 1$}. The fact that these operators commute 'up to lower order terms' is what will enable us to obtain a well defined functional calculus combining these two families, the \emph{calculus of pseudodifferential operators}.
...
Fix an open set $\Omega \subset \RR^d$. In the most general setting, for any distribution $a(x,\xi)$ which is tempered in the $\xi$ variable, i.e. any distribution $a \in \DD(\Omega_x)^* \CT \SW(\RR^d_\xi)^*$, we can define an operator $a(x,D): \DD(\Omega) \to \DD(\Omega)^*$ such that for $f,g \in \DD(\Omega)$,
%
\[ \langle a(x,D) f, g \rangle = \int a(x,\xi) \widehat{f}(\xi) e^{2 \pi i \xi \cdot x} g(x)\; dx\; d\xi. \]
%
We call any operator $T$ which can be given in the form $a(x,D)$ a \emph{pseudodifferential operator}. The symbol $a$ is determined uniquely by the operator $T$, since the action of $a(x,D)$ determines the behaviour of $a$ on $\DD(\Omega_x) \otimes \mathcal{S}(\RR^d_\xi)$, which is dense in $\DD(\Omega_x) \CT \mathcal{S}(\RR^d_\xi)$. For any set $S \subset \DD(\Omega_x)^* \CT \SW(\RR^d_\xi)^*$, the notation $\text{Op}(S)$ is often used to denote the family of all pseudodifferential operators defined by an element of $S$.
...
Any pseudodifferential operator $T = a(x,D)$ is continuous from $\mathcal{S}(\RR^d)$ to $\DD(\RR^d)^*$, and has kernel
%
...
%
converging unconditionally, in the sense of distributions, and then
%
\[ a(x,D) \phi = \sum_{i = 1}^\infty u_i \cdot (v_i * \phi). \]
%
Thus $a(x,D)$ is a sum of convolutions and multiplication operators.
...
\begin{itemize}
    ...
    \item If $a \in \loc{\mathcal{S}^t}(\Omega \times \RR^d)$, then $a(x,D)$ extends to a continuous operator from $\EC(\Omega)^*$ to $\DD(\Omega)^*$ and from $\DD(\Omega)$ to $\EC(\Omega)$.
    ...
    Conversely, let $T: \DD(\Omega) \to \EC(\Omega)^*$ be \emph{any} proper operator. Then it's kernel $K$ lies in $\DD(\Omega_x)^* \CT \EC(\Omega_y)^*$. Thus we can define $a \in \DD(\Omega_x)^* \CT \SW(\RR^d_\xi)^*$ by setting
    %
...
There are two varieties of the theory of pseudodifferential operators, roughly analogous to one another. The first, which works best when considering pseudodifferential operators on $\RR^d$, and works with operators specified by symbols $a \in \mathcal{S}^t(\RR^d \times \RR^d)$, and thus works with operators on Schwartz functions. However, due to our desire to work in different systems of coordinates / on manifolds, we choose to describe the theory of operators described by \emph{local symbols} $a \in \loc{\mathcal{S}^t}(\Omega \times \RR^d)$.

Throughout this chapter, we fix an open set $\Omega \subset \RR^d$. We consider any symbol $a \in \loc{\mathcal{S}^t}(\Omega \times \RR^d)$, i.e. a smooth function $a: \Omega_x \times \RR^d_\xi \to \CC$ such that for any integers $n$ and $m$,
%
\[ |\nabla_x^n \nabla_\xi^m a(x,\xi)| \lesssim_{n,m} \langle \xi \rangle^{t - m}, \]
%
where the implicit constant is locally uniform in $x$. From this symbol, we can define a continuous operator $T_a: \DD(\Omega) \to \EC(\Omega)$ by setting
%
\[ T_a f(x) = \int a(x,\xi) e^{2 \pi i \xi \cdot x} \widehat{f}(\xi)\; d\xi, \]
%
where the integral can now be interpreted in the usual Riemann / Lebesgue sense, and gives a smooth function since $a \in C^\infty(\Omega_x, \SW(\RR^d_\xi)^*)$. We then say $T_a$ is a \emph{pseudodifferential operator of order $t$}.

The kernel of a pseudodifferential operator given by a symbol $a(x,\xi)$ of the class above is of the form
%
\[ K_a(x,y) = \int a(x,\xi) e^{2 \pi i \xi \cdot (x - y)}\; d\xi, \]
%
where the integral must be interpreted as defining an oscillatory integral distribution. In particular, we know from that discussion that
%
\[ \text{WF}(K_a) \subset \{ (x,x;\xi,\xi) : x, \xi \in \RR^d \}. \]
%
The microlocal analysis of distributions implies the existence of a continuous extension $T_a: \EC(\Omega) \to \DD(\Omega)^*$, and we find that $\text{WF}(T_au) \subset \text{WF}(u)$. This is the first instance of the \emph{microlocal nature} of pseudodifferential operators; these operators roughly preserve the location of the mass and frequency support of a function, but with some additional `fuzz' that is usually neglible to the problem, but must be managed.
...
\begin{theorem}
    ...
    %
    \[ |\nabla^n_x \nabla^m_z k_a(x,z)| \lesssim_{n,m,N} |z|^{-t-d-m-N}, \]
    %
...
\begin{proof}
    ...
    %
    \[ |\nabla^n_x \nabla^m_z k_a(x,z)| \lesssim_N 1 / |z|^N, \]
    %
    ...
    %
    where the convergence is in the sense of tempered distributions. We claim that for any $i$, and any $n,m,N \geq 0$,
    %
    \[ |\nabla^n_x \nabla^m_z k_i(x,z)| \lesssim_{n,m,N} |z|^{-N} 2^{i(t + d + m - N)}, \]
    %
    ...
    %
    \[ k_i(x,z) = \int \rho(\xi / 2^i) a(x,\xi) e^{2 \pi i \xi \cdot z}\; d\xi. \]
    %
    These bounds, if $N$ is taken large enough, imply that the sum $k = \sum k_i$ converges uniformly on any set of the form $\{ (x,z): x \in K, |z| > \varepsilon \}$, where $K \subset \Omega$ is compact. Moreover, summing up these bounds for sufficiently large $N$ gives the result for $|z| \geq 1$. For $0 < |z| \leq 1$, we break the sum into two parts, i.e. writing
    %
...
\begin{lemma}
    ...
    %
    Let $k$ be the convolution kernel for $K$. Suppose that for some $t$, and any non-negative integers $n,m$, and $N$, the kernel $k$ satisfies the growth condition
    %
    \[ |\nabla^n_x \nabla^m_z k(x,z)| \lesssim |z|^{-t-d-m-N} \]
    %
    ...
    \begin{itemize}
        ...
        %
        \[ \int k(x,z) \phi(x-z)\; dz = \lim_{\varepsilon \to 0} \int_{|z| > \varepsilon} k(x,z) \phi(x-z)\; dz, \]
        %
        ...
        %
        \[ \left| \int D^\alpha_x k(x,z) \phi(z/R)\; dz \right| \lesssim_{\phi,\alpha} R^t, \]
        %
...
\begin{theorem}
    ...
    Then for any $N > 0$ and $\lambda > 0$, we can write $T_a \{ f e^{2 \pi i \lambda \phi} \}(x)$ as
    %
    \begin{align*}
        e^{2 \pi i \lambda \phi(x)} \left\{ \sum_{|\beta| < N} \frac{1}{\beta! \cdot (2 \pi i)^{\beta}} \cdot D_\xi^\beta a(x,\lambda \nabla \phi(x)) \left. D^\beta_y \{ e^{2 \pi i \lambda r_x} f \} \right|_{y = x} + R_N(x,\lambda) \right\},
    \end{align*}
    %
    where $\lambda^{t - \lceil N/2 \rceil} R_N \in L^\infty(\RR^d \times (0,\infty))$. In particular, for $N = 3$, we find that
    %
    \begin{align*}
        T_a & \{ f e^{2 \pi i \lambda \phi} \}(x)\\
        &= e^{2 \pi i \lambda \phi(x)} \Bigg\{ a(x, \lambda \nabla \phi(x)) f(x)\\
            &\quad\quad\quad\quad + \frac{1}{2\pi i} \sum_{k = 1}^d (D^k_\xi a)(x,\lambda \nabla \phi(x)) \cdot (D^k_x f)(x)\\
            &\quad\quad\quad\quad - \frac{i \lambda}{4 \pi} \sum_{|\beta| = 2} (D^\beta_\xi a)(x,\lambda \nabla \phi(x)) (D^\beta_x \phi)(x) f(x)\\
            &\quad\quad\quad\quad\quad\quad\quad + O(\lambda^{t - 2}). \Bigg\}
    \end{align*}
    ...
    %
    \[ T_a \{ f e^{2 \pi i \lambda \phi} \}(x) \sim \sum_{\beta} \frac{1}{\beta! (2 \pi i )^\beta} D_\xi^\beta a(x,\lambda \xi) \cdot D^\beta_x f(x). \]
\end{theorem}
\begin{proof}
    We write
    ...
    \[ R_{N,\lambda}(x,\xi) = \sum_{|\beta| = N} \xi^\beta \frac{N}{\beta!} \int_0^1 (1 - t)^{N-1} \lambda^N D^\beta_\xi a(x, \lambda \nabla \phi(x) + t \lambda \xi)\; dt. \]
    %
    We only care about this formula when $|\xi| \sim 1$, and differentiating this formula gives that, for such $\xi$,
    %
    \[ |D_\xi^\alpha R_{N,\lambda}(x,\xi)| \lesssim_\alpha \lambda^{t - |\alpha|}, \]
    %
    and $D_\xi^\alpha R_{N,\lambda}(x,0) = 0$ for all $|\alpha| < N$. Stationary phase thus implies that
    %
    ...
    \begin{align*}
        ...
        &= \lambda^{-(d + \beta)} (2 \pi i)^{-\beta} \left. D_y^\beta \{ e^{2 \pi i \lambda r_x(y)} f(y) \} \right|_{y = x}
    \end{align*}
    % 
...
Most of the theory of pseudodifferential operators works \emph{modulo smoothing operators}, which is mostly fine in harmonic analysis, where we attempt to find localized estimates, and smoothing operators naturally satisfy all the necessary localized estimates we might be interested in. In particular, this often makes the theory more flexible. For instance, one interesting operator to study is the square root of the Laplacian, which is the pseudodifferential operator with symbol $a(x,\xi) = 2 \pi |\xi|$. The fact that $a$ is not smooth near the origin means that $a(x,D)$ does not quite fit the theory of pseudodifferential operators given above. Nonetheless, for any bump function $\rho \in \DD(\RR^d)$ equal to one in a neighborhood of the origin, the symbol $\tilde{a}(x,\xi) = 2 \pi |\xi| (1 - \rho(\xi))$ \emph{is} of order one, and since $a(x,\xi) - \tilde{a}(x,\xi)$ is compactly supported in the $\xi$ variable, it follows that $a(x,D) - \tilde{a}(x,D)$ is a smoothing operator. To remedy this situation, we define $\dot{\mathcal{S}}^t$ to be the space of all distributional symbols which differ from an element of $\loc{S}^t$ by a distributional symbol inducing a smoothing operator. Then $a(x,\xi) = 2 \pi |\xi|$ is a symbol of order one in $\dot{\mathcal{S}}^1$. One important application of this is to the theory of pseudodifferential operators defined in terms of homogeneous functions, of which the square root of the Laplacian is a special example, since homogeneous functions often fail to be smooth at the origin.

Another virtue of the theory modulo smoothing operators is that asymptotics become exact, e.g. a symbol is uniquely defined by an asymptotic formula
%
\[ a(x,\xi) \sim \sum_{k = 0}^\infty a_k(x,\xi) \]
%
where the orders of $\{ a_k \}$ tend to $-\infty$, since any two choices differs by a smoothing operator.
...
%
\[ C_1 \langle \xi \rangle^t \leq |a(x,\xi)| \leq C_2 \langle \xi \rangle^t. \]
%
...
We can use similar asymptotic tools to construct formal fractional powers of an elliptic pseudodifferential operator. For simplicity, we assume the range of the symbols involved avoid the half line $\{ x : x < 0 \}$ so that we can use the principal branches of the fractional powers, but the technique works with any other choice of branch.

\begin{theorem}
    Let $a \in \dot{S}^t$ be an elliptic symbol, and suppose that
    %
    \[ | \text{arg}(a(x,\xi)) - \pi | \gtrsim 1 \]
    %
    for $\xi \gtrsim 1$, where the implicit constants are locally uniform in $x$. Then for any pair of positive integers $p$ and $q$, there exists a unique symbol equivalence class $b \in \dot{S}^{t(p/q)}$ with principal symbol $a(x,\xi)^{p/q}$, such that $a(x,D)^p = b(x,D)^q$, modulo smoothing operators.
\end{theorem}
\begin{proof}
...
\begin{example}
    ...
    %
    \[ |D^\alpha_x D^\beta_y K_a(x,y+n)| \lesssim_N |n|^{-N}. \]
    %
    This implies that
    %
    \[ (x,y) \mapsto \sum_{n \neq 0} K_a(x,y+n) \]
    ...
    To give a particular example of this kind of construction, consider the operator $T: \EC(\TT) \to \EC(\TT)$ given by
    %
...
%
The characteristic set is closed and conic, and $T$ is elliptic on the complement of $\Char(T)$. Thus it follows that $u \in \DD(\Omega)^*$, and $Tu \in \EC(\Omega)$, then $\text{WF}(u) \subset \Char(T)$.
...
We can also consider pseudodifferential operators valued in linear maps between two vector bundles on a space. As examples, the exterior derivative $\Omega^n(M) \to \Omega^{n+1}(M)$ is a pseudodifferential operator with symbol associating with each $(x,\xi) \in \Lambda^n(M)$ the value $i \cdot \xi$. We thus see that if $n = 0$, the exterior derivative is elliptic, but if $n > 0$, the exterior derivative is not elliptic TODO: Treves Example 7.3.
...
%
The wave-front set contained in $\Lambda_\phi = \{ (x,\nabla_x \phi(x,\theta)) : \nabla_\xi \phi(x) = 0 \}$, and, provided $\phi$ is non-degenerate, $\Lambda_\phi$ is a Lagrangian manifold. Thus, given a symplectomorphism $\Phi: T^* \RR^n \to T^* \RR^n$, if we can find a phase $\phi: \RR^n \times \RR^n \to \RR^n$ such that $\Lambda_\Phi = \Lambda_\phi$, then, modulo $C^\infty$ kernels, we would expect to find a symbol $a$ such that
%
