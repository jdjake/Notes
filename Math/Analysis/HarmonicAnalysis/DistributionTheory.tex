...
\begin{theorem}
    Let $u$ be a homogeneous distribution of order $a \in \CC - \{ -n, -(n+1), \dots \}$. Then $u$ has a unique extension to a distribution $E(u)$ on $\RR^n$, such that for any homogeneous polynomial $P$, $E(Pu) = P E(u)$, and if $u$ is not a distribution of order $1-n$, $E(\partial_i u) = \partial_i E(u)$. Moreover, the map $u \mapsto E(u)$ is continuous from $\DD(\RR^n - \{ 0 \})^*$ to $\DD(\RR^n)^*$.
\end{theorem}
\begin{proof}
...
\begin{example}
    ...
    %
    In other words, $\nabla \Phi$ is given by integrating against $\nabla \Phi$ away from the origin. Again, we approximate away from the origin by a $\varepsilon$ ball, integrate by parts, and calculate that
    %
    \begin{align*}
...
An alternate approach to computing the fundamental solution above is to take the Fourier transform of the equation above, giving the equation
%
\[ - 4 \pi^2 |\xi|^2 \cdot \widehat{\Phi}(\xi) = 1. \]
%
Thus we find a fundamental solution for $n > 2$, by setting
%
\[ \widehat{\Phi}(\xi) = \frac{-1}{4 \pi^2 |\xi|^2}, \]
%
and this is an alternate expression of the fundamental solution given above. For $n = 2$, a version of this equation remains true provided one interprets the distribution $1/|\xi|^2$ at the origin in the right way.
...
\begin{example}
    ...
    %
    That these two distributions are also fundamental solution follows from the symmetry of all objects involved under the Lorentz transformations, which include the map $t \mapsto -t$. These are actually the \emph{unique} fundamental solutions supported on the forward and backward light cone and it's interior. Indeed, if $u \in \DD^*(\RR^n \times \RR)$ is supported on the forward light cone and $\Box u = 0$, then
    %
...
The existence and uniqueness of solutions to the Cauchy problem for the wave equation follow immediately from this calculation. Namely, if $\phi_0, \phi_1 \in C^\infty(\RR^n)$, and $f \in C^\infty(\RR^n \times [0,\infty))$, then there exists a unique $u \in C^\infty(\RR^n \times [0,\infty))$ such that $\Box u = f$ on $\RR^n \times [0,\infty)$, and we have
%
...
%
and since $\Phi_+(0) = 0$, and $\partial_t \Phi_+(0) = \delta$ (TODO: WHY?), we find that
%
\begin{align*}
...
which completes the proof.
    ...
    One way to find a solution to the Cauchy problem $\Box \Phi = 0$, with $\Phi(0) = 0$ and with $\partial_t \Phi(0) = \delta_x$, is by considering $\Phi = (1/2) (\Phi_+ - \Phi_-)$. Another way to solve the problem is by taking the Fourier transform in the $x$-variable, i.e. assuming $\Phi \in C^\infty(\RR_t, \SW(\RR^d_x)^*)$ and reducing the equation to
    %
    \[ \partial_t^2 \widehat{\Phi(t)} + 4 \pi^2 |\xi|^2 \widehat{\Phi(t)} = 0, \]
    %
    with initial conditions $\widehat{\Phi(0)} = 0$, and with $\partial_t \widehat{\Phi(0)} = 1$. Thus has the unique solution
    %
    \[ \widehat{\Phi(t)} = \frac{1}{4 \pi i |\xi|} \left( e^{2 \pi i t|\xi|} - e^{-2 \pi i t|\xi|} \right). \]
    %
    Thus
    %
    \[ \Phi(t,x) = \int \frac{1}{4 \pi i |\xi|} \left( e^{2 \pi i (t |\xi| + \xi \cdot x)} - e^{2 \pi i (-t |\xi| + \xi \cdot x)} \right)\; d\xi, \]
    %
    where we interpret this equation as an oscillatory integral. In particular, the singular support of $\Phi$ is contained on the boundary of the light cone.
...
Let us discuss the phenomenon of \emph{uniqueness} for the Cauchy problem. Thus our goal is to determine, for a given set $X \subset \DD(\Omega_0)^*$, whether $L: X \to \DD(\Omega_0)^*$ is injective. If $X$ is a linear subspace of $\DD(\Omega_0)^*$, it suffices to show that if $u \in X$, and $Lu = 0$, then $u = 0$. Of course, we deal with this situation. To make the problem possibly to study microlocally, we will focus on the case where $X$ solely contains functions which vanish on $\Omega_0^-$, and will try and find conditions which guarantee that if $Lu = 0$, then $u$ vanishes on a neighborhood of $S$.
...
\begin{theorem}
    ...
    %
    \begin{align*}
        e^{2 \pi i \lambda \phi(x)} \left\{ \sum_{|\beta| < N} \frac{1}{\beta! \cdot (2 \pi i)^{\beta}} \cdot \partial_\xi^\beta a(x,\lambda \nabla \phi(x)) \left. \partial^\beta_y \{ e^{2 \pi i \lambda r_x} f \} \right|_{y = x} + R_N(x,\lambda) \right\},
    \end{align*}
    %
    ...
    \begin{align*}
        &a(x,\lambda \nabla \phi(x)) f(x)\\
        &\quad + \frac{1}{2\pi i} \sum_{k = 1}^d (\partial^k_\xi a)(x,\lambda \nabla \phi(x)) \cdot (\partial^k_x f)(x)\\
        &\quad - \frac{i \lambda}{4 \pi} \sum_{|\beta| = 2} (\partial^\beta_\xi a)(x,\lambda \nabla \phi(x)) (\partial^\beta_x \phi)(x) f(x)\\
...
\section{Compact Pseudodifferential Operators and the Index Theorem}
...
\begin{align*}
    ...
    &\quad + \frac{1}{2 \pi i} \sum_{j = 1}^d \partial^j_\xi a(x, \nabla_x \Phi(t,x,y,\eta)) \partial^j_x b(t,x,y,\eta)
\end{align*}


\begin{align*}
    e^{-2 \pi i \Phi(t,x,y,\eta)} (2 \pi i T) & \{ b(t,\cdot,y,\eta) e^{2 \pi i \Phi(t,\cdot, y, \eta)} \}(x)\\
    &= \sum_{|\beta| < 2} \frac{1}{\beta! (2 \pi i)^{|\beta|}} \partial^\beta_\xi a(x, \nabla_x \Phi(t,x,y,\eta)) \partial^\beta_x b(t,x,y,\eta)\\
    &\quad - (i/\pi) \sum_{|\beta| = 2} \partial^\beta_\xi a(x,\nabla_x \Phi(t,x,y,\eta)) \partial^\beta_y \rho(Z) b(t,x,y,\eta)\\
    &\quad + R(t,x,y,\eta),
\end{align*}
%
...
The surprising feature of this problem is that these operators need not even have solutions if we switch from studying analytical initial conditions to say, compactly supported smooth initial conditions. For instance, suppose there exists a distribution $u$ on $\RR_t \times \RR_x$, tempered in the $x$-variable, such that $\partial_t u - Du = 0$, and let $u_0 \in \SW(\RR_x)^*$ be the initial value of the distribution. Then, taking Fourier transforms in the $x$ variable, we conclude that $\partial_t \widehat{u}(t,\xi) = \xi \widehat{u}(t,\xi)$, which implies that $\widehat{u}(t,\xi) = \widehat{u_0}(\xi) e^{\xi x}$. But this distribution is \emph{never} tempered in the $\xi$ variable; if it was tempered for one positive value of $t$, and one negative value of $t$, then we could conclude that
%
\[ |\widehat{u_0}(\xi)| \lesssim e^{- \varepsilon |\xi|} \]
%
for some $\varepsilon > 0$. But the Paley-Wiener theorem and it's variants therefore imply that $u_0$ is analytic, and actually extends to a holomorphic function on a small strip containing the real line. Thus a tempered solution to the Cauchy problem $\partial_t u - Du = 0$ \emph{does not exist} if, say, we want to consider initial conditions in $\DD(\RR^d_x)$.

The problem here is that the roots of the polynomial $P(\tau,\xi) = \tau - \xi$ in the $\tau$ variable are real-valued; a desire to find a more powerful existence statement will in general force us to choose polynomials $P(\tau,\xi)$ that have purely imaginary roots in the $\tau$ variable. Indeed, if $\tilde{E}_0(t,\xi),\dots,\tilde{E}_{m-1}(t,\xi)$ are the analytical functions which satisfy $P(\partial_t, \xi) \tilde{E}_i = 0$ together with the initial conditions $\partial_t^i \tilde{E}_j(0,\xi) = 1$ for $i = j$, and $\partial_t^i \tilde{E}_j(0,\xi) = 0$ for $i \neq j$. We can calculate the functions $\{ \tilde{E}_i \}$ explicitly. Namely, if we fix $\xi_0$, and we assume for simplicity at first that the roots of $P(\tau,\xi_0)$ in $\tau$ are distinct. Then the roots are distinct locally around $\xi_0$. If we let $\tau_1(\xi), \dots, \tau_m(\xi)$ be the roots of the equation, then they are analytic functions in $\xi$, and the functions $h_j(t,\xi) = e^{t \tau_j(\xi)}$ satisfy $P(\partial_t, \xi) h_j = 0$, and with initial conditions $\partial_t^i h_j(0,\xi) = \tau_j(\xi)^i$. The uniqueness of analytical solutions implies that
%
\[ h_j(t,\xi) = \tilde{E_0}(t,\xi) + \tilde{E_1}(t,\xi) \tau_j(\xi) + \dots + \tilde{E_{m-1}}(t,\xi) \tau_j^{m-1}(\xi). \]
%
We can use Cramer's rule to solve this system of equations in terms of the $\{ \tilde{E_j} \}$. Namely, if $V(\tau_1,\dots,\tau_m)$ is the Vandermonde determinant, i.e. the determinant of the matrix $\{ \tau_a^b \}$, and if $V_i(\tau_1,\dots,\tau_m;t)$ is the determinant of the matrix $\{ \tau_a^b \}$ with the $b$th column replaced with $\{ e^{t \tau_a(\xi)} \}$, then for $\xi \in \RR^d$ such that the roots $\tau_1(\xi),\dots,\tau_m(\xi)$ are distinct,
%
\[ \tilde{E_j}(t,\xi) = \frac{V_j(\tau_1(\xi),\dots,\tau_m(\xi);t)}{V(\tau_1(\xi),\dots,\tau_m(\xi))}. \]
%
The right hand side is entire and symmetric in the variables $\tau_1,\dots,\tau_m$. Since
%
\[ (\partial_t - \tau_1) \cdots (\partial_t - \tau_n) \left\{ \frac{V_j(\tau_1, \dots, \tau_m;t)}{V(\tau_1,\dots,\tau_m)} \right\} = 0 \]
%
and
%
\[ \partial_t^i \left. \left\{ \frac{V_j(\tau_1,\dots,\tau_m;t)}{V(\tau_1,\dots,\tau_m)} \right\} \right|_{t = 0} = \begin{cases} 1 &: i = j, \\ 0 &: i \neq j \end{cases} \]
%
holds for distinct $\tau_1,\dots,\tau_m$, the analyticity tells us that this equation still specifies $\tilde{E_j}$ even when the roots are not distinct.

By virtue of the roots switching around, it is hard to define the functions $h_1,\dots,h_n$ as globally defined functions in $\xi$. But one can certainly define $h(t,\xi)$, defined locally in $\xi$ by $|h_1(t,\xi)| + \dots + |h_n(t,\xi)|$. If $\tilde{E_0},\dots,\tilde{E_{m-1}}$ are tempered in the $x$ variable, then it follows that the function $h$ is tempered in the $\xi$ variable, and thus satisfies some equation of the form
%
\[ |h(t,\xi)| \lesssim_t \langle \xi \rangle^{N_t} \]
%
for some $N > 0$. But this means that
%
\[ |\text{Re}(\tau_1(\xi))| + \dots + |\text{Re}(\tau_m(\xi))| \lesssim 1 + \log \langle \xi \rangle. \]
%
But the theory of semialgebraic sets implies that this can only be possible if
%
\[ |\text{Re}(\tau_1(\xi))| + \dots + |\text{Re}(\tau_m(\xi))| \lesssim 1. \]
%
This is actually a necessary and sufficient condition for the Cauchy problem to be solvable (a result of Garding). We restrict ourselves to a special case of this condition.
