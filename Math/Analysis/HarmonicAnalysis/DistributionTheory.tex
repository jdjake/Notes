...
\end{equation}
%
then we conclude $\phi u \in C^\infty(\RR^d)$. Thus we can infer the singular support of $u$ via purely spectral properties, provided we are first able to localize about a point.

We can also gain more detailed information about the singularities of a distribution $u$ through the Fourier transform. If $x_0$ is a singularity of $u$, then for any bump function $\phi \in C^\infty(\RR^d)$ with $\phi(x) = 1$ in a neighbourhood of $x_0$, there must exist a value $\xi_0 \neq 0$ such that there exists no conical neighbourhood $U$ of $\xi_0$ such that for all $\xi \in U$ and all $N > 0$,
%
\begin{equation} \label{nonsingularfourierdecay}
  |\widehat{u \phi}(\xi)| \lesssim_N \langle \xi \rangle^{-N}.
\end{equation}
%
...
\begin{equation} \label{fastDecayEquation}
  |\widehat{u}(\xi)| \lesssim_N \langle \xi \rangle^{-N}.
\end{equation}
%
It is simple to verify that if $\Gamma(u) = \emptyset$, then $u \in C_c^\infty(\RR^d)$.
