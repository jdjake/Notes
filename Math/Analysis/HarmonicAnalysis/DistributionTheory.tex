...
We fix an open subset $\Omega$ of $\RR^d$, and let $\DD(\Omega) = C_c^\infty(\Omega)$ denote the family of all smooth functions on $\Omega$ with compact support. Our goal is to equip $\DD(\Omega)$ with a complete locally convex topology, so that we can consider the dual space $\DD^*(\Omega)$ of \emph{distributions} on $\Omega$.

The first problem emerges because it is \emph{impossible} to make $C_c^\infty(\Omega)$ into a Fr\'{e}chet space, such that each of the subspaces
%
\[ C_c^\infty(K) = \{ f \in C_c^\infty(\Omega): \text{supp}(f) \subset K \} \]
%
are closed. The interior of each of these subspaces is empty because no translation of these subspaces is absorbing. The Baire category theorem would then imply that a countable union of such subspaces has nonempty interior, but $C_c^\infty(\Omega)$ is the countable union of such spaces.
...
To make $\DD(\Omega)$ complete, we instead assign $\DD(\Omega)$ a stronger locally convex topology which prevents convergent functions from `escaping open sets'; the cost, however, is that the topology is no longer a Fr\'{e}chet space, i.e. metrizable, but instead an \emph{LF Space}. The spaces $C_c^\infty(K)$ of smooth functions supported on $K$ forms a Fre\'{c}het space, and we define $\DD(\Omega)$ to be the inductive limit of these spaces. It follows from the general theory that $\DD(\Omega)$ is complete in this topology. In fact, the space $\DD(\Omega)$ is also a Montel space, since it is the inductive limit of Montel spaces.
...
We now have the tools to explain the idea of a distribution. If $f \in \loc{L^1}(\Omega)$, then the linear functional $\Lambda[\psi]$ on $\DD(\Omega)$ defined for each $\phi \in \DD(\Omega)$ by setting
%
...
%
This definition has the property that $A(\Lambda[\phi]) = \Lambda[A \phi ]$ for any $\phi \in \DD(\Omega)$.
...
As a dual space to an LF space, we can equip the space of distributions $\DD(\Omega)^*$ with several topologies. The most important for our purposes in the strong topology, which makes $\DD(\Omega)^*$ into a complete locally convex space, and the weak $*$ topology, which is quasi-complete. These topologies are roughly the same for many purposes. For instance, the uniform boundedness theorem implies that a sequence $\{ u_n \}$ of distributions converges in the weak $*$ topology if and only if it converges in the strong dual topology, and we call this convergence \emph{distributional convergence}.
...
We have seen that all translation-invariant operators $T: \DD(\RR^d) \to C^\infty(\RR^d)$ are given by convolution by a distribution. Thus convolutions by a distribution are suitably general to represent all continuous translation-invariant operations. When studying non translation invariant operators, we consider a more general family of operators. Instead of studying operators of the form
%
...
%
where $K$ is a distribution on $\Omega_2 \times \Omega_1$ and $\phi \in \DD(\Omega_1)$. To formally interpret the output of this operator, we need to test it against another bump function, i.e. for $\psi \in \DD(\Omega_2)$ we consider
%
\[ \int T\phi(x) \psi(x)\; dx = \int K(x,y) \psi(x) \phi(y)\; dx\; dy, \]
%
which is $K$ tested against $\phi \otimes \psi$. Thus $T\phi$ is naturally a distribution on $\RR^m$, and this definition gives a continuous map $T: \DD(\RR^n) \to \DD^*(\RR^m)$. The behaviour of the operator $T$ uniquely determines the distribution $K$, and we call $K$ the \emph{Schwartz kernel} of $T$. In 1953, Schwartz showed that essentially every linear operator encountered in analysis is of this form (continuity from $\DD(\RR^n)$ to $\DD^*(\RR^m)$ is a very weak condition), which explains the prevalence of kernel operators in analysis.
...
\begin{remark}
    There are many variants of Schwartz's result. for instance, if $T: \mathcal{S}(\RR^n) \to \mathcal{S}(\RR^m)^*$ is a continuous map, then it certainly restricts to a map from $\DD(\RR^n) \to \DD(\RR^m)^*$, and so the vanilla version of Schwartz's theorem shows that there is a distribution kernel $K$ for $T$. The fact that $\langle K, \phi \otimes \psi \rangle$ is well defined and bilinearly continuous for any $\phi \in \mathcal{S}(\RR^n)$ and $\psi \in \mathcal{S}(\RR^m)$ implies that $K$ is actually a tempered distribution itself. Thus every continuous map $T: \mathcal{S}(\RR^n) \to \mathcal{S}(\RR^m)^*$ has a tempered distribution as it's kernel.
\end{remark}
...
\begin{remark}
    ...
    %
    for any $u \in \mathcal{E}(\Omega_1)^*$, where we interpret the integral in a distributional sense. The continuity of $u$ implies that $Tu \in C^\infty(\Omega_2)$, and the operator is then actually a continuous map $T: \mathcal{E}(\Omega_1)^* \to C^\infty(\Omega_2)$. Conversely, the vanilla version of Schwartz's theorem implies that any operator $T$ is represented by some distributional kernel $K$. Now for each $y \in \Omega_1$, we can define $K_y \in C^\infty(\Omega)$ by setting $K_y = T\delta_y$, and because $y \mapsto \delta_y$ is continuous from $\Omega_1 \to \mathcal{E}(\Omega_1)^*$, it follows that the map $\tilde{K}(x,y) = K_y(x)$ is a continuous function, differentiable in the $x$-direction, and any partial derivative $D^\alpha_x \tilde{K}$ is also continuous. It is actually also differentiable in the $y$-variable, since
    %
    \[ D^i_y \tilde{K}(x,y) = \lim_{h \to 0} \frac{\tilde{K}(x,y+he_i) - \tilde{K}(x,y)}{h} = T \left( \lim_{h \to 0} \frac{\delta_{y + he_i} - \delta_y}{h} \right)(x) = T \left( D^i \delta_y \right)(x). \]
    %
    More generally, one can prove that $\tilde{K}$ is actually has all derivatives in the $y$-direction, with $D^\alpha_y \tilde{K} = T( D^\alpha \delta_y )(x)$, and thus one can show that $\tilde{K}$ is actually a smooth function. One can show that any distribution $u$ formed from a finite linear combination of derivatives of Dirac deltas at various points, we have
    %
    \[ Tu = \int \tilde{K}(x,y) u(y)\; dy. \]
    %
    But both the right hand side and left hand side give continuous operators on $\mathcal{E}(\Omega_1)^*$, and since finite linear combinations of derivatives of Dirac deltas at various points are weak $*$ dense in $\mathcal{E}(\Omega_1)^*$, it follows that for any compactly supported distribution $u$ we have
    %
    \[ Tu = \int \tilde{K}(x,y) u(y)\; dy. \]
    %
    Thus the kernel of $T$ is smooth.
\end{remark}

Given an operator $T: \DD(\Omega_1) \to \DD(\Omega_2)^*$ with associated kernel $K(x,y)$, consider the operator $T^tx : \DD(\Omega_2) \to \DD(\Omega_1)^*$ given by the kernel $K(y,x)$. Then
%
...
%
\begin{itemize}
    \item An operator $T: \DD(\Omega_1) \to \DD(\Omega_2)^*$ is \emph{separately regular} if it maps $C_c^\infty(\Omega_2)$ to $C^\infty(\Omega_1)$, and extends to a map from $\mathcal{E}(\Omega_2)$ to $\DD(\Omega_1)^*$. This is equivalent ot saying that the kernel $K$ of $T$ can be viewed both as an element of $C^\infty(\Omega_2, \DD(\Omega_1)^*)$, i.e. $K$ is a smooth map from $\Omega_2$ into the space of distributions $\DD(\Omega_1)^*$, and also an element of $C^\infty(\Omega_1, \DD(\Omega_2)^*)$.

    \item We say that an operator $T: \DD(\Omega_1) \to \DD(\Omega_2)^*$ is \emph{separately regular} if both $T$ and $T^t$ (the operator with kernel $K(y,x)$ instead of $K(x,y)$) map $C_c^\infty(\Omega_2)$ to $C^\infty(\Omega_1)$. This holds if and only if we can view $K$ as a $C^\infty$ function

    \item We say that an operator $T: \DD(\Omega) \to \DD(\Omega)^*$ is \emph{very regular} if it is separately regular, and it's kernel $K$ agrees with a $C^\infty$ function away from the diagonal. Such an operator is then \emph{pseudolocal}, i.e for any compactly supported distribution $u$, $\text{sing supp}(Tu) \subset \text{sing supp}(u)$.

    \item We say an operator $T: \DD(\Omega_1) \to \DD(\Omega_2)^*$ is \emph{smoothing}, or \emph{regularizing}, if $T$ extends to a map from $\mathcal{E}(\Omega_2)^*$ to $C^\infty(\Omega_1)$. We have seen that this holds if and only if the kernel $K$ of $T$ is $C^\infty$.

    \item We say an operator $T: \DD(\Omega_1) \to \DD(\Omega_2)^*$ is \emph{proper} if $T$ maps $\DD(\Omega_1)$ to $\mathcal{E}(\Omega_2)^*$, and $T^t$ maps $\DD(\Omega_2)$ to $\mathcal{E}(\Omega_1)^*$. This is equivalent to saying that for any $\phi \in \DD(\Omega_1)$ and $\psi \in \DD(\Omega_2)$, the distributions $K(x,y) \phi(y)$ and $\psi(x) K(x,y)$ are compactly supported.
\end{itemize}
%
The pseudodifferential operators we will study later are very regular. In fact, they satisfy a stronger condition of \emph{microlocality}, that the \emph{wavefront set} of $Tu$ is contained in the wavefront set of $u$, a subtle refinement of the concept of the singular support of a function we will also define later.
...
\end{theorem}
\begin{proof}
    Suppose $L$ has degree $k$. Since $L$ is elliptic, there exists $R > 0$ such that the polynomial $P(\xi) = \sum_\alpha c_\alpha \xi^\alpha$ satisfies $|P(\xi)| \sim |\xi|^k$ for $|\xi| \geq R$. If $\chi \in C_c^\infty(\RR^d)$ is chosen such that $\chi(\xi) = 1$ for $|\xi| \leq R$, and we define a distribution $\Phi_0$ such that
    %
    ...
    %
    Since $\chi \in C_c^\infty(\RR^d)$, it follows by the Paley-Wiener theorem, taking the inverse Fourier transform that $L \Phi_0 = \delta - w$, where $w$ is an entire analytic function of at most polynomial increase. Applying the Cauchy-Kovalevskaya theorem (expanding things in power series), we can find an entire analytic function $u$ of at most exponential increase such that $Lu = w$. Then $\Phi = \Phi_0 + u$ is a fundamental solution for $L$.
\end{proof}

The theory of pseudodifferential operators was mainly used to generalize the kinds of construction that occured in this proof. In particular, a \emph{parametrix} for a differential operator $L$ is an `approximate inverse' for $L$, i.e. an operator $S$ such that $1 - S \circ L$ is a \emph{smoothing operator}, i.e it is an operator whose kernel is smooth. Because differential operators are local, for any differential operator $L$, and any compactly supported distribution $u$, $\text{WF}(Lu) \subset \text{WF}(u)$. But if $L$ has a pseudolocal parametrix $S$, then it follows that because $1 - S \circ L$ is a smoothing operator, $(1 - (S \circ L))u$ is a smooth function, and so
%
\[ \text{WF}(u) = \text{WF}((S \circ L) u + (1 - (S \circ L)) u) = \text{WF}((S \circ L) u) \subset \text{WF}(Lu). \]
%
Thus for any differential operator $L$ with a pseudolocal parametrix, $\text{WF}(Lu) = \text{WF}(u)$. We shall find that it is precisely the family of elliptic differential operators that have pseudolocal parametrices, and thus the elliptic differential operators preserve wavefront sets.
...
\end{align*}
%
We use this integral formula to define the general operators $a(x,D)$. In the most general setting, for any distribution $a(x,\xi)$ which is tempered in the $\xi$ variable, i.e. any distribution in $(\DD(\RR^d) \otimes \mathcal{S}(\RR^d))^*$, and any $f \in \mathcal{S}(\RR^d)$, we can define $a(x,D) f$ to be the distribution such that for any $g \in \DD(\RR^d)$,
%
\[ \langle a(x,D) f, g \rangle = \int a(x,\xi) \widehat{f}(\xi) e^{2 \pi i \xi \cdot x} g(x)\; dx\; d\xi. \]
%
We call any operator $T$ which can be given in the form $a(x,D)$ a \emph{pseudodifferential operator}. For any given pseudodifferential operator, the symbol $a$ is unique since the action of $a(x,D)$ determines the behaviour of $a$ against all test functions in $\DD(\RR^d) \otimes \mathcal{S}(\RR^d)$. For any set $S$ of tempered distributions on $\RR^d \times \RR^d$, the notation $\text{Op}(S)$ is often used to denote the family of all pseudodifferential operators defined by an element of $S$.
...
\begin{itemize}
    ...
    \item Suppose that $a(x,\xi)$ is a $\mathcal{D}(\RR^d) \otimes \mathcal{O}$

    \item If $a$ is a local symbol of order $t$ on $\RR^d \times \RR^d$ for some $t \in \RR$, i.e. $a \in \mathcal{S}^t_{\text{loc}}(\RR^d \times \RR^d)$, then $a(x,D)$ extends to a continuous operator from $\mathcal{E}(\RR^d)^*$ to $\DD(\RR^d)^*$ and from $\DD(\RR^d)$ to $C^\infty_{\text{loc}}(\RR^d)$.
...
Consider any symbol $a \in \mathcal{S}^t_{\text{loc}}(\RR^d \times \RR^d)$, i.e. a smooth function $a \in C^\infty(\RR^d \times \RR^d)$ such that for any $n_1,n_2,m \geq 0$,
%
\[ |\nabla_x^{n_1} \nabla_y^{n_2} \nabla_\xi^m a(x,\xi)| \lesssim_{n,m} \langle \xi \rangle^{t - m}, \]
%
where the implict constant is locally uniform in $x$. From this symbol, we can define a continuous operator $T_a: \DD(\RR^d) \to C^\infty_{\text{loc}}(\RR^d)$ by setting
%
\[ T_a f(x) = \int a(x,\xi) e^{2 \pi i \xi \cdot x} \widehat{f}(\xi)\; d\xi, \]
%
where the integral can now be interpreted in the usual Riemann / Lebesgue integral. We say $T$ is then a \emph{pseudodifferential operator of order $t$}. If the estimates above are \emph{uniform} in $x$, i.e. $a \in \mathcal{S}^t(\RR^d \times \RR^d)$, then the operator $T_a$ extends to a map from $\mathcal{S}(\RR^d)$ to $C^\infty(\RR^d)$. We let $\Phi^t(\RR^d) = \text{Op}(\mathcal{S}^t)$, and $\Phi^t_{\text{loc}}(\RR^d) = \text{Op}(\mathcal{S}^t_{\text{loc}})$.
...
\begin{theorem}
    Let $a \in \mathcal{S}^t_{\text{loc}}(\RR^d)$. Then for any $n_1,n_2,N \geq 0$,
    %
...
\begin{lemma}
    ...
    If $t \geq 0$, $k$ satisfies the growth conditions above, and in addition, for any bounded subset $\mathcal{B}$ of $C_c^\infty(\RR^d)$, satisfies the \emph{cancellation conditions}
    %
...
%
which is a symbol of order $-\infty$. Any smoothing operator has the property that $Tu \in \mathcal{C}_c^\infty(\RR^d)$ for any $u \in \mathcal{E}(\RR^d)^*$. Most of the theory of pseudodifferential operators works \emph{modulo smoothing operators}, which is mostly fine in harmonic analysis, where we work estimates of localized functions, and when localized, smoothing operators satisfy all the estimates we are interested in.
...
\begin{remark}
    ...
    %
    for all $n$ and $m$, it is not necessarily possible to apply the operator to Schwartz functions, and tempered distributions. One can consider asymptotics, as long as we work modulo a weaker family of smoothing operators, i.e. those whose kernels lie in $C^\infty_{\text{loc}}(\RR^d \times \RR^d)$.
\end{remark}

\section{Compositions of $\Psi$DOs, and Parametrices}

Let $a(x,D)$ and $b(x,D)$ be pseudodifferential operators defined by local symbols of order $t$ and $s$. It is not always possible to define the composition $a(x,D) \circ b(x,D)$. This is because the image of an element of $\DD(\RR^d)$ under the operator $b(x,D)$ in general lies in the Schwartz space $\mathcal{S}(\RR^d)$, and one cannot necessarily apply $a(x,D)$ to Schwartz functions. One way to fix this is to assume that $a(x,D)$ is defined by a symbol lying in $\mathcal{S}^t$ rather than $\mathcal{S}^t_{\text{loc}}$, for then $a(x,D)$ extends to a map from $\mathcal{S}(\RR^d)$ to itself, which means we can define $a(x,D) \circ b(x,D)$ on $\DD(\RR^d)$. Another approach is to assume that $b(x,D)$ maps $\DD(\RR^d)$ to itself, which is a slightly weaker property that $b(x,D)$ being a proper operator (an operator is proper if both $b(x,D)$ and it's transpose map $\DD(\RR^d)$ to itself).
...
\begin{theorem}
    For any $a \in \mathcal{S}^t_{\text{loc}}(\RR^d)$, $a(x,D)$ extends to a continuous operator from $H^s(\RR^d)$ to $H^{s-m}_{\text{loc}}(\RR^d)$.
\end{theorem}
\begin{proof}
