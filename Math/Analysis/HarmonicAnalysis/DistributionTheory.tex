...
%
even if $\Lambda$ is not given by integration against some function. The space $\mathcal{D}(\Omega) = C_c^\infty(\Omega)^*$ will be called the space of distributions on $\Omega$.

One huge advantage of this approach is that we can generalize many analytical operations defined on $C_c^\infty(\Omega)$ \emph{distributionally} to give an operation on $\mathcal{D}(\Omega)$, even if the original analytical operations required some degree of smoothness to define. If $A$ is an operator on $C_c^\infty(\RR^d)$ with adjoint $A^*$, then for any $\phi,\psi \in C_c^\infty(\RR^d)$,
%
...
For instance, we can use this idea to define the derivative of an arbitrary distribution. For $\phi,\psi \in C_c^\infty(\RR)$, integration by parts tells us that
%
\[ \int_{-\infty}^\infty \phi'(x) \psi(x)\; dx = - \int_{-\infty}^\infty \phi(x) \psi'(x)\; dx. \]
%
Thus if $A\phi = \phi'$ is the derivative operator then it's adjoint is $A^* \psi = - \psi'$. Thus, for a distribution $\Lambda$ on $\RR$, we define it's derivative to be the distribution $\Lambda'$ such that for $\phi \in C_c^\infty(\RR)$,
%
...
In general, we define a \emph{distribution} to be a continuous linear functional on the space of test functions $C_c^\infty(\Omega)$, i.e. an element of $C_c^\infty(\Omega)^*$. In the last section, our exploration of continuous linear transformations on $C_c^\infty(\Omega)$ guarantees that a linear functional $\Lambda$ on $C_c^\infty(\Omega)$ is continuous if and only if for every compact $K \subset X$ there is an integer $n_k$ such that $|\Lambda \phi| \lesssim_K \| \phi \|_{C^{n_k}(K)}$ for $\phi \in C_c^\infty(K)$. If one integer $n$ works for all $K$, and $n$ is the smallest integer with such a property, we say that $\Lambda$ is a distribution of \emph{order $n$}. If such an $n$ doesn't exist, we say the distribution has infinite order. If such an $n$ doesn't exist, we say the distribution has infinite order. Applying the Hahn-Banach theorem shows that if $\Lambda \in \mathcal{D}(\Omega)$ has order $n$, then it can be tested against any function in $C^n_c(\Omega)$.
...
\begin{example}
    ...
    %
    The fact that this bound does not require information about the derivatives of $\phi$ implies that $\Lambda[\mu]$ is a distribution of order zero.
\end{example}
...
\begin{example}
    ...
    %
    The distribution can also be described as the distributional derivative of the locally integrable function $\log |x|$, since an integration by parts shows that for each $\phi \in C_c^\infty(\RR^d)$,
    %
    \begin{align*}
...
\begin{theorem}
  If $u \in \mathcal{D}(\RR^d)$ and there exists an index $i$ such that $D^i u = 0$, then there exists $v \in \mathcal{D}(\RR^d)$ such that
  %
...
There are many other important operations one can apply to distributions. If $\Omega$ is a conic subset of $\RR^d$, and $\phi,\psi \in C_c^\infty(\Omega)$, we find
%
...
For $f \in C^\infty(\Omega)$, we have an operator $\phi \mapsto f \phi$ on $C_c^\infty(\Omega)$. The adjoint is clearly $\psi \mapsto f \psi$, so for a distribution $\Lambda$ on $\Omega$, we define $f \Lambda$ by setting $(f\Lambda)(\phi) = \Lambda(f \phi)$. Thus $\mathcal{D}(\Omega)$ is naturally a $C^\infty(\Omega)$ module.
...
Thus differentiation is continuous in the space of distributions. It is simple to verify that for $f \in C^\infty(\Omega)$, $\Lambda \mapsto f \cdot \Lambda$ is a continuous operation.
...
\end{theorem}
\begin{proof}
  For each $\phi \in C_c^\infty(\RR^d)$, the map
  %
  \[ (\Lambda_\alpha, g_\alpha) \mapsto \int \Lambda_\alpha(x) g_\alpha(x) \phi(x)\; dx \]
  %
...
\begin{theorem}
    Suppose $\mathcal{U} \subset \mathcal{D}(\Omega)$ is a family of distributions such that for each $\phi \in C_c^\infty(\Omega)$, $\sup_{u \in \mathcal{U}} |u(\phi)| < \infty$. Then for every compact set $K$, there exists $m$ such that for an $u \in \mathcal{U}$ and $\phi \in C_c^\infty(K)$,
    %
    \[ |u(\phi)| \lesssim \| \phi \|_{C^m(K)}. \]
    %
    If $\{ u_n \}$ is a sequence of distributions for which $\lim_n u_n(\phi)$ exists for every $\phi \in C_c^\infty(\Omega)$, then $u(\phi) = \lim_n u_n(\phi)$ defines a distribution, and for every compact set $K$ there is an integer $m$ such that for each $\phi \in C_c^\infty(K)$,
    %
...
In the language of commutative algebra, the association of $\mathcal{D}(U)$ to each open subset $U$ of $\Omega$ gives the structure of a sheaf of modules on $\Omega$.

Given a distribution $\Lambda$, we might have $\Lambda(\phi) = 0$ for every $\phi$ supported on some open set $U$. The complement of the largest open set $U$ for which this is true is called the \emph{support} of $\Lambda$. This agrees with the sheaf theoretic definition.
...
\begin{example}
        A linear differential operator $P: C_c^\infty(\RR^d) \to C_c^\infty(\RR^d)$ is translation invariant, from which it follows that there exists a distribution $\Lambda$ such that $P\phi = \Lambda * \phi$. Of course, $\Lambda(\phi) = P\phi(0)$ is just given by applying the differential operator at the origin.
\end{example}
...
%
where $K$ is a distribution on $\Omega_1 \times \Omega_2$ and $\phi \in C_c^\infty(\Omega_1)$. To formally interpret the output of this operator, we need to test it against another bump function, i.e. for $\psi \in C_c^\infty(\Omega_2)$ we consider
%
\[ \int T\phi(y) \psi(y)\; dy = \int K(x,y) \phi(x) \psi(y)\; dx\; dy, \]
%
which is $K$ tested against $\phi \otimes \psi$. Thus $T\phi$ is naturally a distribution on $\RR^m$, and this definition naturally gives a continuous map from $C_c^\infty(\RR^n)$ to $\mathcal{D}(\RR^m)$. In 1953, Schwartz showed that essentially every linear operator encountered in Euclidean analysis is of this form, which explains the prevalence of kernel operators in analysis.

\begin{theorem}
  Let $T: C_c^\infty(\Omega_1) \to \mathcal{D}(\Omega_2)$ be a continuous linear operator. Then there exists a unique distribution $K \in \mathcal{D}(\Omega_1 \times \Omega_2$ such that for $\phi \in C_c^\infty(\RR^n)$ and $\psi \in C_c^\infty(\RR^m)$,
  %
  \[ \int T\phi(y) \psi(y)\; dy = \int K(x,y) \phi(x) \psi(y)\; dx\; dy. \]
\end{theorem}
...
This fact means we can obtain a consistant localization about a point. If $u$ is a distribution, $\phi_1,\phi_2 \in C_c^\infty(\RR^d)$ are given, and the support of $\phi_2$ is compactly supported on the support of $\phi_1$, then $\phi_2/\phi_1 \in C_c^\infty(\RR^d)$, and so we conclude that
%
...
Using this change of variables formula, we can define the wavefront set of any smooth manifold; the wavefront set of a distribution on $M$ will then be a closed subset of $T^* M$.

\section{Oscillatory Integral Distributions}

Fix a symbol $a \in C^\infty(U \times (\RR^p - \{ 0 \})$ of some order $m \in \RR$, i.e. satisfying estimates of the form
%
\[ |\nabla_x^k \nabla_\theta^l a(x,\theta)| \lesssim_{k,l} \langle \theta \rangle^{m - l} \]
%
for all $k,l \geq 0$, $x \in U$, and $\theta \in \RR^p - \{ 0 \}$. Next, consider a \emph{phase function} $\phi$, i.e. a function in $C^\infty(U \times \RR^p)$, such that $\nabla_x \phi$ and $\nabla_\theta \phi$ have no common vanishing point on the support of $a$, and such that $\phi$ is homogeneous of degree one, i.e. $\phi(x,\lambda \theta) = \lambda \phi(x,\theta)$ for $\lambda > 0$. We then consider the oscillatory integral distribution $I_\phi$ on $\RR^d$, formally defined by the formula
%
\[ I_\phi(x) = \int a(x,\theta) e^{2\pi i \phi(x,\theta)}\; d\theta. \]
%
If $m < -d$, then $I_\phi$ can be interpreted as an absolutely convergent integral, and in this case $I_\phi$ is actually a locally integrable function. On the other hand, if $m \geq -d$, then $I_\phi$ will no longer act as a locally integrable function; for instance, our definition will show that the distribution
%
\[ \int_{\RR^d} \xi^\alpha e^{2 \pi i x \cdot \xi}\; d\xi \]
%
acts on functions as a constant multiple of the differential operator $D^\alpha$.

To define the oscillatory integral distribution rigorously, we fix $\psi \in C_c^\infty(\RR^d)$, and $\rho \in C_c^\infty(\RR^p)$, equal to one in a neighborhood of the origin. We claim that the quantity
%
\[ \lim_{R \to \infty} \int a(x,\theta) \psi(x) \rho(\theta / R) e^{2 \pi i \phi(x,\theta)}\; d\theta \]
%
exists, and is independent of the choice of bump function $\rho$. To prove the limit exists, we fix $R_1 \leq R_2$, and let $\tilde{\rho}(\theta) = \rho(\theta/R_2) - \rho(\theta/R_1)$. Then $\tilde{\rho}$ is supported on $R_1 \lesssim |x| \lesssim R_2$. Assume first that $R_2 \leq 2R_1$. Rescaling, we find that
%
\begin{align*}
...
\end{align*}
%
Then $\eta$ is supported on $1/2 \lesssim |x| \lesssim 1$. Thus the support of $\eta$ is independant of $R_1$ and $R_2$. It is simple to verify that
%
\[ |D^\alpha_x D^\beta_\theta \eta(x,\theta)| \lesssim_{\alpha,\beta} R_2^m \sum_{\alpha' \leq \alpha} |D^{\alpha'}_x \phi(x)|, \]
%
...
%
\[ \left| R_2^p \int_{\RR^n} \int_{\RR^p} e^{2 \pi i R_2 \phi(x,\theta)} \eta(x,\theta) \right| \lesssim_N R_2^{p + m - N} \sum_{|\alpha| \leq N} \| D^\alpha \psi \|_{L^\infty(\RR^d)}. \]
%
...
%
\begin{align*}
    \left| \int_{\RR^n} \int_{\RR^p} e^{2 \pi i \phi(x,\theta)} a(x,\theta) \phi(x) \tilde{\rho}(\theta) \right| &= \left| \sum_{k = 0}^l \int_{\RR^n} \int_{\RR^p} e^{2 \pi i \phi(x,\theta)} a(x,\theta) \phi(x) (\rho(\theta/a_{k+1}) - \rho(\theta / a_k)) \right|\\
    &\lesssim \sum_{k = 0}^l a_{k+1}^{p + m - N} \sum_{|\alpha| \leq N} \| D^\alpha \psi \|_{L^\infty(\RR^d)}. 
\end{align*}
%
If we choose $N > p + m$, then we conclude that
%
\[ \left| \int_{\RR^n} \int_{\RR^p} e^{2 \pi i \phi(x,\theta)} a(x,\theta) \phi(x) \tilde{\rho}(\theta) \right| \lesssim (R_1^{p + m - N} + R_2^{p + m - N}) \sum_{|\alpha| \leq N} \| D^\alpha \psi \|_{L^\infty(\RR^d)} \leq R_1^{p + m - N} \sum_{|\alpha| \leq N} \| D^\alpha \psi \|_{L^\infty(\RR^d)}. \]
%
In particular, this quantity tends to zero as $R_1 \to \infty$, which gives convergence of the limit, and also gives boundedness, showing $I_\phi$ is a distribution of order $N$, where $N$ is the smallest integer bigger than $p + m$. A very similar argument shows that if $\rho$ is equal to zero in a neighborhood of the origin, then
%
\[ \lim_{R \to \infty} \int_{\RR^n} \int_{\RR^p} e^{2 \pi i \phi(x,\theta)} a(x,\theta) \phi(x) \rho(\theta) \phi(x) = 0. \]
%
It follows from this that the definition is independent of the original choice of $\rho$.
...
\begin{example}
    ...
    %
    Thus we conclude that $I_\phi$ is smooth away from the origin. If $a(x,\theta) = a(x)$, then this result says that the Fourier transform of any symbol is smooth away from the origin. This should be compared to the result that the Fourier transform of a homogeneous distribution which is $C^\infty$ away from the origin is $C^\infty$.
\end{example}
...
\begin{lemma}
    Let $\mathcal{U}$ be a family in $\mathcal{D}(\Omega)$ such that $\sup_{u \in \mathcal{U}} |u(\phi)| < \infty$. Then for any $\phi \in C_c^\infty(\Omega)$, there exists $m > 0$ such that for any $\xi \in \mathbf{R}^d$,
    %
...
\begin{theorem}
    $C_c^\infty(\Omega)$ is sequentially dense in $\mathcal{D}_\Gamma(\Omega)$.
\end{theorem}
\begin{proof}
...
To see how this is possible, we note that for $\phi,\psi \in C_c^\infty(\RR^d)$, we might expect us to be able to take Fourier transforms, so that
%
\begin{align*}
...
\begin{example}
    ...
    %
    which is disjoint from $0_{\RR^2}$, and so a product $\Lambda_1 \cdot \Lambda_2$ is well defined. To determine what the product is, we consider a non-negative bump function $\phi \in C_c^\infty(\RR^d)$ equal to one in a neighborhood of the origin, and define
    %
...
Let us begin with the pullback. For a smooth map $f: \Omega \to \Psi$, not necessarily a diffeomorphism, and $\phi \in C_c^\infty(\Psi)$, we can define $f^* \phi = \phi \circ f \in C^\infty(\Omega)$. This map is continuous in the appropriate topology, and if $f$ is a proper map, $f^*$ is continuous from $C_c^\infty(\Psi) \to C_c^\infty(\Omega)$. To obtain a distributional definition, we apply the Fourier inversion formula; if $\psi \in C_c^\infty(\RR^d)$, then
%
...
\begin{theorem}
    ...
    %
    Fix a closed cone $\Gamma$ with $\Gamma \cap N = \emptyset$. Then $f^*: C_c^\infty(\Psi) \to C^\infty(\Omega)$ extends to a continuous map from $\mathcal{D}_\Gamma(\Psi) \to \mathcal{D}_{f^* \Gamma}(\Omega)$, where
    %
...
Next, let us define the tensor product. Given a distribution $u_1$ on $\Omega_1$ and a distribution $u_2$ on $\Omega_2$, we define a distribution $u_1 \otimes u_2$ on $\Omega_1 \times \Omega_2$ such that for $\phi \in C_c^\infty(\Omega_1 \times \Omega_2)$,
%
...
Finally, we define the pushforward of a distribution. This is most naturally defined distributionally. Given a smooth map $f: \Omega \to \Psi$, $\phi \in C_c^\infty(\Omega)$, and $\psi \in C_c^\infty(\Psi)$, we define
%
...
%
for a distribution $u$ on $\Omega$ and $\psi \in C_c^\infty(\Psi)$. To understand the wavefront set of $u$, we consider a bump function $\phi$ supported in a neighbourhood  on $\Omega$ and consider
%
...
\begin{example}
    Given $\phi,\psi \in C_c^\infty(\Omega)$, we have
    %
...
Let us consider an important example which occurs in the theory of kernel operators. Recall that the Schwartz kernel theorem says that if $T: C_c^\infty(\Omega) \to \mathcal{D}(\Psi)$ is any continuous linear map, then there exists a distribution $K \in \mathcal{D}(\Omega \times \Psi)$ such that for any $\phi \in C_c^\infty(\Omega)$ and $\psi \in C_c^\infty(\Psi)$,
%
\[ \int T\phi(y) \psi(y)\; dy = \int K(x,y) \phi(x) \psi(y)\; dx\; dy. \]
%
...
How about the \emph{composition} of kernel operators? Intuitively, if $B: C_c^\infty(\Psi) \to \mathcal{D}(\Phi)$, $A: C_c^\infty(\Omega) \to \mathcal{D}(\Psi)$, with kernels $K_A(x,y)$ and $K_B(y,z)$, then, if we could define a kernel operator $C = B \circ A: C_c^\infty(\Omega) \to \mathcal{D}(\Phi)$, then it should have kernel
%
\[ K_C(x,z) = \int K_A(x,y) K_B(y,z)\; dy. \]
%
Slightly more precisely, we might want to define $K_C = \pi_* \Delta^*(K_A \otimes K_B)$, where $\Delta: X \times Y \times Z \to X \times Y \times Y \times Z$, and $\pi: X \times Y \times Z \to X \times Z$. In order for this to make sense, we require the projection map $(x,y) \to x$ to be proper on $\text{supp}(K_A)$. Provided that $\text{WF}'(K_B) \circ \text{WF'}(K_A) \circ 0_\Omega$ does not contain a zero section, the composition $B \circ A$ is well defined as an operator from $C_c^\infty(\Omega) \to \mathcal{D}(\Phi)$, where the kernel is defined by the formula above. Moreover,
%
...
\begin{example}
    Given a kernel $K \in \mathcal{D}(\Omega \times \Psi)$ generating an operator $T: C_c^\infty(\RR^n) \to \mathcal{D}(\RR^m)$, the adjoint map $T^*: C_c^\infty(\RR^m) \to D'(\RR^n)$ is induced by the kernel $K^*(y,x) = \overline{K(x,y)}$. It is simple to verify that
    %
...
Given a pseudodifferential operator $T: \mathcal{S}(\RR^d) \to \mathcal{S}(\RR^d)$, we calculate that
%
\begin{align*}
...
The choice of symbol associated to a pseudodifferential operator is not standard in the literature. For instance, for any symbol $a(x,\xi)$ we can also define the \emph{adjoint Kohn-Nirenberg quantization}
%
...
%
Since the conjugate of a symbol is a symbol, and the adjoint of a pseudodifferential operator is a pseudodifferential operator, we still get the same family of operators using the adjoint Kohn-Nirenberg quantization. For instance, the symbol corresponding to the differential operator
%
\[ Lf(x) = \sum_{|\alpha| \leq n} c_\alpha(x) D^\alpha f(x) \]
%
...
To begin studying the orders of psedodifferential operators, let us begin by assuming strong conditions on the function $a$. In particular, we assume $a(x,\xi)$ is homogenous of degree zero in $\xi$, and that moreover, there exists a smooth, homogenous function $b(\xi)$ such that for any integers $n_1,n_2$, and $n_3$,
%
