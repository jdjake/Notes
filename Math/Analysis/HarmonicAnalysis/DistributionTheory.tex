...
One huge advantage of this approach is that we can generalize many analytical operations defined on $C_c^\infty(\Omega)$ \emph{distributionally} to an operation on $C_c^\infty(\Omega)^*$, which is a much more general class. If $A$ is an operator on $C_c^\infty(\RR^d)$ with adjoint $A^*$, then for any $\phi,\psi \in C_c^\infty(\RR^d)$,
%
...
There are many other important operations one can apply to distributions. If $\Omega$ is a star shaped set about the origin, and $\phi,\psi \in C_c^\infty(\Omega)$, we find
%
...
Given a psuedodifferential operator $T: \mathcal{S}(\RR^d) \to \mathcal{S}(\RR^d)$, we calculate that
%
\begin{align*}
...
\begin{theorem}
    Given a smooth map $f: U \to V$, let $N(f) = \{ (f(x),\eta): f'(x)^T \eta = 0 \}$. Fix a closed cone $\Gamma$ with $\Gamma \cap N(f) = \emptyset$. Then $f^*: C_c^\infty(V) \to C^\infty(U)$. Extends to a continuous map from $D_\Gamma'(V) \to D'(U)$. Moreover, $\text{WF}(f^* u) \subset f^*(\text{WF}(u))$.
\end{theorem}
...
Finally, we define the pushforward of a distribution. This is most naturally defined distributionally. Given a smooth map $f: U \to V$, $\phi \in C_c^\infty(U)$, and $\psi \in C_c^\infty(V)$, we define
%
...
Another useful operation is to define the composition of two kernel operators. Recall that the Schwartz kernel theorem says that if $T: C_c^\infty(U) \to C_c^\infty(V)'$ is any continuous linear map, then there exists a distribution $K \in C_c^\infty(U \times V)'$ such that for any $\phi \in C_c^\infty(U)$ and $\psi \in C_c^\infty(V)$,
%
\[ \int T\phi(y) \psi(y)\; dy = \int K(x,y) \phi(x) \psi(y)\; dx\; dy. \]
%
...
%
Let us use this equation is extend $T$ to be defined for certain compactly supported distributions. First, if $u$ is a distribution, $\Delta^*(u \otimes K)$ is well defined provided that $\text{WF}(u \otimes K)$ is disjoint from
%
\[ N(\Delta) = \{ (x,x,y,\xi,-\xi,0) : x \in U, y \in V, \xi \in \RR^n \}. \]
%
This means precisely that there does not exist $x$, $y$, and $\xi$ such that $(x,\xi) \in \text{WF}(u)$ such that $(x,y,-\xi,0) \in \text{WF}(K)$. In a more notationally convenient way, if we define
%
\[ \text{WF}'(K)_X = \{ (x,\xi) : (x,y,0,-\xi) \in \text{WF}(K)\ \text{for some $y \in V$} \} \]
%
then $\Delta^*(u \otimes K)$ is well defined provided that $\text{WF}(u) \cap \text{WF}'(K)_X = \emptyset$. Now if $u$ is supported on some compact set $X$, then $\Delta^*(u \otimes K)$ is supported on $X \times \RR^m$, and $\pi$ is a proper map when restricted to a map from $X \times \RR^m$ to $\RR^m$, so $\pi_*(\Delta^*(u \otimes K))$ is well defined. Thus we conclude that $Tu$ is well defined if $u$ is a compactly supported distribution with $\text{WF}(u) \cap \text{WF}'_X(K) = \emptyset$.

Let us also track through our calculations to control the wavefront set of $Tu$. The wavefront set of $\Delta^*(\phi \otimes K)$ is equal to 
