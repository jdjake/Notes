...
\begin{example}
    ...
    %
    Such functions are dense in $\mathcal{D}(\RR)$. We claim $\Lambda$ extends to a continuous functional on the entirety of $\mathcal{D}(\RR)$. To prove this, fix $\phi \in C_c^\infty[-N,N]$ vanishing on a neighbourhood $(-\varepsilon,\varepsilon)$ of the origin. Then
    %
    ...
    %
    Since $N$ was arbitrary, it follows that $\Lambda$ is continuous in the topology induced by that of $\mathcal{D}(\RR)$, and thus extends uniquely to a distribution on the entirety of $\mathcal{D}(\RR)$. To be precise, we often denote the application of $\Lambda$ to $\mathcal{D}(\RR)$ as
    %
    \[ \text{p.v} \int_{-\infty}^\infty \frac{\phi(x)}{x}\; dx. \]
    %
    A simple approximation argument shows that for any $\phi \in \mathcal{D}(\RR)$,
    %
    \[ \Lambda \phi = \lim_{\varepsilon \to 0} \int_{|x| \geq \varepsilon} \frac{\phi(x)}{x}\; dx. \]
    %
    The distribution can also be described as the distributional derivative of the locally integrable function $\log |x|$, since an integration by parts shows that for each $\phi \in \mathcal{D}(\RR^d)$,
    %
    \begin{align*}
...
%
and $\phi \in C^\infty(U \times \RR^d)$ is homogeneous of degree one in $\theta$, such that $(\nabla_x \phi, \nabla_\theta \phi)$ is nonvanishing on the support of $a$.
...
To understand this operator, our goal is to construct a parametrix for the family of operators $e^{-2 \pi i t T}$. Clearly $e^{2 \pi i t T}$ is an inverse operator, but we wish to find an integral formula for this operator. Let $a(x,\xi)$ be the principal symbol for $T$. We shall find that, for $|t| \ll 1$, we can find a parametrix $S(t)$ for $e^{-2 \pi i t T}$ given by a Fourier integral, i.e. such that for $f \in C_c^\infty(\RR^d)$,
%
\[ S(t) f(x) = \int s(t,x,y,\xi) e^{2 \pi i[\phi(x,y,\xi) + ta(y,\xi)]} f(y)\; dy\; d\xi, \]
%
...
Next, we must understand what conditions ensure that
%
\[ (\partial / \partial t + 2 \pi i T) \circ S(t) \]
%
is a smoothing operator. The kernel of this operator is
%
\[ K(t,x,y) = \int  \]