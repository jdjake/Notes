...
\begin{example}
    ...
    %
    \[ \Phi(x) = \begin{cases} (2\pi)^{-1} \log |x| &: \text{if $d = 2$} \\ \frac{1}{c_d (d-2) |x|^{d-2}} &: \text{if $d > 2$.} \end{cases} \]
    %
    ...
    \end{align*}
    %
    The proof here follows by approximating the integral on the left by a small $\varepsilon$ ball about the origin, applying Gauss' formula, and then taking limits carefully.
\end{example}
...
\begin{example}
    ...
    and
    %
    \[ \int_\Omega \left( u \frac{\partial v}{\partial \overline{z}} + \frac{\partial u}{\partial \overline{z}} v \right)\; dx\; dy = \frac{1}{2} \int_{\partial \Omega} uv\; dz. \]
    %
...
%
\[ b(\kappa(x),\eta) = \left\{ S e^{2 \pi i \eta \cdot z} \right\}(\kappa(x)) = \int a(x,\xi) e^{2 \pi i [\xi \cdot (x - y) + \eta \cdot \kappa(y)]}\; dy\; d\xi \]
%
Provided we show that $b \in S^t(V)$, then $b(x,D)$ will be a continuous operator whose kernel is compactly supported in $V \times V$, and the fact that $S e^{2 \pi i \eta \cdot z} = b(y,D) e^{2 \pi i \eta \cdot z}$ for all $\eta$ implies, by continuinty and the fact that linear combinations of exponentials are dense in $\mathcal{D}(V)^*$, that $S = b(y,D)$. Thus our analysis would be completed by an analysis of the function $b(y,\eta)$. If $\phi \in C_c^\infty(V)$ is chosen such that $\phi(x) = \phi(y) = 1$ for $(x,y)$ in the kernel of $S$, then
%
\[ b(\kappa(x),\eta) = \left\{ S \left\{ \phi \cdot e^{2 \pi i \eta \cdot z} \right\} \right\}(\kappa(x)), \]
%
which implies that $b \in C^\infty(T^* V)$. If we set
%
\[ \Phi(\xi,\eta) = \int \phi(y) e^{2 \pi i [ \eta \cdot \kappa(y) - \xi \cdot y ]}\; dy, \]
%
then
%
\[ b(\kappa(x), \eta) = \int \Phi(\xi,\eta) e^{2 \pi i \xi \cdot x}\; d\xi. \]
%
Now $\Phi$ is a standard oscillatory integral, whose phase has a stationary point for values of $y$ such that $D\kappa(y)^T \eta = \xi$. Since $D\kappa(y)^T$ is invertible, this can only happen when $|\eta| \sim |\xi|$. More precisely, suppose $|D\kappa(y)|, |D\kappa(y)^{-1}| \leq C$. Then if $|D\kappa(y)^T \eta - \xi| \geq |\xi|/2$ for $|\eta| \leq |\xi|/2C$, and $D\kappa(y)^T \eta - \xi| \geq |\eta|/2$ for $|\xi| \geq |\eta|/2C$.



A change of variables, setting $w = \kappa(y)$ and $\alpha = (D\kappa^{-1})^T \xi$, gives
%
\[ Sf(z) = \int \det \left| D\kappa^{-1}(w) \right| a(\kappa^{-1}(z), \xi) e^{2 \pi i \xi \cdot (\kappa^{-1}(z) - \kappa^{-1}(w))} f(w)\; dw\; d\xi \]
