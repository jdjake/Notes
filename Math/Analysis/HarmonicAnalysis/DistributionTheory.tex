...
%
If $a \in \mathcal{S}(\RR^d \times \RR^d)^*$, then we can define $a(x,D) f \in \mathcal{S}(\RR^d)^*$ for any $f \in \mathcal{S}(\RR^d)$. The Schwartz kernel theorem easily implies that \emph{any} continuous linear operator from $\mathcal{S}(\RR^d)$ to $\mathcal{S}(\RR^d)^*$ is given by such a symbol. To get a more sophisticated calculus, we will for most part restrict our attention to much more regular choices of the distribution $a$.
...
%
where, since $\widehat{f}$ decays rapidly in $\xi$, we can interpret the integral in the Lebesgue sense. Any operator $T$ which can be given as $T_a$ for some symbol $a$ is called a \emph{pseudo-differential operator}, or $\Psi$DO for short.
...
%
Varying $\xi_0$ gives the complete symbol $a$. More formally, if $K(x,y)$ is the kernel of the operator $T_a$, then we can write this more formally as saying that
%
\[ a(x,\xi) = \int K(x,y) e^{2 \pi i \xi \cdot (y-x)}\; dy = \int K(x,x-y) e^{-2 \pi i \xi \cdot y}\; dy, \]
%
where we must interpret the integral on the right hand side as an oscillatory integral distribution. The advantage of this formula is that is essentially the Fourier transform of an appropriate function, which aids in calculations.
...
%
Thus general microlocal analysis shows that the kernel $K$ is smooth for $x \neq y$, and that pseudodifferential operators extend to continuous operators from $\mathcal{E}^*(\RR^d) \to \mathcal{D}^*(\RR^d)$, and satisfy the relation $\text{WF}(Tu) \subset \text{WF}(u)$. This is the first instance of the \emph{pseudolocal nature} of pseudodifferential operators; these operators roughly preserve the position of the mass of a function, but with some `fuzz'. Here is another quantitative estimate on the kernel also showing the pseudolocal nature, which implies that $T_a f$ is a \emph{Schwartz function} whenever $f$ is Schwartz.
