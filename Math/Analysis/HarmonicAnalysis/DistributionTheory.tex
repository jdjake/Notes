...
%
When $d = 1$, this actually implies that $u$ is a multiple of $|x|^a$ on each coordinate axis. The identity also implies that for any $\psi \in \DD(\RR^d - \{ 0 \})$ with $\int_0^\infty r^{a + d-1} \psi(rx)\; dr = 0$ for all $x$, $\langle u, \phi \rangle = 0$ by rewriting the formula in polar coordinates.
...
\begin{proof}
    ...
    %
    Thus we need only study the behaviour of $u$ near the unit sphere, which is supported away from the origin. Doing this more formally yields the extension map $E$. For any $\phi \in \DD(\RR^d)$, define $R_a \phi(x) = \langle t^{a + d-1}_+, \phi(tx) \rangle$. Then $R_a \phi$ is homogeneous of degree $-n-a$, and is continuous from $\DD(K)$ to $C^\infty(\RR^d - \{ 0 \})$ for any compact set $K$. If $\psi \in C_c(\RR^d - \{ 0 \})$ and $\int_0^\infty \psi(tx)/t\; dt = 1$ for all $x \neq 0$, then $\psi R_a \phi \in \DD(\RR^d - \{ 0 \})$, and $R_a(\psi R_a \phi) = R_a \phi$, so that (because of our observations before the proof) $E(u) = \langle u, \psi R_a \phi \rangle$ is independent of the choice of $\psi$. Moreover, $\langle u, \psi R_a \phi \rangle = \phi$ for each $\phi \in \DD(\RR^d - \{ 0 \})$. The continuity of the map $u \mapsto E(u)$ is not too difficult to see, completing the proof.
\end{proof}
...
\begin{example}
  ...
  %
  and so $\Lambda$ is tempered of order 1. The $\text{p.v.}$ is called the \emph{(Cauchy) principal value} of $1/x$.
\end{example}
...
\begin{itemize}
    ...
    %
    If $K$ is very regular, then $T$ is \emph{pseudolocal}, i.e. for any compactly supported distribution $u$, $\text{sing supp}(Tu) \subset \text{sing supp}(u)$.
...
\begin{proof}
    ...
    %
    The right hand side defines a smooth function of $x$ for $x \in \Omega - \text{supp}(u)$. But this gives us the initial estimate $\text{sing supp}(Tu) \subset \text{supp}(u)$. To prove the full statement, if $u \in \EC(\Omega)^*$, and $x_0 \not \in \text{sing supp}(u)$, then there is $\phi \in \DD(\Omega)$ which equals one in a neighborhood $U$ of $x_0$, such that $\phi u \in \DD(\Omega)$. Thus $T(\phi u) \in \EC(\Omega)$. But $\text{supp}((1 - \phi) u)$ does not contain $x_0$, which implies that $x_0$ does not lie in $\text{sing supp}(T((1 - \phi)u))$. But this means that both $T(\phi u)$ and $T((1 - \phi)u)$ are both smooth in a neighborhood of $x_0$, which means the same is true of $Tu$. Thus $x_0 \not \in \text{sing supp}(Tu)$.

    We have already proved (v) in a remark following the statement of the Schwartz kernel theorem.

    The proof of (vi) is left to the reader.
\end{proof}
%
...
TODO: See Rudin, Functional Analysis, or Treves, Chapter 29.
...
\section{Holomorphic Distributions}

TODO: Treves Chapter 22, and H\"{o}rmander (certain chapters).
...
\begin{example}
    ...
    %
    Here $c_d$ is the volume of the unit ball in $\RR^d$. To prove this, it suffices to show for any $\phi \in \DD(\RR^d)$,
    %
...
\begin{example}
    ...
    %
    and then $|\Phi(t,x)| = H(t) / (4 \pi t)^{n/2}$, which is not a locally integrable function. Nonetheless, the oscillation present here does allow us to interpret the distribution correctly, e.g. by interpreting $\Phi$ as the distribution such that the integral against any test function $\phi \in \DD(\RR \times \RR^d)$ is
    %
    \[ \lim_{\varepsilon \to 0^+} \int_\varepsilon^\infty \int_{\RR^d} \Phi(t,x) \phi(t,x)\; dx\; dt. \]
    %
    We claim that this is a distribution of order at most $n+1$. One way to do this is via the method of oscillatory integrals. First, assume that $\text{Re}(A^{-1}) = 0$, i.e. that $A^{-1} = -iB$, for some invertible matrix $B$. Then for any $\phi \in \DD(\RR \times \RR^d)$, and for any $t > 0$, stationary phase shows that if $\phi$ is supported on a compact set $K \subset \RR \times \RR^d$, then 
    %
    \begin{align*}
...
\end{lemma}
\begin{proof}
    It is simple to see that $\text{sing supp}(Lu) \subset \text{sing supp}(u)$. Conversely, fix a compact set $K \subset U$, and consider $\psi \in \DD(U)$ equal to one on a neighbourhood of $K$. Then for any distribution $u$ on $U$ with $Lu = 0$,
    %
    \begin{align*}
...
The family of such operators is very useful in analysis, since families of functions are more amenable to intuition than families of operators, and we ask questions such as what features of the functions $m$ imply about the resulting operators $m(D)$. Any \emph{constant coefficient} linear differential operator can be represented as $m(D)$ for a suitable polynomial $m(x)$, which allows us to use these results in the regularity theory of partial differential equations. For instance, these ideas immediately lead to a fairly powerful theory. For the next theorem, recall that a constant coefficient partial differential equation $\sum c_\alpha D^\alpha$ of degree $k$ is \emph{elliptic} if the homogeneous polynomial $\sum_{|\alpha| = k} c_\alpha \xi^\alpha$ is non-vanishing away from the origin.

\begin{theorem}
    Let $L = \sum c_\alpha D^\alpha$ be an elliptic partial differential operator with constant coefficients on $\RR^d$. Then $L$ has a fundamental solution, i.e. a distribution $\Phi \in \DD(\RR^d)$ such that $L(\Phi) = \delta$.
\end{theorem}
\begin{proof}
    Suppose $L$ has degree $k$. Since $L$ is elliptic, there exists $R > 0$ such that the polynomial $P(\xi) = \sum_\alpha c_\alpha \xi^\alpha$ satisfies $|P(\xi)| \sim |\xi|^k$ for $|\xi| \geq R$. If $\chi \in \DD(\RR^d)$ is chosen such that $\chi(\xi) = 1$ for $|\xi| \leq R$, and we define a distribution $\Phi_0$ such that
    %
    ...
    %
    Since $\chi \in \DD(\RR^d)$, it follows by the Paley-Wiener theorem, taking the inverse Fourier transform that $L \Phi_0 = \delta - w$, where $w$ is an entire analytic function of at most polynomial increase. Applying the Cauchy-Kovalevskaya theorem (expanding things in power series), we can find an entire analytic function $u$ of at most exponential increase such that $Lu = w$. Then $\Phi = \Phi_0 + u$ is a fundamental solution for $L$.
\end{proof}

The theory of pseudodifferential operators was mainly used to generalize the kinds of construction that occured in this proof. In particular, a \emph{(left) parametrix} for a linear differential operator $L$ with smooth coefficients on a domain $\Omega$ is an `approximate inverse' for $L$, i.e. an operator $S: \DD(\Omega) \to \DD(\Omega)^*$ such that $1 - S \circ L$ is a \emph{smoothing operator}. The existence of a regular parametrix, which will be justified by the theory of pseudodifferential operators, is quite important in the theory of differential equations. In particular, it proves that certain differential operators are \emph{hypoelliptic}, i.e. that if $u \in \DD(\Omega)$, then $\text{sing supp}(Lu) = \text{sing supp}(u)$ (the nontrivial property is that $\text{sing supp}(u) \subset \text{sing supp}(Lu)$).

\begin{theorem}
    If an operator $L$ has a very regular left parametrix $S$, then $L$ is hypoelliptic.
\end{theorem}
\begin{proof}
    For any very regular operator $S$, $\text{sing supp}(Su) \subset \text{sing supp}(u)$. It suffices to prove that for any \emph{compactly supported distribution}, $\text{sing supp}(u) \subset \text{sing supp}(Lu)$, since the general case follows by localization. Since $1 - S \circ L$ is a smoothing operator, we have
    %
    \begin{align*}
        \text{sing supp}(u) &\subset \text{sing supp}((1 - S \circ L) u) \cup \text{sing supp}((S \circ L) u)\\
        &\subset \text{sing supp}((S \circ L) u)\\
        &\subset \text{sing supp}(Lu). \qedhere
    \end{align*}
\end{proof}

In fact, pseudodifferential operators are \emph{microlocal}, i.e. they do not expand wavefront sets when applied to distributions. Thus it follows that if an operator $L$ has a left parametrix $S$ which is a pseudodifferential operator, then for any distribution $u$, $\text{WF}(Lu) = \text{WF}(u)$. We shall find that it is precisely the \emph{elliptic} differential operators that have pseudodifferential parametrices, and thus elliptic differential operators preserve wavefront sets.
...
\end{align*}
%
We use this integral formula to define the general operators $a(x,D)$.

In the most general setting, for any distribution $a(x,\xi)$ which is tempered in the $\xi$ variable, i.e. any distribution $a \in \DD(\RR^d_x)^* \CT \SW(\RR^d_\xi)^*$, and any $f \in \mathcal{S}(\RR^d)$, we can define $a(x,D) f$ to be the distribution such that for any $g \in \DD(\RR^d)$,
%
\[ \langle a(x,D) f, g \rangle = \int a(x,\xi) \widehat{f}(\xi) e^{2 \pi i \xi \cdot x} g(x)\; dx\; d\xi. \]
%
We call any operator $T$ which can be given in the form $a(x,D)$ a \emph{pseudodifferential operator}. The symbol $a$ is determined uniquely by the operator $T$, since the action of $a(x,D)$ determines the behaviour of $a$ on $\DD(\RR^d_x) \otimes \mathcal{S}(\RR^d_\xi)$, which is dense in $\DD(\RR^d_x) \CT \mathcal{S}(\RR^d_\xi)$. For any set $S$ of tempered distributions on $\RR^d \times \RR^d$, the notation $\text{Op}(S)$ is often used to denote the family of all pseudodifferential operators defined by an element of $S$.
...
Any pseudodifferential operator of the form above is continuous from $\mathcal{S}(\RR^d)$ to $\DD(\RR^d)^*$, and has kernel
%
...
%
converging absolutely in $\DD(\RR^d)^* \CT \SW(\RR^d)^*$, and then
%
\[ a(x,D) f = \sum_{i = 1}^\infty u_i \cdot (v_i * f) \]
%
is a linear combination of convolution operators.

As we increase the regularity of $a$, we no longer need to treat this integral quite as formally, and can define the operator for a more general family of functions. Here are some non-comprehensive examples of this phenomenon:
%
\begin{itemize}
    \item If $a \in \mathcal{S}(\RR^d \times \RR^d)^*$, then $a(x,D)$ extends to a continuous linear operator from $\mathcal{S}(\RR^d)$ to $\mathcal{S}(\RR^d)^*$. The Schwartz kernel theorem implies that any continuous linear operator from $\mathcal{S}(\RR^d)$ to $\mathcal{S}(\RR^d)^*$ is of this form, which means that a powerful general theory of pseudodifferential operators of this form is likely not possible.

    \item If $a$ is a local symbol of order $t$ on $\RR^d \times \RR^d$ for some $t \in \RR$, i.e. $a \in \loc{\mathcal{S}^t}(\RR^d \times \RR^d)$, then $a(x,D)$ extends to a continuous operator from $\mathcal{E}(\RR^d)^*$ to $\DD(\RR^d)^*$ and from $\DD(\RR^d)$ to $C^\infty_{\text{loc}}(\RR^d)$.

    \item If $a$ is a symbol of order $t$ on $\RR^d \times \RR^d$ for some $t \in \RR$, i.e. $a \in \mathcal{S}^t(\RR^d \times \RR^d)$, then $a(x,D)$ extends to a continuous operator from $\DD(\RR^d)$ to $\mathcal{S}(\RR^d)$.

    \item If $a \in \mathcal{S}^{-\infty}_{\text{loc}}(\RR^d \times \RR^d) = \lim_{t \to -\infty} \loc{\mathcal{S}^t}(\RR^d \times \RR^d)$, then we will see later in this chapter that $a(x,D)$ has a kernel in $C^\infty(\RR^d \times \RR^d)$, and extends uniquely to a continuous operator from $\mathcal{E}(\RR^d)^*$ to $C^\infty_{\text{loc}}(\RR^d)$. Thus $a(x,D)$ is a smoothing operator.
\end{itemize}
%
Thus the regularity of $a$ gives regularity of $a(x,D)$.

Suppose that $T: \DD(\RR^d) \to \EC(\RR^d)$ is a proper operator. Then it's convolution kernel $k$ lies in $\DD(\RR^d_x)^* \CT \EC(\RR^d_z)^*$, and in particular, is a continuous $\EC(\RR^d_z)^*$ valued function in the $x$-variable. Then we can take the Fourier transform of $k$ in the $z$-variable, giving us an element $a$ in
%
\[ \DD(\RR^d_x)^* \CT \left( C^\infty(\RR^d_\xi) \cap \mathcal{S}(\RR^d_\xi)^* \right). \]
%
We verify using the Fourier multiplication formula that
%
\[ a(x,D) f = \int a(x,\xi) \widehat{f}(\xi) e^{2 \pi i \xi \cdot x}\; d\xi = \int k(x,z) f(z-x)\; dz = T f. \]
%
Thus every proper operator is a pseudodifferential operator, which shows that properness, a kind of `locality' principle, implies between pseudodifferential. That such general operators are pseudodifferential indicates we must restrict the theory to a more regular family of operators to obtain results. We will mostly focus on pseudodifferential operators defined by various symbol classes, since most practical operator occuring ub PDE and analysis are of this form, and we can still get a sophisticated calculus.

\section{Basic Definitions}

Consider any symbol $a \in \loc{\mathcal{S}^t}(\RR^d \times \RR^d)$, i.e. a smooth function $a \in C^\infty(\RR^d \times \RR^d)$ such that for any $n_1,n_2,m \geq 0$,
%
\[ |\nabla_x^{n_1} \nabla_y^{n_2} \nabla_\xi^m a(x,\xi)| \lesssim_{n,m} \langle \xi \rangle^{t - m}, \]
%
where the implict constant is locally uniform in $x$. From this symbol, we can define a continuous operator $T_a: \DD(\RR^d) \to \EC(\RR^d)$ by setting
%
\[ T_a f(x) = \int a(x,\xi) e^{2 \pi i \xi \cdot x} \widehat{f}(\xi)\; d\xi, \]
%
where the integral can now be interpreted in the usual Riemann / Lebesgue sense. We say $T$ is then a \emph{pseudodifferential operator of order $t$}. If the estimates above are \emph{uniform} in $x$, i.e. $a \in \mathcal{S}^t(\RR^d \times \RR^d)$, then the operator $T_a$ extends to a map from $\mathcal{S}(\RR^d)$ to $C^\infty(\RR^d)$. We let $\Phi^t(\RR^d) = \text{Op}(\mathcal{S}^t)$, and $\Phi^t_{\text{loc}}(\RR^d) = \text{Op}(\loc{\mathcal{S}^t})$.
...
%
This means that we can actually define $T_a u$ for \emph{any} compactly supported distribution $u$, and we will find that $\text{WF}(T_a u) \subset \text{WF}(u)$. This is the first instance of the \emph{pseudolocal nature} of pseudodifferential operators; these operators roughly preserve the location of the mass and frequency support of a function, but with some additional `fuzz' that is usually neglible to the problem, but must be managed.
...
Here is a quantitative estimate on the kernel of a pseudodifferential operator, which shows it's pseudolocal nature. In particular, the result implies that $T_a$ maps Schwartz functions to Schwartz functions. Now the wavefront set calculation we performed shows that $K_a(x,y)$ agrees with a $C^\infty$ function away from the line $x = y$, and so $k_a(x,z)$ agrees with a $C^\infty$ function away from the line $z = 0$. Thus if $f \in \DD(\RR^d)$, and $x \not \in \text{supp}(f)$, then the multiplication formula for tempered distributions implies that
%
...
\begin{theorem}
    Let $a \in \loc{\mathcal{S}^t}(\RR^d)$. Then for any $n_1,n_2,N \geq 0$,
    %
...
\begin{proof}
    ...
    %
    so this case is easy. Thus, without loss of generality, in the remainder of the proof we may assume $a(x,\xi) = 0$ for $|\xi| \leq 1$. We can then perform a Littlewood-Paley decomposition, i.e. writing
    %
    \[ a(x,\xi) = \sum_{n = 0}^\infty a_n(x,y,\xi), \]
    %
    where $a_n(x,\xi) = \rho(\xi / 2^n) a(x,\xi)$ is supported on $|\xi| \sim 2^n$. Let $k_n$ be the convolution kernel for the pseudodifferential operator $a_n(x,D)$. Then
    %
    \[ k(x,z) = \sum_{n = 0}^\infty k_n(x,z), \]
    %
    where the convergence is in the sense of tempered distributions. We claim that for any $n$, and any $n_1,n_2,N \geq 0$,
    %
    ...
    %
    If $N$ is large, it follows that the sum $k = \sum k_n$ converges uniformly on any set of the form $\{ |z| > \varepsilon \}$ for some $\varepsilon > 0$. Moreover, summing up these bounds for sufficiently large $N$ gives the result for $|z| \geq 1$. For $0 < |z| \leq 1$, we break the sum into two parts, i.e. writing
    %
...
\begin{lemma}
    Let $k \in \DD(\RR^d \times \RR^d)^*$ be a convolution kernel which agrees with a $C^\infty$ function away from the plane $\{ z = 0 \}$. Suppose that for some $t$, and any indices $n_1,n_2,N \geq 0$, the kernel $k$ satisfies the growth condition
    %
    \[ \sup \left\{ |z|^{t + d + n_2 + N} |\nabla^{n_1}_x \nabla^{n_2}_z k(x,z)| : x \in \RR^d, z \in \RR^d - \{ 0 \} \right\} < \infty. \]
    %
    If $t < 0$, this implies that $k$ agrees with a function $\tilde{k}$ away from the origin that is uniformly integrable in the $z$-variable. Provided that the action of $k$ agrees with integration by this variable, i.e. $k = \tilde{k}$, so nothing strange is happening when $z = 0$, then $k$ is the kernel of some pseudodifferential opereator of order $t$.

    If $t \geq 0$, $k$ satisfies the growth conditions above, and in addition, for any bounded subset $\mathcal{B}$ of $\DD(\RR^d)$, satisfies the \emph{cancellation conditions}
    %
    \[ \sup \left\{ R^{-t} \left| \int D^\alpha_x k(x,z) \phi(z/R)\; dz \right| : x \in \RR^d, z \in \RR^d - \{ 0 \}, \phi \in \mathcal{B} \right\} < \infty, \]
    %
    where the implicit constant is a continuous seminorm on $\DD(\RR^d)$ for each multi-index $\alpha$, then $k$ is the kernel of some pseudodifferential operator of order $t$.
\end{lemma}
\begin{proof}
...
%
This is correct up to first order, and in fact, we can obtain a complete asymptotic development as $\lambda \to \infty$. For simplicity, we assume $\text{supp}(a)$ is compact.

\begin{theorem}
    Fix a symbol $a \in \mathcal{S}^t(\RR^d \times \RR^d)$, compactly supported in the $x$-variable, a smooth function $f \in C^\infty(\RR^d)$, and a smooth, real-valued function $\phi$ on $\RR^d$ with $\nabla \phi$ nonvanishing on $\text{supp}_x(a)$. Let
    %
    \[ \tilde{\phi}(x,y) = \nabla \phi(x) \cdot (x - y) - (\phi(x) - \phi(y)). \]
    %
    Then for any $N > 0$, we can write $e^{-2 \pi i \lambda \phi(x)} T_a \{ f e^{2 \pi i \lambda \phi} \}(x)$ as
    %
    \begin{align*}
        \sum_{|\beta| < N} \frac{1}{\beta! \cdot (2 \pi i)^{\beta}} \cdot D_\xi^\beta a(x,\lambda \nabla \phi(x)) \left. D^\beta_y \{ e^{2 \pi i \lambda \tilde{\phi}(x,y)} f \} \right|_{y = x} + R_N(x,\lambda),
    \end{align*}
    %
    where $\lambda^{t - \lceil N/2 \rceil} R_N \in L^\infty(\RR^d \times (0,\infty))$. In particular, for $N = 3$,
    %
    we can write $e^{-2 \pi i \lambda \phi(x)} T_a \{ f e^{2 \pi i \lambda \phi} \}(x)$ as
    %
    \begin{align*}
        &a(x,\lambda \nabla \phi(x)) f(x)\\
        &\quad + \frac{1}{2\pi i} \sum_{k = 1}^d D^k_\xi a(x,\lambda \nabla \phi(x)) \cdot D^k f(x)\\
        &\quad - \frac{i \lambda}{4 \pi} \sum_{|\beta| = 2} D^\beta_\xi a(x,\lambda \nabla \phi(x)) \cdot D^\beta \phi(x) \cdot f(x)\\
        &\quad\quad\quad + O(\lambda^{t - 2}).
    \end{align*}
\end{theorem}
\begin{proof}
    We write
    %
    \[ e^{-2 \pi i \lambda \phi(x)} T_a \{ f e^{2 \pi i \lambda \phi } \}(x) = \lambda^d \int e^{2 \pi i \lambda ( (\xi - \nabla \phi(x)) \cdot (x - y) - \tilde{\phi}(x,y))} a(x, \lambda \xi) f(y)\; d\xi dy. \]
    %
    Since $|\tilde{\phi}(x,y)| \lesssim |x - y|^2$, the principal contributions to this integral occurs when $y \approx x$ and $\xi \approx \nabla \phi(x)$. Without loss of generality, we may therefore assume that $a(x,\xi) = 0$ whenever it is not true that $|\xi| \sim \lambda$. We rewrite our integral as
    %
    \[ \lambda^d \int e^{2 \pi i \lambda ( \xi \cdot (x - y) - \tilde{\phi}(x,y) )} a(x, \lambda \nabla \phi(x) + \lambda \xi) f(y)\; d\xi\; dy. \]
    %
    ...
    %
    \[ \left| \lambda^d \int \int e^{2 \pi i \lambda ((x - y) \cdot \xi + \tilde{\phi}(x,y))} R_{N,\lambda}(x,\xi) f(y)\; d\xi\; dy \right| \lesssim \lambda^{t - \lceil N/2 \rceil}. \]
    %
    Finally, we note that via an integration by parts,
    %
    \begin{align*}
        \int & e^{2 \pi i \lambda (\xi \cdot (x - y) - \tilde{\phi}(x,y))} \xi^\beta f(y)\; d\xi\; dy\\
        &= (1/\lambda)^{d + \beta} (2 \pi i)^{-\beta} \left. D_y^\beta \{ e^{- 2 \pi i \lambda \tilde{\phi}(x,y)} f(y) \} \right|_{y = x}
    \end{align*}
    %
    and substituting them into the formula completes the proof.
\end{proof}

As the order of the symbol $a$ decreases, we expect the behaviour of the corresponding pseudodifferential operator to become more and more regular. In particular, for $t < - d$, the symbol is actually \emph{integrable} in $\xi$. A $\Psi$DO of order $-\infty$ has a kernel $k$ lying in $C^\infty(\RR^d \times \RR^d)$, and satisfying estimates of the form
%
\[ | \nabla^{n_1}_x \nabla^{n_2}_z k(x,z)| \lesssim_{n,N} \langle z \rangle^{-N} \]
%
for any $N \geq 0$. Thus a $\Psi$DO of order $-\infty$ is smoothing. Conversely, if $k$ satisfies estimates of the form above, then it is the $\Psi$DO corresponding to the symbol
%
\[ a(x,\xi) = \int k(x,z) e^{2 \pi i \xi \cdot z}\; dz, \]
%
which is a symbol of order $-\infty$. In particular, all properly supported smoothing operators are $\Psi$DOs of order $-\infty$.

Most of the theory of pseudodifferential operators works \emph{modulo smoothing operators}, which is mostly fine in harmonic analysis, where we attempt to find localized estimates, and smoothing operators naturally satisfy all the necessary localized estimates we might be interested in. In particular, this makes the theory more flexible. For instance, one interesting operator to study is the square root of the Laplacian, which is the pseudodifferential operator with symbol $a(x,\xi) = 2 \pi |\xi|$. The fact that $a$ is not smooth near the origin means that $a(x,D)$ does not quite fit the theory of pseudodifferential operators given above. Nonetheless, for any bump function $\rho \in \DD(\RR^d)$ equal to one in a neighborhood of the origin, the symbol $\tilde{a}(x,\xi) = 2 \pi |\xi| (1 - \rho(\xi))$ \emph{is} of order one, and since $a(x,\xi) - \tilde{a}(x,\xi)$ is compactly supported in the $\xi$ variable, it follows that $a(x,D) - \tilde{a}(x,D)$ is a smoothing operator. To remedy this situation, we define $\dot{\mathcal{S}}^t$ to be the space of all distributional symbols which differ from an element of $\loc{S}^t$ by a distributional symbol inducing a smoothing operator. Then $a$ is an element of $\dot{\mathcal{S}}^1$.

Thus, abusing notation, we might speak of $a(x,\xi) = 2 \pi |\xi|$ as being a symbol of order one, since $a(x,D)$ differs from a pseudodifferential operator of order one. One important application of this is to the theory of pseudodifferential operators defined in terms of homogeneous functions, since homogeneous functions often fail to be smooth at the origin.
...
\begin{remark}
    If $b(x,\xi) = a(x,x,\xi)$, then the formula above can be written more formally as $a(x,y,\xi) \sim e^{2 \pi i D_x \cdot D_\xi} b$, where $D_x$ and $D_\xi$ are the normalized derivative operators defined above. This makes sense, since if $K$ is the kernel of $T_a$, and $T_a$ corresponds to a pseudodifferential operator, then it's symbol would correspond precisely to
    %
    \begin{align*}
    ...
    \end{align*}
    %
    If we define $a_x(y,\xi) = a(x,y,\xi)$, and $c(x,\xi) = e^{-2 \pi i \eta \cdot y}$, then $a(x,\xi) = (a_x * c)(x,\xi)$. But $c$ is a Gaussian, and thus if $(\widehat{x}, \widehat{\xi})$ are the dual variables to $(x,\xi)$, then $\widehat{c}(\widehat{x}, \widehat{\xi}) = e^{2 \pi i \widehat{\xi} \cdot \widehat{x}}$. Thus if $D_x$ and $D_\xi$ are the differential operators normalized by $2\pi i$ introduced at the beginning of the section (the Fourier multiplier operators corresponding to multiplication by $\widehat{x}$ and $\widehat{\xi}$ respectively), then we can write
    %
...
Let $a(x,D)$ and $b(x,D)$ be pseudodifferential operators defined by local symbols of order $t$ and $s$. It is not always possible to define the composition $a(x,D) \circ b(x,D)$. This is because the image of an element of $\DD(\RR^d)$ under the operator $b(x,D)$ in general lies in the Schwartz space $\mathcal{S}(\RR^d)$, and one cannot necessarily apply $a(x,D)$ to Schwartz functions. One way to fix this is to assume that $a(x,D)$ is defined by a symbol lying in $\mathcal{S}^t$ rather than $\loc{\mathcal{S}^t}$, for then $a(x,D)$ extends to a map from $\mathcal{S}(\RR^d)$ to itself, which means we can define $a(x,D) \circ b(x,D)$ on $\SW(\RR^d)$. Another approach is to assume that $b(x,D)$ maps $\DD(\RR^d)$ to itself, which is true in particular if $b(x,D)$ is a proper pseudodifferential operator.
...
\begin{remark}
    This result shows that $\Phi = \bigcup_t \Phi^t$ and the subfamily $\Phi_{\text{loc},\text{prop}}$ of proper pseudodifferential operators in $\Phi_{\text{loc}} = \bigcup_t \Phi^t_{\text{loc}}$ form graded algebras. We note for any pseudodifferential operator $T \in \Phi^t_{\text{loc}}$, there is a properly supported pseudodifferential operator $\tilde{T} \in \Phi_{\text{loc},\text{prop}}^t$ such that $T - \tilde{T}$ is a smoothing operator. If we define $\dot{\Phi}^t$ to be the space of all pseudodifferential operators which differ from an element of $\loc{\Phi}_t$ by a smoothing operator, modulo the set of smoothing operators, then $\dot{\Phi} = \bigcup_t \dot{\Phi}^t$ is a graded algebra under addition and composition, which induces, by bijection, an alternate graded algebra structure on $\dot{\mathcal{S}}$, agreeing up to first order with the standard algebra structure on this space given by multiplication.
\end{remark}

\section{Parametrices for Elliptic Operators}

A \emph{pseudodifferential parametrix} for a pseudodifferential operator $T$ is a pseudodifferential operator $S$ such that $S \circ T$ is the identity operator, modulo smoothing operators. One useful result of our calculations is that we can easily construct \emph{parametrices} for suitable pseudodifferential operators. A symbol $a \in \dot{\mathcal{S}}^t$ is called \emph{elliptic} if in a neighborhood of each $x_0$, there exists $R > 0$ and $C_1,C_2 > 0$ such that for $|\xi| \geq R$,
%
\[ C_1 (1 + |\xi|)^t \leq |a(x,\xi)| \leq C_2 (1 + |\xi|)^t. \]
%
In this case, $b_0(x,\xi) = 1 / a(x,\xi)$ is a well defined element of $\dot{\mathcal{S}}^{-t}$, and by the composition calculus, $1 - (a \circ b_0)(x,\xi)$ is a symbol $r_1 \in \dot{\mathcal{S}}^{-1}$. For $i \geq 1$, given $r_i \in \dot{\mathcal{S}}^{-i}$, if we define $b_i = r_i / a$ in $\dot{\mathcal{S}}^{t-i}$, then the composition calculus tells us that $r_{i+1} = 1 - (a \circ (b_0 + \dots + b_i))$ lies in $\dot{\mathcal{S}}^{-i-1}$, and we can continue the iteration. If we consider $b \in \dot{\mathcal{S}}^{-t}$ is defined by the asymptotic development
%
...
%
so $b = c$, modulo smoothing operators. Thus a left parameterix for an elliptic operator is automatically a right parameterix.

\begin{remark}
    The condition that $|a(x,\xi)| \sim \langle \xi \rangle^t$ for large $\xi$ is necessary in order to construct a parametrix of order $-t$. Without loss of generality, it suffices to analyze the case $t = 0$, since in general we can replace $a(x,\xi)$ with $a(x,\xi) \langle \xi \rangle^{-t}$ and $b(x,\xi)$ with $b(x,\xi) \langle \xi \rangle^t$. If there is $b \in \dot{\mathcal{S}}^0$ such that $b(x,D)$ is a parametrix for $a(x,D)$, then the composition calculus tells us that $1 - a(x,\xi) b(x,\xi)$ is a local symbol of order $-1$. Thus in a neighborhood of any $x_0$, there exists $R > 0$ such that for $|\xi| \geq R$,
    %
    \[ |a(x,\xi) b(x,\xi) - 1| \leq 1/2, \]
    %
    so that $|a(x,\xi)| \geq 1/2 |b(x,\xi)|$, and combined with the fact that we can choose this neighborhood and $R > 0$ such that $|b(x,\xi)| \lesssim 1$, this implies that $|a(x,\xi)| \gtrsim 1$.
\end{remark}

It follows from our theory that if $T$ is an elliptic pseudodifferential operator on $\Omega$, then for any $u \in \EC(\Omega)^*$, $\text{sing supp}(Tu) = \text{sing supp}(u)$. This is very similar to hypoellipticity, except that for differential operators, this condition can be applied to arbitrary distributions, not just the compactly supported distributions.
...
\begin{theorem}
    For any $a \in \loc{\mathcal{S}^t}(\RR^d)$, $a(x,D)$ extends uniquely to a continuous operator from $H^s(\RR^d)$ to $H^{s-m}_{\text{loc}}(\RR^d)$.
\end{theorem}
\begin{proof}
...
\begin{theorem}
    ...
    %
    \[ a_\kappa(\kappa(x),\eta) \sim \sum \frac{1}{\alpha!} (D^\alpha_\xi a) (x,D\kappa(x)^T \eta) \left. \left\{ D^\alpha_y e^{2\pi i \eta \cdot r_x(y)} \right\} \right|_{y = x}, \]
    %
