...
\begin{example}
    ...
    %
    \[ E(x) = \begin{cases} (2\pi)^{-1} \log |x| &: \text{if $d = 1$} \\ \frac{1}{c_d (d-2) |x|^{d-2}} &: \text{if $d \geq 1$.} \end{cases} \]
    %
    ...
    %
    \[ \int_{|x| \geq \varepsilon} \nabla E(x) \cdot \nabla \phi(x)\; dx = \int_{|x| = \varepsilon} \phi(x)\ [\nabla E(x) \cdot n(x)] dS - \int_{|x| \geq \varepsilon} \Delta E(x) \phi(x)\; dx = \int_{|x| = \varepsilon} \phi(x)\ [\nabla E(x) \cdot n(x)] dS. \]
...
\begin{example}
    ...
    %
    This fundamental solution is \emph{not necessarily locally integrable} as $t \to 0$, but it is a distribution order at most $d + 1$. To verify this is a fundamental solution, it is simple to see that $LE = 0$ away from $t = 0$. Gauss' theorem implies that provided $E$ is locally integrable, then for any $\phi \in C_c^\infty(\RR^{d+1})$,
    %
