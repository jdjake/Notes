...
%
is well defined for any $f$ bounded in a ball around the origin, and with a suitable decay for the terms of the limit to exist (for instance, any decay on the order of $O(1/|x|^\delta)$ will do, for some $\delta$), because using the fact that $r^{d-1} \Phi(r) = o(1)$ as $r \to 0$, and $f$ is bounded near the origin, we conclude
%
\[ \int_{\varepsilon_0 \leq |x| \leq \varepsilon_1} f(x) \Phi(x) = O \left( \int_{\varepsilon_0}^{\varepsilon_1} r^{d-1} \Phi(r) \right)\; dr  = o(1) \]
%
as $\varepsilon_1 \to 0$. Thus the limit is Cauchy. In particular, we have
%
\[ \left| \int_{\mathbf{R}^n} f(x) \Phi(x) \right| \lesssim \| f \|_{L^\infty(B_1)} + \| x f \|_{L^\infty(\mathbf{R}^d - B_1)} \]
%
and so $\Phi$ can be interpreted as a tempered distribution. In particular, we can calculate $\Delta \Phi$ in a distribution sense.
...
\begin{proof}
    ...
    %
    \[ D_\varepsilon = - \int_{|x| > \varepsilon} (\nabla \Phi) \cdot (\nabla f) = \int_{S_\varepsilon} f (\nabla \Phi) \cdot \eta + \int_{|x| > \varepsilon} f (\Delta \Phi \cdot \eta) = \int_{S_\varepsilon} f \nabla \Phi \cdot \eta \]
    %
...
Harmonic functions possess an incredibly useful integral formula which makes them as regular as we desire.
...
%
\[ \fint_{B_r(x)} f(y)\; dy = \frac{1}{r^d \alpha(d)} \int_0^r \int_{\partial B_s(x)} f(y)\; dy\; ds = d\alpha(d) f(x) \int_0^r\; s^{d-1} ds = r^d d \alpha(d) f(x) \]
