...
\end{align*}
%
So the $L^1$ norm of $D_n$ grows, albeit slowly, to $\infty$. This reflects the fact that $D_n$ oscillates very frequently, and also that the pointwise convergence of the Fourier series is much more subtle than that provided by good kernels. However, in the next section we show that, if we interpret our convergence of series in a different manner, we get a family of good kernels, and therefore we obtain pointwise convergence for suitable reinterpretations of partial sums.
...
\section{Returning to Harmonic Functions on the Disk}
...
%
then $f * K_\delta$ converges to $f$ in the $L^1$ norm as $\delta \to 0$. If in addition, the $K_\delta$ satisfy $|K_\delta(x)| \lesssim \delta^{-n}$, and $|K_\delta(x)| \lesssim \delta/|x|^{n+1}$, then $f * K_\delta$ converges to $f$ almost everywhere. A common way to form such a kernel is to take some function $K \in L^1(\mathbf{R}^n)$ with $\int K = 1$ and to write $K_\delta(x) = \delta^{-n} K(x/\delta)$. The integrality condition is expressed by the fact that
%
