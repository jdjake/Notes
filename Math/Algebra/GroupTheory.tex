...
In the next proof, we note that for any set $X$ and any two permutations $\pi, \sigma \in S(X)$, and any $x \in X$, recalling $\prescript{\pi}{}{\sigma} = \pi \circ \sigma \circ \pi^{-1}$, then
%
...
Let $p$ be a prime number. A group $G$ is called a \emph{p-group} \index{p-group} if the groups order is a power of $p$.
...
Let $G$ be a group of order $p^mq$, where $p$ is a prime and $q$ and $p$ are relatively prime. Then a subgroup is called a \emph{p-Sylow Subgroup} \index{Sylow Subgroup} if the order of the subgroup is a power of $p^m$ -- the maximum order of a $p$ subgroup in $G$.
