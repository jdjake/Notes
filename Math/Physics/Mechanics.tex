...
%\begin{proof}
%    Consider first the case of three objects. Let $A = F(1,2)$, $B = F(2,3)$, and $C = F(3,1)$. Given the accelerations and masses of the three objects, we can determine the three cumulative forces
    %
%    \[ F(1) = -A + C \quad F(2) = A - B \quad F(3) = B - C, \]
    %
%    which we can write as the matrix equation
    %
%    \[ F = \begin{pmatrix} -1 & 0 & +1 \\ +1 & -1 & 0 \\ 0 & +1 & -1 \end{pmatrix} \begin{pmatrix} A \\ B \\ C \end{pmatrix}. \]
%\end{proof}
...
% Mass 1 vs. Mass m
% Mass 1 Accelerates at a magnitude a, Mass m at a magnitude b
% a = mb
% So m can be determined from this.

% Three Forces F12, F13, F23
% Mass 1, M2, M3
% Accelerations e_1, X, Y
%
% Then
%   -F_{12} - F_{13} = e_1
%   M2 ( F_{12} - F_{23} ) = X
%   M3 ( F_{13} + F_{23} ) = Y
%
% We have nine equations, and 11 unknowns
%
% If the forces are assumed to be radial, we have
% nine equations in 5 unknowns, so the system is determined
%


On the other hand, we are only directly able to observe the accelerations of these $N$ objects. We thus only observe $3N$ parameters of the problem, and since $3N < N + 3 \binom{N}{2}$ for $N > 3$, it is impossible to recover the masses and forces on a family of objects solely from what can be directly observed.
...
%The first law and second law can be used to define mass operationally, provided that we have some way to apply a constant force with fixed magnitude. Given two objects with masses $m_0$ and $m_1$, if we apply a constant force of equal magnitude to each, and measure the resulting magnitudes of acceleration $a_0$ and $a_1$ (which are essentially directly observable from the positions of the object at different times), then Newton's Second Law states that $m_0 / m_1 = a_0 / a_1$, so we have a way to directly compare the masses of two different objects (if linear acceleration is hard to measure, one can instead apply a centripedal force constraining the objects, and then measure rotational acceleration).

On the other hand, if we make assumptions on the relative strength and behaviour of the various forces, it \emph{is} possible to recover the masses of the objects, and once th
...
\chapter{Oscillations: Linear Differential Equations}
...
%
for some new component functions $p_r, p_\theta: I \to \RR$. In the variables $(r,\theta,z, v_r, v_\theta, v_z)$,
%
...
\begin{align*}
    F_r &= (\cos \theta) F_x + (\sin \theta) F_y\\
        ...
        &= \frac{dp_r}{dt} - \frac{r p_\theta^2}{m},
\end{align*}
%
and
%
...
%
% d/dq^i = (dx^a/dq^i) (d/dx^a)
% sum q'^i d/dq^i = sum q'^i (dx^a/dq^i) (d/dx^a)
%    so
% x'^a(v_p) = sum q'^i(v_p) (dx^a/dq^i)(p)
%
% Differentiating with respect to dot{q}^i gives
%
% dx'^a / dq^j = sum q'^i (d^2x^a/dq^i dq^j)

% (d/dx^a)(p) = sum (dq^i/dx^a)(p) (d/dq^i)(p) + (dr^i/dx^a) (d/dr^i)(p)
%\
% x'^a = 
%
\[ \frac{\partial \dot{x}^\alpha}{\partial \dot{q}^i} = \frac{\partial x^\alpha}{\partial q^i}. \]
...
Look up \emph{Hamilton's optico-mechanical analogy}.
