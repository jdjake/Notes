...
\begin{remark}
    There is also a variant of the theorem where the charts $\{ x_\alpha \}$ can be bijections into open submanifolds of a family of manifolds rather than open subsets of Euclidean space.
\end{remark}
...
\end{theorem}
\begin{proof}
    Let $M$ be a manifold, let $p \in M$ be a point, and let $U$ be a neighbourhood of $p$. Then there is a coordinate chart $(x,V)$, where $V \subset U$ and $p \in V$.I f $B$ is a closed ball around $x(p)$ in $x(U)$, then $B$ is a compact neighbourhood of $x(p)$, hence $x^{-1}(B)$ is a compact neighbourhood of $p$, contained in $U$. Thus $M$ has a basis of pre-compact coordinate balls, and is therefore locally compact.
\end{proof}
...
Let us be given a map $f:M \to N$ between manifolds. Given a correspondence $b = f(a)$, a reasonable inquiry would be to consider two charts $(x,U)$ and $(y,V)$, where $U$ is a neighbourhood of $a$ and $V$ is a neighbourhood of $b$. We obtain a map $y \circ f \circ x^{-1}$, defined between open subsets of Euclidean space. We have `expressed $f$ in coordinates'. The function $f$ shall then be differentiable at $a$ if $y \circ f \circ x^{-1}$ is differentiable at $x(a)$. Unfortunately, without additional structure this idea is doomed to fail, for we can hardly expect that the statement holds for all charts when it holds for a single pair of them.
...
This implies, in particular, that we may specify a smooth structure by just giving a family of $C^\infty$ related transition maps covering a manifold, which is often easier to specify than giving a complete maximal atlas.
...
In particular, we can define the notion of a $C^k$ atlas on a manifold $M$ for any $k \geq 1$. Two charts $(x,U)$ and $(y,V)$ are $C^k$ related if $U \cap V = \emptyset$, or $U \cap V \neq \emptyset$ and $y \circ x^{-1}$ is a $C^k$ map on Euclidean space. Thus a $C^k$ manifold is a manifold equipped with a $C^k$ atlas, and on a $C^k$ manifold we can consider differentiable maps, and moreover, $C^i$ maps for all $i \leq k$.

Topologically speaking, the family of $C^k$ manifolds is no more general than a $C^\infty$ manifold, since any $C^k$ manifold can be given a $C^\infty$ structure. But sometimes we can equip a manifold with a $C^k$ atlas naturally, but it is not natural to equip it with a smooth structure. Our first example of this occurs in our discussion of submanifolds.

Alternatively, we can consider an even more rigid structure. If we require transition maps between charts to be (real) analytic, and we build up an atlas of charts compatible with respect to this requirement, we obtain a \emph{real-analytic manifold}. If the dimension of the manifold is equal to $2n$, and we identify $\RR^{2n}$ with $\CC^n$, then we can require our transition maps to be holomorphic, we obtain the family of complex manifolds. But we do not dwell on these classes of manifolds here.

\begin{example}
    Let $M$ be a manifold, and $U$ an open submanifold. Then we can define an atlas on $U$, by considering all smooth charts $(x,V)$ on $M$, with $V \subset U$. This is a maximal atlas, and is the unique smooth structure such that
    %
    \begin{enumerate}
...
\begin{example}
    On $\mathbf{R}^2$, we have polar coordinates $(\theta, \mathbf{R}^2 - \{0\})$ defined `in inverse coordinates' by the map $\theta^{-1}(r,u) = re^{iu}$. This chart is locally injective and has full rank at every point, so that $\theta^{-1}$ is actually invertible, and $\theta$ is $C^\infty$, by the inverse function theorem. Thus the polar coordinate system is a smooth coordinate system. On $\mathbf{R}^3$, we have the spherical and cylindrical coordinate systems to work with, which one can also verify are smooth coordinate systems.
\end{example}
...
An assignment of orientation can also been as a `section', not of the tangent bundle, but of an associated `orientation bundle'. Orientations on a vector space do not form a vector space, so this orientation bundle is not a vector bundle, but instead a member of a more general family called `fibre bundles'. Given a topological space $M$, known as a \emph{model space}, and a \emph{base space} $X$, we consider the family of all \emph{$M$ bundles}, which are topological spaces $E$ together with projection maps $\pi: E \to X$ such that each $x \in X$ has a neighbourhood $U$ such that $\pi^{-1}(U)$ is homeomorphic to $M \times U$. Of course, we can form the \emph{trivial $M$ bundle} $\varepsilon(X,M) = M \times X$, but there are certainly more interesting bundles.

The orientation bundle is a fibre bundle with model space $M = \{ -1, 1 \}$. For each $n$ dimensional vector bundle $\xi$, consider the $M$ bundle $O(\xi)$ whose topological extension space is $E = F/\sim$, where $F$ is the subspace of the $n$ fold direct sum $\xi \oplus \dots \oplus \xi$ consisting solely of $v^1_p \oplus \dots \oplus v^n_p$ where $\{ v^1_p, \dots, v^n_p \}$ are linearly independant, and therefore form a basis, and $\sim$ identifies $v^1_p \oplus \dots \oplus v^n_p$ with $w^1_p \oplus \dots \oplus w^n_p$ if the linear isomorphism $T: T_pM \to T_pM$ with $T(v^i_p) = w^i_p$ for each $i$ has positive determinant (the determinant is basis independant and thus defined on automorphisms of any vector space). In particular, this means that for each $x \in X$, $E_x$ consists of two points, each corresponding to the two orientations of $F_x$. If $\xi = \varepsilon^n(X)$, then it is easy to see that $O(\xi)$ is just $\varepsilon(X,M)$, where we identify $a_x \in (M \times X)_x$ with the equivalence class of $v^1_p \oplus \dots \oplus v^n_p \in (\RR^n \times X)^n_p$ where $\det(v^1, \dots, v^n) > 0$, if $a = 1$, and the class with $\det(v^1, \dots, v^n) < 0$, if $a = -1$. This is easily verified to be continuous. Since any vector bundle $\xi$ is locally equivalent to a trivial bundle, one can use this fact that show that $O(\xi)$ is locally equivalent to a trivial bundle over $M$. Thus $O(\xi)$ is a fibre bundle. One can view a choice of orientation on $\xi$ as a continuous section of $O(\xi)$, which results in the following theorem.

\begin{theorem}
    If $X$ is connected, then $\xi$ is orientable if and only if there exists a global section $s: X \to O(\xi)$, and then $O(\xi)$ is equivalent to $\varepsilon(X,M)$, corresponding to the two choices of orientation on $\xi$.
\end{theorem}
...
\begin{example}
    If $\xi = \pi: \mathbf{M} \to S^1$ is the M\"{o}bius bundle over $S^1$, then $O(\xi)$ is equivalent to the fibre bundle $\eta: S^1 \to S^1$ given by $\eta(z) = z^2$, viewing $S^1$ as the unit circle in the complex plane. We essentially argued that $\xi$ was not orientable by tracing this fibre bundle around the manifold.
\end{example}
...
\begin{example}
    For any smooth real-valued function $f: M \to \mathbf{R}$, we can define a covector field $df: M \to T^*M$, such that $df_p(v) = v(f)$. The covector field $df$ is known as the {\emph differential} of $f$. If $X$ is a vector field, then $df(X) = X(f)$ is a smooth function.
\end{example}
...
%
Thus we have a coordinate independant way to find a critical points on the set. In the special case where $g: M \to \mathbf{R}$, we need only find $\lambda$ such that $df(p) = \lambda dg(p)$.
...
%
If $(y,V)$ is another coordinate system, then there are functions $b_{i_1 \dots i_n}^{j_1 \dots j_m} \in C^\infty(V)$ such that on $V$,
%
...
There is one way to switch our point of view from vector fields so that global existence results may be obtained. Consider a manifold $M$, and suppose that for each point $p \in M$, we associate a one-dimensional subspace $\Delta_p \in M$. If we assume that around each point $p \in M$, we can find a neighbourhood $U$ and a smooth vector field $X \in \Gamma(TU)$ such that $\Delta_q$ is spanned by $X_q$ for each $q \in U$. Our goal is now to find a one-dimensional immersed submanifold $i: N \to M$ such that $i_*(T_pN) = \Delta_{i(p)}$ for each $p \in N$. The existence and uniqueness theory of differential equations gives the local existence of the manifold $N$, but the advantange of this approach is that we can join overlapping solutions. Thus $M$ can be written as the disjoint union of connected manifolds $N$. The function $\Delta$ is known as a \emph{one dimensional distribution}.

A function $\Delta$ on a manifold $M$ is called a \emph{$m$ dimensional distribution} if each associates with each point $p \in M$ with a $m$-dimensional plane $\Delta$ in $T_p M$ (these are nothing to do with the distributions of analysis), and such that locally around each point $p \in M$, there exists a neighbourhood $U$ and $m$ linearly independent vector fields $X^1, \dots, X^m \in \Gamma(TU)$ such that $\Delta_q$ is spanned by $X^1_q, \dots, X^m_q$ for each $q \in U$. Our goal, given an $m$ dimensional distribution $\Delta$, is to find an immersed $m$ dimensional submanifolds $i: N \to M$ such that $i_*(T_pN) = \Delta_{i(p)}$ for each $p \in N$, which we call an \emph{integral manifold} to $\Delta$.

In general, the problem of finding integral manifolds is not, in general, solvable. The simplest example is given in $\RR^3$ by the distribution $\Delta$ given by the equation $dz = y dx$, i.e. such that
%
...
\begin{example}
    ...
    %
    then $\phi$ automatically solves the Cauchy Riemann equations. Setting mixed derivatives equal here, shows integrality conditions hold provide the Cauchy-Riemann equations hold for $f$, and thus a function $\phi$ exists locally in a neighbourhood of any point.
\end{example}
...
The most important property of $f$ relatedness is it's relation to the Lie bracket.
...
%
(at least over a field which is of characteristic not equal to two). We let $\Omega^k(V)$ denote the family of all alternating tensors.

If $T$ is alternating, then for any coefficients $A = a^{ij}$,
%
\[ T \left(\sum_{j = 1}^k a^{1j} v_j, \dots, \sum_{j = 1}^k a^{kj} v_j \right) = \det(A) \cdot T(v_1, \dots, v_k). \]
%
We let $\Omega^k(V)$ denote the family of all alternating tensors. In particular, if, for each permutation $\sigma \in S_k$, we let
%
\[ (v_1, \dots, v_k) \cdot \sigma = (v_{\sigma(1)}, \dots, v_{\sigma(k)}), \]
%
Then we have a right group action, i.e. $((v_1, \dots, v_k) \cdot \sigma) \cdot \tau = (v_1, \dots, v_k) \cdot (\sigma \tau)$. In particular, given any $T \in T^k(V)$, we let
%
\[ (T \cdot \sigma)(v_1, \dots, v_k) = T((v_1, \dots, v_k) \cdot \sigma) \]
%
then we have a right group action on the space of tensors. We note that if $T$ is alternating, then for any $\sigma \in S_k$, $T \cdot \sigma = \text{sgn}(\sigma) \cdot T$. In particular, if we let
%
\[ \text{Alt}(T) = \frac{1}{k!} \sum_{\sigma \in S_k} T \cdot \sigma, \]
%
then $\text{Alt}(T) \in \Omega^k(V)$, and if $T \in \Omega^k(T)$, $\text{Alt}(T) = T$. Thus $\text{Alt}$ is a projection map from $T^k(V)$ to $\Omega^k(V)$.
...
%
where the funny coefficient is not essential, but makes some things work more nicely. We note that $\wedge$ is bilinear, and `anti-commutative', i.e. $\omega \wedge \nu = (-1)^{kl} \nu \wedge \omega$. Furthermore, if $f: W \to V$, then $f^*(\omega \wedge \nu) = f^*(\omega) \wedge f^*(\nu)$. It takes some work, but the wedge product is also associative.

\begin{lemma}
    If $S \in T^k(V)$, $T \in T^l(V)$, and $\text{Alt}(S) = 0$, then $\text{Alt}(S \otimes T) = \text{Alt}(T \otimes S) = 0$.
\end{lemma}
\begin{proof}
    We calculate
    %
    \[ (k+l)! \text{Alt}(S \otimes T) = \sum_{\sigma \in S_{k+l}} \text{sgn}(\sigma) \cdot (S \otimes T) \cdot \sigma. \]
    %
    ...
    %
    Thus summing over each representative completes the calculation in the case $\text{Alt}(S \otimes T)$. $\text{Alt}(T \otimes S)$ is treated similarily.
\end{proof}
...
Thus the wedge produce turns $\oplus_k \Omega^k(V)$ into a graded algebra, which has an identity if we let $\Omega^0(V)$ denote real numbers, and $a \wedge \omega = a \omega$. Unlike $T(V)$, $\Omega(V)$ is finite dimensional if $V$ is finite dimensional.
