...
\begin{remark}
    If we define $V,W \subset \RR^d$ to be transversal if $V + W = \RR^d$, then two manifolds $M$ and $N$ are transversal if $T_p M$ and $T_p N$ are transversal at each $p \in M \cap N$.
\end{remark}
...
Things get only slightly more technical if we deal with manifolds with boundary, but we still need some knowledge of transversality in this scenario to deal with homotopies of maps. Recall that if $f: M^n \to N^m$, $\partial N = \emptyset$, and $q \in N$ is a regular value of both $f$ and $\partial f: \partial M \to N$, then $f^{-1}(q)$ is a manifold with boundary, and $\partial (f^{-1}(q)) = (\partial f)^{-1}(q)$. We can extend this notion to obtain a kind transversality condition.

\begin{theorem}
    Suppose $M$ and $N$ are submanifolds of some common manifold $E$, with $\partial E = \partial N = \emptyset$, and suppose that both $M^\circ$ and $\partial M$ intersect $N$ transversally. Then $M \cap N$ is a manifold with boundary such that
    %
    \[ \codim_E(M \cap N) = \codim_E(M) + \codim_E(N), \]
    %
    and such that $\partial (M \cap N) = (\partial M) \cap N$.
\end{theorem}

\begin{remark}
    More generally, if $f: M \to E$ is smooth, $N$ is a submanifold of $E$ with $\partial N = \emptyset$, and both $f$ and $\partial f$ are transversal to $N$, then $f^{-1}(N)$ is a manifold with boundary with $\partial (f^{-1}(N)) = (\partial f)^{-1}(N)$.
\end{remark}

The reason we care so much about transversality is that it is a regularity condition which is essentially `generic', i.e. any pair of manifolds may be deformed so that they intersect transversally. We consider a `smooth deformation' in the same manner as by homotopies.

\begin{theorem}
    Let $H: S \times M \to E$, where $M$, $S$, and $E$ are smooth manifolds, and only $X$ has boundary. Let $N$ be a submanifold of $E$ with $\partial N = \emptyset$. For each $s \in S$, we let $f_s: M \to E$ be the smooth function $f_s(p) = H(s,p)$. If $H$ and $\partial H$ are transversal to $N$, then for almost every $s \in S$, both $f_s$ and $\partial f_s$ are transversal to $N$.
\end{theorem}
\begin{proof}
    At each point $a = (s,p) \in S \times M$, we know that $T_a(S \times M)$ can be decomposed into $T_a(S) \oplus T_a(M)$. By the transversality assumption, if $q = H(s,p)$, then
    %
    \[ T_q(E) = H_*(T_a(S)) + H_*(T_a(M)) + T_q(N). \]
    %
    Now $(f_s)_*(T_p(S)) = H_*(T_a(S))$, so it suffices to show that for almost all $s \in S$, for every $p \in M$,
    %
    \[ T_q(E) = H_*(T_a(M)) + T_q(N). \]
    %
    By transversality, $N_0 = H^{-1}(N)$ is a submanifold of $S \times M$. Consider the projection map $\pi: N_0 \to S$, and suppose $s \in S$ is a regular value of $\pi$. Now $H_*(T_a(N_0)) \subset T_q(N)$, but since $\pi_*(T_a(N_0)) = T_s(S)$, this means that $T_a(S \times M) = T_a(M) + T_a(N_0)$, and thus
    %
    \begin{align*}
        T_q(E) &= H_*(T_a(S \times M)) + T_q(N)\\
        &= H_*(T_a(M)) + H_*(T_a(N_0)) + T_q(N) = H_*(T_a(M)) + T_q(N).
    \end{align*}
    %
    Thus $f_s$ is transverse at $p$. A similar argument involving $\partial H$ and $\partial \pi$ shows that $\partial f_s$ is transverse for almost every $s \in S$.
\end{proof}
...
Often, we will need to extend `partially' transverse maps to fully transverse maps in a compatible way. Given a map $f: M \to E$, a submanifold $N$ of $E$, and a set $C \subset M$, we will say $f$ is transverse to $N$ on $C$ if at each point $p \in f^{-1}(N) \cap C$,
%
\[ f_*(T_p(M)) + T_{f(p)}(N) = T_{f(p)}E. \]
%
Such maps extend to transverse maps everywhere.

\begin{theorem}
    Let $f: M \to E$ be a smooth map, $N$ a submanifold of $E$, such that neither $N$ nor $E$ have boundary, and consider a closed set $C \subset M$. Suppose $f$ and $\partial f$ are transverse to $N$ on $C$. Then there exists a smooth map $g: M \to E$ homotopic to $f$, such that $g$ and $\partial g$ are transverse to $N$, and $f$ and $g$ agree on a neighbourhood of $C$.
\end{theorem}
\begin{proof}
    It is simple to prove that if $f$ is transverse to $N$ on $C$, then $f$ is actually transverse to $N$ on a neighbourhood $U$ of $C$. Let $\gamma$ be an Urysohn function for $U^c$, equal to one on $U^c$, but vanishing in a neighbourhood of $C$. Then define a function $G: M \times B \to E$ by setting
    %
