...
%
\[ (\nabla_v s)(p) = \lim_{t \to 0} \frac{\Gamma(c)_t^0 \{ s(c(t)) \} - v}{t}. \]
%
...
If $c: I \to M$ is a curve, and $s$ is a section of a vector bundle defined along $c$, we define 
%
\[ \frac{Ds}{Dt} = (\nabla_{c'(t)} s)(c(t)), \]
%
A curve $c: I \to M$ is an autoparallel if and only if
%
\[ \frac{Dc'}{Dt} = 0. \]
%
Indeed, we calculate that this is true in coordinates if and only if for all $i$,
%
...
%
We call $\nabla_X Y$ the \emph{covariant derivative} of $Y$ in the direction of $X$. The linearity in the $X$ variable over $C^\infty(M)$ means that $(\nabla_X Y)(p)$ depends only on $X_p$.

The \emph{fundamental theorem of Riemannian geometry} states that there is a unique affine connection, the \emph{Riemannian connection} on $M$, which satisfies
%
\[ \nabla_X Y - \nabla_Y X = [X,Y], \]
%
and such that 
...
On Riemannian manifolds, we also have a natural way to define the length of any curve $c: [a,b] \to M$ of bounded variation, using the formula
%
\[ L(c) = \int_a^b |c'(t)|_g\; dt. \]
%
We can make any \emph{connected} Riemannian manifold into a metric space by defining
%
\[ d(p,q) = \inf\nolimits_{c \in \mathcal{C}} L(c), \]
%
where $\mathcal{C}$ is the set of all piecewise smooth curves $c: [0,1] \to M$ with $c(0) = p$, $c(1) = q$, and $c'(t) \neq 0$ at all but finitely many points. Under this assumption, by applying a reparameterization (under which the length functional is invariant), we conclude that
%
\[ d(p,q) = \inf\nolimits_{c \in \mathcal{C}_0} L(c), \]
%
where $\mathcal{C}_0 \subset \mathcal{C}$ is the set of \emph{unit speed curves}, piecewise smooth curves $c$ such that $|c'(t)|_g$ is constant on each interval where the function is smooth.
...
\begin{proof}
    ...
    %
    Thus $L(c) \gtrsim \varepsilon$, uniformly over all curves $c \in \mathcal{C}$. Thus $d(p,q) > 0$.
\end{proof}
...
The distance $d(p,q)$ is not \emph{always} realized by a particular curve. For instance, on $\RR^d - \{ 0 \}$, with the standard Riemannian metric, the induced metric is the standard one. Thus for all $v \in \RR^d - \{ 0 \}$, $d(v, -v) = 2 |v|$, but there is no curve from $v$ and $-v$ in $\RR^d - \{ 0 \}$ with this length. We will see this is a lack of the \emph{completeness} of $\RR^d - \{ 0 \}$, i.e. the induced metric is not a complete metric.

We now analyze the equation $\inf\nolimits_c L(c)$ using the calculus of variations. Consider a piecewise smooth map $\alpha: [-\varepsilon,\varepsilon] \times [a,b]$, which defines the smooth family of curves $s \mapsto c_s$. The map is a \emph{variation} of a curve $c$ if $c_0 = c$. We define the \emph{variation} $\delta L(\alpha)$ to be the derivative of $s \mapsto L(c_s)$ at $s = 0$. We say $c$ is a \emph{geodesic} if $\delta L(\alpha) = 0$ for all variations $\alpha$ \emph{fixing endpoints}, i.e. so that $c_s(0) = c(0)$ and $c_s(1) = c(1)$ for all $s$. Thus we think of a geodesic as an extreme point of the length functional $L$. A reparameterization argument shows that it suffices to consider unit length geodesics.

We now consider the \emph{first variation formula} for geodesics. Let $Q = [a,b] \times [-\varepsilon,\varepsilon]$ and fix a variation $\alpha: Q \to M$ of a unit speed geodesic $c$. For simplicity to begin with, assume $c$ is everywhere smooth. Let $T = \alpha_*(\partial_t)$ and $V = \alpha_*(\partial_s)$ be vector fields on $\alpha^*(TM)$. By differentiating under the integral sign and applying the chain rule, we calculate that
%
\begin{align*}
    \delta L(\alpha) &= \frac{1}{2} \int_0^1 \langle T, T \rangle^{-1/2} V \{ \langle T, T \rangle \} = \int_0^1 \langle T, T \rangle^{-1/2} \langle \nabla_V T, T \rangle.
\end{align*}
%
Because $c$ is unit speed geodesic, $\langle T, T \rangle^{-1/2} = C$ is a constant. Since $[\partial_t, \partial_s] = 0$, we have $\nabla_V T = \nabla_T V$. Integration by parts gives
%
\begin{align*}
    \int_0^1 \langle T, T \rangle^{-1/2} \langle \nabla_T V, T \rangle &= C \left[ \int_0^1 T \langle V, T \rangle - \langle V, \nabla_T T \rangle \right]\\
    &= C \left[ \left. \langle V, T \rangle \right|^1_0 - \int_0^1 \langle V, \nabla_T T \rangle \right].
\end{align*}
%
This is the \emph{first variation formula} for geodesic. A general formula for only piecewise smooth geodesics $c$ is given by breaking $c$ into pieces, and applying variations on each smooth piece of $c$. If $\alpha$ is a variation fixing endpoints, then $V(0) = 0$ and $V(1) = 0$, so we conclude that
%
\[ \delta L(\alpha) = - C \int_0^1 \langle V, \nabla_T T \rangle. \]
%
\emph{Any} vector field $V$ on the cruve $c$ can be realized as the varaition field for some variation, which fixes endpoints if $V(0) = 0$ and $V(1) = 0$ -- one example is $\alpha(s,t) = \exp_{c(t)}(s V)$. But taking $V = \phi(t) \nabla_T T$ for a smooth function $\phi$ with $\phi(0) = \phi(1) = 0$, we conclude that $c$ is a geodesic if and only if $\nabla_T T = 0$.
...
%
The advantage is that the integrand here is smooth at the origin. The equation is no longer invariant under a reparameterization. If $s: [0,1] \to [0,1]$ is a monotone diffeomorphism, then
%
\[ E(c \circ s^{-1}) = \int_0^1 \frac{|c'(s^{-1}(s))|_g^2}{s'(s^{-1}(s))^2}\; ds = \int_0^1 |c'(t)|_g^2 s'(t)^{-1}\; dt. \]
%
Using the fact that $\int_0^1 s'(t)\; dt = 1$, Cauchy-Schwartz implies that
%
\begin{align*}
    l &= \int_0^1 |c'(t)|_g\\
    &= \int_0^1 [s'(t)^{1/2}] [s'(t)^{-1/2} |c'(t)|_g ]\\
    &\leq \left( \int_0^1 s'(t) \right)^{1/2} \left( \int_0^1 s'(t)^{-1} |c'(t)|_g \right)^{1/2}\\
    &\leq E(c \circ s^{-1}).
\end{align*}
%
Moreover, this inequality is an equality if and only if $s'(t) = \lambda |c'(t)|_g$, which forces $s(t) = l^{-1} \int_0^t |c'(s)|_g\; ds$, and this is the unique minimizer of the energy equation. Thus if $c$ is a curve such that $\delta E(\alpha) = 0$ for all variations fixing endpoints, then $c$ must be a unit speed geodesic. Note furthermore that in this case we have $E(c) = L(c)$.

For a variation $\alpha$ of a unit speed curve $c$ with $\langle T, T \rangle = C$, we calculate that
%
\begin{align*}
    \delta E(\alpha) &= 4 \int_0^1 \langle T, T \rangle \langle \nabla_V T, T \rangle\\
    &= 4 C \int_0^1 \langle \nabla_T V, T \rangle\\
    &= 4C \left[ \int_0^1 T \langle V, T \rangle - \langle V, \nabla_T T \rangle \right] \\
    &= 4C \left[ \langle V, T \rangle|^1_0 - \int_0^1 \langle V, \nabla_T T \rangle \right] .
\end{align*}
%
This is the first variation formula for the energy functional. Again, we see that critical points of the energy functional are critical points of the length functional that have constant speed.
...
Let $V$ be a vector space. A \emph{strongly convex Minkowski norm} is a function $F: V \to [0,\infty)$, smooth away from the origin, homogeneous of degree one, and such that for each $v \in V$, the Hessian of the function $F^2$ on $V - \{ 0 \}$ is positive definite. The \emph{indicatrix} of $F$ is then $F^{-1}(1)$, and by homogeneity, uniquely determines $F$. A \emph{Finsler metric} on a manifold $M$ will be a smooth choice of strongly convex Minkowski norms on each tangent space of $M$, so it is profitable to begin by studying such functions. T

\begin{example}
    Let $Q: V \times V \to \RR$ be an inner product on $V$. Then $F(v) = Q(v,v)^{1/2}$ is a strongly convex Minkowski norm on $V$.
\end{example}

In general, strongly convex Minkowski norms are precisely norms that can be closely approximated up to second order by the square root of an inner product. For each $v \in V - \{ 0 \}$, define an inner product $Q_v$ on $V$ using the Hessian of $F^2 / 2$ at $v$. One may then check that $F(w)$ and $Q_v(w,w)^{1/2}$ agree up to second order at $v$. 
...
\begin{example}
    ...
    %
    \emph{are} strongly convex.
\end{example}

Now we consider the case of manifolds. A \emph{Finsler Metric} on a manifold $M$ is a homogeneous function $F: TM \to [0,\infty)$, smooth away from the zero section, such that for each $x \in M$, the function is a strongly convex Minkowski norm on $T_x M$.

\begin{example}
    Let $M$ be a Riemannian manifold, fix a one form $\omega \in \Omega^1(M)$ with $\| \omega(x) \|_g < 1$ for all $x \in M$, and define a function $F(x,v) = \| v \|_g + \omega(v)$. Then one can check that $F$ is strongly convex, and thus gives a Finsler metric on $M$.

    To give an explicit example on $M = \RR^2 - \{ 0 \}$, for each $x \in M$, we define $F$ so that at each $x$, the indicatrix of $F$ is an ellipse of eccentricity $e(x)$ with one focii at the origin, and the directrix passes through $0$ perpendicularly to the line segment from $0$ to $x$. One can check that
    %
    \[ F(x,v) = e^{-1} \frac{|v|}{|x|} - \frac{x \cdot v}{|x|^2}. \]
\end{example}
...
If $M$ is a Finsler manifold, then there is a \emph{distinguished section} $l$ of the bundle $\pi^*(TM)$, given in coordinates by $l^j(x,v) = v^j / F(x,v)$. It's dual is the \emph{Hilbert form} $\omega$ on $\pi^*(T^*M)$, given in cordinates by $\omega_j = (\partial_{v_j} F)(x,v)$. Then $\omega(l) = 1$ by Euler's theorem, and $l^\flat = \omega$, where we use the inner product on $\pi^*(TM)$ to perform musical isomorphism (in the future, we will freely raise and lower indices of tensors on $\pi^*(TM)$ by using this musical isomorphism). If $\sigma: [a,b] \to M$ is a piecewise smooth curve, then identifying $\sigma$ with the resulting piecewise curve on $TM - \{ 0 \}$ by taking the derivative, we have
%
\[ L(\sigma) = \int_a^b F(\sigma, \sigma') = \int_\sigma \omega, \]
%
a formula called \emph{Hilbert's invariant integral}.

The vector bundle $\pi^*(TM)$ on $SM$ has an inner product $g$ induced by the Finsler metric on $M$, given in coordinates by the formula
%
\[ g_{ij} = \partial_{v_i} \partial_{v_j} \{ F^2 / 2\} = F \partial_{v_j} \partial_{v_k} F + \partial_{v_j} F \partial_{v_k} F. \]
%
The inner product $g$ is called the \emph{fundamental tensor}. We also define a tensor $A$ on $\pi^*(SM)$ by the equation
%
\[ A_{ijk} = (F/2) \partial_{v_k} g_{ij} = (F/4) \partial_i \partial_j \partial_k \{ F^2 \}. \]
%
This is the \emph{Cartan tensor} (the last formula shows it is a symmetric tensor). Sometimes, in the literature the tensor $C_{ijk} = A_{ijk} / F$ is used as the definition of the Cartan tensor, but it is often more natural to work with tensors that are homogeneous of order zero so that they are naturally defined on the sphere bundle. Roughly speaking, $g$ is the best inner product approximating $F$ to second order, and $A$ somehow provides a measure of how non-Riemannian the function $F$ is (if $F$ was induced by a Riemannian metric, than $\partial_{v_k} g_{ij} = 0$ uniformly).

The calculations we have done for Minkowski norms imply that in $\pi^*(TM - 0)_{(x,v)}$,
%
\[ v^\flat = dF(x,v) \]
%
\[ g(v,v) = F(x,v)^2 \]
%
and for any $w,u \in \pi^*(TM - 0)_{(x,v)}$,
%
\[ A_{(x,v)}(v, w, u) = 0, \]
%
the latter follows from Euler's theorem.

We define the \emph{formal Christoffel symbols}
%
\[ \gamma^i_{jk} = \frac{g^{is}}{2} \left( \frac{\partial g_{sj}}{\partial x^k} - \frac{\partial g_{jk}}{\partial x^s} + \frac{\partial g_{ks}}{\partial x^j} \right), \]
%
and also the \emph{nonlinear connection}
%
\[ N^i_j = \gamma^i_{jk} v^k - C^i_{jk} \gamma^k_{rs} v^r v^s. \]
%
Neither of these are tensors. Nonetheless, the quantities $N^i_j / F$, homogeneous of order zero, change somewhat nicely under coordinates, with the quantities $\tilde{N}^a_b$ in new coordinates $y$ given by
%
\[ \tilde{N}^a_b / \tilde{F} = \frac{\partial y^a}{\partial x^i} \frac{\partial x^j}{\partial y^b} (N^i_j / F) + \frac{\partial y^a}{\partial x^i} \frac{\partial^2 x^i}{\partial y^b \partial y^s} \tilde{l}^s. \]
%
Define elements in $T(TM)$ of the form
%
\[ \frac{\delta}{\delta x^j} = \frac{\partial}{\partial x^j} - N^i_j \frac{\partial}{\partial v^j}. \]
%
and
%
\[ \delta v^i = dv^i + N^i_j dx^j. \]
%
Then $TM - 0$ has a natural Riemannian metric
%
\[ g_{ij} dx^i \otimes dx^j + g_{ij} \frac{\delta v^i}{F} \otimes \frac{\delta v^j}{F} \]
%
known as the \emph{Sasaki metric}. The \emph{horizontal subspace} spanned by $\delta / \delta x^j$ is then orthogonal to the \emph{vertical subspace} $F \partial / \partial x^j$. The manifold $TM - 0$ thus obtains an \emph{Ehresman connection} through this splitting.

If we define $G^i = \gamma^i_{jk} y^j y^k$, then we will see that Finsler geodesics satisfy the equation $x'' = -G(x,x')$.
