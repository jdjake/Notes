...
If $(x,U)$ is a chart on $M$, which identifies $TM|_U$ with $U \times \RR^n$, i.e. $(x,v)$ coordinates for $TM|_U$. Given some other chart $(y,U)$, with induced coordinates $(y,w)$ on $TM|_U$, if $\tau = y \circ x^{-1}$, then we have
%
\[ y = \tau(x) \quad\text{and}\quad w = (D \tau)(x) \{ v \}. \]
%
If we now consider the induced identification of $T(TM)|_U$ with $U \times \RR^n \times \RR^n \times \RR^n$ induced by $(x,v)$, i.e. a coordinate system $(x,v,\dot{x}, \dot{v})$, then given some other coordinate system $(y,w,\dot{y}, \dot{w})$, if we define a matrix $(D^2 \tau)(x,v)$ by letting $(D^2 \tau)_{jk} = \sum (\partial_\alpha \partial_k \tau^j) v_\alpha$, then
%
\[ \begin{bmatrix} \dot{y} \\ \dot{w} \end{bmatrix} = \begin{bmatrix} (D \tau)(x) & 0 \\ (D^2\tau)(x,v) & (D \tau)(x) \end{bmatrix} \begin{bmatrix} \dot{x} \\ \dot{v} \end{bmatrix}. \]
%
Given $p \in M$, and two tangent vectors $v,w \in T_p M$, we can define a tangent vector $\text{vlift}_v(w)$ in $T_{v_p} (TM)$, the \emph{vertical lift} of $w$ over $v$, to be the tangent vector to the curve $\gamma(t) = v + t w$ when $t = 0$. In coordinates, if $v$ and $w$ are given in coordinates by $(x,a)$ and $(x,b)$, then the vertical lift is given in coordinates by $(x,a,0,b)$, and it is simple to check this definition is independent of the coordinate system because the top right of the matrix above vanishes. A \emph{vector field} $X$ on $M$ induces a flow $\{ \Phi^X_t \}$, and we define the \emph{tangent lift} of $X$ at $v_p \in T_pM$ to be the tangent vector to the curve $\gamma(t) = (\Phi^X_t)_* \{ v_p \}$ when $t = 0$. In coordinates, if the vector field $X$ is given in coordinates by $(x,a(x))$, then the tangent lift is given in coordinates by $(x,v,a(x), Da(x)\{ v \})$.

It follows that if two vectors in $T_p M$ are given in coordinates by $(x(p),v)$ and $(x(p),w)$, then the vector $(x(p),v,0,w)$ is independent of the coordinate system, and is called the \emph{vertical lift} of $w$ over $v$.
%
\[ (x(p),v,0,w) \{ f \} = (D_2 f) \{ w \} \]

The vector field on $TM|_U$ given in coordinates $(x,v) \mapsto (x,v,0,v)$ is thus independent of the coordinate system, and we therefore have a natural vector field on $TM$, called the \emph{verticlal lift} of $TM$.
...
On $\RR^n$, an inner product enables us to measure the lengths of vectors, and angles between them. By considering inner products of the tangent space of a manifold, we can find a way to measure lengths of smooth curves, and angles between curves passing through a point. This is the theory of riemannian geometry.

First, we recall the necessary linear algebra. A bilinear form on a real vector space $V$ is a bilinear map $\beta: V \times V \to \RR$. This form is \emph{symmetric} if $\beta(v,v') = \beta(v',v)$ for all $v,v' \in V$, \emph{positive-definite} if $\beta(v,v) > 0$ for all $v \in V - \{ 0 \}$, and \emph{non-degenerate} if for all $v \in V - \{ 0 \}$, there is $v' \in V$ with $\beta(v,v') \neq 0$. Given a non-degenerate form on $V$, we can naturally define a map $v \mapsto v^\flat$ from $V$ to $V^*$ via the equation $v^\flat(w) = \beta(v,w)$. This map is injective precisely when $\beta$ is non-degenerate, and thus naturally identifies $V$ with $V^*$ if $V$ is finite dimensional.

A \emph{Riemannian manifold} is a manifold $M$ equipped with a \emph{Riemannian metric}, a smooth assignment of a positive-definite symmetric bilinear form $\langle \cdot, \cdot \rangle_p$ on the tangent spaces $T_p M$, for each point $p \in M$. We can characterize this smoothness in several equivalent ways:
%
\begin{itemize}
    \item We can view the Riemannian metric as a section of the $(0,2)$ tensor bundle $T^2 M$, and such sections naturally have a definition of smoothness.

    \item The Riemannian metric will be smooth if and only if for any two smooth vector fields $X$ and $Y$ on $M$, $\langle X, Y \rangle$ is a smooth function on $M$.

    \item In any coordinate system $x: U \to M$, we can write the Riemannian metric uniquely as $\sum g_{ij}(x) dx^i \otimes dx^j$, where $g_{ij} = g_{ji}$. Then the metric is smooth if and only if the functions $g_{ij}$ are smooth.
\end{itemize}
%
A smooth choice of \emph{non-degenerate} bilinear forms is called a \emph{pseudo Riemannian metric} on $M$, and gives $M$ the structure of a \emph{pseudo Riemannian manifold}.
...
\begin{example}
    ...
    %
    Often, the field is just denoted by the Kronecker delta $\delta$, since the coordinates $g_{ij}$ associated with this metric is just the Dirac delta function $\delta_{ij}$.
\end{example}

\begin{example}
    For some more explicit Riemannian metrics defined in coordinates, it is useful to consider the \emph{Riemannian space forms} on $\RR^n$. Let $s_\lambda$ be the unique solution to the differential equation $s_\lambda'' = - \lambda s_\lambda$ with $s_\lambda(0) = 0$ and $s_\lambda'(0) = 1$. Then define $\psi(x) = [s_\lambda(|x|) / |x|]^2$, and set
    %
    \[ g_{jk} = (1 - \psi) \frac{x_j x_k}{|x|^2} + \psi \delta_{jk}. \]
    %
    This defines a metric on $\RR^d$ for $\lambda \leq 0$, and on an open ball of radius $\pi / \sqrt{\lambda}$ for $\lambda > 0$. Using more advanced terminology, this definition here is precisely the metric in normal coordinates on a Riemannian manifold with constant curvature $\lambda$. TODO: Use Jacobi fields to prove this.

    TODO: Merge with previous example.
...
\begin{example}
    A surface $\Sigma$ in $\RR^3$ inherits a Riemannian metric from the ambient space it lies in by restricting the inner product on $T_p \RR^3$ to $T_p \Sigma$ for each $p \in \Sigma$. More generally, an immersed submanifold of a Riemannian manifold naturally obtains a Riemannian structure. If $i: M \to N$ is an immersion, and $N$ has a Riemannian metric, then we can define the Riemannian metric on $M$ by the equation
    %
    \[ \langle X, Y \rangle_M = \langle i_*X, i_*Y \rangle_N \]
    %
    The same construction does \emph{not} necessarily work in the pseudo Riemannian setting, since the restriction of a non-degenerate bilinear form to a subspace need not be non-degenerate.
\end{example}
...
An \emph{isometry} between two Riemannian manifolds is a diffeomorphism which acts as an isometry on each tangent space. The study of Riemannian geometry can be considered the study of properties of Riemannian manifolds which are invariant under isometries.

Psuedo Riemannian metrics often occur in physical theories such as relativity theory. Since the eigenvalues corresponding to the symmetric operator change smoothly, and cannot be zero by non-degeneracy, the number of negative and positive eigenvalues do not change across a connected psuedo-Riemannian manifold. The number of negative eigenvalues for a pseudometric is known as the \emph{index} of the manifold. An index 0 metric is precisely a Riemannian metric, and an index 1 metric is known as a \emph{Lorentz metric}. 

\begin{example}
    The classical example of a Lorentz metric occurs in special relativity, known as the Minkowski metric on $\RR \times \RR^n$, given by the metric
    %
    \[ g = dx_1^2 + \dots + dx_n^2 - dt^2. \]
    %
    This metric is natural in the study of special relativity, since the symmetries of that theory are precisely the isometries of this space, the \emph{Lorentz transforms}. More generally, in the study of general relativity, the $\RR^n \times \RR$ by a general $n+1$ dimensional manifold, and the Minkowski metric by an arbitrary Lorentz metric, satisfying certain physical equations known as the Einstein equations, which control how the Lorentz metric behaves on the space.
\end{example}

\section{The Gradient Vector}

Since a non-degenerate form enables us to identify a vector space $V$ with its dual, a metric enables us to identify $TM$ and $T^*M$ in a natural way. This identification is also called the \emph{musical isomorphism}, since we use $v \mapsto v^\flat$ to denote the map from $TM$ to $T^*M$, and $\omega \mapsto \omega^\sharp$ to denote the map from $T^* M$ to $TM$. In coordinates, for a vector $v = \sum v^j \partial_{x^j}$ and $\omega = \sum \omega_j dx^j$, we have
%
\[ v^\flat = \sum g_{ij} v^i dx^j \quad\text{and}\quad \omega^\sharp = \sum g^{ij} \omega_i \frac{\partial}{\partial x^j}, \]
%
where $g^{ij}$ denotes the entries of the inverse of the matrix with coefficients $g_{ij}$, i.e. so that $\sum g^{ij} g_{jk} = \delta^i_k$ for each fixed $i$ and $k$. The entries $g^{ij}$ then transform as a $(2,0)$ tensor, and thus give rise to an inner product on each of the cotangent spaces $T_p^* M$.

For any scalar valued function $f: M \to \RR$, the differential $df$ is a section of $T^* M$. Taking the dual using the bilinear form, we obtain the \emph{gradient} $\nabla f$, which is a section of $TM$, defined by the equation
%
\[ \langle \nabla f, X \rangle = df(X). \]
%
In coordinates, we have $\nabla f = \sum g^{ij} (\partial_{x^i} f) \partial_{x^j}$. Over Euclidean space, with the standard metric, we obtain the standard gradient.

\begin{example}
    On the sphere $S^2$, we have spherical coordinates
    %
    ...
    \end{align*}
    %
    Thus the basis given by spherical coordinates is orthogonal, but nor orthonormal. We have
    %
...
\begin{example}
    In Minkowski space, we have a time coordinate $t$, and three spatial coordinates $x$, $y$, and $z$. If $t$ is chosen as the $0$th coordinate, the Minkowski metric is chosen such that
    %
...
If $M$ is a Riemannian manifold, we consider a submanifold $SM$ of $TM$ defined by the equation $\langle v, v \rangle = 1$. It is a submanifold of dimension $2n - 1$, if $M$ has dimension $n$.
...
On any manifold, we can define $SM$ by taking the \emph{projectivization} of the tangent space $TM$, but then this manifold is not so naturally embedded as a submanifold of $TM$.

\section{The Volume Form}

Any Riemannian manifold has a natural notion of volume. Tensorizing the definition of the metric, we can obtain a natural inner product on all of the tensor bundles of $M$; it is precisely the unique inner product that agrees with the Riemannian metric on tangent and cotangent vectors, and satisfies
%
...
%
This means that the resulting scalar density $dV = |dx^1 \wedge \dots \wedge dx^n| |\det(G)^{-1/2}|$ is invariantly defined on $M$, which we call the \emph{volume element} on $M$. If $M$ is \emph{oriented}, we can choose an oriented choice of the norm one elements, and thus obtain a globally defined $n$ form, the \emph{volume form}. Thus for any Riemannian manifold $M$, we can define it's \emph{volume} $\text{Vol}(M)$ as
%
\[ \text{Vol}(M) = \int_M dV. \]
%
...
Let $c: I \to M$ be a smooth curve into a manifold $M$. Without additional structure, it does not make sense to state that $c$ has \emph{constant velocity}. Indeed, on $\RR^d$ constant velocity curves travel on straight lines, but under an arbitrary diffeomorphism these curves can be mapped onto curves that behave in an arbitrary fashion. We begin by discussing the analogous problem with such definitions infinitisimally on the tangent bundle, and then discuss how the concept of a \emph{connection} allows one to define constant velocity curves, which in differential geometry are called \emph{autoparallels}, and which we will relate to the study of \emph{geodesics} in later sections.

Consider the bundle $T(TM)$, the tangent bundle to the tangent bundle. For any coordinate system $x$ on $U \subset M$, we obtain an induced coordinate system $(x,\dot{x})$ on $\pi_{TM}^{-1}(U) \subset TM$, where $x(v)$ is just the coordinates of the base point of $v$, and $\dot{x}(v) \in \RR^d$ gives the coefficients of the expansion of $v$ in the basis $\partial_{x^i}$. Thus we can write every element of $T_v(TM)$ as
%
\[ \sum a^i \left. \frac{\partial}{\partial x^i} \right|_v + b^i \left. \frac{\partial}{\partial \dot{x}^i} \right|_v. \]
%
In another coordinate system $y$ on $U$, we have
%
\[ \dot{y}^i(p) = \sum \left. \frac{\partial y^i}{\partial x^j} \right|_p \dot{x}^j(p). \]
%
Thus
%
\[ \left. \frac{\partial \dot{y}^i}{\partial \dot{x}^j} \right|_v = \left. \frac{\partial y^i}{\partial x^j} \right|_p \quad\text{and}\quad \left. \frac{\partial \dot{y}^i}{\partial x^j} \right|_v = \sum \left. \frac{\partial^2 y^i}{\partial x^k \partial x^j} \right|_p \dot{x}^k(p). \]
%
But this means that
%
\[ \left. \frac{\partial}{\partial x^i} \right|_v = \sum \left. \frac{\partial y^j}{\partial x^i} \right|_p \left. \frac{\partial}{\partial y^j} \right|_v + \sum \left. \frac{\partial^2 y^j}{\partial x^i \partial x^k} \right|_p \dot{x}^k(p) \left. \frac{\partial}{\partial \dot{y}^j} \right|_v \]
%
and
%
\[ \left. \frac{\partial}{\partial \dot{x}^i} \right|_v = \sum \left. \frac{\partial y^j}{\partial x^i} \right|_p \left. \frac{\partial}{\partial \dot{y}^j} \right|_v. \]
%
Setting $A(x)$ to be the matrix with entries $A^i_j = \partial y^i / \partial x^j$, and $B(v)$ the matrix with entries $B^i_j = \sum \partial^2 y^i / \partial x^j \partial x^k \dot{x}^k$, then if
%
\[ \sum a^i \left. \frac{\partial}{\partial x^i} \right|_v + b^i \left. \frac{\partial}{\partial \dot{x}^i} \right|_v = \sum c^i \left. \frac{\partial}{\partial y^i} \right|_v + d^i \left. \frac{\partial}{\partial \dot{y}^i} \right|_v, \]
%
then
%
\[ \begin{pmatrix} c \\ d \end{pmatrix} = \begin{pmatrix} A & 0 \\ A & B \end{pmatrix} \begin{pmatrix} a \\ b \end{pmatrix}. \]
%
Thus if $a = 0$, then $c = 0$, and so the span of the vectors $\{ \partial_{\dot{x}^i} \}$ at each point generates a vector bundle $V(TM)$ which is invariant under coordinate changes in $M$. We can see this in a coordinate invariant way also, because $V(TM)$ is the kernel of the bundle map $\pi_*: T(TM) \to TM$ induced by $\pi: TM \to M$. We call $V(TM)$ the \emph{vertical bundle}.

On the other hand, the span of the vectors $\{ \partial_{x^i} \}$ is only invariant if $B = 0$, i.e. under \emph{linear coordinate changes}. This makes sense from our original discussion. If we could form a vector bundle $H(TM)$ in a coordinate independent way from the span of vectors $\{ \partial_{x^i} \}$, then from $c: I \to M$, we can differentiate and obtain a curve $c': I \to TM$ and again differentiate to obtain $c'': I \to T(TM)$. In coordinates, we would have
%
\[ c''(t) = \sum \frac{d(x^i \circ c)}{dt} \left. \frac{\partial}{\partial x^i} \right|_{c'(t)} + \frac{d^2(x^i \circ c)}{dt} \left. \frac{\partial}{\partial \dot{x}^i} \right|_{c'(t)}, \]
%
and thus we could then declare $c$ to have constant velocity if $c''(I) \subset H(TM)$, and this would then be a coordinate independent property, which we have seen is not possible.

A connection is a structure on a manifold that allows us to obtain some definition of a constant velocity curve. It is as easy to define this for a general vector bundle $\pi: E \to M$, which is often helpful in computations, so we might as well do so. For any vector bundle $\pi: E \to M$, we have a short exact sequence
%
\[ 0 \to VE \to TE \to \pi^*(TM) \to 0. \]
%
A connection is a \emph{splitting} of this short exact sequence. This requires us to perform one of the three equivalent processes:
%
\begin{itemize}
    \item We could form a decomposition $TE = VE \oplus HE$ for some bundle $HE$, known as the \emph{horizontal bundle} of the connection.

    \item We could \emph{split on the left}, defining a connection as a morphism $\omega: TE \to VE$, known as the \emph{connection form}.

    \item We could \emph{split on the right}, defining a connection as a morphism $l: \pi^*(TM) \to TE$, known as the \emph{lifting map}.
\end{itemize}
%
If one performs any of these three properties, there is a canonical way to obtain the other three maps. The connection form has $HE$ as a kernel, and the lifting map has $HE$ as an image. Given a manifold whose tangent bundle equipped with a connection, an \emph{autoparallel} is then a curve $c: I \to M$ such that $c''(I) \subset H(TM)$, which we think of as having constant velocity with respect to the connection.

Suppose $\pi: E \to M$ is a vector bundle, and consider an open set $U \subset M$ such that we have both a coordinate system $x: M \to \RR^d$, and sections $e_\alpha: M \to E$ given a frame on $E$. Then $(x,a)$ forms a coordinate system on $\pi^{-1}(U)$, where $x(v) = x(\pi(v))$ is the basepoint coordinate, and $a(v)$ gives the coefficient expansion of $v$ in the frame $\{ e_\alpha \}$. Thus $TE \to E$ has a natural frame, which we denote by $\{ e_i \} \cup \{ \dot{e}_\alpha \}$. In coordinates, we can write
%
\[ \omega \left(  \sum a^i e_i + b^\alpha \dot{e}_\alpha \right) = \sum ( a^i N^\alpha_i + b^\alpha ) \dot{e}_\alpha \]
%
for smooth functions $N^\alpha_i$ on $E$. They are called the \emph{(nonlinear) connection coefficients}. In general, the functions are only homogeneous as functions on $TE$, and do \emph{not} transform tensorially under a change of coordinates (the formula for the transformation is somewhat technical, see the remark below). However, if the coefficients $N_\alpha^j$ are always \emph{linear} in $v$, then we call the connection \emph{linear}, and we can then write $N_i^\alpha( \sum v^\beta e_\beta ) = \sum \Gamma^\alpha_{i \beta} v^\beta$ for smooth coefficients $\Gamma^\alpha_{i \beta}$ on $M$, which also do not transform tensorially. We call these the \emph{linear connection coefficients}. The tangent vectors
%
\[ \dpardnoline{x^i} = \pardnoline{x^i} - \sum N_i^\alpha \dot{e}_\alpha, \]
%
which we also denote by $\delta e_i$, then form a frame for $HE$, and the associated lifting map $l: \pi^*(TM) \to H(TM)$ is then given by $l( \sum a^i e_i ) = \sum a^i \delta e_i$. The vectors $\delta e_i$ transform as
%
\[ \dpardnoline{x^j} = \sum \pardnoline[y^i]{x^j} \dpardnoline{y^i}, \]
%
just like the usual tangent vectors. On the cobundle $E^*$, $VE^\perp$ can be defined without reference to a connection, and has a frame $\{ dx^j \}$. A basis in coordinates to $H(TM)^\perp$ can be defined by introducing the covectors $\delta (\dot{e}^\alpha)^* = \sum N^\alpha_i dx^i + d \dot{e}^\alpha$.

\begin{comment}
In coordinates, we can write
%
\[ \omega \left( \sum a^i \pard{x^i}{v} + b^i \pard{ \dot{x}^i }{v} \right) = \sum ( a^i N^j_i(v) + b^j ) \partial_{\dot{x}^j} \]
%
for smooth functions $N_i^j \in C^\infty(TM)$. They are called the \emph{(nonlinear) connection coefficients}. In general, the functions are only homogeneous as functions on $TM$, and do \emph{not} transform tensorially under a change of coordinates. If the coefficients $N_i^j$ are \emph{linear} in $v$, then we call the connection \emph{linear}, and we can then write $N_i^j( \sum v^k \partial_{x^k} ) = \sum \Gamma^j_{ik} v^k$ for smooth functions $\Gamma^j_{ik} \in C^\infty(M)$, which also do not transform tensorially, the \emph{(linear) connection coefficients}. The associated lifting map $l: TM \to H(TM)$ is given by
%
\[ l \left( \sum a^i \pard{x^i}{v} \right) = \sum a^i \dpard{x^i}{v}, \]
%
where
%
\[ \dpardnoline{x^i} = \pardnoline{x^i} - N_i^j \pardnoline{\dot{x}^j}. \]
%
These tangent vectors then form a basis for the horizontal bundle $H(TM)$, and transform as
%
\[ \dpardnoline{x^j} = \sum \pardnoline[y^i]{x^j} \dpardnoline{y^i}, \]
%
just like with the usual tangent vectors.  On the cotangent bundle, $V(TM)^\perp$ can be defined without reference to a connection, and is spanned in coordinates by the quantities $\{ dx^j \}$. Given access to a connection, we can define a basis in coordinates to $H(TM)^\perp$ by introducing the covectors $\delta \dot{x}^j = N^a_b dx^b + d \dot{x}^j$.
\end{comment}

A connection gives rise to \emph{parallel transports along curves}.

\begin{theorem}
    Let $E \to M$ be a vector bundle equipped with a connection. Then for any curve $c: I \to M$, and any $t_0$ and $t_1$, there exists a bijective map $\Gamma(c)_{t_0}^{t_1}: E_{c(t_0)} \to E_{c(t_1)}$ called the \emph{parallel transport map}, such that for each $v \in E_{c(t_0)}$, if we consider the curve $\gamma: I \to E$ given by
    %
    \[ \gamma(t) = \Gamma(c)_{t_0}^t \{ v \}, \]
    %
    then $\gamma$ is the unique curve with $\gamma(t) \in E_{c(t)}$ for all $t$ and $\gamma'(t) \in HE$ for all $t \in I$. The curve $\gamma$ is called the \emph{parallel transport of $v$ along $c$}. If the connection is a linear connection, then the parallel transport maps are linear.
\end{theorem}
\begin{proof}
    The proof is fairly simple. Define $F = c^* E$, a vector bundle on $[0,1]$. The pull back of any vector bundle with a connection naturally has a connection (we simply pull back the horizontal subbundle). The connection on $F$ gives us a lifting map $l: \pi^*(TI) \to TF$, which induces a vector field $X \in \Gamma(TF)$ by the formula $X_v = l( \partial_t|_v )$. The integral curves of this vector field are precisely the parallel transport maps needed above. The calculations in the following argument verify that if the connection is linear, then the parallel transport maps are linear.
\end{proof}

\begin{remark}
    Suppose $\gamma: I \to E$ is a parallel transport curve, and we can write $\gamma$ in a frame as
    %
    \[ \gamma(t) = \sum \gamma^\alpha(t) e_\alpha(c(t)). \]
    %
    Our constraints on $\gamma$ amounts to $\omega(\gamma'(t)) = 0$, which we can write as
    %
    \[ \omega \left( (c^i)'(t) \pard{x^i}{\gamma(t)} + (\gamma^\alpha)'(t) \dot{e}_\alpha(\gamma(t)) \right) = 0, \]
    %
    or
    %
    \[ (\gamma^\alpha)' = - \sum (c^i)' N_i^\alpha(\gamma). \]
    %
    Thus if we set $\Gamma(c)_0^t(v) = \sum A^\alpha(t,v) e_\alpha$, then $A$ satisfies the system of nonlinear differential equations
    %
    \[ (\partial_t A^\alpha) = - \sum (c^i)' N_i^\alpha(A), \]
    %
    such that $A(0,v) = v$. If $N_i^\alpha(v) = \sum \Gamma_{i\alpha}^\beta \dot{e}_\beta$, then we obtain a linear differential equation
    %
    \[ (\partial_t A^\alpha) = - \sum (c^i)' \Gamma_{i\beta}^\alpha(c) A^\beta. \]
    %
    This in particular implies that $A(c)_0^t$ is then a linear function of $v$. We can thus write $A^\alpha = \sum A^\alpha_\beta v^\beta$, and then $(A^\alpha_\beta)' = - \sum (c^j)' \Gamma_{j \lambda}^\alpha(c) A^\lambda_\beta$.
\end{remark}

\begin{remark}
In the next remark, we calculation the change of variables formula for the nonlinear connection coefficients. If in a coordinate system $y$ we have connection coefficients $\tilde{N}^j_k(y,w)$, and we write
%
\[ A^i_a = \frac{\partial y^i}{\partial x^a} \quad B^i_{ab} = \frac{\partial^2 y^i}{\partial x^a \partial x^b} \quad C^a_i = \frac{\partial x^a}{\partial y^i} \quad D^a_{ij} = \frac{\partial^2 x^a}{\partial y^i \partial y^j}, \]
%
then
%
\begin{align*}
    \omega \left( \pardnoline{x^a} \right) &= \omega \left( \sum A^i_a \pardnoline{y^i} + B^j_{ab} \dot{x}^b \pardnoline{\dot{y}^j} \right)\\
    &= \sum (A^i_a \tilde{N}_i^j + B^j_{ac} \dot{x}^c) \pardnoline{\dot{y}^j}\\
    &= \sum (A^i_a \tilde{N}_i^j + B^j_{ac} \dot{x}^c) C^b_j \pardnoline{\dot{x}^b}.
\end{align*}
%
Thus
%
\[ N_a^b = \tilde{N}^j_i A_a^i C^b_j + B_{ac}^j \dot{x}^c C^b_j \]
%
Inverting this equation, we obtain that
%
\[ \tilde{N}^j_i = N_a^b C_i^a A_b^j + D^b_{ik} \dot{y}^k A_b^j \]
%
%
%
If the connection is linear, we can also directly express a relationship between linear connection coefficients $\Gamma^j_{il}$ and $\tilde{\Gamma}^j_{il}$, i.e.
%
\[ \Gamma_{ac}^b = \tilde{\Gamma}^j_{ik} A_a^i A^k_c C^b_j + B_{ac}^j C^b_j \quad\text{and}\quad \tilde{\Gamma}_{ik}^j = ( C_i^a \Gamma_{ac}^b C^c_k + D_{ik}^b ) A^j_b. \]
%
Note that these quantities \emph{only transform tensorially} under linear coordinate changes, due to the presense of the additional correction factor appearing in the formulas.


%$\Gamma^j_{il} = C^j_k (A_i^r \Gamma^k_{rl} + B^k_{il})$ and $\tilde{\Gamma}^j_{il} = A^j_k ( C_i^r \Gamma^k_{rl} + D^k_{il} )$.
\end{remark}

\section{Covariant Derivative Operators}

On a general manifold, it is possible to differentiate a smooth function $f: M \to \RR$ at a point $p$ in a direction $v \in T_pM$. It is \emph{not} possible to define the derivative of a smooth vector field $X \in \Gamma(TM)$ at a point $p$ in a direction $v$, because in general it is impossible to compare tangent spaces at two different points. We do have a Lie derivative, which allows us to define derivatives if we have a flow in a manifold, but this only allows us to define the derivative of a smooth vector field $X$ in the direction of another vector field $Y$, and so the derivative of $X$ with respect to $Y$ also involves derivatives of $Y$, which isf undesirable in many contexts.

Nonetheless, we now see it \emph{is} possible to define derivatives once we have defined a connection on $M$. More generally we can define the derivative of sections of any vector bundle $E$ equipped with a connection, where the derivative is defined with respect to a vector in the tangent bundle $TM$. Given $s: M \to E$ and $v \in T_p M$, we consider a curve $c: [0,1] \to M$ with $c'(0) = v$, and set
%
\[ (\nabla_v s)(p) = \lim_{t \to 0} \frac{\Gamma(c)_t^0 \{ s(c(t)) \} - s(c(0))}{t}. \]
%
To see this operator is independent of the choice of $c$, we consider a frame $\{ e_\alpha \}$ on $E$, writing
%
\[ s = \sum s^\alpha e_\alpha \quad\text{and}\quad \Gamma(c)_t^0(s) = \sum A^\alpha_t(s) e_\alpha(c(t)). \]
%
Then one can verify that
%
\[ \Gamma(c)_t^0 \{ s(c(t)) \} = \sum A^\alpha_t(s(c(t))) e_\alpha(c(t)), \]
%
and the derivative at the origin then becomes, via the chain rule, equal to
%
\[ \sum [ (\partial_t A^\alpha)(s) + (\partial_i s^\alpha) v^i] e_\alpha, \]
%
which is invariant of the curve $c$. We can further simplify this to
%
\[ \sum [\partial_i s^\alpha + N_i^\alpha(s) ] v^i e_\alpha. \]
%
This is, in general, a \emph{nonlinear} differential equation in $s$, since the coefficients depend on $s$. If the connection is linear, we can further simplify this to
%
\[ \sum [\partial_i s^\alpha + \Gamma_{i\beta}^\alpha s^\beta ] v^i e_\alpha, \]
%
and so we get a linear differential equation in $s$.

For a general connection, we see that for constants $\lambda_1$ and $\lambda_2$, we have
%
\[ \nabla_{\lambda_1 v_1 + \lambda_2 v_2}(s) = \lambda_1 \nabla_{v_1}(s) + \lambda_2 \nabla_{v_2}(s). \]
%
In addition, \emph{if the connection is linear}, we have
%
\[ \nabla_v(\lambda_1 s_1 + \lambda_2 s_2) = \lambda_1 \nabla_v(s_1) + \lambda_2 \nabla_v(s_2) \]
%
and
%
\[ \nabla_v(a s) = v(a) s + a \nabla_v(s). \]
%
In fact, if you have a linear operator $\nabla$ satisfying these properties, you can define coefficients
%
\[ \Gamma_{i\beta}^\alpha = e_\alpha^*( \nabla_{\partial_i}( e_\beta ) ) \]
%
and check these coefficients define a linear connection, so one can take a covariant derivative operator as an equivalent definition of a connection. TODO: Equivalence of existence of this operator with definition of connection, and check whether nonlinear connection derivative determines the connection.

If $c: I \to M$ is a curve, and a bundle $E$ on $M$ equipped with a connection $\nabla$, we can pullback the connection to get a connection on $c^* E$, which, given that $TI$ is one dimensional, we should really think of as a map $D_t$ defined on sections $s$ of $E$ along $c$, given by
%
\[ D_t s = \frac{Ds}{dt} = (\nabla_T s)(c(t)), \]
%
where $T = \dot{c}$. A curve $c: I \to M$ is an autoparallel if and only if $D_t s = 0$. Indeed, we calculate that this is true in coordinates if and only if for all $i$,
%
\[ \frac{d^2 c^i}{dt^2} + \sum N^i_j(c') \frac{dc^j}{dt} = 0. \]
%
But this implies that $c''(I) \subset H(TM)$.

\section{Affine Connections}

We shall have need to consider affine connections on a vector bundle $\pi: E \to M$. An \emph{affine connection} on $E$ is a bilinear map $\nabla: \Gamma(TM) \times \Gamma(E) \to \Gamma(E)$, such that for $a \in C^\infty(M)$, $X \in \Gamma(TM)$, and $Y \in \Gamma(E)$,
%
\[ \nabla_{aX}(Y) = a \nabla_X Y \quad\text{and}\quad \nabla_X(a Y) = X(a) Y + a \nabla_X Y.  \]
%
We call $\nabla_X Y$ the \emph{covariant derivative} of $Y$ in the direction of $X$. The linearity in the $X$ variable over $C^\infty(M)$ means that $(\nabla_X Y)(p)$ depends only on $X_p$, and so we may think of $\nabla$ as a map $\Gamma(E) \to \Omega^1(TM) \otimes \Gamma(E)$.

The \emph{fundamental theorem of Riemannian geometry} states that there is a unique affine connection, the \emph{Riemannian connection} on $M$, which is \emph{torsion free}, i.e.
%
\[ \nabla_X Y - \nabla_Y X = [X,Y], \]
%
and \emph{metric compatible}, i.e.
%
\[ X \{ \langle Y, Z \rangle \} = \langle \nabla_X Y, Z \rangle + \langle Y, \nabla_X Z \rangle. \]
%
For an affine connection on $TM$, we can define the \emph{torsion} of the connection to be the bilinear map $\text{Tor}(X,Y) = (\nabla_X Y - \nabla_Y X) - [X,Y]$. Thus the Riemannian connection is \emph{torsion free} in the sense that $\text{Tor} = 0$.

TODO: Prove fundamental theorem.

Given a map $\phi: N \to M$ and a vector bundle $E \to M$, we can consider the vector bundle $\phi^* E$ on $M$, such that $(\phi^* E)_p$ is equal to $E_{\phi(p)}$, with the smooth structure such that if $\{ e_j \}$ is a smooth frame on $E$, then $e_j \circ \phi$ is a smooth frame on $\phi^* E$. If $\nabla$ is an affine connection on $E$, then we naturally have an affine connection $\phi^* \nabla$ on $\phi^* E$, which in a local frame $\{ e_j \}$, is given by the formula
%
\[ \phi^* \nabla_X( \sum a^j e_j )(p) = \sum [X(a^j) e_j + a^j \nabla_{\phi_*(X_p)} e_j](p). \]
%
One can check this formula is independent of the particular frame we choose. If $M$ is a Riemannian manifold, and $\nabla$ is the Riemannian connection on $M$, then the pullback connection satisfies
%
\[ X \langle Y, Z \rangle = \langle \phi^* \nabla_X Y, Z \rangle + \langle Y, \phi^* \nabla_X Z \rangle. \]
%
Moreover, given vector fields $X,Y \in \Gamma(TN)$, we can interpret $\phi_* Y$ and $\phi_* Z$ as elements of $\phi^* E$, and then
%
\[ \phi^* \nabla_X \{ \phi_*(Y) \} - \phi^* \nabla_Y - \phi_* [X, Y] = 0. \]
%
When we are willing to abuse notation, we will write $\phi^* \nabla$ as $\nabla$. These formulas will be useful when calculating variations associated with geodesics on a manifold.





The \emph{curvature} $R$ of a connection is a $(1,2)$ tensor on $TM$ given by
%
\[ R^a_{bc} (dx^b \wedge dx^c) \otimes \frac{\partial}{\partial \dot{x}^a}, \]
%
where $R^a_{bc} = dx^a \left[ \frac{\delta}{\delta x^b}, \frac{\delta}{\delta x^c} \right]$

 given by $R^a_{bc}$

 is given by
%
\[ R = R^a_{bc} (dx^b \wedge dx^c) \]
...
On Riemannian manifolds, we can define the length of any curve $c: I \to M$ of bounded variation, by using the formula
%
\[ L(c) = \int_I \sqrt{ \langle \dot{c}, \dot{c} \rangle }\; dt. \]
%
We can thus make any \emph{connected} Riemannian manifold into a metric space by defining
%
\[ d(p,q) = \inf L(c), \]
%
where the infinum is taken over all piecewise smooth curves $c: [0,1] \to M$ from $p$ to $q$, such that $c'$ is non-vanishing at all but finitely many points.

It is clear the distance function $d$ is symmetric. Modifying the concatenation of two paths slightly so that concatenation is smooth also shows the metric satisfies the triangle inequality. The next lemma justifies the final property that guarantees $d$ is a metric.

\begin{theorem}
    For distinct $p,q \in M$, $d(p,q) \neq 0$.
\end{theorem}
\begin{proof}
    Find a coordinate system $(x,U)$ centered at $p$. If $x(U)$ contains the closed ball $B$ of radius $\varepsilon > 0$ centered at the origin, then by smoothness,
    %
    \[ | \sum g_{ij}(p) v_i, v_j |^2 \sim \sum v_i^2 \]
    %
    with implicit constants uniform in $B$. Fix $c: [0,1] \to M$ in $\mathcal{C}$. Let $t_0$ be the first time at which $|x(c(t_0))| = \varepsilon$. We can thus identify the restriction of the curve $c$ to the interval $[0,t_0)$ with a curve in $x(U)$. Thus
    %
    \[ L(c) \geq \int_0^{t_0} |c'(t)|_g \gtrsim \int_0^{t_0} \sqrt{ \sum c_j'(t)^2 }\; dt. \]
    %
    If we define $r(t) = |c(t)|^2$, then $r(0) = 0$, $r(t_0) = \varepsilon^2$, and $r'(t) = 2 c(t) \cdot c'(t)$. Thus
    %
    \[ \varepsilon^2 = \int_0^{t_0} r'(t) = 2 \int_0^{t_0} c(t) \cdot c'(t) \leq 2 \varepsilon \int_0^{t_0} |c'(t)| \lesssim L(c). \]
    %
    Thus $L(c) \gtrsim \varepsilon$ with a uniform implicit constant. Thus $d(p,q) > 0$.
\end{proof}

\begin{remark}
    A similar argument shows that the metric $d$ gives $M$ the same topology as it's differentiable structure, i.e. because, locally in coordinates, $d(p,q) \sim |p - q|$.
\end{remark}
...
The distance $d(p,q)$ is not \emph{always} realized by a particular curve. For instance, on $\RR^d - \{ 0 \}$, with the standard Riemannian metric, the induced metric is the standard one. Thus for all $v \in \RR^d - \{ 0 \}$, $d(v, -v) = 2 |v|$, but there is no curve from $v$ and $-v$ in $\RR^d - \{ 0 \}$ with this length. We will see this is a lack of the \emph{completeness} of $\RR^d - \{ 0 \}$, i.e. the induced metric is not a complete metric, or equivalently, the manifold is not \emph{geodesically complete}, i.e. there are geodesics that cannot be continued indefinitely.





\section{The First Variation For Arclength and Energy}

We now analyze the behaviours of the minimizers of the length functional by using the calculus of variations. Fix a curve $c: I \to M$ which is piecewise smooth, i.e. smooth away from $a = t_0 < t_1 < \dots < t_n = b$. Let $\Box = I \times (-\varepsilon,\varepsilon)$, and consider a map $\alpha: \Box \to M$, which is smooth which restricted to each box $[t_i, t_{i+1}] \times (-\varepsilon,\varepsilon)$, and such that if $c_s(t) = \alpha(t,s)$, then $c_0 = c$. We define the \emph{first variation} $\delta L(\alpha)$ to be the derivative of $L(c_s)$ in the $s$ variable when $s = 0$. We say $c$ is a \emph{geodesic} if $\delta L(\alpha) = 0$ for all variations $\alpha$ which \emph{fix endpoints}, i.e. so that $c_s(0) = c(0)$ and $c_s(1) = c(1)$ for all $s \in (-\varepsilon,\varepsilon)$. We thus think of a geodesic as an extreme point of the length functional $L$. Any length minimizing curve must be a geodesic, and we will see that all sufficiently short geodesics are length minimizing.

Given a variation $\alpha$, define vector fields $T$ and $V$ by setting
%
\[ T(t,s) = \alpha_*( \partial_t|_{(t,s)} ) \quad\text{and} V(t,s) = \alpha_*( \partial_s|_{(t,s))} ). \]
%
We now compute a formula for $\delta L(\alpha)$ in terms of $T$ and $V$. By breaking up the interval $I$ into pieces, we may assume without loss of generality that $\alpha$ is smooth on $I \times (-\varepsilon,\varepsilon)$. Differentiating under the integral sign, using the metric compatibility of the affine connection on $M$, and letting
%
\[ \widehat{T} = \langle T, T \rangle^{-1/2} T \]
%
be the unit vector normalization of the vector field $T$, we conclude that
%
\begin{align*}
    \delta L(\alpha) &= \frac{d}{ds} \int_I \langle T, T \rangle^{1/2}\\
    &= \frac{1}{2} \int_I  \frac{V \{ \langle T, T \rangle \}}{\langle T, T \rangle^{1/2}}\\
    &= \int_I \langle \nabla_V T, \widehat{T} \rangle.
\end{align*}
%
Since $[T,V] = 0$ and the affine connection on $M$ is torsion free, we have $\nabla_V T = \nabla_T V$. Integrating by parts, using the identity
%
\[ \frac{d}{dt} \langle V, \widehat{T} \rangle = \langle \nabla_T V, \widehat{T} \rangle + \langle V, \nabla_T \widehat{T} \rangle, \]
%
if $I = [a,b]$, we conclude that
%
\[ \delta L(\alpha) = \left.\langle V, \widehat{T} \rangle \right|_a^b - \int_I \langle V, \nabla_T \widehat{T} \rangle. \]
%
If our variation is only piecewise smooth, then decomposing into smaller pieces gives the formula
%
\begin{align*}
    \delta L(\alpha) = \sum_i \left.\langle V, \widehat{T} \rangle \right|_{t_i^+}^{t_{i+1}^-} - \int_I \langle V, \nabla_T \widehat{T} \rangle.
\end{align*}
%
This is the \emph{first variation formula} for arclength. 

\emph{Any} vector field $V$ on the curve $c$ can be realized as the variation field for some variation. Moreover, if $V(0) = V(1) = 0$, then we can choose this variation to fix endpoints. One choice of such a variation is setting $\alpha(s,t) = \exp_{c(t)}(s V)$. But this means that $\delta L(\alpha) = 0$ for all variations if and only if $\widehat{T}(t_i^-) = \widehat{T}(t_i^+)$ for all $0 < i < n$, so that $\widehat{T}$ is a continuous vector field on $I$, and if $\nabla_T \widehat{T} = 0$ on $c$. That $\widehat{T}$ is continuous implies that the only reason $c$ could fails to be $C^1$ is if $\langle \dot{c}, \dot{c} \rangle$ is not continuous.

There is another approach to defining geodesics. Instead of the length functional $L$ directly, we can instead study the \emph{energy functional}
%
\[ E(c) = \frac{1}{2} \int_0^1 \langle \dot{c}, \dot{c} \rangle\; dt. \]
%
The advantage is that the integrand here is smooth at the origin, but the equation is now no longer invariant under reparameterization, and energy is minimized by minimizing changes of velocity. We can see this in two different ways:
%
\begin{itemize}
    \item If $s: [0,1] \to [0,1]$ is a monotone diffeomorphism, then
    %
    \[ 2 E(c \circ s^{-1}) = \int_0^1 \frac{|c'(s^{-1}(s))|_g^2}{s'(s^{-1}(s))^2}\; ds = \int_0^1 |c'(t)|_g^2 s'(t)^{-1}\; dt. \]
    %
    Using the fact that $\int_0^1 s'(t)\; dt = 1$, Cauchy-Schwartz implies that
    %
    \begin{align*}
        l &= \int_0^1 |c'(t)|_g = \int_0^1 [s'(t)^{1/2}] [s'(t)^{-1/2} |c'(t)|_g ] \leq \left( \int_0^1 s'(t)^{-1} |c'(t)|_g \right)^{1/2} \leq 2 E(c \circ s^{-1}).
    \end{align*}
    %
    Moreover, this inequality is an equality if and only if $s'(t) = \lambda |c'(t)|_g$. Thus energy is minimized over all reparameterizations of $c$ by that parameterization that makes $c$ into a constant speed curve.

    \item We will now see minimizers of the energy equation are precisely the constant speed geodesics.
\end{itemize}
%
To obtain the second point, we compute the first variation $\delta E(\alpha)$ using an analogous method to that for arclength. Applying the torsion free criterion, and integration by parts tells us that
%
\begin{align*}
    \delta E(\alpha) &= \int \langle \nabla_V T, T \rangle = 2 \int \langle \nabla_T V, T \rangle = \sum \left. \langle V, T \rangle \right|_{t_i^+}^{t_{i+1}^-} - \int \langle V, \nabla_T T \rangle.
\end{align*}
%
This is the \emph{first variation formula} for energy. We see now that $c$ is an extremal for energy if and only if $\nabla_T T = 0$ and $T$ is a continuous vector field on $c$, i.e. so that $c$ is $C^1$. Note then that
%
\[ \partial_t \langle T, T \rangle = 2 \langle T, \nabla_T T \rangle = 0, \]
%
so if $\nabla_T T = 0$, the curve $c$ must be constant speed, and then $\nabla_T \widehat{T} = 0$. Thus the extremals for energy are precisely the \emph{constant speed geodesics}.

\begin{comment}

We claim that the critical points of the energy functional are precisely the unit speed geodesics. Indeed, the functional $L: TM \to [0,\infty)$ given by $L(x,v) = |v|_g^2$ is strictly convex, so the standard theory of the calculus of variations imply that the 

This is because
%
\begin{align*}
    \partial_\alpha E(c_\alpha) &= \int_0^1 \partial_\alpha |c_\alpha'(t)|_g^2\; dt = 2 \int_0^1 \langle c_\alpha'(t), \partial_\alpha c_\alpha'(t) \rangle_g\; dt.
\end{align*}
%
This can only possibly be minimized for 

The advantage of the length functional is that it is invariant under reparametrization of the curve $c$, whereas the advantage of the energy functional is that $|\cdot|_g^2$ is smoother than $|\cdot|_g$ near the origin. For the purposes of studying the critical points of the length equation, both can be used, because if $c$ is a \emph{unit speed geodesic}, satisfying $|c'|_g = 1$ at all but finitely many points, then $L(c) = E(c)$.

We analyze both approaches for the purposes of completeness, but start with the approach using the energy functional. Fix a curve $c: [a,b] \to M$ between two points $p$ and $q$ which is entirely contained in a single coordinate system. We consider a \emph{variation} of the curve, i.e. a map
%
\[ \alpha: [a,b] \times [-\varepsilon,\varepsilon] \to M. \]
%


\begin{remark}
    If $(x,U)$ is a coordinate system, then it is simple to see, using arguments similar to the coordinate argument above, that there exists a constant $C$ such that if $p$ and $q$ are points in $U$ with $d(p,q) \leq C \min(d(p,\partial U))$, then the shortest geodesic between $p$ and $q$ must lie entirely within $U$. Thus a shortest geodesic, if it exists, must be given in the coordinate system by the Euler-Lagrange formula above. 
\end{remark}
...
\section{The Euler-Lagrange Equation For Geodesics}

We can also approach the study of geodesics from the study of Euler Lagrange equations. The Lagrangian we are consdering in our length functional is
%
\[ L(x,v) = \frac{1}{2} \sum g_{ij}(x) \dot{x}_i \dot{x}_j. \]
%
Now
%
...
%
Geodesics must satisfy the Euler-Lagrange equation, which in our case reads that for each $l$,
%
...
%
then, inverting our equation by multiplying by the inverse metric coefficients, geodesics are the solutions to the equation
%
\[ c''_l(t) + \sum_{i,j} \Gamma^l_{ij}(c(t)) c_i'(t) c_j'(t) = 0. \]
%
This is a \emph{second order} ordinary differential equation with smooth coefficients, and thus has a unique smooth solution for small times, given some initial values $c(0)$ and $c'(0)$. We conclude that \emph{geodesics must be smooth}.

The homogeneity of the, Euler-Lagrange equation tells us that this family of solutions to the Euler-Lagrange equation, and so there exists a map $\exp$, defined on an open, star-shaped subset $\Omega$ of $TM$, with codomain $M$, such that for any geodesic $\gamma: [0,1] \to M$ with $\gamma(0) = p$ and $\gamma(0)' = v$, $\gamma(t) = \exp(p,tv)$. We call $\exp$ the \emph{exponential map} of the manifold $M$.

The exponential map is a diffeomorphism in the tangent variables in a neighborhood of the origin. More precisely, if we define $\phi(x,v) = (x,\exp(x,v))$, then by the initial conditions condition on the exponential map, $D_v \phi(0,0)$ is the identity. Since $\phi(x,0) = (x,x)$, $D_x \phi(0,0) = I + I$. This means that for any $x_0$, there exists an open neighborhood $W$ of $(0,0)$ and an open set $U \times U \subset M \times M$ such that $\exp$ is a diffeomorphism from $W$ to $U \times U$. This means that for any $x_0, x_1 \in U$, there exists a unique vector $v$ such that $(x_0,v) \in W$ and $\phi(x_0,v) = (x_0,x_1)$.

It is often handy to take the Euler-Lagrange equations, which are a family of \emph{second order differential equations} on $M$, and convert them into a system of \emph{first order differential equations} on $T^* M$. This is analogous to the conversion of Lagrangian mechanics to Hamiltonian mechanics in classical physics. Given a geodesic $c(t)$, consider the curve $\xi(t) = c'(t)^\flat$ in $T^* M$, i.e. in coordinates, setting
%
\[ \xi_i(t) = \sum\nolimits_j g_{ij} c_j'(t). \]
%
...
\end{theorem}
\begin{proof}
    Suppose $M$ is geodesically complete. Fix two points $x_0,x_1 \in M$. Consider a normal coordinate system $(x,U)$ centered at $x_0$, where $x(U) = 2B$, where $B$ is a ball of some small radius $\varepsilon > 0$, a small enough ball to guarantee that $d(x_0,q) = |x(q)|$ for $q \in U$. Now let $S$ denote the inverse image of $\partial B$. By compactness, there exists a point $r \in S$ which minimizes $d(x_1,r)$. We claim that the radial geodesic $\gamma$ obtained from extending the minimum geodesic from $x_0$ to $r$ eventually touches $x_1$, and moreover, is a minimal length geodesic between $x_0$ and $x_1$. Since every curve from $x_0$ to $x_1$ must pass through $S$,
    %
    \[ d(x_0,x_1) = d(r,x_1) + \varepsilon. \]
    %
    Our proof would be completed if we could show that for all $\varepsilon \leq t \leq d(x_0,x_1)$,
    %
    \[ d(\gamma(t),x_1) = d(x_0,x_1) - t. \]
    %
    This is easily seen for $\varepsilon \leq t \leq 2 \varepsilon$ by working in the normal coordinate system around $x_0$. To prove the result for $2\varepsilon \leq t \leq d(x_0,x_1)$, let $t_0$ be a supremum of $t$ such that this equation held. If we have $t_0 = d(x_0,x_1)$, then by continuity our result would hold, so assume $t_0 < d(x_0,x_1)$. Let $p = \gamma(t_0)$, and form a normal coordinate system $(x_0,U_0)$ around $p$, where $x_0(U_0) = 2 B_0$, where $B_0$ is a $\varepsilon_0$ ball satisfying the analogous properties that $B$ satisfies. Let $r_0$ be the point on the inverse image $S_0$ of the boundary of $B_0$ which minimizes distance to $x_1$. Then we find that
    %
    \[ d(r_0,x_1) = d(p,x_1) - \varepsilon_0 = d(x_0,x_1) - t - \varepsilon_0 = d(x_0,x_1) - d(x_0,p) - \varepsilon_0. \]
    %
    By the triangle inequality, we have
    %
    \[ d(x_0,r_0) \geq d(x_0,x_1) - d(r_0,x_1) \geq \varepsilon_0 + t_0. \]
    %
    But this means that the path that travels from $x_0$ to $p$, and then radially from $p$ to $r_0$, is a minimum length path from $x_0$ to $r_0$, which implies this path is a smooth geodesic. This is only the case if $\gamma$ agree with the radial length geodesic. Thus we obtain a contradiction to the fact $t_0$ was the upper bound, which concludes the construction of the minimal length geodesic.

    We have thus proven that on any geodesically complete manifold, any two points are connected by a length minimizing geodesic. We claim it follows simply from this that $M$ is a complete metric space. If $K$ is a closed, bounded subset of $M$, fix a point $p$, and choose $R$ such that $K$ is contained in the open ball of radius $R$ centered at $p$. Now if $B \subset T_p M$ is the closed ball of radius $R$ centered at the origin, then $B$ is compact. By continuity, $\exp_p(B)$ is compact. But $K \subset \exp_p(B)$, so $K$ is compact. Thus $M$ has the Heine-Borel property. It follows from this that $M$ must be a complete metric space, by standard topological arguments.

    Conversely, suppose that $M$ is a complete metric space. Given some geodesic $\gamma: (a,b) \to M$, if $\{ t_i \}$ is a sequence converging to $b$, then $\{ \gamma(t_i) \}$ is a Cauchy sequence in $M$. Thus this sequence converges to some point $\gamma(b)$, and we can now use the uniqueness of geodesics to extend $\gamma$. A least upper bound argument shows that $\gamma$ extends to a globally defined geodesic.
\end{proof}

\section{The Second Variation Formula For Energy}

For a curve $c$, we defined the quantity $\delta E$ as a function on variations $\alpha$. But the first variation formula shows that this quantity depends only on the variation vector field $W$ along $c$, and so we can consider $\delta E$ as a function of this vector field $W$, and then $\delta E$ is a linear functional on the vector space of all such vector fields along $c$.

For a curve $c: I \to M$, we want to consider a second variation of energy. Thus we consider a 2-parameter variation $\alpha: (-\varepsilon, \varepsilon)^2 \times [a,b] \to M$ with $\alpha(0,0,\cdot) = c(\cdot)$, thus defining a family of curves $c_{u_1,u_2}$ for each $|u_1|, |u_2| < \varepsilon$. We then define
%
\[ \delta^2 E(\alpha) = \frac{\partial^2 E(c_u)}{\partial u_1 \partial u_2}. \]
%
The \emph{second variation formula} says that for a variation \emph{of a constant speed geodesic} if we define $V_1$ and $V_2$ to be the variation fields along $c$ with respect to each parameter, then
%
\[ \delta^2 E(\alpha) = - \sum \left. \langle V_2, D_t V_1 \rangle \right|_{t_i}^{t_{i+1}} - \int_a^b \langle V_2, D_t^2 V_1 + R( V_1, T ) T \rangle \]
%
As a consequence, this quantity depends only on $V_1$ and $V_2$, and so we write it as $\delta^2 E( V_1, V_2 )$.

To prove the second variation formula, we can use metric compatibility and the lack of torsion to write
%
\begin{align*}
    \delta^2 E(\alpha) &= \int_I \langle \nabla_{V_1} \nabla_{V_2} T, T \rangle + \langle \nabla_{V_2} T, \nabla_{V_1} T \rangle\\
    &= \int_I \langle \nabla_{V_1} \nabla_T V_2, T \rangle + \langle \nabla_T V_2, \nabla_T V_1 \rangle.
\end{align*}
%
Now we use the fact that $\nabla_{V_1} \nabla_T V_2 = \nabla_T \nabla_{V_1} V_2 + R(V_1, T) T$. An integration by parts, using that $\nabla_T T = 0$, yields that
%
\[ \int \langle \nabla_T \nabla_{V_1} V_2, T \rangle = \langle \nabla_{V_1} V_2, T \rangle|_a^b. \]
%
Thus we have
%
\[ \delta^2 E(\alpha) = \langle \nabla_{V_1} V_2, T \rangle|_a^b + \int_I \left[ \langle \nabla_T V_1, \nabla_T V_2 \rangle - \langle R(V_1,T) T, T \rangle \right]. \]
%
For piecewise smooth variations, summing up the endpoint terms gives
%
\[ \delta^2 E(\alpha) = \sum \langle \nabla_{V_1} V_2, T \rangle|_{t_i^+}^{t_{i+1}^-} + \int_I \left[ \langle \nabla_T V_1, \nabla_T V_2 \rangle - \langle R(V_1,T) T, T \rangle \right]. \]
%
If we are consider a \emph{smooth} variation rather than a piecewise smooth variation, and also assume that this variation fixes endpoints, then the endpoint terms completely vanish, and so
%
\[ \delta^2 E(\alpha) = \int_I \langle \nabla_T V_1, \nabla_T V_2 \rangle - \langle R(V_1,T) T, T \rangle. \]
%
We call this quantity the \emph{index form} of $V_1$ and $V_2$, denoted $I(V_1,V_2)$. It is symmetric and bilinear over vector fields. A further integration by parts also allows us to write
%
\[ I(V_1,V_2) = \left.\langle \nabla_T V_1, V_2\rangle \right|_{t_i^+}^{t_i^-} - \int \langle \nabla_T \nabla_T V_1, V_2 \rangle + \langle R(V_1,T) T, T \rangle. \]
%
If $\alpha: \Box \to M$ is a variation fixing endpoints, defining a family of curves $\{ c_s \}$, with $c_0$ a geodesic and with $V$ the associated vector field, then
%
\[ E(c_s) = E(c_0) + s^2 \frac{I(V,V)}{2} + O(s^3). \]
%
It thus follows that if $I(V,V) < 0$, then $E(c_s) < E(c_0)$ for $s \neq 0$ and suitably close to zero. Thus $c_0$ is not a minimizer of energy (and as a result, not a length minimizing geodesic). Conversely, if $I(V,V) < 0$ for some vector field on $c$ vanishing at endpoints, then we can find a variation of $c$ under which $c$ is not a minimum. It will be of use to introduce a notation for the set of all vector fields vanishing at the endpoints on $c$: we denote the space of such vector fields by $\Omega_c$.

Conversely, if $I(V,V) \geq 0$ for all vector fields $V \in \Omega_c$, we would hope that $c$ \emph{actually is} a minimum length curve between two points. To do this, we begin by studying `critical points' of the index form, vector fields $J$ along $c$ with $I(J,V) = 0$ for all $V \in \Omega_c$. Vector fields with this property are called \emph{Jacobi fields}. The second variation formula shows that a vector field $J$ is Jacobi if and only if it satisfies the differential equation
%
\[ \nabla_T \nabla_T J + R(J,T) T = 0. \]
%
This is a system of $d$ second order ordinary differential equations. Thus the family of all Jacobi fields on a curve $c$ is $2d$ dimensional.

Two points $a$ and $b$ on a curve are \emph{conjugate} if there exists a Jacobi field $J$ on $c$ such that $J(a) = J(b) = 0$. The multiplicity of the pair is then the dimension of the vector space of Jacobi fields vanishing at $a$ and $b$. There exists Jacobi fields taking on every possible value of $J(a)$ by existence and uniqueness of solutions to ODEs, and thus by rank-nullity, the space of Jacobi fields vanishing at $a$ is $d$ dimensional. A direct computation shows that the vector field $J = (t - a) T(t)$ is a Jacobi field vanishing at $a$, but non-zero everywhere else. It thus follows that the multiplicity of a pair of conjugate points is \emph{at most $d-1$}.

This is tight. For instance, on the sphere $S^d$, antipodal points are conjugate along geodesics, with multiplicity $d-1$. To see this, we note that every Jacobi field $J$ arises as the variation field of a variation $\alpha: \Box \to M$ with the property that the resulting family of curves $\{ c_s \}$ are all geodesics. If $J(a) = J(b) = 0$, we can choose this family such that $c_s(a) = c(a)$ for all $s$, but not of the endpoints, which only agree up to second order.

Then all geodesics beginning at one antipodal point meet at the other antipodal point, and there are a $d-1$ dimensional family of such geodesics, giving rise to $d-1$ linearly independent Jacobi fields vanishing at each point.

Now $(at + b) T$ is a Jacobi field for all $a$ and $b$, as may be computed directly. It follows that every Jacobi field $J$ can be written uniquely as $J^\perp + (at + b) T$ for a Jacobi field $J^\perp$ everywhere perpendicular to $c$. In particular, if $J$ is perpendicular to $c$ at one point, it is perpendicular to $c$ at every point.

TODO: Theorem 8.6 of Spivak Volume 4; If a curve $c:[a,b] \to M$ contains a point conjugate to $a$ in it's interior, then there exists $W$ with $I(W,W) < 0$, and thus $c$ is not length minimizing.

\section{TODO: Separate Jacobi Field Notes}

Given a vector $v \in T_p M$, define a variation $\alpha(t,s) = \exp_p( t (v + sw) )$. Then the variation field
%
\[ J(t) = (\partial_s \alpha)(t,0) = (\exp_p)_*(tv)(tw) \]
%
satisfies the differential equation $\nabla_T J = \nabla_J T$, where $\nabla_r = (\exp_p)_*( tr )( \partial_r )$.

% t[ v + s(av + w) = t ( (1 + as) v + sw ) ]
...
Let $V$ be a vector space. A \emph{strongly convex Minkowski norm} is a function $F: V \to [0,\infty)$, smooth away from the origin, homogeneous of degree one, and such that for each $v \in V$, the Hessian of the function $F^2$ on $V - \{ 0 \}$ is positive definite. The \emph{indicatrix} of $F$ is then $F^{-1}(1)$, and by homogeneity, uniquely determines $F$. A \emph{Finsler metric} on a manifold $M$ will then be a smooth choice of strongly convex Minkowski norms on each tangent space of $M$, and so it is profitable to start by studying such norms.

\begin{example}
    Let $Q: V \times V \to \RR$ be an inner product on $V$. Then $F(v) = Q(v,v)^{1/2}$ is a strongly convex Minkowski norm on $V$. All such inner products are equivalent by a change of basis. But this is \emph{not} the case with Minkowski norms.
\end{example}

In general, strongly convex Minkowski norms are precisely norms that can be closely approximated up to second order by the square root of an inner product like above. For each $v \in V - \{ 0 \}$, define an inner product $Q_v$ on $V$ by taking half the Hessian of $F^2$ at the point $v$. One may then check that $F(w)$ and $Q_v(w,w)^{1/2}$ agree up to second order at $v$. 
% The jth derivative of Q_v(w,w) is
%
% Q_v(w,w)^{-1/2} g_{jk} w^k
%
% at v, the jth derivative is thus F_j
%
%
% The jth and ath derivative of Q_v(w,w) is
%
% - Q_v(w,w)^{-3/2} ( g_{jk} w^k ) ( g_{ak} w^k ) + Q_v(w,w)^{-1/2} g_{ja}(v)
%
% Thus the jth and ath derivative at v is
%
% - F(v)^{-1} F_j(v) F_a(v) + F_{ja}(v) + F(v)^{-1} [ F_j(v) F_a(v) ]
% F_{ja}
%
In coordinates, we can write $Q_v(w,u) = \sum g_{jk}(v) w^j u^k$ for constants
%
\[ g_{jk} = F\; \partial_j \partial_k F + (\partial_j F) (\partial_k F). \]
%
We have the \emph{fundamental inequality} $dF(v) \{ w \} \leq F(w)$, with equality if and only if $w$ is a scalar multiple of $v$. This inequality implies that $F(v + w) \leq F(v) + F(w)$, with equality if and only if $w$ is a scalar multiple of $w$. Thus a strongly convex Minkowski norm is really a norm in the sense of functional analysis. One may view the fundamental inequality as a strengthening of Euler's theorem $dF(v) \{ v \} = F(v)$, which holds for all homogeneous functions. Using the metric, we can convert the fundamental inequality to read
%
\[ Q_v(v,w) \leq F(v) F(w), \]
%
with equality if and only if $w$ is a positive scalar multiple of $v$, and then the fundamental inequality can be seen as a generalization of Cauchy-Schwartz.

\begin{example}
    Consider the convex limaçon in $\RR^2$ given in polar coordinates by $r = 3 + \cos \theta$. It is described in Cartesian coordinates by $f(x,y) = 0$, where $f(x,y) = (x^2 + y^2) - 3 \sqrt{x^2 + y^2} - x$. Using \emph{Okubo's technique}, we can find a homogeneous function $F$ equal to one on the convex limaçon by solving
    %
    \[ f(z / F(z)) = 0, \]
    %
    and it is given by $F(x,y) = (x^2 + y^2) / ( 3 \sqrt{x^2 + y^2} + x )$. This function is strongly convex.
\end{example}

\begin{example}
    The quartic unit ball in $\RR^2$ given by the equation $x^4 + y^4 = 1$ has a strictly convex interior, but \emph{is not the unit ball of a strongly convex metric}. The associated function is $F(x,y) = ( x^4 + y^4 )^{1/4}$, but the Hessian becomes singular on the $x$ and $y$ axes. On the other hand, the pertubed functions
    %
    \[ F(x,y) = \sqrt{(x^4 + y^4)^{1/4} + \varepsilon (x^2 + y^2)} \]
    %
    for $\varepsilon > 0$ \emph{are} strongly convex.
\end{example}

Now we consider the case of manifolds. A \emph{Finsler Metric} on a manifold $M$ is a homogeneous function $F: TM \to [0,\infty)$, smooth away from the zero section, such that for each $x \in M$, the function is a strongly convex Minkowski norm on $T_x M$. As a shorthand, we write $\partial_{\alpha|\beta}$ for $\partial_{x^\alpha} \partial_{v^\beta}$, or $\partial_\alpha$ or $\overline{\partial}_\beta$ for $\partial_{x^\alpha}$ and $\partial_{v^\beta}$.

\begin{example}
    Let $M$ be a Riemannian manifold, fix a one form $\omega \in \Omega^1(M)$ with $\| \omega(x) \|_g < 1$ for all $x \in M$, and define a function $F(x,v) = \| v \|_g + \omega(v)$. Then one can check that $F$ is strongly convex, and thus gives a Finsler metric on $M$. To give an explicit example on $M = \RR^2 - \{ 0 \}$, for each $x \in M$, we define $F$ so that at each $x$, the indicatrix of $F$ is an ellipse of eccentricity $e(x)$ with one focus at the origin, and such that the directrix of the ellipse associated with that focus given by the line passing through $0$ perpendicular to the line segment from $0$ to $x$. One can check that this metric is given by
    %
    \[ F(x,y) = \frac{\sqrt{ e(x) \rho(x,v) + \lambda(x,v)^2 } + \lambda(x,v)}{e(x)}, \]
    %
    where $\lambda(x,v) = (x \cdot v) / |x|^2$ and $\rho(x,v) = |v|^2 / |x|^2$. See (Antonelli, Ingarden, Matsumoto) for more details. If $e$ is constant, then the geodesics of this Finsler manifold are logarithmic spirals through the origin.
    %   
    % e(x)^{-1} |x| |v| = |x|^2 + (x * v)
    %
    % e(x)^{-2} |x|^2 |v|^2 = |x|^4 + 2 |x|^2 (x * v) + (x * v)^2
    %
    % e(x)^{-2} |x|^2 ( v_1^2 + v_2^2 ) = |x|^4 + 2 |x|^2 x1 v1 + 2 |x|^2 x2 v2 + x1^2 v1^2 + x2^2 v2^2 + 2 x1x2 v1v2
    %
    % v_1^2 ( e(x)^{-2} |x|^2 - x1^2  ) + v_2^2 ( e(x)^{-2} |x|^2 - x2^2 )
    %       - v1 (2 |x|^2 x1 ) - v2 ( 2 |x|^2 x2 )
    %       - v1v2 (x1x2)
    %       - |x|^4
\end{example}

\begin{example}
    Consider the Finsler metric on the ball $B$ of radius 2 at the origin with constant negative Gaussian curvature $-1/4$, of the form
    %
    \[ F(x,v) = \frac{4|v|}{4 - |x|^2} + \frac{16|x|}{(4 - |x|^2)(4 + |v|^2)} \frac{v_1 + v_2}{|x|}. \]
    %
    The Finsler metric distance from the origin to the rim of the disk is infinite, but the distance from the rim of the disk to the origin is $\log 2$. We will see the geodesics of this Finsler manifold are precisely the hyperbolic trajectories on $B$.
\end{example}

Consider the \emph{sphere bundle} $\pi: SM \to M$ and \emph{slit tangent bundle} $\pi: TM - 0 \to M$. The sphere bundle $SM$ is obtained from $TM - 0$ by projectivizing, i.e. identifying vectors with positive scalar multiples of themselves, or alternatively if $M$ has a Finsler metric, by taking the submanifold of $TM - 0$ of elements with norm one.

We will often work with the vector bundle $B = \pi^*(TM)$ on $TM$, as well as the dual bundle $B^*$, which we can identify with $\pi^*(T^* M)$. Two natural sections $l$ and $\pi$ of $B$ and $B^*$ are given in coordinates by
%
\[ l^i = F^{-1} v^i \quad\text{and}\quad \omega_i = \overline{\partial}_i F. \]
%
The vector field $l$ is called the \emph{distinguished section}, and $\omega$ is called the \emph{Hilbert form}, due to Hilbert's observation that for a curve $c: [0,1] \to M$,
%
\[ \int_0^1 F \left( c, \frac{dc}{dt} \right) = \int_0^1 (\overline{\partial}_i F) \left(c, \frac{dc}{dt} \right) \frac{dc^i}{dt} = \int_c \omega. \]
%
This formula is also called \emph{Hilbert's Invariant Integral}. The bundle $B$ is also equipped with an inner product $g$ induced by the Finsler metric on $M$, given in coordinates by the formula
%
\[ g_{ij} = \overline{\partial}_{ij} \{ F^2 \} / 2 = F (\overline{\partial}_{jk} F) + (\overline{\partial}_j F) (\overline{\partial}_k F), \]
%
and which we can also think of as a section of $B^* \otimes B^*$ (in which case the inner product $g$ is called the \emph{fundamental tensor}). Conversely, $B^*$ has an inner product induced by the dual Finsler metric on $M$, and the coordinates $g^{ij}$ in coordinates are given precisely by the entries of the matrix given by taking the inverse of the matrix with coordinates $g_{ij}$. With respect to the metric $g$, $l$ and $\omega$ are dual, i.e. $\sum g_{ij} l^j = \omega_i$, or for any vector $w$, $g(l,w) = \omega(w)$.

We also define the \emph{Cartan tensor}
%
\[ A_{ijk} = \frac{F}{2} \overline{\partial}_k g_{ij} = \frac{F}{4}\; \overline{\partial}_{ijk} F^2. \]
%
This is the \emph{Cartan tensor}. It is a symmetric tensor, a section of $B^* \otimes B^* \otimes B^*$. Sometimes, in the literature the tensor $C_{ijk} = A_{ijk} / F$ is used as the definition of the Cartan tensor, but it is often more natural to work with tensors that are homogeneous of order zero so that they are naturally defined on the sphere bundle. Roughly speaking, $g$ is the inner product that approximates $F$ to second order, and $A$ somehow provides a measure of how non-Riemannian the function $F$ is; if $F$ was induced by a Riemannian metric, than it is a theorem of Deicke, found in a 1953 paper, that $A_{ijk} = 0$ uniformly if and only if the Finsler metric is Riemannian. Some useful formulas are that $g(l,l) = \sum g_{ij} l^i l^j = 1$, and that $A_{ijk} l^i = 0$.

We often choose to work with special frames $\{ e_a \}$ for $B$, which are $g$ orthonormal, and satisfy $e_n = l$. The dual frame $\{ \omega^a \}$ to $\{ e_a \}$ then has the property that $\omega^n = \omega$. If we write $e_a = \sum e_a^i \partial_i$ and $\omega^a = \sum \omega^a_i dx^i$, then $\sum e_a^i \omega^b_i = \delta^b_a$. Thus we have $\partial_i = \sum \omega^a_i e_a$ and $dx^i = \sum e^i_a \omega^a$. For $a,b \neq n$, 
%
\[ F (\overline{\partial}_{ij} F) e_a^i e_b^j = \delta_{ab}, \]
%
because $g(e_a,e_b) = \delta_{ab}$, and $\sum \overline{\partial}_i F e_a^i = \sum \omega^n_i e_a^i = \delta^n_a$. Conversely, for $a,b \neq n$,
%
\[ \sum\nolimits_i \omega^a_i \omega^b_i = F (\overline{\partial}_{ij} F). \]
%
This follows simply by inverting the last equation.

We define the \emph{formal Christoffel symbols}
%
\[ [jk,s] = \frac{\partial_j g_{sk} + \partial_k g_{sj} - \partial_s g_{jk}}{2}, \]
%
and then define $\gamma^i_{jk}$ by `raising indices', defining
%
\[ \gamma^i_{jk} = g^{is} [jk,s]. \]
%
In the future, we will raise and lower indices by summing over the metric in this way. We also define the \emph{nonlinear connection}
%
\[ N^i_j = \gamma^i_{jk} v^k - C^i_{jk} \gamma^k_{rs} v^r v^s. \]
%
To make the nonlinear connection homogeneous of order zero, we consider
%
\[ \tilde{N}^i_j = F^{-1} N^i_j = \gamma^i_{jk} l^k - A^i_{jk} \gamma^k_{rs} l^r l^s. \]
%
There is another formula, which proves more useful in the analysis of geodesics.

\begin{lemma}
    If we set $G^i = \gamma^i_{ab} v^a v^b$, then $N^i_j = (1/2) \overline{\partial}_{j} G^i$.
\end{lemma}
\begin{proof}
    Using homogeneity, we see that if $G_a = g_{ab} G^b = \gamma_{ajk} v^j v^k$, then $\overline{\partial}_i G_a = 2 \gamma_{aij} v^j$, because
    %
    \begin{align*}
        \overline{\partial}_i \{ \gamma_{ajk} \} v^j v^k &= \frac{1}{2} \left( \partial_{j|i} g_{ak} + \partial_{k|i} g_{aj} - \partial_{a|i} g_{jk} \right) v^j v^k\\
        &= \frac{ \partial_{j|aik} \{ F^2 \} + \partial_{k|aij} \{ F^2 \} - \partial_{a|ijk} \{ F^2 \} }{4} v^j v^k = 0.
    \end{align*}
    %
    And so
    %
    \begin{align*}
        \overline{\partial}_i G_a = 2 \gamma_{aij} v^j.
    \end{align*}
    %
    But now we see that
    %
    \begin{align*}
        \frac{1}{2} \overline{\partial}_{j} G^i &= \frac{1}{2} \overline{\partial}_{j} \{ g^{ik} G_k \}\\
        &= \frac{1}{2} [ \overline{\partial}_{j} \{ g^{ik} \} G_k + g^{ik} \overline{\partial}_{j} \{ G_k \} ] \\
        &= - \frac{1}{4} g^{it} \overline{\partial}_{tuj} \{ F^2 \} g^{uk} G_k + g^{ik} \gamma_{kja} v^a\\
        &= - C^i_{uj} G^u + \gamma^i_{ja} v^a\\
        &= \gamma^i_{jk} v^k - C^i_{jk} G^k\\
        &= \gamma^i_{jk} v^k - C^i_{jk} \gamma^k_{rs} v^r v^s = N^i_j. \qedhere
    \end{align*}
\end{proof}

None of the quantities $\{ N^i_j \}$, $\{ G^a \}$, or $\{ \gamma^i_{jk} \}$ transform tensorially. But the quantities $\tilde{N}^i_j$ have a somewhat nice transformation formula. The quantities $\tilde{N}^a_b$ in a new coordinates $\tilde{x}$ are given by the old quantities $N^i_j$ by the formula
%
\[ \tilde{N}^a_b = \frac{\partial \tilde{x}^a}{\partial x^i} \frac{\partial x^j}{\partial \tilde{x}^b} N^i_j + \frac{\partial \tilde{x}^a}{\partial x^i} \frac{\partial^2 x^i}{\partial \tilde{x}^b \partial \tilde{x}^s} \tilde{v}^s. \]
%
The coefficients $N^i_j$ define a (nonlinear) Ehresman connection on $TM - 0$, and thus split $T(TM - 0)$ into two subbundles $H = H(TM - 0)$ and $V = V(TM - 0)$, and conversely split $T^*(TM - 0)$ into $H^\perp$ and $V^\perp$. We define $\delta_j = \partial_j - N^i_j \overline{\partial}_{i}$ and $\delta v^i = dv^i + N^i_j dx^j$. Then $H$ is spanned by $\{ \delta_j \}$, $V$ is spanned by $\{ \overline{\partial}_j \}$, $H^\perp$ is spanned by $\delta v^i$, and $V^\perp$ is spanned by $dv^i$. Now $TM - 0$ has a natural Riemannian metric
%
\[ g_{ij} dx^i \otimes dx^j + g_{ij} \frac{\delta v^i}{F} \otimes \frac{\delta v^j}{F} \]
%
known as the \emph{Sasaki metric}, which has the property that $H$ and $V$ are then orthogonal.

We consider an orthonormal frame $\{ \widehat{e}_a \} \oplus \{ \widehat{e}_{n + a} \}$ and $\{ \widehat{\omega}^a \} \oplus \{ \widehat{\omega}^{a + n} \}$ for $H \oplus V$ and $V^\perp \oplus H^\perp$ which are dual to one another. Then we can write
%
\[ \widehat{e}_a = \sum \widehat{e}_a^i \delta_i \quad\text{and}\quad \widehat{e}_{n + a} = \sum \widehat{e}_{n+a}^i \overline{\partial}_i \]
%
and
%
\[ \widehat{\omega}^a = \sum \widehat{\omega}^a_i dx^i \quad\text{and}\quad \widehat{\omega}^{n + a} = \sum \widehat{\omega}^{n + a}_i \frac{\delta v^i}{F}. \]
%
We will make the additional assumption that $\widehat{e}_{n+a}^i = \widehat{e}_a^i$, and thus that $\widehat{\omega}^{n+a}_i = \widehat{\omega}^a_i$. Moreover, we will assume that $\widehat{\omega}_n = \omega$ and $\widehat{e}_n = l$.

We leave it an exerc,se to prove that for such a frame,
%
\[ d ( \log F ) = \omega_i \frac{\delta y^i}{F} = \widehat{\omega}^{n+n}. \]
%
It follows from this that $d (\log F) \in H^\perp$, and thus $F$ is constant along every \emph{horizontal curve} on $M$, i.e. $\delta_i F = 0$ for all $i$.

A linear connection $\nabla$ on the bundle $B$ is \emph{torsion free} if
%
\[ \nabla_X Y - \nabla_Y X = [X,Y]. \]
%
It is \emph{metric compatible} if
%
\[ X( g(Y,Z) ) = g ( \nabla_X Y, Z ) + g( Y, \nabla_X Z ). \]
%
There \emph{does not exist} a $g$-compatible, torsion free linear connection on the bundle $B$ unless the Finsler metric on $M$ is induced by a Riemannian metric. There are thus \emph{several} linear connections on $B$ which approximate these properties. The \emph{Chern connection} is the \emph{unique} torsion free connection on $B$ which is \emph{almost metric compatible},  in the sense that
%
\[ X( g(Y,Z) ) = g( \nabla_X Y, Z ) + g( Y, \nabla_X Z ) + 2 A( \nabla_X l, Y, Z ), \]
%
where $C$ is the Cartan tensor. In coordinates, write $\nabla \partial_i = \omega_i^j \partial_j$ for one-forms $\omega_i^j$ (the notation overloads $\omega$, which is unfortunate, but we will try and keep these notations distinct by indexing elements of a frame by indices such as $a$ and $b$, and indexing coordinates bi indices such as $i$ and $j$), then the connection is torsion free if and only if for all $j$,
%
\[ \sum dx^j \wedge \omega_j^i = 0, \]
%
or equivalently, if $\omega_j^i = \Gamma^i_{jk} dx^k$ for functions $\Gamma^i_{jk}$ symmetric in $j$ and $k$. If we write $\Gamma_{i|jk} = g_{is} \Gamma^s_{jk}$, then metric compatibility is equivalent to an expression of the form
%
\[ \Gamma_{i|jk} = \frac{1}{2} \left( \delta_j g_{ik} + \delta_k g_{ij} - \delta_i g_{jk} \right). \]
%
This should be compared to the formula for $\gamma_{i|jk}$.

If we consider a special $g$-orthonormal basis for $B$, then we can also consider connection coefficients $\tilde{\omega}^a_b$ such that
%
\[ \nabla e_b = \tilde{\omega}^a_b e_a. \]
%
Then one can check that
%
\[  \tilde{\omega}^a_b = (de_b^i) \omega^a_i + e_b^j \omega_j^i \omega^a_i. \]
%
Conversely,
%
\[ \omega_j^i = (d\omega^a_j) e_a^i + \omega^b_j \tilde{\omega}_b^a e_a^i. \]
%
The torsion free criterion then reads that
%
\[ d \omega^a - \omega^b \wedge \tilde{\omega}_b^a = 0 \]
%
and almost metric compatibility reads that
%
\[ \tilde{\omega}^a_b + \tilde{\omega}^b_a = - 2 A_{abc} \widehat{\omega}^{n+c}. \]
%
These equations imply that $\tilde{\omega}^n_n = 0$ (and more generally, $\tilde{\omega}^n_a = - \tilde{\omega}_a^n$), since we have
%
\[ A_{abc} = A_{ijk} e_a^i e_b^j e_c^k = F/4 \overline{\partial}_{ijk} \{ F^2 \} e_a^i e_b^j e_c^k. \]
%
Since $\sum \overline{\partial}_{ijk} \{ F^2 \} l^i = 0$, if $n \in \{ a, b, c \}$, then $A_{abc} = 0$.
% A_{nab} = 0 for any $a$ and $b$
%
%\begin{align*}
%    \tilde{\omega}^n_n &= d(e_n^i) \omega_i^n + e_n^j \omega_j^i \omega_i^n\\
%    &= \left( [- l^i / F] (\partial_j F) ( \overline{\partial}_i F ) dx^j \right) + \left( e_n^j \Gamma^i_{jk} (\overline{\partial}_i F) dx^k \right)\\
%    &= \left( - \frac{\partial_i F}{F} + \Gamma_{k|ij} l^j l^k \right) dx^i\\
%    &= \left( - \frac{\partial_i F}{F} + \frac{\partial_i F}{F} \right) dx^i = 0.
    % partial_i g_{kj} l^j l^k + partial_j g_{ki} l^j l^k - partial_k g_{ij} l^j l^k
    % 4 partial_i F / F
%\end{align*}
%
We will mainly use the Chern connection, but there are other options of a linear connection. For instance, the \emph{Cartan connection} is the connection with coefficients $\omega^i_j + A^i_{jk} \delta v^k / F$. It is metric compatible, but \emph{has torsion}. We refer to (Bao-Chern) for other connections of common use.

Before we move on, we list several other formulas which prove useful. First,
%
\[ \Gamma^i_{jk} l^j = F^{-1} N^i_k \]
%
and so if we write $\nabla l = (\nabla l)^i \partial_i$, then $(\nabla l)^i = dl^i + l^j N_j^i$, then
%
\[ \frac{\delta v^i}{F} = dl^i + l^j \omega_j^i + l^i d(\log F) = (\nabla l)^i + l^i d (\log F). \]
%
Thus
%
\[ \widehat{\omega}^{n+a} = \omega^a_i \frac{\delta v^i}{F} = \omega^a (\nabla l) + \delta^a_n d (\log F). \]
%
But now we get that
%
\[ g(l, \nabla l) = \omega( \nabla l ) = \widehat{\omega}^{n + n} - d (\log F) = 0. \]
%
For $a \neq n$, we have $\widehat{\omega}^{n+a} = \tilde{\omega}^a_n$, but it is not true that $\widehat{\omega}^{n+n} = \omega^n_n$. The latter is equal to zero, whereas the former is never zero. %Finally, if we set $\sqrt{g} = \sqrt{\det(g_{ij})}$, then $d \log \sqrt{g} = (g^{ab}/2) dg_{ab}$. Thus
%
%\begin{align*}
%    \delta_i \log \sqrt{g} &= \partial_i \log \sqrt{g} - N_i^j \overline{\partial}_j \log \sqrt{g}\\
%    &= (g^{ab} / 2) \partial_i g_{ab} - g^{ab} N_i^j C_{abj}\\
%    &= \Sum \Gamma^j_{ij} - g^{ab} \tilde{N}_i^j A_{abj}\\
%    &= \Sum \Gamma^j_{ij}.
%\end{align*}
%
%This follows because
% g^{ab} g_{ab} = n
% g^{ab} A_{abj} = - [partial_j g^{ab}] g_{ab}
%\begin{align*}
%    g^{ab} A_{abj}
%\end{align*}
% A^b_{bj} = A^{bj}_b A^k_{ij} gamma_{k|rs} l^r l^s

% Sum Gamma^j_{ij} = g^{ja}/2 [ di_i g_{aj} + di_j g_{ai} - di_a g_{ij} ]
%                  = g^{ab}/2 di_i g_{ab}


% 5.3.1 of Page 125 Geodesics of constant speed satisfy
% [d^2 sigma^i / dt^2] + [dsigma^j / dt] [ dsigma^k/dt ] (gamma^i_{jk})(sigma, sigma') = 0
% In normal coordinates on M, is it true that gamma^i_{jk}(0,v) = 0 at the origin for all v?
% If this is true, then by C^1, |gamma^i_{jk}(x,v)| << |x|, so d^2 sigma^i / dt^2 = 0.
%
%
% Geodesics satisfy x'' = - G(x,x'), where
% G_i = (partial_{x_k} g_{ij}) v^j v^k - (1/2) (partial_{x_i} g_{jk}) v^j v^k
%
% Does G(0,v) = 0?
% We know that x(t) = tw satisfy this equation. Thus
%
% G(tw, w) = 0 for all t and w
% IN particular, G(0,w) = 0 for all w.
%
% So I *think* this should be true.

\section{Curvature Forms}

The \emph{curvature $2$-forms} of the Chern connection are $\Omega_j^i = d\omega_j^i - \omega_j^k \wedge \omega_k^i$. They can be expanded as
%
\[ \Omega_j^i = \frac{1}{2} R^i_{jkl} dx^k \wedge dx^l + P^i_{jkl} dx^k \wedge \frac{\delta y^l}{F}+ frac{1}{2} Q^i_{jkl} \frac{\delta y^k}{F} \wedge \frac{\delta y^l}{F}. \]
%
The objects $R$, $P$, and $Q$ are called the $hh$, $hv$, and $vv$ \emph{curvature tensors}. We will assume without loss of generality that $R$ and $Q$ are antisymmetric in the $k$ and $l$ index (though we will soon see that $Q$ vanishes for the Chern connection).

TODO: Bianchi identities.



\chapter{Variation Formulas for Finsler Manifolds}

Let $\Box = [0,r] \times (-\varepsilon,\varepsilon)$, and consider 
%
\[ 0 = t_0 < t_1 < \dots < t_a = r. \]
%
Consider the space of all regular curves $c: [0,r] \to M$ which are piecewise smooth on $[t_i,t_{i+1}] \times (-\varepsilon,\varepsilon)$. A \emph{smooth variation} of a curve in $X$ is a function $\sigma: \Box \to M$ which is piecewise smooth on each box $[t_i,t_{i+1}] \times (-\varepsilon,\varepsilon)$ and with $\sigma(\cdot,0) = c(\cdot)$.

Arising from any variation is a pair of vector fields
%
\[ T = \frac{d\sigma}{dt} \quad\text{and}\quad U = \frac{d\sigma}{du}, \]
%
given by differentiation in the first and second variables. They are well defined away from the times $\{ t_j \}$. The map $\sigma$ lifts to a map $\widehat{\sigma}: \Box \to TM - 0$, and thus gives rise to two vector fields
%
\[ \widehat{T} = \frac{d\widehat{\sigma}}{dt} \quad\text{and}\quad \widehat{U} = \frac{d\widehat{\sigma}}{du}. \]
%
We consider a special frame $\{ e_s \}$ for the bundle $B$ on $TM - 0$, and use this frame to write $T = T^s e_s$ and $U = U^s e_s$. We also write
%
\[ \widehat{\sigma}^* \omega^s = a^s dt + b^s du \quad\text{and}\quad \widehat{\sigma}^* \tilde{\omega}_s^r = A_s^r dt + B_s^r du. \]
%
Since $\omega^s \in V^\perp$,
%
\[ a^s = (\widehat{\sigma}^* \omega^s)(\partial_t) = \omega^s( \widehat{T} ) = \omega^s(T) = F(T) \omega^s(l) = \delta^s_n F(T). \]
%
Similarily, $b^s = U^s$, $A_s^r = \tilde{\omega}_s^r( \widehat{T} )$, $B_s^r = \tilde{\omega}_s^r( \widehat{U} )$. Since $\tilde{\omega}_s^n = - \tilde{\omega}_n^s$, the almost metric compatibility implies that $A^n_n = B^n_n = A_s^n + A^s_n = B_s^n + B^s_n = 0$. If we pull back the torsion free condition, then for $s \neq n$,
%
\begin{align*}
    0 &= \widehat{\sigma}^* \left[ d\omega^s - \omega^r \wedge \tilde{\omega}_r^s \right]\\
    &= d ( b^s du ) - (b^r du) \wedge \left( A^s_r dt + B^s_r du \right) - ( F(T) dt ) \wedge ( A^s_n dt + B^s_n du )\\
    &= \left( \frac{\partial b^s}{\partial t} + b^r A^s_r - F(T) B_n^s \right) (dt \wedge du),
\end{align*}
%
and for $s = n$, noting that $dF$ lies in $H^\perp$,
%
\begin{align*}
    0 &= \widehat{\sigma}^* \left[ d\omega^s - \omega^r \wedge \tilde{\omega}_r^s \right]\\
    &= d ( F(T) dt + b^n du ) - (b^r du) \wedge \left( A^n_r dt + B^n_r du \right) - ( F(T) dt ) \wedge ( A^n_n dt + B^n_n du )\\
    &= \left( - dF ( \widehat{U} ) + \frac{\partial b^n}{\partial t} + b^r A^n_r \right) (dt \wedge du)\\
    &= \left( - \frac{\partial a^n}{\partial u} + \frac{\partial b^n}{\partial t} + b^r A^n_r \right).
\end{align*}
%
Now the length $L(u)$ of the curve $t \mapsto \sigma(t,u)$ is given by
%
\[ \sum_i \int_{t_i}^{t_{i+1}} F(T)\; dt = \sum_i \int_{t_i}^{t_{i+1}} a^n\; dt. \]
%
Thus
%
\begin{align*}
    L'(u) &= \sum_i \int_{t_i}^{t_{i+1}} \frac{\partial a^n}{\partial u}\\
    &= \sum_i \int_{t_i}^{t_{i+1}} \left[ \frac{\partial b^n}{\partial t} + b^r A^n_r \right]\; dt\\
    &= \sum_i \left. b^n \right|_{t_i}^{t_{i+1}} + \int_{t_i}^{t_{i+1}} b^r A^n_r\; dt.
\end{align*}
%
If a curve is an extreme point among the space of functions under which we are considering variations, then $L'(0) = 0$ for any variation. We may isolate the variation so that $b^r = 0$ outside of a single interval $[t_i, t_{i+1}]$. It then follows that for any choice of functions $\{ b^r \}$ vanishing at $t_i$ and $t_{i+1}$,
%
\[ \int_{t_i}^{t_{i+1}} b^r A^n_r\; dt = 0. \]
%
This is only possible if $A^n_r = 0$ for all $r$. If a curve is $C^1$ on $[0,r]$ and $A^n_r = 0$ for all $r$, then we will call that curve a \emph{geodesic}. Noting that $b^n = \omega(U) = g(U, T) / F(T)$, it follows that for any variation of a geodesic curve, not necessarily fixed at the endpoints,
%
\[ L'(u) = \sum_i \left. \frac{g(U,T)}{F(T)} \right|_{t_i}^{t_{i+1}}. \]
%
To work out a system of equations that a geodesic must satisfy, for $r \neq n$ we write
%
\[ A^n_r = \tilde{\omega}^n_r( \widehat{T} ) = - \tilde{\omega}_r^n( \widehat{T} ) = - \widehat{\omega}^{n+r} ( \widehat{T} ). \]
%
These quantities vanish if and only if $\widehat{T}$ lies in the span of $\widehat{e}_a$ for $1 \leq a \leq n$, i.e. if $\widehat{T}$ is a horizontal vector with respect to the Ehresman connection. Now we know that
% e_a^i delta_i  = e_a^i partial_i - e_a^i N_i^j partialbar_j
\begin{align*}
    \widehat{T} &= \sum \frac{dc^i}{dt} \partial_i + \frac{d^2 c^i}{dt^2} \overline{\partial}_i.
\end{align*}
%
This vector can only be in the span of $\widehat{e}_a$ if
%
\[ \widehat{T} = \sum \frac{dc^i}{dt} \omega_i^a \widehat{e}_a = F l^i \delta_i. \]
%
Thus $(c,c')$ is an integral curve of the vector field $F l^i \delta_i$. The vector field $F l^i \delta_i$ is $C^\infty$, and \emph{thus it follows that geodesics are $C^\infty$}. Note then that
%
 \[ \frac{d^2 c^k}{dt^2} = - F l^i N_i^k = \frac{dc^i}{dt} N_i^k. \]
%
Recalling the function $G$, which is homogeneous of order two, we have
%
\[ \frac{d^2 c^k}{dt^2} = - \frac{1}{2} \frac{dc^i}{dt} \overline{\partial}_i G^k = - G^k. \]
%
Thus geodesics are given by solutions to the equation $\ddot{c} = -G(c,\dot{c})$.

There is an alternate approach we can take to the problem, involving vector fields and covariant derivatives rather than differential forms. But there is some technicalities. In the usual, Riemannian approach to the problem, for a vector field $W$ along a curve $c: [0,1] \to M$ with velocity vector $T$, we would define the covariant derivative as
%
\[ D_T W = \left( \frac{dW^i}{dt} + W^j T^k \Gamma^i_{jk} \right) \partial_i, \]
%
% Delta W = omega_i^j W^i partial_j
%
The problem on a Finsler manifold is \emph{which} coefficients $\Gamma^i_{jk}$ to use; two natural options are to use $\Gamma^i_{jk}(T)$ \emph{or} $\Gamma^i_{jk}(W)$. We call the first the covariant derivative $D_TW$ with reference vector $T$, and the second the covariant derivative $D_T W$ with reference vector $W$. Now consider a variation of a curve $c$, with associated vector fields $T$ and $U$.

\begin{lemma}
    $D_T U = D_U T$, if $T$ is used as the reference vector in both covariant derivatives.
\end{lemma}
\begin{proof}
    The first vector field is
    %
    \[ \left( \frac{dU^i}{dt} + U^j T^k \Gamma^i_{jk}(T) \right) \partial_i \]
    %
    and the second is
    %
    \[ \left( \frac{dT^i}{du} + T^j U^k \Gamma^i_{jk}(T) \right) \partial_i \]
    %
    By equality of mixed derivatives, $dU^i/dt = dT^i/du$, and
    %
    \[ U^j T^k \Gamma^i_{jk}(T) = T^j U^k \Gamma^i_{jk}(T) \]
    %
    because $\Gamma^i_{jk}(T) = \Gamma^i_{kj}(T)$.
\end{proof}

Differentiating the equation
%
\[ L(u) = \int_I g_T(T,T)^{1/2} \]
%
and using the metric compatibility condition that
%
\[ \partial_u \{ g_T( T,T ) \} = U \{ g_T( T,T ) \} = 2 g( D_U T, T ), \]
%
where $D_U T$ is taken with reference vector $T$, we find that
%
\[ L'(u) = \int_I \frac{g_T(D_U T, T)}{g_T(T,T)^{1/2}} = \int_I g_T( D_T U, l). \]
%
Using Metric compatibility again gives that
%
\[ \partial_t \left\{ g_T(U,l) \right\} = g_T( D_T U, l ) + g_T( U, D_T l ). \]
%
where covariant derivatives are taken with reference vector $T$. Integration by parts thus yields
%
\[ L'(u) = \sum \left. g_T(U,l) \right|_{t_i}^{t_{i+1}} - \int_I g_T(U, D_T l). \]
%
This quantity can only be zero for all variations (and thus all vector fields $U$ fixed at the endpoints) if $D_T l = 0$. Working in a special orthogonal basis, we have $D_T l = \tilde{\omega}_n^a(T) e_a$, and so we recover the condition we obtained by working with differential forms. We now that if the curve has constant speed $F(T) = A$, then $D_T l = A^{-1} D_T T$, and so such a curve is a geodesic if and only if $D_T T = 0$. Conversely, if $D_T T = 0$, then metric compatibility implies that $\partial_t \{ g_T(T,T) \} = 0$, i.e. so that the curve has constant speed, and then $D_T l = 0$ follows immediately.

Now we consider the \emph{second variation formula}. Consider a variation as above, using the same notation. Now pulling back the identity
%
\[ d \omega_j^i - \omega_j^k \wedge \omega_k^i = \frac{1}{2} R^i_{jkl} \widehat{\omega}^k \wedge \widehat{\omega}^l + P^i_{jkl} \widehat{\omega}^k \wedge \widehat{\omega}^{n+l}. \]
%
Pulling back this identity gives that
%
\[ \frac{\partial B^i_j}{\partial t} - \frac{\partial A^i_j}{\partial u} = A_j^k B_k^i - B_j^k A_k^i + R^i_{jnl} a^n b^l + \sum_{l \neq n}  P^i_{jnl} a^n B_n^l - P^i_{jkl} b^k A_n^l. \]
%
If $i = n$ and $j \neq n$, then the Bianchi identities imply that $P^n_{inj} = 0$, and so
%
\[ \frac{\partial A_j^n}{\partial u} = \frac{\partial B^n_j}{\partial t} - A_j^k B_k^n + B_j^k A_k^n + \sum_{l \neq n} - R^n_{jnl} a^n b^l + P^n_{jkl} b^k A_n^l \]
%
Substituting this into the equation for length gives that
%
\[ L''(0) = \left.\left( \frac{\partial b^n}{\partial u} - \sum\nolimits_i b^i B_n^i \right) \right|_0^r + \int_0^r a^n ( \sum\nolimits_i B_n^i B_n^j - b^i R_{ij} b^j ), \]
%
where $R_{ij} = l^j R^i_{jkl} l^L$.

%
% If we vary the angle, but keep length fixed, we must have
% (partial_u b^n - sum_i b^i B_n^i)(L) + int_0^L a^n ( sum B_n^i B_n^j - b^i R_{ij} b^j )
