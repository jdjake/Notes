...
Verifying two topological spaces are homeomorphic is often a fairly easy ordeal. One needs only find a single homeomorphism between them. The converse, verifying two topological spaces are not homeomorphic, is much more tricky. One trick is to find fundamental topological properties of spaces which are able to distinguish between two separate spaces. For instance, connectedness, compactness, and being Hausdorff is preserved under homeomorphism, so two spaces for which these properties differ cannot be homeomorphic. But these invariants are not sufficient to distinguish between a sphere and a torus. Algebraic topology consists of obtaining algebraic topological invariants which enable us to distinguish between different topological spaces.

Let us consider things more concretely using category theory, which is a powerful tool invented by Samuel Eilenberg and Saunders Maclane precisely to unify the foundations of algebraic topology. Let $\textsf{C}$ be a category, and consider a functor $F$ from the category $\textsf{Top}$ of topological spaces to $\textsf{C}$. Algebraic topology attempts to study those functors $F$ from $\textsf{Top}$ to algebraic categories $\textsf{C}$, like the category of groups, rings, or modules, since many topological problems can be reduced to algebraic problems by means of functors.

\begin{example}
    Let $F: \textsf{Top} \to \textsf{C}$ be a functor. If $X$ and $Y$ are homeomorphic topological spaces, then $F(X)$ must be isomorphic to $F(Y)$ in $\textsf{C}$. In particular, if we can show $F(X)$ is \emph{not} isomorphic to $F(Y)$, then it is impossible for $X$ and $Y$ to be homeomorphic.
\end{example}

\begin{example}
    Consider three topological spaces $X$, $Y$, and $Z$, and suppose we fix two continuous maps $f: X \to Z$ and $g: X \to Y$. The \emph{extension problem} asks whether there exists a continuous map $h: Y \to Z$ such that $h \circ g = f$. If $F: \textsf{Top} \to \textsf{C}$ is a functor, and such a map $h$ exists, then we find that $F(h) \circ F(g) = F(f)$. In particular, if there does not exist any morphism $\varphi: F(Y) \to F(Z)$ such that $\varphi \circ F(g) = F(f)$, then no such extension $h$ can exist.
\end{example}

\begin{example}
    A simple, but sometimes effective functor to use assigns with each topological space $X$ the set $\pi_0(X)$ of path connected components of $X$. If $f: X \to Y$ is a continuous map, then it induces a natural map $\pi_0(f): \pi_0(X) \to \pi_0(Y)$, since if $x_0, x_1 \in X$ are path connected, then so too are $f(x_0)$ and $f(x_1)$. It is clear that this gives a functor from the category $\textsf{Top}$ of topological spaces to the category $\textsf{Set}$ of sets. In particular, we conclude that if $X$ and $Y$ are homeomorphic, then $\pi_0(X)$ should have the same cardinality as $\pi_0(Y)$. Thus this functor really only distinguishes between homotopy equivalences. This is a fairly trivial invariant, but its idea can be extended to a multitude of powerful algebraic invariants, the simplest of which is known as the \emph{fundamental group} of a topological space.
\end{example}

\section{Homotopy}

It will not only be important that the algebraic invariant we study give powerful invariants, but also that they are easily computable. The cost of this is that most of the functors we study will be invariant under a kind of continuous deformation. Fix two topological spaces $X$ and $Y$, and consider two functions $f_0$ and $f_1$ from $X$ to $Y$. Though $f_0$ might not be equal to $f_1$, it may `have the same shape' in the sense that we may continuously deform the function $f_0$ into the function $f_1$. Thus we say $f_0$ and $f_1$ are \emph{homotopic}, written $f \simeq g$, if there exists a continuous map $H: X \times I \to Y$, such that for each $x \in X$, $H(x,0) = f(x)$ and $H(x,1) = g(x)$. If we write, for $t \in I$, $f_t(x) = H(x,t)$, then we can think of the family $\{ f_t \}$ as a family of continuously varying functions from $f_0$ to $f_1$. Homotopy induces an equivalence relation on the family of continuous maps between topological spaces $X$ and $Y$; we let $[X,Y]_H$ be the set of equivalence classes of maps in $C(X,Y)$ under homotopy, and for a function $f: X \to Y$, we let $[f]_H \in [X,Y]_H$ denote it's \emph{homotopy class}.

It is easy to prove that if $X$, $Y$, and $Z$ are topological spaces, and we are given $f_0, f_1 \in C(X,Y)$ and $g_0, g_1 \in C(Y,Z)$ such that $f_0 \simeq f_1$ and $g_0 \simeq g_1$, then $g_0 \circ f_0 \simeq g_1 \circ f_1$. Thus the operator $[g]_H \circ [f]_H = [g \circ f]_H$ is well defined, and so we can define a \emph{quotient category}, known as the homotopy category and denoted $\mathsf{Toph}$, whose objects consist of topological spaces and whose morphisms consist of homotopy classes of maps, from the category $\mathsf{Top}$. Most of the functors $F: \mathsf{Top} \to \mathsf{C}$ we consider will descend to functors from $\mathsf{Toph}$ to $\mathsf{C}$, in the sense that $F(f_0) = F(f_1)$ for $f_0, f_1 \in C(X,Y)$ with $f_0 \simeq f_1$. Thus most of the functors we deal with can only hope to classify spaces up to homotopy equivalence.

\begin{example}
    The space $S^{n-1}$ is homotopy equivalent to $\RR^n - \{ 0 \}$. Indeed, a homotopy equivalence is given by the inclusion map $i: S^{n-1} \to \RR^n - \{ 0 \}$. A homotopy inverse to $i$ is given by the map $j: \RR^n - \{ 0 \} \to S^{n-1}$ given by setting $j(x) = x / |x|$. It is clear that $j \circ i$ is the identity map on $S^{n-1}$, and the homotopy $H: \RR^n - \{ 0 \} \times I \to \RR^n - \{ 0 \}$ given by setting
    %
    \[ H(x,t) = (1 - t) x + t \cdot (i \circ j)(x) = (1 - t) x + t x / |x|, \]
    %
    shows that $i \circ j$ is homotopic to the identity map on $\RR^n - \{ 0 \}$.
\end{example}

It is sometimes important to consider a restricted family of homotopies from the general family of homotopies. Given two topological spaces $X$ and $Y$, and a subspace $A \subset X$, we say two continuous maps $f_0,f_1: X \to Y$ are homotopic \emph{relative to $A$}, denoted $f_0 \simeq_A f_1$, if $f_0(x) = f_1(x)$ for all $x \in A$, and there exists a map $H: X \times I \to Y$ such that for each $x \in A$ and $t \in I$, $H(x,t) = f_0(x) = f_1(x)$. Again, this gives an equivalence relation, and we let $[f_0]_{H(A)} \in [X,Y]_{H(A)}$ denote the homotopy equivalence class of $f_0$ relative to $A$. A \emph{topological pair} $(X,A)$ is a tuple of topological spaces with $A \subset X$. We let $\textsf{pTop}$ denote the category whose objects consist of topological pairs, and whose maps between two pairs $(X,A)$ and $(Y,B)$ consist of continuous maps $f: X \to Y$ with $f(A) \subset f(B)$. We let $\textsf{RelToph}$ denote the quotient category of $\textsf{pTop}$ such that we identify morphisms $f: (X,A) \to (Y,B)$ that are homotopic relative to $A$.

A space $X$ is \emph{contractible} if it is homotopy equivalent to a point. In the sense of homotopy, the most trivial maps are those which are \emph{null homotopic}, i.e. homotopic to a constant map. A space is contractible if and only if the identity map is null homotopic. Moreover, if $Y$ is contractible, then all maps in $C(X,Y)$ are null homotopic, and thus essentially trivial. It is not necessarily true, however, that if $Y$ is contractible, and $f_0,f_1 \in C(X,Y)$ agree on some set $A \subset X$, then $f_0 \simeq_A f_1$, though this is true if $Y$ is a convex subset of $\RR^n$ for some $n$, since we can then define $H(t,x) = t f_1(x) + (1 - t)f(x)$.

\begin{example}
    Let $X$ be the \emph{comb space}
    %
    \[ X = \{ (x,y) \in [0,1]^2 : y = 0\ \text{or}\ x \in \{ 1,1/2,1/3,\dots, 0 \} \}. \]
    %
    Define three maps $\text{id}_X, \pi, 0 \in C(X,X)$ by setting $\text{id}_X(x,y) = 0$, $\pi(x,y) = (x,0)$, and $0(x,y) = (0,0)$. The homotopy $H: X \times I \to X$ given by setting
    %
    \[ H(x,y,t) = (x,(1-t)y) \]
    %
    shows that $\text{id}_X \simeq \pi$. The homotopy
    %
    \[ H(x,y,t) = ((1-t)x,0) \]
    %
    shows that $\pi \simeq 0$. Thus $\text{id}_X \simeq 0$, showing that $X$ is contractible. If $1 \in C(X,X)$ is given by setting $1(x,y) = (0,1)$, then it is \emph{not} true that $\text{id}_X \simeq 1$ relative to $(0,1)$. In particular, suppose $H: X \times I \to X$ is a continuous map such that for each $(x,y) \in X$ and $t \in I$,
    %
    \[ H(x,y,0) = (x,y), \quad H(x,y,1) = (0,1), \quad\text{and}\quad H(0,1,t) = (0,1). \]
    %
    By virtue of the connectedness of the space, for each $n$, there must exist $t_n \in I$ such that $H(1/n,1,t_n) = (1/n,0)$. Since $X \times I$ is compact, there must exist a subsequence of the sequence $\{ (1/n,1,t_n) \}$ which converges. If $\{ n_k \}$ denotes the indices of this sequence, then
    %
    \[ \lim_{k \to \infty} (1/n_k,1,t_k) = (0,1,*). \]
    %
    By continuity of $H$, we find $H(0,1,*) = (0,0)$, which is impossible given the assumptions.
\end{example}

\section{Deformations}

A \emph{retraction} is a continuous map $r:X \to X$ for which $r^2 = r$. If $Y = \text{im}(r)$, and $r$ is a homotopy equivalence, then we say $X$ {\bf deformation retracts} to $Y$. The deformation retraction is {\bf strong} if $i \circ r$ is homotopic to the identity relative to $Y$. We showed that $\mathbf{R}^{n+1} - \{ 0 \}$ has a strong deformation retract onto $S^n$. A space is {\bf contractible} or {\bf null-homotopic} if it is deformation retracts to a point.
...
\begin{proof}
    ...
    %
    \[ H_*(M_\infty) = \text{colim} ( H_*(M) \to H_*(M) \to \dots ) = H_*(M)[(H_* \widehat{m})^{-1}] \] 
    %
