...
%
With some analysis, it is easy to calculate $\mu^*$ for some basic sets.

\begin{example}
  Consider the set $E = \mathbf{Q} \cap [0,1]$, which elementary sets could not `penetrate' deep enough to upper bound the area of. But now, consider an enumeration $E = \{ x_1, x_2, \dots \}$, and for each $n$, consider a box $R_n$ containing $x_n$ which volume $\varepsilon/2^n$. Then
  %
  \[ \mu^*(E) \leq \sum_{k = 1}^\infty |R_n| = \sum_{k = 1}^\infty \frac{\varepsilon}{2^n} = \varepsilon \]
  %
  Letting $\varepsilon \to 0$, we conclude $\mu^*(E) = 0$. More generally, we find that {\it any} countable set has exterior Lebesgue measure zero, and this holds even if the set is unbounded.
\end{example}

The next computation was first introduced by Borel, who invented the theorem we now call the Heine-Borel theorem, and the modern definition of compactness, to perform the computation.

\begin{example}
  Let $R$ be a rectangle in $\mathbf{R}^d$. Then $J^*(R) = |R|$. Because $R$ covers itself, we know $J^*(R) \leq |R|$. But consider a countable covering of $R$ by rectangles $S_1, S_2, \dots$. For each $S_n$, consider a slightly larger {\it open} rectangle $S_n'$ with $|S_n'| = |S_n| + \varepsilon/2^n$. Then the $S_n'$ are an open cover of $R$, and we therefore by compactness, we have a finite subcover $S_1', \dots, S_N'$. But by the theory of elementary sets, we know
  %
  \[ |R| \leq |S_1'| + \dots + |S_N'| \leq \varepsilon + \sum |S_n| \]
  %
  Since the $S_n$ are arbitrary, we conclude that $J^*(R) \geq |R| - \varepsilon$, and taking $\varepsilon \to 0$, we conclude that $J^*(R) \geq |R|$, hence we have equality.
\end{example}

\begin{remark}
  Note that as an easy consequence of this theorem, we conclude that $\mathbf{R}^d$ is uncountable for each $d$, because, if $\mathbf{R}^d$ was uncountable, we would conclude that $\mu^*(\mathbf{R}^d) = 0$, hence every set has $\mu^*(E) = 0$. Thus the last example must use a fact about $\mathbf{R}^d$ which distinguishes it from other countable sets, such as $\mathbf{Q}$. This was provided by the Heine-Borel theorem, which exploited the completeness of $\mathbf{R}^d$ as a metric space.
\end{remark}
...
\[ |U| = \sum |I_i| \]
%
The definition of measure we will eventually give will give us this theorem as an identity, but it is not clear how we can generalize this to non open sets, and the theorem in $\mathbf{R}^d$ is not nearly as pleasant, because uniqueness does not hold here. Here is the analogous theorem. We say two boxes $R$ and $S$ are `almost` disjoint if they only intersect at their boundary. The technique of this proof uses an important fact about the {\it dyadic intervals} in $\mathbf{R}^d$, which are those with rational endpoints of the form $n/2^m$ -- these intervals are either nested, or almost disjoint.
...
\begin{proof}
  ...
  %
  \[ \inf \{ \mu^*(U): U\ \text{open} \} \leq  \mu^*(\bigcup S_n') \leq \sum \mu^*(S_n') = \sum |S_n| + \varepsilon \]
  %
...
Though the exterior measure is not necessarily additive, it is additive if sets are not entangled from one another. If $E$ and $F$ are subsets of a metric space, then
%
\[ d(E,F) = \inf \{ d(x,y) : x \in E, y \in F \} \]
%
which is the {\bf Hausdorff distance} between the two sets, and measures the degree to which they are separated.
...
As with the theory of Jordan measure, it is not in general true that for any two sets $E$ and $F$, $\mu^*(E \cup F) = \mu^*(E) + \mu^*(F)$, as Banach and Tarski warned us. For the class of Lebesgue measurable sets, we will show that we can obtain additivity, and even more generally, countable additivity -- if $E_1, E_2, \dots$ are disjoint {\it Lebesgue measurable} sets, then
%
\[ \mu^* \left( \bigcup E_i \right) = \sum \mu^*(E_i) \]
%
Unfortunately, their isn't a great definition of the Lebesgue inner measure for Lebesgue measurable sets, so we cannot define the Lebesgue measurable sets to be those which can be approximated well from within and without. We instead use the criterion that a set $E$ is Jordan measurable if there is an elementary set $F$ such that $J^*(E \bigtriangleup F)$ is small, so $E$ is {\it well approximated} by elementary sets. We say a set $E$ is {\bf Lebesgue measurable} in the same way, replacing $F$ by a countable union of rectangles, or, because of a neglible change of measure, by open sets. Precisely, we say $E$ is Lebesgue measurable if, for any $\varepsilon > 0$, there is an open set $F$ with $\mu^*(E \bigtriangleup F) \leq \varepsilon$. The utility of the Lebesgue measure is that essentially every subset of $\mathbf{R}^d$ is Lebesgue measurable.

\begin{theorem}
  These \mu^*(\bigcup R_i \bigtriangleup E)properties are used to show Lebesgue measurability:
  \begin{enumerate}
    \item[(a)] Every open set is measurable.

    \item[(b)] If $\mu^*(E) = 0$, then $E$ is Lebesgue measurable. If $F$ is a set, $E$ is Lebesgue measurable, and $\mu^*(E \bigtriangleup F) = 0$, then $F$ is Lebesgue measurable, a result known as the {\it completeness property} of the Lebesgue measurable sets.

    \item[(c)] Countable unions of Lebesgue measurable sets are Lebesgue measurable.

    \item[(d)] Closed sets are measurable.

    \item[(e)] The complement of a measurable set is measurable.

    \item[(f)] Countable intersections of Lebesgue measurable sets are Lebesgue measurable.
  \end{enumerate}
\end{theorem}
\begin{proof}
  ...
  %
  and letting $\varepsilon \to 0$ gives that $E$ is measurable. If $E$ is Lebesgue measurable, and $\mu^*(E \bigtriangleup F) = 0$, then choose some open set $U$ with $\mu^*(E \bigtriangleup U) \leq \varepsilon$, we conclude that, since $\bigtriangleup$ is commutative, and $E \bigtriangleup E = \emptyset$,
  %
  \[ \mu^*(F \bigtriangleup U) = \mu^*((F \bigtriangleup E) \bigtriangleup (E \bigtriangleup U)) \leq \mu^*(F \bigtriangleup E) + \mu^*(E \bigtriangleup U) \leq \varepsilon \]
  %
  and we can then take $\varepsilon \to 0$ to obtain that $F$ is measurable. This proves (b). Now if $E_1, E_2, \dots$ are measurable, then we can find $U_1, U_2 ,\dots$ with $\mu^*(E_n \bigtriangleup U_n) \leq \varepsilon/2^n$, then since
  %
  \[ \bigcup E_n \bigtriangleup \bigcup U_n \subset \bigcup E_n \bigtriangleup U_n \]
  %
  \[ \mu^* \left( \bigcup E_n \bigtriangleup \bigcup U_n \right) \leq \sum \mu^* \left( U_n \bigtriangleup E_n \right) \leq \varepsilon \]
  %
  which proves (c). To prove (d), it suffices to show that a compact set $K$ is measurable, because every closed set is the countable union of compact sets. We know $\mu^*(K) < \infty$, because $K$ is contained in some open box $R$, and monotonicity implies $\mu^*(K) \leq |R| < \infty$.
\end{proof}
