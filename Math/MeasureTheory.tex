...
\begin{example}
  ...
  %
  Therefore $f$ is in $L^p(\mathbf{R}^d - B_\varepsilon(0))$ only if $p \alpha > d$, and $f$ is in $L^p(B_1(0))$ if and only if $p \alpha < d$. Notice that for large $\alpha$ $f$ is peaked at the origin, but decays rapidly away from the origin, hence the inclusions above.
\end{example}
