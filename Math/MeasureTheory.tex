...
If a function $f$ is Lebesgue measurable, then $f^{-1}((a,b])$ is measurable for each $a < b$, since
%
\[ f^{-1}((a,b]) = \{ x: f(x) \leq b \} \cap \{ x : f(x) \leq a \}^c \]
%
Since $f^{-1}((a,b))$ is the countable union of $f^{-1}((a,b_i])$, as $b_i \uparrow b$, and $f^{-1}([a,b])$ is the countable intersection of $f^{-1}((a_i,b])$, we conclude the inverse image of every interval is measurable. By taking unions of open intervals, we obtain that the inverse image of every open set is measurable, and by taking complements, that the inverse image of every closed set is as well. and then $f^{-1}(X)$ is measurable for every set $X$ which is open and closed.

\begin{lemma}
  If $f: \mathbf{R}^n \to \mathbf{C}$ is continuous, and $g_1, \dots, g_n$ are measurable, then $f(g_1, \dots, g_n)$ is measurable.
\end{lemma}
\begin{proof}
...
\begin{example}
  ...
  %
  which implies $\| f \|_p \to |A_m|$ as $p \to \infty$. On the other hand, we have $\lim_{p \to 0} \| f \|^p_p \to \sum |E_n|$, so the width of the function becomes more and more emphasized.
\end{example}
...
The spaces $L^p(X)$ are not necessarily included in one another, except in two circumstances, which occur often. If $X$ is a finite measure space, then for $0 < p \leq q < 1$, H\"{o}lder's inequality implies
%
