...
\begin{theorem}
  ...
  \begin{enumerate}
      ...
      \item[(e)] The symmetric difference $E \Delta F = (E - F) \cup (F - E)$ is an elementary set.
...
\[ |F| = |E| + |F - E| \geq |E| \]
%
so the measure is {\it monotone}. These properties uniquely define the measure of a set up to a scalar factor. Since all the properties are intuitive to us, this tells us that our construction is going in the right direction!

\section{Jordan Measurable Sets}

In the last section, we constructed a consistant measure on the family of elementary sets. However, this family is certainly limited. We cannot even use this quantity to measure the area of a circle, or the volume of a sphere. However, we have really only applied the first ancient technique of measuring area, forming disjoint unions of simple shapes. We haven't used the method of approximating shapes by simple sets from above and below. The next two theorems say that the family of elementary sets is large in the family of open sets, which are the next family we set are eyes on measuring the size of.
...
In calculus, the Riemann integral was used to measure the area under certain curves in space. Recall that if $f$ is a bounded function on a box $R$, then we lower and upper bound the integral by partitions $P$ of the box into subboxes, setting
%
...
%
and a bounded set $E$ is Jordan measurable precisely when $J_*(E) = J^*(E)$, and this common value is then the measure of the set. Note that it is easy to see that $J_*$ and $J^*$ are both monotone, and therefore the Jordan measure is monotone when it exists. Furthermore, $J^*$ is subadditive -- for any two sets $E$ and $F$, regardless of whether they are Jordan measurable or not, we conclude $J^*(E \cup F) \leq J^*(E) + J^*(F)$. The next theorem also offers another important criterion for Jordan measurability.

\begin{theorem}
    A bounded set $E$ is Jordan measurable if and only if for every $\varepsilon > 0$, there is an elementary set $F$ with $J^*(E \Delta F) \leq \varepsilon$.
\end{theorem}
\begin{proof}
  ...
  %
  \[ J^*(E \Delta E_2) = J^*(E_2 - E) \leq |E_2 - E_1| \leq \varepsilon \]
  %
  Conversely, suppose $E$ is a bounded set, and for any $\varepsilon$, we can find an elementary set $F$ with $J^*(E \Delta F) \leq \varepsilon$. This means that we can find an elementary set $L$ covering $E \Delta F$ with $|L| \leq 2\varepsilon$, and then $F - L \subset E \subset F \cup L$, and
  %
...
This theorem can be used to show easily that $E$ and $F$ are Jordan measurable sets, then $E \cup F$ and $E \cap F$ are Jordan measurable, since if there are elementary sets $E_1$ and $F_1$ with $J^*(E_1 \Delta E), J^*(F_1 \Delta F) \leq \varepsilon$, then
%
\[ (E \cup F) \Delta (E_1 \cup F_1) \subset (E \Delta E_1) \cup (F \Delta F_1) \]
%
and $J^*((E \Delta E_1) \cup (F \Delta F_1)) \leq J^*(E \Delta E_1) + J^*(F \Delta F_1) \leq 2\varepsilon$. If $E_1 \subset E \subset E_2$ and $F_1 \subset F \subset F_2$ are given with $|E_2 - E_1|, |F_2 - F_1| \leq \varepsilon$, then
%
...
%
so $|E_2 \cap F_2 - E_1 \cap F_1| \leq 2\varepsilon$. Elementary properties of the Riemann integral guarantee that if $E$ and $F$ are two disjoint Jordan measurable sets, then $E \cup F$ is Jordan measurable, and $|E \cup F| = |E| + |F|$. Similarily, if $E \subset F$ are both Jordan measurable, then $F - E$ is Jordan measurable, and $|F - E| = |F| - |E|$. Thus we have {\it finite additivity}. If $E \subset F$, then $|E| \leq |F|$, because $\chi_E \leq \chi_F$. We also fairly easily see we have translation invariance. Thus Jordan measurability has all the properties we liked about the measure of elementary sets, and the family of Jordan measurable sets form an algebra over every {\it bounded} subset of space.
