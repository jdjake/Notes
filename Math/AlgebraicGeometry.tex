...
\begin{example}
    The set of points $(x,y)$ in the real affine plane satisfying $y = \sin(x)$ cannot form a planar curve, because the curve intersects the $X$ axis infinitely often, yet the set of points does not contain the $X$ axis.
\end{example}
...
\begin{corollary}
    Every variety is expressible as the common zeroes of a finite set of polynomials.
\end{corollary}

The ideal of leading coefficients of an ideal $\mathfrak{a}$ in $R[X]$ is extremely useful to understanding $\mathfrak{a}$. We can generalize this process to studying the rings $R[X_1, \dots, X_n]$, provided that we have a way of selecting, from a polynomial $f(X_1, \dots, X_n)$, the coefficient in the expansion related to the `highest monomial'. Thus we assume that we have specified a linear ordering on the set of monomials, such that if $X^\alpha \geq X^\beta$, then $X^{\alpha + \lambda} \geq X^{\beta + \lambda}$, for any $\alpha, \beta, \lambda \in \mathbf{Z}^n$. Once this is fixed, the {\it leading term} of a polynomial is well understood. Given any ideal $\mathfrak{a}$ in $R[X_1, \dots, X_n]$, the family of leading terms of elements of $\mathfrak{a}$ also forms an ideal of $R$.

we have a {\it well ordering} on the set of monomials, such that if $X^\alpha \geq X^\beta$, then $X^{\alpha + \lambda} \geq X^{\beta + \lambda}$. This allows us, given an arbitrary polynomial $f$, to extract the coefficient related to the largest monomial in the expansion of $f$. Hilbert's basis theorem enables us to conclude that any total ordering on $R[X_1, \dots, X_n]$ which satisfies $X^{\alpha + \lambda} \geq X^{\beta + \lambda}$ if $X^\alpha \geq X^\beta$ is a well

In particular, if we are studying the ring $K[X_1, \dots, X_n]$, then $R = K[X_1, \dots, X_{n-1}]$, and the `leading coefficient' of an element

The set $f_1, \dots, f_M$ is known as a {\bf Gr\"{o}bner basis}
...
\begin{example}
    If $K$ is finite, then all subsets of $\mathbf{A}^n$ are varieties, because all subsets of $\mathbf{A}^n$ are finite subsets.
\end{example}
...
A {\bf subsheaf} of a sheaf $\mathcal{F}$ is a choice of subgroups $\mathcal{G}(U)$ of $\mathcal{F}(U)$ for each open set $U$, which is closed under restriction and satisfies the sheaf axiom. The kernel of a morphism of sheafs is a subsheaf, whereas the image and cokernel of the morphism may not be.
