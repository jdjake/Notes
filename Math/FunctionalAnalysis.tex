...
Functional analysis is the interlace of algebra and analysis, in which algebraic structures are endowed with topological structure. The approach's utility counts for the rapid growth of applications over the past century, be it in quantum mechanics, statistics, or computing science. Rarely are we concerned with a single object, like a function, a random variable, or a measure, but instead consider large classes of such objects. One technique for handling these classes is to add algebraic structure which elaborates on the natural relation between these objects. Functional analysis adds in an analytical structure to the class as well, so we may combine the algebraic and analytical structure to obtain powerful results about the objects involved.
...
Most of the important spaces which occur in functional analysis can be formulated as vector spaces over a field of characteristic zero. For analytic niceties, we will often require our vector spaces to be `topologicall complete'. We can therefore begin with real vector spaces, for a complete, rational vector space can always be reformulated as a real vector space. From the real numbers, we can only really extend our field to the complex numbers. Real vector spaces are spaces which can be naturally scaled in some manner, whereas complex vector spaces can also be `twisted'. As an example, we may scale continuous functions from the unit interval $[0,1]$ into the real numbers pointwise, so that the space of continuous, real valued functions on is a real vector space. On the other hand, continuous, complex valued functions on the unit interval can be twisted; each function in the space can be visualized as a twisted corkscrew corkscrew -- the domain is the length, the range is the extremities of the screw. Twisting a corkscrew is natural, so we have a complex vector space. To be more specific, if we have a real vector space $V$ with a specified `twisting' linear transformation $J: V \to V$ with $J^2 = -1$, then there is a natural way to make $V$ into a complex vector space, by defining $(a + ib)v = av + b(Jv)$. The algebra of the complex numbers tells us that if we are able to twist by a right angle, we can twist by all other angles.

\chapter{Hilbert Spaces}

The nicest vector space is $\mathbf{R}^n$. It is here that we may measure angles and distance, which give rise to the topological properties of a space. Recall that the inner product on $\mathbf{R}^n$ is naturally related to the measure of angles between vectors. Without an inner product, we can only talk about linear dependance, which is a binary relation, hardly suited to analysis. With an inner product, we obtain an abstract definition of angle, which is a continuous degree of similarity between two vectors, and this makes studying independence in inner product spaces particularly interesting.

Recall that a bilinear form on a $K$ vector space $V$ is a map $f: V \times V \to K$ which is linear in each variable. A bilinear form over the real numbers is an {\bf inner product} if $f$ is symmetric, and $f(x,x) > 0$ for all $x \neq 0$, and an {\bf inner product space} is a real vector space equipped with a fixed bilinear form. In $\mathbf{R}^n$, the euclidean inner product gives us a way to measure the cosine of angles, so we can think of an arbitrary inner product as giving a way to measure angles in a real vector space. The standard notation for a general inner product is $\langle \cdot, \cdot \rangle$, as it is used for the Euclidean inner product on $\mathbf{R}^n$.

We would also like to consider `complex inner product spaces', but we have a problem. If $\langle \cdot, \cdot \rangle$ is bilinear, and $\langle v, v \rangle > 0$ for all $v \neq 0$, then $0 \leq \langle i v, i v \rangle = - \langle v, v \rangle \leq 0$, so $\langle v, v \rangle = 0$ for all $v$, so the only `complex inner product space' is $V = 0$ The problem results because the definition of an inner product over the real numbers restricts our attention to considering real values of the inner product of $v$ with itself, yet we can vary $v$ by a complex coefficient. To determine how a `complex inner product' results, let us return to the fundamentals of forming a complex vector space. A complex vector space $V$ is just a real vector space with a `twisting map' $J: V \to V$. We would expect $J$ to twist vectors in $V$ by a right angle, so that if $\langle \cdot, \cdot \rangle$ is any {\it real} inner product, then we might add the extra condition that $\langle v, Jv \rangle = 0$ for all $v \in V$. We would also like the rotation operator $J$ to be an isometry, so that $\langle Jv, Jw \rangle = \langle v, w \rangle$. Since $J^2 = -1$, we find $\langle Jv, w \rangle = \langle Jv, J^2(-w) \rangle = \langle v, -Jw \rangle$. Given any real inner products with these conditions, we wish to extend $\langle \cdot, \cdot \rangle$ to a complex `form' $(\cdot, \cdot)$, in the sense that $\Re (\cdot, \cdot) = \langle \cdot, \cdot \rangle$. The new form $(\cdot, \cdot)$ cannot be linear in both factors, since $\Re (v, iw) = \langle v, iw \rangle = \langle -iv, w \rangle$. The closest thing we could have is that $(\cdot, \cdot)$ is {\bf sequilinear} (sequi means `one and a half' in greek), linear in the first factor, and `anti-linear' in the second factor, in the sense that
%
\[ (v, \lambda w + \gamma u) = \overline{\lambda} (v,w) + \overline{\gamma} (v,u) \]
%
And almost symmetric, in the sense that $(v,w) = \overline{(w,v)}$. If $(\cdot, \cdot)$ is a sesquilinear form extending $\langle \cdot, \cdot \rangle$, then $\Im (v,w) = - \Re [i (v, w)] = - \langle iv, w \rangle = \langle v, i w \rangle$, so if we have any hope of extending $\langle \cdot, \cdot \rangle$, we must define $(v,w) = \langle v, w \rangle + i \langle v, iw \rangle$. With the properties we have displayed, this map is sesquilinear, and $(v,v) > 0$ for $v \neq 0$. We call a positive-definite antisymmetric sesquilinear form $(\cdot, \cdot)$ a {\bf Hermitian inner product}, and a space with a fixed Hermitian product is a {\bf Hermitian inner product space}. We will refer to both Hermitian products and standard inner products as just `inner products', so that an {\bf inner product space} is either a real vector space with a real inner product, of a complex vector space with a Hermitian inner product. We will use the notation $(v, w)$ for the inner product of two vectors $v$ and $w$, regardless of whether the inner product is real-valued or complex-valued. A Hilbert space is a `complete' inner product space, which we will define in due time.

Looking back to the inspiration of the inner product enlightens us on the new definition. The inner product on a real vector space measures the ratio of projection from one vector to another. When we encounter complex spaces, this ratio of projection becomes complex, so a complex inner-product must be complex valued. The complex plane spanned by a vector $w$ is the real span of the vectors $w$ and $iw$. If we project onto this plane, it is natural for this projection to be the complex combination of the real projection onto $w$ and the real projection onto $iw$, and this is exactly the definition we obtained above. The real part of such a product measures the `real' angle between $v$ and $w$, the imaginary part measures a `rotational angle'.

\begin{example}
    Given any measure space $X$, let $L^2 X$ denote the space of complex-valued square-integrable measurable functions on $X$ (where functions are identified if they agree up to a subset of measure zero), where a function is square integrable if
    %
    \[ \int | f |^2 < \infty \]
    %
    The space of real-valued functions on $L^2 X$ forms a subspace of the complex-valued functions. There is no clear distinction between the notation for the space of real and complex valued square integrable functions, but normally the context tells you which space is under consideration (and almost everything true for the space of real-valued measurable functions is true for the space of complex-valued functions as well, so we might as well work in the space of complex valued functions). $L^2 X$ also has the structure of an inner product space, if we define
    %
    \[ (f, g) = \int f \cdot \overline{g} \]
    %
    which is well defined, because by applying the H\"{o}lder inequality, we conclude that
    %
    \[ \int |fg| \leq \sqrt{\int |f|^2} \sqrt{\int |g|^2} < \infty \]
    %
    This is an inner product, because $\int |f|^2 = 0$ if and only if $f = 0$ almost everywhere. It requires a little bit of work to show that $L^2 X$ is actually a Hilbert space, and is one of the prime examples of the Hilbert space.
\end{example}

\begin{example}
    The easiest Hibert space to analyze is the space $l_2$ of `square-summable' sequences, those real/complex valued sequences $x_1,x_2, \dots$ such that $\sum |x_i|^2 < \infty$. More generally, for a set $X$, the space of all `square-summable' functions $f$ on $X$, such that $\sum_{x \in X} |f(x)|^2 < \infty$ forms a Hilbert space. These are just $L_2 \mathbf{N}$ and $L_2 X$ where $X$ is a measure space under the counting measure. We shall find these are essentially the only examples of a Hilbert space we need analyze. If $F: X \to \mathbf{R}$ is any positive function on $X$, then the space of $f: X \to \mathbf{R}$ for which $\sum F(x) |f(x)|^2 < \infty$ forms a Hilbert space which might not be equivalent to the standard $l_2$ space. As examples, we can take the set of sequences $\{ x_n \}$ such that
    %
    \[ \sum \frac{|x_n|^2}{n} < \infty\ \ \ \ \sum \frac{|x_n|^2}{n^2} < \infty\ \ \ \ \sum n^2 |x_n|^2 < \infty \]
    %
    and these are all naturally Hilbert spaces.
\end{example}

\begin{example}
    If $D$ is an open subset of the complex plane, the space $L_2^a D$ of analytic real/complex valued square-integrable functions (with respect to the Lebesgue measure on $\mathbf{C}$) is a closed (topologically) subspace of $L_2D$, and is therefore a Hilbert space. If $H$ is any Hilbert space, the subspaces of $H$ which form Hilbert spaces are exactly the closed subspaces.
\end{example}

Because we can measure angles in a Hilbert space, we can also measure right angles. Two vectors $v$ and $w$ are {\bf orthogonal}, denoted $v \perp w$, if $v \cdot w = 0$. This is a symmetric relation, since if $(v,w) = 0$,
%
\[ (w,v) = \overline{(v,w)} = \overline{0} = 0 \]
%
The theory of right triangles essentially manifests in the theory of equations involving two to three vectors.

\begin{lemma}
    If $v$ is orthogonal to $w$, then $\| v + w \|^2 = \| v \|^2 + \| w \|^2$.
\end{lemma}
\begin{proof}
    We just calculate
    %
    \[ \| v + w \|^2 = (v + w, v + w) = \| v \|^2 + \| w \|^2 + (v,w) + (w,v) = \| v \|^2 + \| w \|^2 \]
    %
    so `Pythagoras theorem' holds in Hilbert spaces.
\end{proof}

Equations involving finitely many vectors in an infinite dimensional vector space seem intuitive, but remember that these finitely many vectors span a finite dimensional subspace of the vector space, so at least in the elementary theory of infinite dimensional vector spaces, the theorems essentially reduce to basic finite algebra.

\begin{theorem}[Cauchy-Schwarz inequality]
    For any two vectors $x,y$, in an inner product space,
    %
    \[ |\langle x, y \rangle| \leq \| x \| \| y \| \]
\end{theorem}
\begin{proof}
    If we define the projection of $y$ onto $x$ as the operator
    %
    \[ P_x(y) = \frac{\langle y, x \rangle}{\ \|x\|^2} x \]
    %
    Theb $\| P_x(y) \| = |\langle y, x \rangle| |x|^{-1}$, and the Cauchy-Schwarz inequality says exactly that $\| P_x(y) \| \leq \| y \|$, which in two dimensions makes sense intuitively, since a projection of a vector should be shorter than the original vector. We should also expect to project onto a right angle in the space, so if we define $z = y - P_x(y)$, we find
    %
    \[ \langle z, P_x(y) \rangle = \frac{\langle x, y \rangle \langle y, x \rangle}{\|x\|^2} - \frac{|\langle x, y \rangle|^2}{\|x\|^2} = 0 \]
    %
    so $z \perp P_x(y)$, and hence
    %
    \[ \| y \|^2 = \| z + P_x(y) \|^2 = \| z \|^2 + \| P_x(y) \|^2 \geq \| P_x(y) \|^2 \]
    %
    and this inequality is an equality exactly when $z = 0$, in which case $P_x(y) = y$, and so $y$ is a scalar multiple of $x$.
\end{proof}

\begin{theorem}
    If $\langle \cdot, \cdot \rangle$ is an inner product, and we define a function $\| \cdot \|$ by the equation $\| v \|^2 = \langle v, v \rangle$, then the triangle inequality holds, in the sense that $\| v + w \| \leq \| v \| + \| w \|$, and $\| \lambda v \| = |\lambda| \| v \|$.
\end{theorem}
\begin{proof}
    Applying the Cauchy-Schwarz inequality, we conclude that
    %
    \begin{align*}
        \| v + w \|^2 &\leq \langle v + w, v + w \rangle\\
        &= \| v \|^2 + \| w \|^2 + 2 \Re \langle v, w \rangle\\
        &\leq \| v \|^2 + 2 \| v \| \| w \| + \| w \|^2\\
        &= (\| v \| + \| w \|)^2
    \end{align*}
    %
    hence $\| v + w \| \leq \| v \| + \| w \|$. It is easiest to verify that $\| \lambda v \|^2 = \langle \lambda v, \lambda v \rangle = |\lambda|^2 \| v \|^2$, and the scalar equality is obtained by taking square roots.
\end{proof}

Note that because the triangle inequality is essentially the Cauchy-Schwarz inequality in disguise, we have $\| v + w \| = \| v \| + \| w \|$ precisely when $w$ is a {\it non-negative} multiple of $v$.

A {\bf norm} on a vector space is a function $\| \cdot \|: V \to \mathbf{R}$ such that $\| v \| > 0$ if $v \neq 0$, $\| \lambda v \| = |\lambda| \|v\|$, and $\| v + w \| \leq \| v \| + \| w \|$. Inner product spaces form a class of norm spaces with incredibly rich structure, where we take the norm $\| v \| = \langle v, v \rangle$. A norm allows us to measure the distance of vectors, if we define the distance between two vectors $v$ and $w$ to be $\| v - w \|$. This defines a metric space structure to the vector space $V$, and if a norm gives rise to a complete metric space structure on a vector space, we call this vector space a {\bf Banach space}. $\| \cdot \|$ is a continuous function with respect to the topology it induces, because if $x_i \to x$, then $\| x_i - x \| \to 0$, hence $\| x \| \leq \| x_i \| + \| x - x_i \|$, so $\| x \| \leq \liminf \| x_i \|$, and also $\| x \| \geq \| x_i \| - \| x_i - x \|$, so $\| x \| \geq \limsup \| x_i \|$, and therefore $\| x \| = \lim \| x_i \|$.

We will study Banach spaces in due time, but now we focus on the {\bf Hilbert spaces}, which are inner product spaces whose induces norm gives rise to a complete vector space structure. They are the easiest of the classes of objects in functional analysis to understand, an occur in applications  in a wide range of fields, in mathematics, quantum physics, and computing science. In this chapter, we shall let $H$ denote a general Hilbert space.

\begin{theorem}[Parallelogram Law]
    If $x,y$ are in a Hilbert space, then
    %
    \[ \| x + y \|^2 + \| x - y \|^2 = 2 \| x \|^2 + 2 \| y \|^2 \]
    %
    which is obvious viewing $0, x, y$, and $x + y$ as vertices of a parallogram.
\end{theorem}
\begin{proof}
    We simply calculate
    %
...
One of the biggest advantages of Hilbert spaces is that we have powerful theorems about the convex sets in the space. A {\bf convex set} in a vector space is a set $C$ such that the line between any two points is contained in $C$. That is, for any $x, y \in C$, and $\lambda \in [0,1]$, $\lambda x + (1 - \lambda) y \in C$.
...
Thus every locally convex space has a basis of convex, balanced neighbourhoods of the origin.
