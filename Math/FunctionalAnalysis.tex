...
A topological vector space is, of course, an abelian topological group under multiplication, since addition is continuous, and inversion can be specified by the continuous operation of multiplication.
%
...
We recall some terminology from linear algebra, which will become even more prevalent in the theory of topological vector spaces. A subset $C$ of a vector space $X$ is {\bf convex} if $x, y \in C$ imply $\lambda x + (1 - \lambda) y \in C$ for $0 \leq \lambda \leq 1$ (so $C$ contains all line segments between each of its points), or in set notation,
%
\[ \lambda C + (1 - \lambda) C \subset C \]
%
{\bf balanced} if it is closed under multiplication from scalars with norm $\leq 1$, so that if $|\lambda| \leq 1$, then
%
\[ \lambda C \subset C \]
%
(this is always an equality) and {\bf absorbing} if every $x$ is equal to $\lambda y$ for some scalar $\lambda > 0$ and $y \in C$, so that
%
\[ X = \bigcup_{\lambda > 0} C \]
%
If a topological vector space has neighbourhoods possessing these geometric properties, we will be able to deduce analytic properties of the topology on the space. Some of these properties we obtain for free. To begin with our exploration of this topology, we note that by continuity of addition, we need only talk about topology around the origin of a topological vector space.

\begin{theorem}
    Every neighbourhood of the origin is absorbing.
\end{theorem}
\begin{proof}
    Given a topological vector space $X$, and a neighbourhood $U$ of the origin, fix $x \in X$, and consider the continuous map $f(\lambda) = \lambda x$. By continuity, since $f(0) = 0 \in U$, there is $(-\varepsilon, \varepsilon)$ such that $f((-\varepsilon, \varepsilon)) \in U$, hence if $\gamma < \varepsilon$, $\gamma x \in U$. Hence $x \in \gamma^{-1} U$, and so $x \in \lambda U$ for $\lambda > \varepsilon^{-1}$.
\end{proof}
...
In a topological vector space, a set $K$ is {\bf bounded} (to be precise, {\bf Von Neumann bounded}, because we may not have a metric on the space) if, for every neighbourhood $U$ of the origin, there is a scalar $\lambda > 0$ such that if $\gamma > \lambda$, $\gamma U \supset K$. We note that because every neighbourhood of the origin is absorbing, compact sets are bounded. Furthermore, note that if $V$ is a neighbourhood of the origin which is bounded, and if $\lambda_i$ are a sequence of scalars tending to zero, then $\lambda_i V$ is a neighbourhood basis of the origin, bcause if $W$ is an arbitrary neighbourhood, then $V \subset \lambda_i^{-1} W$ for some $\lambda_i$, and then $\lambda_i V \subset W$. Thus having a bounded neighbourhood is closely related to being first countable.

\begin{theorem}
    If $V$ is a neighbourhood of the origin, and $A$ is any set, then $\overline{A} \subset A + V$, and in fact $\overline{A} = \bigcap_V A + V$.
\end{theorem}
\begin{proof}
    Given $V$, we can use the continuity of addition to find $W = -W$ such that $W + W \subset V$. If $x \not \in A + V$, then $x - V$, contains no points in $A$, and $-V$ is a neighbourhood of the origin, hence $x \not \in \overline{A}$. If $x$ is contained in $A + V$, for every $V$, then $x - V$ contains points in $A$ for every neighbourhood $V$, and $V \mapsto -V$ is a bijection of the neighbourhood of the origin, so every neighbourhood of $x$ contains points in $A$, ergo $x \in \overline{A}$.
\end{proof}
