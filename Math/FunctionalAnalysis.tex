...
To be as general as possible, we start with the weakest assumptions for our framework. A {\bf topological vector space} is a vector space $X$, together with a topology making addition and scalar multiplication continuous. This means exactly that if $x_\alpha \to x$, $y_\beta \to y$, and $\lambda_\gamma \to \lambda$, then $\lambda_\gamma x_\alpha \to \lambda x$ and $x_\alpha + y_\beta \to x + y$. To be brusque, we need only check that $\lambda_\gamma (x_\alpha + y_\beta)$ converges to $\lambda (x + y)$, because we can always let $\lambda_\gamma = 1$, or $y_\beta = 0$. A topological vector space is, of course, an abelian topological group, since addition is continuous, and inversion can be specified as the composition of the continuous operations $x \mapsto (-1,x) \mapsto (-1) \cdot x = -x$. However, unlike in the case of topological groups, interesting topological vector spaces are rarely locally compact, hence why the analysis of the two basic theories proceeds down different paths. However, just like topological groups, the continuity operations imply the space is uniform, with the translation operators $T_g: f \mapsto f + g$ and $M_\lambda: f \mapsto \lambda f$ homeomorphisms of $X$, so that we need only concentrate on the topology around the origin.
...
\begin{itemize}
    ...
    \[ \lambda B \subset B \]
    %
    and this is an equality if $|\lambda| = 1$, since then
    %
...
\end{theorem}
\begin{proof}
    Given a topological vector space $X$, and a neighbourhood $U$ of the origin, fix $x \in X$, and consider the continuous map $\lambda \mapsto \lambda x$. By continuity, since $0$ maps to $0$, there is $\varepsilon > 0$ such that if $|t| < \varepsilon$, then $t x \in U$. But then if $y = tx \in U$, then $x = t^{-1} y \in t^{-1} U$.
\end{proof}
...
Since $C + W$ is open, the closure of $K + W$ does not intersect $W$. In particular, this means that every neighbourhood $V$ of a point $x$ contains a neighbourhood $W$ with $\smash{\overline{W}} \subset V$. This is one of the main results showing how nice we can choose neighbourhoods of a point to be. We've already used the fact that we can always choose symmetric neighbourhoods $U$ with $-U = U$. In other words, every topogical vector space has a basis at the origin consisting solely of symmetric sets. We will soon show that we can choose these sets to be balanced as well, and, if we begin with a convex neighbourhood, convex and balanced.

\begin{theorem}
    Every neighbourhood of the origin contains a balanced subneighbourhood of the origin.
\end{theorem}
\begin{proof}
    Since multiplication is continuous, if $V$ is any neighbourhood of the origin, and $0 \cdot 0 = 0$, there is a neighbourhood $U$ and $\varepsilon > 0$ such that for $|\lambda| < \varepsilon$, $\lambda U \subset V$. In particular,
    %
    \[ W = \bigcup_{|\lambda| < \varepsilon} \lambda U \subset V \]
    %
    and $W$ is balanced.
\end{proof}
...
Given any bounded open neighbourhood $V$ of an origin, we can obtain a local basis by taking a family of scalars $\lambda_1 > \lambda_2 > \dots \to 0$, and considering $\lambda_1 V, \lambda_2 V, \dots$. This follows by continuity of multiplication. Thus if a space has a single bounded convex neighbourhood, the space is locally convex.
...
A topological vector space is said to be {\bf metrizable} if it's topology is induced by a metric on the space. Then the balls of radius $1/n$ around the origin form a basis for the entire space. Surprisingly, this is all we need to show that a topological vector space is countable.

\begin{theorem}
    If $X$ is a topological vector space with a countable local base, then there exists an invariant metric $d$ such that the open balls at the origin are balanced. If $X$ is locally convex, then we can choose $d$ such that the open balls are convex.
\end{theorem}
\begin{proof}
    TODO
\end{proof}
