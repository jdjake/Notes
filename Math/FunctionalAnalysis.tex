...
The most common analytic operations are scaling and rotation. As a result, the most common vector spaces which occur in functional analysis are over the real and complex numbers. To be more specific, a real vector space $V$ is a space equipped with scaling operations $v \mapsto \lambda v$, for $\lambda \in \mathbf{R}$. If $V$ is a real vector space with a specified `counterclockwise twisting' linear transformation $J: V \to V$ with $J^2 = -1$, then there is a natural way to turn $V$ into a complex vector space by defining $(a + ib)v = av + b(Jv)$. The geometry of the complex numbers then enables us to define a counterclockwise twist by any angle $\theta$, by the operation $v \mapsto e^{i\theta} v$. A picky algebraist should note that, if we require our fields to be of characteristic zero, then we will find that the topologically complete criterion forces our vector space to be over the real numbers {\it at least}, so fixing our attention here isn't being too particular.
...
Originally, functional analysis dealt with norm spaces, providing vectors with an explicit way of measuring their size, and the distance between vectors. But soon, vector spaces appeared in problems with topological properties which couldn't be identified with a norm structure. Some are easy to fix -- for instance, the space of square integrable functions becomes a norm space if we identify functions equal almost everywhere. However, there turn out to be interesting notions of convergence on classes of functions in analysis which give rise to even stranger topologies than seminorm spaces. For instance, the space of holomorphic functions under the topology of locally uniform convergence, or the class of Schwartz functions on Euclidean space under the topology of rapid uniform convergence of the function and all of its derivatives. Thus we are forced to treat vector spaces with topologies which do not correspond to a norm, and so we obtain the theory of topological vector spaces.

In order to be completely general, we start with the weakest assumptions corresponding to the framework we wish to form. A {\bf topological vector space} is a vector space, together with a topology making addition and scalar multiplication continuous. In terms of nets, this means that if $x_\alpha \to x$ and $y_\beta \to y$, then $x_\alpha + y_\beta \to x + y$, and if we have a net of scalars $\lambda_\gamma \to \lambda$, then $\lambda_\gamma x_\alpha \to \lambda x$. In terms of neighbourhoods, the continuity of addition says that if $V$ is any neighbourhood of $x + y$, then there are neighbourhoods $W$ of $x$ and $U$ of $y$ such that $W + U \subset V$. Note that in particular, if $V$ is a neighbourhood of the origin, then since $0 + 0 = 0$, we can find a neighbourhood $W$ of the origin with $W + W \subset V$, and, by induction, find a neighbourhood with $W + W + \dots + W \subset V$ for any finite, but fixed number of additions we desire. The continuity of multiplication says that if $V$ is any neighbourhood of $\lambda x$, then there is $\varepsilon > 0$ and a neighbourhood $W$ of $x$ such that if $|\gamma - \lambda| < \varepsilon$, then $\gamma W \subset V$.

A topological vector space is, of course, an abelian topological group, since addition is continuous, and inversion can be specified as the composition of the continuous operations $x \mapsto (-1,x) \mapsto (-1) \cdot x = -x$. Note that, unlike in the case of topological groups, interesting topological vector spaces are rarely locally compact, hence why the analysis of the two basic theories proceeds down different paths.
...
%
which is essentially a form of locally uniform convergence for the automorphisms.
...
%
\begin{itemize}
    \item A {\bf convex} subset $C$ of a vector space $X$ is such that if $x, y \in C$ imply $\lambda x + (1 - \lambda) y \in C$ for $0 \leq \lambda \leq 1$ (so $C$ contains all line segments between each of its points), or in set notation,
    %
    ...
    \[ \lambda B \subset B \]
    %
    If $|\lambda| = 1$, this is always an inequality, for then
    %
    \[ B = \lambda^{-1}(\lambda B) \subset \lambda B \subset B \]
    %
    which implies $B = \lambda B$.s
...
In a topological vector space, a set $K$ is {\bf (Von Neumann) bounded} if, for every neighbourhood $U$ of the origin, there is a scalar $\lambda > 0$ such that if $\gamma > \lambda$, $\gamma U \supset K$. We note that because every neighbourhood of the origin is absorbing, compact sets are bounded. In the context of topological vector spaces, we shall always use bounded to refer to this definition, even if the vector space is metrizable.
...
\end{proof}
>>>>>>> 05f676888eaf6abc855b85cb2e2b9da767b5ec0c
...
From now on, we will assume all the vector spaces we consider are Hausdorff. In most cases this can be obtained by taking a quotient vector space of the original spaces, ala identifying functions almost everywhere in the theory of $L^p$ spaces.
...
Since $C + W$ is open, we find that, in fact, $\overline{K + W}$ does not intersect $C + W$. If we choose $K = \{ 0 \}$, and $C = V^c$ for some neighbourhood $V$ of the origin, we find a neighbourhood $W$ with $\smash{\overline{W}} \subset V$. Thus for any neighborhood $V$ we can find a subneighbourhood whose closure is contained in $V$.
...
\begin{theorem}
    if $X$ is a first countable vector space with a countable base, then $X$ is metrizable by an invariant metric $d$ whose open balls are balanced
\end{theorem}
\begin{proof}
...
In the sequel, we will often use the following fact: If $f: X \to Y$ is a map between two complete metric spaces, and $d(f(x), f(y)) \geq d(x,y)$, then $f(X)$ is closed. This is simple to prove, for if $f(x_n) \to y$, then $f(x_n)$ is Cauchy, hence $x_n$ is Cauchy, so $x_n \to x$, and by continuity $f(x) = y$.
...
%
hence $\| x - x' \| = 0$, and since this is true for every seminorm, $x = x'$.

If $\| \cdot \|$ is a seminorm, then the `unit ball' with respect to this seminorm is convex, balanced, and absorbing. Conversely, suppose $A$ is a convex subset of a topological vector space. Then we can define the {\bf Minkowski functional}
%
\[ \mu_A(x) = \inf \{ \lambda > 0: x \in \lambda A \} \]
%
If $A$ is an absorbing set, $\mu_A$ is a finite function. If $A$ is, in addition, balanced, then $\mu_A$ defines a seminorm. The connection between convex sets and Minkowski functionals gives a close correspondence between locally convex spaces and spaces with metrics given by seminorms.
...
If $x \neq 0$, then $x, 2x, 3x, \dots$ {\it cannot} be bounded. For there is a balanced neighbourhood $U$ with $x \not \in U$, hence $nx \not \in nU$.
