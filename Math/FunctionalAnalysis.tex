...
%
The resolvent is the complement of the spectrum. When the underlying algebra is canonical, we just use $\sigma(M)$ and $\rho(M)$ to denote the spectrum. I view the addition of a complex number to an operator as an `infinitisimal shifts' in the effects of the operator. The spectrum tells us in which directions the operator goes bad under infinitisimal changes, and we shall find that knowledge of the spectrum will allow us to perturb operators more finely. A large number of theorems are based on relating these perturbations to the original operator.
...
In a commutative $C^*$ algebra, every element is trivially normal, which implies that
...
For normal operators, we can extend the holomorphic functional calculus to all continuous functions.
...
The natural morphism between $*$-algebras is a $*$-morphism, an algebra homomorphism $T$ which satisfies $T(M^*) = T(M)^*$.
...
Thus every $*$-morphism is continuous. Injective $*$-morphisms are isometries. Let $A$ be a $C^*$ algebra, and let $M \in A$. We let $C^*(M)$ be the smallest $C^*$ algebra containing $M$. It can be described as the smallest closed subalgebra which contains $1$, $M$, and $M^*$. Certainly, the latter is contained in the former. Consider a monomial, consider of products of $M$ and $M^*$. Then the involution of a monomial is a monomial, and by linearity, the span is closed under involution, hence the closed span is also closed under involution.

\begin{prop}
    An injective $*$-morphism is an isometry.
\end{prop}
\begin{proof}
    Let $M \in A_{\text{sa}}$, and let $N = \pi(M)$. Restrict $\pi$ to a map between $C^*(M)$ and $C^*(N)$. Thus we may assume that $A$ and $B$ are abelian. By adjoining an identity, we may also assume the algebras are unital, and thus we may explicitly introduce isometries $A \cong C(X)$ and $B \cong C(Y)$. An isomorphism $\pi: C(X) \to C(Y)$ induces the dual map $\pi^*: Y \to X$ on the character space. Since $Y$ is compact, $\pi^*(Y)$ is closed in $\Phi_A$. Suppose $\pi^*$ was not surjective. Let $f$ be the constant 1 function on $\pi^*(Y)$, which vanishes outside some proper neighbourhood of $\pi^*(Y)$. Then there is a non-zero function $g$ for which $fg = 0$, and so
    %
    \[ \pi(fg) = \pi(f) \pi(g) = 0 \]
    %
    and $\pi(f) = 1$, for
    ...
    \[ \pi(f)(x) = f(\pi^*(x)) = 1 \]
    %
    Thus $\pi(f)$ is invertible, which is impossible, for this would imply $\pi(g) = 0$, a contradiction to the fact that $g$ is non-zero and $\pi$ is injective. By contradiction, we find $\pi^*(Y) = X$, and for any $h \in C(X)$
    %
    \[ \| \pi(h) \|_\infty = \sup \{ |\pi(h)(y)|: y \in Y \} = \sup \{ |h(\pi^*(y))| : y \in Y \} = \| h \|_\infty \]
    %
    So $\pi$ is an isometry on $C^*(M)$ and $C^*(N)$. In general, for any $M$,
    %
    \[ \| M \|^2 = \| MM^* \| = \| \pi(MM^*) \| = \| \pi(M) \pi(M^*) \| = \| \pi(M) \|^2 \]
    %
    so $\pi$ is an isometry.
\end{proof}
...
\end{prop}
\begin{proof}
    There is a sequence $P_i$ of polynomials which uniformly approximate $f$. Thus $P_i(0) \to 0$, and the sequence $Q_i = P_i - P_i(0)$ also converges uniformly to $f$, and so $Q_i(M) \to f(M)$. If we write $Q_i = \sum a_{ij} X^i \overline{X}^j$, then $Q_i(M) = \sum a_{ij} X^{i+j} \in \mathfrak{a}$, and since $\mathfrak{a}$ is closed, we find $f(M) \in \mathfrak{a}$.
\end{proof}
...
If an ideal is closed under involution, then it is a two sided ideal, for involution reverses the multiplication structure. We shall show that this is true of all closed ideals.
