...
%        \draw [blue, thick] \endA  -- ++(1/7,0);
    ...
    \uncover<5->{
        ...
        \foreach \x in {1/7, 3/7, 5/7,6/7}
            \draw[very thick] (\x,-0.8) -- (\x + 1/7,-0.8);
    }
\end{tikzpicture}
    \end{figure}

%    \begin{itemize}
%        \item {\bf Multi Scale Problem}: Find $X$ such that $X^n \cap Z \subset \Delta$.

%        \item {\bf Single Scale Problem}: Given disjoint unions of intervals $I_1, \dots, I_n$, find a large subset $J_1 \subset I_1, \dots, J_n \subset I_n$ such that $J_1 \times \dots \times J_n$ is disjoint from $Z$.
...
%        \item If $x_1, \dots, x_n \in X$, then there are intervals $x_1 \in I_1, \dots, x_n \in I_n$ at some scale.
