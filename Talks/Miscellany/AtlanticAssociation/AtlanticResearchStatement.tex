...
In Section 3, I discuss my plans for future research, emphasizing how my PhD work gives me the tools to succeed in these plans. Professor Eswarathasan works on problems involving nodal sets of sub Laplacians, and the relations between Fourier integral operators and geometric measure theory, and \emph{several projects in this section relate to Professor Eswarathasan work, including using methods of stochastic diffusion to study nodal sets of sub Laplacians, studying highly degenerate sub Laplacians using methods of o-minimality, and studying relations between Fourier integral operators and geometric measure theory, and I explicitly relate these projects to Professor Eswarathasan work in that section.}
...
{\bf Fourier Dimensions on Manifolds} Fourier dimension depends on the Fourier transform, and is thus closely tied to the structure of Euclidean space. A natural question is whether an analogous theory can be developed on manifolds, where Fourier decay is replaced with a lack of correlation with any Laplace-Beltrami eigenfunction. Given this definition, is it possible to recover results connecting Fourier dimension to the existence of patterns in this setting? Such methods will likely involve the use of Fourier integral operators to replace the theory of the Fourier transform, and thus relate to the work of Professor Eswarathasan on geometric measure theory in \cite{EswarathasanIosevichTaylor} and \cite{EswarathasanLetrouit} related to the Falconer distance problem.

{\bf Bounds On Nodal Sets For Sub Laplacians} Let $M^d$ be a compact Riemannian manifold, possibly with boundary, and continue an eigenfunction $e_\lambda$ to the Laplace-Beltrami operator on $M$, i.e. satisfying $\Delta e_\lambda = \lambda e_\lambda$. It is an interesting problem to consider asymptotic properties of the \emph{nodal domains} of $e_\lambda$, i.e. the connected components of the complement of the zero set of $e_\lambda$. Recently in \cite{EswarathasanLetrouit}, Eswarathasan and Letrouit extended classical methods in the theory of nodal domains to study analogous sets for nodal sets associated with \emph{sub Laplacians}, connected components of the complement of the zero set to an eigenfunction to a sums of squares operators of the form $- \sum X_i^* X_i$, where $\{ X_i \}$ are vector fields satisfying H\"{o}rmander's vector field condition. In particular, Eswarathasan and Letrouit prove upper bounds on the number of such connected components, prove a nodal domain cannot contain a ball of suitably large radius, and obtain Yau-type bounds on the Hausdorff measure of the zero set.

In the \cite{GeorgievMukherjee} showed a converse of the results of \cite{EswarathasanLetrouit} in the setting of Laplace operators, in particular showing that a nodal domain \emph{must} contain a ball of suitably large radius. The results involve a use of \emph{stochastic diffusions}, i.e. stochastic processes related to Brownian motion. It is a natural question whether such methods can be used to prove analogous results in the setting of sub Laplacians. One problem with such an approach is that stochastic diffusions are related to \emph{elliptic} differential operators like the Laplacian, and so one must find a more general class of stochastic processes associated with \emph{subelliptic} operators.
