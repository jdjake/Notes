...
Consider the unit sphere $S^d = \{ x \in \RR^{d+1} : |x| = 1 \}$, viewed as a Riemannian manifold. Then we can consider the Laplace-Beltrami operator $\Delta$ on $S^d$. In this paper, we study $L^p$ boundedness properties of multiplier operators associated with $\Delta$. We define $P = \sqrt{-\Delta}$, fix a function $h: (0,\infty) \to \CC$ with $\text{supp}(h) \subset \{ t: 1/2 \leq |t| \leq 2 \}$, and study multiplier operators of the form
%
\[ h \left( P / R \right) = \sum\nolimits_\lambda h(\lambda / R) \mathcal{P}_\lambda, \]
%
where $\mathcal{P}_\lambda$ is the orthogonal projection operator onto the eigenspace of $P$ corresponding to the eigenvalue $\lambda$, using variable-coefficient generalizations of techniques that have been used to obtain the boundedness of radial Fourier multiplier operators of the form $h(|\cdot|)$. In particular, we prove that certain conditions on the Fourier transform of $h$, which are necessary for the operators $\{ h(P/R) \}$ to be uniformly bounded on $L^p(S^d)$, are actually \emph{sufficient} for a restricted range of exponents $p$. To state this condition, define
%
\[ s_q = (d-1) \left| \frac{1}{q} - \frac{1}{2} \right| \quad\text{for $1 \leq q \leq \infty$}, \]
%
and define
%
\[ C_p(h) = \left( \int_0^\infty \left[ \langle t \rangle^{s_p} |\widehat{h}(t)| \right]^p\; dt \right)^{1/p} \quad\text{for $1 \leq p \leq 2$}. \]
%
is finite. This condition is necessary for the operators
%
\[ \{ h(P/R) : R > 0 \} \]
%
