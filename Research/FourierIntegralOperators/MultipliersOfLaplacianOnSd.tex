...
\begin{equation} \label{puttingTogetherPieces}
    \Big\| \sum_{j \gtrsim 1} m_j(P/2^j) \Big\| \lesssim \sup_{j \geq 10} \| m_j(P/2^j) \|.
\end{equation}
%
...
Let us go over our problem more precisely. Spectral theory guarantees the existence of a discrete set $\Lambda_M \subset (0,\infty)$ accumulating only at $\infty$, and an orthogonal decomposition $L^2(M) = \bigoplus_{\lambda \in \Lambda_M} \mathcal{V}_\lambda$, where $\mathcal{V}_\lambda$ is a finite dimensional subspace of $C^\infty(M)$ such that $P f = \lambda f$ for all $f \in \mathcal{V}_\lambda$. For any bounded function $m: (0,\infty) \to \CC$, we define $m(P)$ by setting
%
\[ m(P) = \sum_{\lambda \in \Lambda_M} m \left( \lambda \right) \mathcal{P}_\lambda, \]
%
where $\mathcal{P}_\lambda$ is the orthogonal projection operator onto $\mathcal{V}_\lambda$. As in the last section, we study this operator by breaking it up as $\sum_j m_j(P/2^j)$, where
%
\[ m_j(P/2^j) = \sum_{\lambda \in \Lambda_M} m_j \left( \lambda / 2^j \right) \mathcal{P}_\lambda. \]
%
...
%
To exploit the integrability of the functions $\{ \widehat{m}_j \}$, we must relate the Fourier transform of a function to it's associated spectral multiplier. A standard method in this setting is to apply the Fourier inversion formula; given a function $h: \RR \to \CC$, we have
%
...
The half-wave equation on $M$ is hyperbolic, and like other hyperbolic partial differential equations, the singularities of solutions to the half-wave propogate along characteristic curves, which is this case are the geodesics of the manifold $M$. To obtain more quantitative information about this propogation singularities, we `localize to a particular frequency'. Fix $\tilde{\beta} \in C_c^\infty(\RR)$ with $\text{supp}(\beta) \subset [1/4,4]$ and with $\tilde{\beta}(\lambda) = 1$ for $\lambda \in [1/2,2]$. Then $\beta(t) = \tilde{\beta}(t)^2$ has the exact same properties. For $R > 0$, we define $Q_R = \beta(P/R)$ and $\tilde{Q}_R = \tilde{\beta}(P/R)$. Then $Q_R = \tilde{Q}_R^2$. Both operators have range contained in the finite dimensional subspace $V_R$ of $C^\infty(M)$ spanned by eigenfunctions of $P$ with eigenvalues in $[R/4,4R]$. Since $P$ is a positive-definite and self-adjoint, it is often a useful heuristic that elements of $V_R$ have similar properties to a function on $\RR^d$ with Fourier support on the annulus $\{ \xi: R/4 \leq |\xi| \leq 4R \}$. In particular, the uncertainty principle tells us the kernel of the operator $Q_R \circ e^{2 \pi i tP} \circ Q_R$ should be smooth, and locally constant at the scale $1/R$.

Because $\text{supp}(m(\cdot/2^j)) \subset [2^{j-1}, 2^{j+1}]$, we can introduce these operators by writing
%
\begin{align*}
    m_j(P/2^j) &= Q_{2^j} \circ m_j(P/2^j) \circ Q_{2^j} = \int_{\RR} 2^j \widehat{m}_j(2^j t) (Q_{2^j} \circ e^{2 \pi i t P} \circ Q_{2^j})\; dt.
\end{align*}
%
The half-wave equation is also more easy to understand for small times; this is where we exploit the fact that the manifold $M$ has periodic geodesic flow. Normalizing the metric on $M$ appropriately, we may assume that the geodesic flow has period one. It follows that $e^{2 \pi i (t + n) P} = e^{2 \pi i t P}$ for any $t \in \RR$ and $n \in \ZZ$. We may then write
%
\begin{align*}
    m_j(P/2^j) = \int_{-1/2}^{1/2} b_j(t) (Q_{2^j} \circ e^{2 \pi i tP} \circ Q_{2^j})\; dt,
\end{align*}
%
where $b_j: [-1/2,1/2] \to \CC$ is the periodized function
%
\[ b_j(t) = \sum_{n \in \ZZ} 2^j \widehat{m}_j(2^j (t + n)). \]
%
We will split our analysis of these multipliers into two regimes. We fix a small geometric constant $\varepsilon_M > 0$ depending only on the geometry of $M$, to be specified later (TODO: Where). We thus write $b_j = b_j^0 + b_j^1$, where
%
\[ b_j^0(t) = \psi(t / \varepsilon_M)\; b_j(t) \quad\text{and}\quad b_j^1(t) = \Big( 1 - \psi(t / \varepsilon_M) \Big)\; b_j(t). \]
%
for some smooth, compact supported function $\psi: \RR \to [0,\infty)$ supported on $[-1,1]$ and with $\psi(t) = 1$ for $t \in [-1/2,1/2]$. We will perform a further `wave-packet decomposition' at a frequency scale $2^j$ for the function $b_j^0$. We will later prove that we can write
%
\[ b_j^0 = \sum_{t_0 \in \mathcal{T}_j} b_{j,t_0}, \]
%
where $\mathcal{T}_j = \ZZ/2^j \cap [-\varepsilon_M, \varepsilon_M]$, where $\text{supp}(b_{j,t_0}) \subset [ t_0 - 2/2^j, t_0 + 2/2^j ]$ for $t_0 \in \mathcal{T}$, and such that for each $k \geq 0$,
%
\[ \left( \sum_{t_0 \in \mathcal{T}_j} \Big[ \| \partial_t^k b_{j,t_0} \|_{L^1(\RR)} \langle 2^j t_0 \rangle^{\alpha(p)} \Big]^p \right)^{1/p} \lesssim_k 2^{j(k - 1/p')} C_p(m), \]
%
and that the function $b_j^1$ satisfies
%
\[ \| b_j^1 \|_{L^p(\RR)} \lesssim 2^{-j (1/p' + \alpha(p))} C_p(m). \]
%
We prove the existence of this decomposition, and the required estimates, in Lemma TODO. We can thus write
%
\[ m_j(P/2^j) = \left( \sum_{t_0 \in \mathcal{T}_j} M_{j,t_0} \right) + M_j^1 \]
%
where
%
\[ M_{j,t_0} = \int b_{j,t_0}(t) ( Q_{2^j} \circ e^{2 \pi i tP} \circ Q_{2^j} )\; dt \quad\text{and}\quad M_j^1 = \int b_j^1(t) ( Q_{2^j} \circ e^{2 \pi i tP} \circ Q_{2^j})\; dt. \]
%
We will obtain control over the operator $\{ m_j(P/2^j) \}$ by understanding the interactions of functions of the form $f_{x_0,t_0} = M_{j,t_0} u_{x_0}$, where $u_{x_0}: M \to \CC$ is highly localized, i.e. with $\text{supp}(u_0)$ contained in $B(x_0,2/2^j)$, the radius $2/2^j$ geodesic ball centered at some point $x_0 \in M$. Our main results on these interactions are described in the next section.
...
\section{Density Arguments For Dyadic Pieces}

In this section, we begin proving \eqref{dyadicMainReulst}. Given a general input $u: M \to \CC$, we consider a maximal $1/2^j$ separated subset $\mathcal{X}$ of $M$, then consider a decomposition $u = \sum_{x_0 \in \mathcal{X}} u_{x_0}$, where $u_{x_0}$ is supported on $B(x_0,2/2^j)$, and for all $r \in [1,\infty]$,
%
\[ \| u \|_{L^r(M)} \sim \left( \sum_{x_0 \in \mathcal{X}} \| u_{x_0} \|_{L^r(M)}^r \right)^{1/r}. \]
%
If we set $f_{x_0,t_0} = M_{j,t_0} \{ u_{x_0} \}$, then we can write
%
\[ m_j(P/2^j) \{ u \} = \sum_{t_0 \in \mathcal{T}_j} \sum_{x_0 \in \mathcal{X}} f_{x_0,t_0} + M_j^1 \{ u \}. \]
%


we write $m_j(P/2^j) \{ u \} = \sum M_{j,t_0}(u_{x_0})$


The quasi-orthogonality estimates there are not sufficient to obtain $L^2$ estimates 
