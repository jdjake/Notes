...
\begin{theorem} \label{maintheoremprime}
    Suppose $M$ is a compact Riemannian manifold, and the geodesic flow on $M$ is periodic. If $d \geq 4$, and $1/(d-1) < |1/p - 1/2| \leq 1/2$, then
    %
...
One side of this inequality is already well known. A classical transplantation theorem of Mitjagin \cite{Mitjagin} states that
%
\begin{equation}
...
The path to proving \eqref{upperboundmainresult} is hinted at by the main result of \cite{HeoandNazarovandSeeger}, which states that, for $(d-1)^{-1} \leq |1/p - 1/2| \leq 1/2$, $\| T_m \| \sim C_p(m)$, where
%
...
The techniques that underlie the proof of Proposition \ref{mainTheorem} are also inspired by the proof methods of \cite{HeoandNazarovandSeeger}. As in that proof, we begin by considering a decomposition $m(\cdot) = \sum_j m_j(\cdot / 2^j)$, where $\text{supp}(m_j) \subset [1/2,2]$. We will see that the behaviour of the multipliers $M_j = m_j(P/2^j)$ is fairly benign for $j \lesssim 1$. Proposition \ref{mainTheorem} then follows if we can show that for each $j \gtrsim 10$,
%
\begin{equation} \label{dyadicMainReulst}
    \| M_j \| \lesssim \| \langle t \rangle^{s_p} \widehat{m}_j(t) \|_{L^p(\RR)},
\end{equation}
%
and also that
%
\begin{equation} \label{puttingTogetherPieces}
    \Big\| \sum_{j \gtrsim 1} M_j \Big\| \lesssim \sup_{j \gtrsim 1} \| M_j \|.
\end{equation}
%
...
%
As discussed in the last section, we consider a dyadic decomposition $m(P) = \sum_j M_j$, where $M_j = \sum_\lambda m_j(\lambda / 2^j) \mathcal{P}_\lambda$.
%
...
%
\[ M_j = \int_{-\infty}^\infty 2^j \widehat{m}_j(2^j t) e^{2 \pi i P t}\; dt, \]
%
...
%
\begin{align*}
    M_j = \int_{-1/2}^{1/2} b_j(t) (Q_j \circ e^{2 \pi i tP} \circ Q_j)\; dt,
\end{align*}
%
...
\begin{lemma}
    ...
    %
    \begin{itemize}
        \item $\text{supp}(b_{j,t_0}^I) \subset [t_0 - 2/2^j, t_0 + 2/2^j]$ and $\text{supp}(b_j^{II}) \subset [-1/2,1/2] \smallsetminus [-\varepsilon_M/2,\varepsilon_M/2]$.
...
%
\begin{align*}
    M_j &= M_j^I + M_j^{II} = \left( \sum_{t_0 \in \mathcal{T}_j} M_{j,t_0}^I \right) + M_j^{II}
\end{align*}
%
where
%
\[ M_{j,t_0}^I = \int b_{j,t_0}^I(t) ( Q_j \circ e^{2 \pi i tP} \circ Q_j )\; dt \quad\text{and}\quad M_j^{II} = \int b_j^{II}(t) ( Q_j \circ e^{2 \pi i tP} \circ Q_j)\; dt. \]
%
Given the comparably better bounds for the function $b_j^{II}$ (we have an extra multiplicative factor $2^{-j \alpha(p)}$ to work with), we will be able to obtain bounds on $M_j^{II}$ simply by applying H\"{o}lder's inequality, which reduces our analysis to an endpoint local smoothing inequality. On the other hand, we will obtain control over the operator $M_j^I$ by understanding the interactions of functions of the form $f_{x_0,t_0} = M_{j,t_0}^I u_{x_0}$, where $u_{x_0}: M \to \CC$ is a function with $\text{supp}(u_0)$ contained in $B(x_0,2/2^j)$, the radius $2/2^j$ geodesic ball centered at some point $x_0 \in M$. We begin our analysis of these interactions in the next section.
...
\begin{prop} \label{theMainEstimatesForWave}
    ...
    \begin{itemize}
        ...
        %
        \begin{align*}
            \left| \langle f_0, f_1 \rangle \right| &\lesssim_K \left( \prod_j \| c_j \|_{L^1(\RR)} \| u_j \|_{L^1(M)} \right) \frac{R^d}{( R d_g(x_0,x_1) )^{\frac{d-1}{2}}} \Big\langle R \big| |t_0 - t_1| - d_g(x_1,x_0) \big| \Big\rangle^{-K}.
        \end{align*}
    \end{itemize}
\end{prop}
...
\begin{prop} \label{L2DensityProposition}
    ...
    Then
    %
    \[ \Big\| \sum\nolimits_k \sum\nolimits_{(x_0,t_0) \in \mathcal{E}_k} 2^{k \frac{d-1}{2}} f_{x_0,t_0} \Big\|_{L^2(S^d)} \lesssim 2^{jd} \log(u) u^{\frac{2}{d-1}} \sum_k 2^{k(d-1)} \# \mathcal{E}_k. \]
\end{prop}
...
\begin{itemize}
    ...
    %
    \[ 2^{k \frac{d-1}{2}} 2^{k' \frac{d-1}{2}} \sum_{(x_1,t_1) \in \mathcal{G}_{x_0,t_0,\text{low}}} |\langle f_{x_0,t_0}, f_{x_1,t_1} \rangle| \lesssim_M 2^{jd} 2^{k \frac{d-1}{2}} 2^{k' \frac{d-1}{2}} \Big( u 2^{(k-k')(d-1) + k'} \Big) \Big( 2^{-k \frac{d-1}{2}} \Big). \]
    %
    We can now sum over $\log(u) \lesssim k' \leq k - 10$ and $(x_0,t_0) \in \mathcal{E}_k$ to find
    % d = 2: get an extra O( log k ) factor
    \[ 2^{k \frac{d-1}{2}} 2^{k' \frac{d-1}{2}} \sum_{(x_0,t_0) \in \mathcal{E}_k} \sum_{k' \leq k - 10} \sum_{(x_1,t_1) \in \mathcal{G}_{x_0,t_0,\text{low}}} |\langle f_{x_0,t_0}, f_{x_1,t_1} \rangle| \lesssim 2^{jd} 2^{k (d-1)} \# \mathcal{E}_k. \]
    ...
    Together with Proposition \ref{theMainEstimatesForWave}, we conclude that
    %
    \[ 2^{k \frac{d-1}{2}} 2^{k' \frac{d-1}{2}} \sum_{(x_1,t_1) \in \mathcal{G}_{x_0,t_0,l}} |\langle f_{x_0,t_0}, f_{x_1,t_1} \rangle| \lesssim_N 2^{jd} 2^{k \frac{d-1}{2}} 2^{k' \frac{d-1}{2}} \Big( u 2^{l} 2^{(k-k')(d-1)} \Big) \Big( 2^{-k \frac{d-1}{2}} 2^{-lN} \Big). \]
    %
    Picking $N > 1$, we can sum over $k' + 10 \leq l \leq k - 5$, $\log(u) \lesssim k' \leq k - 10$, and $(x_0,t_0) \in \mathcal{E}_k$ to find
    %
    \[ \sum_{(x_0,t_0) \in \mathcal{E}_k} \sum_{k' \leq k - 10} \sum_{k'+10 \leq l \leq k - 5} \sum_{(x_1,t_1) \in \mathcal{G}_{x_0,t_0,l}} 2^{k \frac{d-1}{2}} 2^{k' \frac{d-1}{2}} |\langle f_{x_0,t_0}, f_{x_1,t_1} \rangle| \lesssim 2^{jd} 2^{k (d-1)} \# \mathcal{E}_k. \]
    ...
    Together with Proposition \ref{theMainEstimatesForWave}, we conclude that
    %
    \[ 2^{k \frac{d-1}{2}} 2^{k' \frac{d-1}{2}} \sum_{(x_1,t_1) \in \mathcal{G}_{x_0,t_0,l}} |\langle f_{x_0,t_0}, f_{x_1,t_1} \rangle| \lesssim_N 2^{jd} 2^{k \frac{d-1}{2}} 2^{k' \frac{d-1}{2}} \Big( u 2^{(l-k')d} 2^{k'} \Big) \Big( 2^{-lN} \Big). \]
    %
    Picking $N > d$, we can sum over $k - 5 \leq l \leq 10 j$, $\log(u) \lesssim k' \leq k - 10$, and $(x_0,t_0) \in \mathcal{E}_k$ to conclude that
    %
    \[ 2^{k \frac{d-1}{2}} 2^{k' \frac{d-1}{2}} \sum_{(x_0,t_0) \in \mathcal{E}_k} \sum_{k' \leq k - 10} \sum_{k-5 \leq l \leq 10 j} \sum_{(x_1,t_1) \in \mathcal{G}_{x_0,t_0,l}} |\langle f_{x_0,t_0}, f_{x_1,t_1} \rangle| \lesssim 2^{jd}. \]
\end{itemize}
%
...
%
\begin{align*}
    \sum_{1 \leq l \leq k - a \log_2 u + 10} \sum_{(x_1,t_1) \in \mathcal{H}_{x_0,t_0,l}} 2^{k(d-1)} |\langle {S\!}_{x_0,t_0}, {S\!}_{x_1,t_1} \rangle| &\lesssim 2^{jd} \sum_{1 \leq l \leq k - a \log_2 u + 10} (2^{k(d-1)}) (2^l u^a)^{- \frac{d-1}{2}} (u^{a+1} 2^l)\\
    &\lesssim 2^{jd} \sum_{1 \leq l \leq k - a \log_2 u + 10} 2^{k(d-1)} 2^{-l \frac{d-3}{2}} u^{1 - a \left( \frac{d-3}{2} \right)}\\
    &\lesssim 2^{jd} 2^{k(d-1)} u^{1 - a \left( \frac{d-3}{2} \right)}.
\end{align*}
%
...
%
\[ |t_0 - t_1| \lesssim 2^k / 2^j \ll 2^l u^a / 2^j. \]
%
...
%
\begin{align*}
    \sum_{l \geq k - a \log_2 u + 10} \sum_{(x_1,t_1) \in \mathcal{H}_{x_0,t_0,l}} 2^{k(d-1)} |\langle {S\!}_{x_0,t_0}, {S\!}_{x_1,t_1} \rangle| &\lesssim 2^{jd} \sum_{l \geq k - a \log_2 u + 10} (2^{k(d-1)}) (2^l u^a)^{-M} u^{1 + ad} 2^{ld} 2^{-k(d-1)}\\
    &\lesssim 2^{jd}.
\end{align*}
%
...
%
\[ \| F_k \|_{L^2(S^d)}^2 \lesssim \sum_\mu \| F_{k,\mu} \|_{L^2(S^d)}^2 + 2^{jd} \left( 1 + 2^{k(d-1)} u^{1 - a \left( \frac{d-3}{2} \right)} \right) \# \mathcal{E}_k. \]
\begin{comment}
...
%
\[ \| F_{k,\mu,t} \|_{L^2(S^d)}^2 \lesssim 2^{jd} 2^{k(d-1)} \# (\mathcal{E}_k \cap (S^d \times \{ t \})). \]
%
But this means that
%
\[ u^a \sum_t \| F_{k,\mu,t} \|_{L^2(S^d)}^2 \lesssim 2^{jd} 2^{k(d-1)} u^a \# \mathcal{E}_{k,\mu}. \]
%
and so
%
\begin{align*}
...
%
\[ \| F_k \|_{L^2(S^d)}^2 \lesssim 2^{jd} 2^{k(d-1)} u^{\frac{2}{d-1}} \# \mathcal{E}_k. \]
%
Thus, returning to our bound for $F$, we conclude that
%
\[ \| F \|_{L^2(S^d)}^2 \lesssim 2^{jd} \log(u) u^{\frac{2}{d-1}} \sum_k  2^{k(d-1)} \# \mathcal{E}_k. \]  
%
Together with Proposition BLAH of the last section, this completes the proof of the $L^2$ density bound.
\end{proof}
...
In this section, we describe the \emph{density decomposition} argument, which will enable us to prove $L^p$ bounds for the operator $M_j^I$. We fix a function $u$. Then
%
\[ M_j u = M_j \{ Q_j u \}. \]
%
...
%
\[ \left( \sum_{x_0 \in \mathcal{X}_j} \| u_{x_0} \|_{L^1(M)}^p \right)^{1/p} \lesssim 2^{-jd/p'} \left( \sum_{x_0 \in \mathcal{X}_j} \| u_{x_0} \|_{L^p(M)}^p \right)^{1/p} \lesssim 2^{-jd/p'} \| Q_j u \|_{L^p(M)} \lesssim 2^{-jd/p'} \| u \|_{L^p(M)}. \]
% L^p is H^p R^{-1}
% L^1 is H^p R^{-p}
Define
%
...
%
Define functions $f_{x_0,t_0} = M_{j,t_0}^I u_{x_0}$. Our computation would be complete if we could show that for any function $c: \mathcal{X}_j \times \mathcal{T}_j \to \CC$,
%
\begin{equation} \label{TheGeneralLpBoundednessEquation}
    \left\| \sum_{l,r} 2^{-(l+r)} \sum_{x_0 \in \mathcal{X}_{j,l}} \sum_{t_0 \in \mathcal{T}_{j,r}} t_0^{\frac{d-1}{2}} c(x_0,t_0) f_{x_0,t_0} \right\|_{L^p(M)} \lesssim 2^{j(\alpha(p) - 1 + d/p')} \left( \sum |c(x_0,t_0)|^p t_0^{d-1} \right)^{1/p}.
\end{equation}
%
Indeed, if we then set $c(x_0,t_0) = 2^{l+r} t_0^{- \frac{d-1}{2}}$ for $x_0 \in \mathcal{X}_{j,l}$ and $t_0 \in \mathcal{T}_{j,r}$, then we find that
%
\begin{align*}
    \| M_j^I \{ Q_j u \} \|_{L^p(M)} &= \left\| \sum f_{x_0,t_0} \right\|_{L^p(M)}\\
...
%
\[ \left\| \sum_{l,r} \sum_{k = 1}^\infty 2^{k \frac{d-1}{2}} \sum_{(x_0,t_0) \in \mathcal{E}_k} 2^{-(l+r)} f_{x_0,t_0} \right\|_{L^{p,\infty}(S^d)}^p \lesssim 2^{j[(d-1)p - d]} \sum_{k = 1}^\infty 2^{k(d-1)} \# \mathcal{E}_k. \]%
...
We now how to bound the operators $\{ M_j^{II} \}$, by a reduction to an endpoint local smoothing inequality, namely, the inequality that
%
\begin{equation} \label{thelocalsmoothinginequality}
    \| e^{2\pi i t P} f \|_{L^q(M) L^q_t[-1/2,1/2]} \lesssim \| f \|_{L^q_{\alpha(p') - 1/p'}}.
\end{equation}
%
This inequality is proved in Corollary 1.2 of \cite{LeeSeeger} for $p < 2(d-1)/(d+1)$, which covers the range of parameters studied in this paper. Alternatively, \eqref{TheGeneralLpBoundednessEquation}, which we proved for a general function $c$, can be used to prove \eqref{thelocalsmoothinginequality}, by a generalization of the method of Section 10 of \cite{HeoandNazarovandSeeger}. This completes the proof of \eqref{dyadicMainReulst}.

\begin{prop} \label{LocalSmoothingLargeTimesTheorem}
    We have
    %
    \[ \| M_j^{II} \{ Q_j u \} \|_{L^p(M)} \lesssim C_p(m) \| f \|_{L^p(M)}.  \]
\end{prop}
\begin{proof}
    For each $j$, the \emph{class} of operators of the form $\{ M_j^{II} \}$ formed from a multiplier $m$ with $C_p(m) < \infty$ is closed under taking adjoints. Indeed, if $M_j^{II}$ is obtained from $m$, then $(M_j^{II})^*$ is obtained from the multiplier $\overline{m}$. Because of this self-adjointness, if we can prove that for any multiplier $m$ satisfying the assumptions of the theorem,
    %
    \[ \| M_j^{II} \{ Q_j u \} \|_{L^{p'}(M)} \lesssim C_p(m) \| f \|_{L^{p'}(M)}, \]
    %
    ...
    %
    \begin{align*}
        |M_j^{II}| &= \left| \int_{-1/2}^{1/2} b_j^{II} e^{2 \pi i tP}\; dt \right|\\
    ...
    %
    \begin{align*}
        \| M_j^{II} \{ Q_j u \} \|_{L^{p'}(M)} &\lesssim C_p(m) 2^{j(1/p' - \alpha(p'))} \| e^{2 \pi i P} (Q_j u) \|_{L^{p'}_t L^{p'}_x}\\
    ...
    %
    \[ \| M_j^{II} \{ Q_j f \} \|_{L^{p'}(M)} \lesssim C_p(m) \| u \|_{L^{p'}(M)}. \qedhere \]
\end{proof}
...
\section{Combining Dyadic Estimates}

As of the last section, we have now completed the argument justifying that the operators $M_j$ are each separately bounded on $L^p(M)$, with operator norm given in inequality \eqref{dyadicMainReulst}. Our goal now is to prove \eqref{puttingTogetherPieces}, which will enable us to sum in $j$ to obtain the required general bound
%
\[ \| m(P) u \|_{L^p(M)} \lesssim C_p(m) \| u \|_{L^p(M)}. \]
%
This argument is made comparatively easier by the fact that the functions $\{ M_j \}$ are supported on different dyadic intervals, which means we can apply variants of Littlewood-Paley theory and other square function estimates. We use these tools to obtain an atomic decomposition for our input functions, inspired by the calculations in \cite{SeegerEndpointEstimatesManifold}, which will yield the required bound.

%Consider the multiplier $m(P)$, and a general input $u \in L^p(M)$. Write $u_j = Q_j u$, so that $m(P) u = \sum_j M_j u_j$. By Plancherel's theorem, we have
%
%\[ \| m(P) u \|_{L^2(M)} = \| \sum_j M_j u_j \|_{L^2(M)} \lesssim \left( \| M_j u_j \|_{L^2(M)}^2 \right)^{1/2}, \]
%
%and by the triangle inequality,
%
%\[ \| m(P) u \|_{L^1(M)} = \| M_j u_j \|_{L^1(M)} \lesssim \sum_j \| M_j u_j \|_{L^1(M)}. \]
%
%Interpolating these two inequalities gives that
%
%\[ \| m(P) u \|_{L^p(M)} \lesssim \left( \sum_j \| M_j u_j \|_{L^p(M)}^p \right)^{1/p} \lesssim C_p(m) \left( \sum_j \| u_j \|_{L^p(M)}^p \right)^{1/p}. \]
%
...
%
\[ m(P) = \sum M_j u_j = \sum_{\alpha,j,s} \sum_{W \in \mathcal{W}_{\alpha,s}} M_j a_{\alpha,j,s,W}. \]
%
We will find it convenient to reorder this sum as
%
\[ m(P) = \sum_{\alpha,j,s,l} \sum_{W \in \mathcal{W}_{\alpha,s,l}} M_j a_{\alpha,j,s,W}, \]
%
where $\mathcal{W}_{\alpha,s,l}$ are the cubes in $\mathcal{W}_{\alpha,s}$ with sidelength $2^{l-j}$. We then write $M_j = M_{j,l}^{\text{Short}} + M_{j,l}^{\text{Long}}$, where
%
\[ M_{j,l}^{\text{Short}} = \int b_j(t) \psi(2^{j-l} t) (Q_j \circ e^{2 \pi i t P} \circ Q_j)\; dt \]
%
and
%
\[ M_{j,l}^{\text{Long}} = \int b_j(t) \Big( 1 - \psi(2^{j-l} t) \Big) (Q_j \circ e^{2 \pi i t P} \circ Q_j)\; dt. \]
%
We bound the short and long range interactions separately.

The function $M_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \}$ is then concentrated on $W^*$. Since the sets $\{ W^* \}$ have bounded overlap, for each fixed $s$, we have a square root cancellation bound
% l >= 0, and the long range interactions become irrelevant for l >= j
\begin{align*}
    &\| \sum_{\alpha,j,l} M_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^2(M)} \lesssim \left( \sum_{\alpha,j,s,l} \| M_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^2(M)}^2 \right)^{1/2}.
\end{align*}
%
But our calculations in previous sections should already have shown (via interpolation between $L^p(M)$ and $L^{p'}(M)$) that
%
\[ \| M_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^2(M)} \lesssim C_p(M) \| a_{\alpha,j,s,W} \|_{L^2(M)}, \]
%
and so we have
%
\[ \| \sum_{\alpha,j,l} M_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^2(M)} \lesssim C_p(m) \left( \sum_{\alpha,j,s,l} \| a_{\alpha,j,s,W} \|_{L^2(M)}^2 \right)^{1/2} \lesssim C_p(m) 2^s |\Omega_s|^{1/2}. \]
%
Applying H\"{o}lder's inequality, since our inputs are supported on $\Omega_s$, we have
%
\[ \| \sum_{\alpha,j,l} M_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^p(M)} \lesssim C_p(m) 2^s |\Omega_s|^{1/p}. \]
%
Summing in $s$, we have
%
\[ \| \sum_{\alpha,j,s,l} M_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^p(M)} \lesssim C_p(m) \sum_s 2^s |\Omega_s|^{1/p} \lesssim C_p(m) \| Su \|_{L^p(M)} \lesssim C_p(m) \| u \|_{L^p(M)}. \]
%
...
%
\[ \| \sum_{\alpha,j,l} M_{j,l}^{\text{Long}} \{ a_{\alpha,j,s,W} \} \|_{L^p(M)} \lesssim \sum_{l \geq 0} \left( \sum_j \| \sum_{l(W) = l - j} M_{j,l}^{\text{Long}} \{ A_{j,W} \} \|_{L^p(M)}^p \right)^{1/p}. \]
%
Suppose we can prove that there exists $\varepsilon > 0$ such that
%
\[ \| \sum_{l(W) = l - j} M_{j,l}^{\text{Long}} \{ A_{j,W} \} \|_{L^p(M)} \lesssim C_p(m) 2^{-l \varepsilon} \left( \sum_{l(W) = l - j} |W| \| A_{j,W} \|_{L^\infty(M)}^p \right)^{1/p}. \]
%
Lemma BLAH then implies that this quantity is bounded by $C_p(m) 2^{-l \varepsilon} 2^s |\Omega_s|$, and summing in $j$ gives that
%
\[ \| \sum_{\alpha,j,l} M_{j,l}^{\text{Long}} \{ a_{\alpha,j,s,W} \} \|_{L^p(M)} \lesssim C_p(m) 2^{-l \varepsilon} 2^s |\Omega_s|. \]
%
Summing in $s$ then gives
%
\[ \| \sum_{\alpha,j,l} M_{j,l}^{\text{Long}} \{ a_{\alpha,j,s,W} \} \|_{L^p(M)} \lesssim C_p(m) \| Su \|_{L^p(M)} \lesssim C_p(m) \| u \|_{L^p(M)}, \]
%
...
\begin{lemma}
    ...
    Then
    %
    \[ \left\| \sum_k \sum_{(x_0,t_0) \in \mathcal{E}_k} 2^{k \frac{d-1}{2}} {S\!}_{x_0,t_0} \right\|_{L^2(S^d)}^2 \lesssim R^{d-2} \log(u) u^{\frac{2}{d-1}} \sum_k 2^{k(d-1)} \# \mathcal{E}_k. \]
\end{lemma}
