...
\begin{lemma}
    Consider the coordinate charts $\{ U_\alpha \}$ and $\{ V_\alpha \}$ introduces in Section \ref{estimatesforwavepackets}. Then we can write
    %
    \[ u_j = \sum_\alpha \sum_s \sum_{W \in \mathcal{W}_{\alpha,s}} a_{\alpha,j,s,W}. \]
    %
    For each $s$, $\mathcal{W}_{\alpha,s}$ is a union of almost disjoint dyadic cubes in the coordinate system $U_\alpha$, and the following properties hold:
    %
    \begin{itemize}
        \item If $W$ has sidelength $2^l$, then $a_{\alpha,j,s,W}$ is only nonzero if $l \geq -s$.

        \item For each such dyadic cube $W$, $a_{\alpha,j,s,W}$ is supported on the inverse image of $W$. We have
        %
        \[ \left( \sum_j \sum_\alpha \sum_{W \in \mathcal{W}_{\alpha,s}} \| a_{\alpha,j,s,W} \|_{L^2(M)} \right)^{1/2} \lesssim 2^s |\Omega_s|, \]

        \item For any assignment $\{ j(\alpha,W) \}$ for each $W \in \mathcal{W}_{\alpha,s}$, and any $0 \leq r \leq 2$, we have
        %
        \[ \left( \sum_\alpha \sum_{W \in \mathcal{W}_{\alpha,s}} |W| \| a_{\alpha,j(\alpha,W),W,s} \|_{L^\infty(M)}^p \right)^{1/p} \lesssim 2^s |\Omega_s|. \]
    \end{itemize}
\end{lemma}
...
%
where $\mathcal{W}_{\alpha,s,l}$ are the cubes in $\mathcal{W}_{\alpha,s}$ with sidelength $2^{l-j}$. We then write $T_j = T_{j,l}^{\text{Short}} + T_{j,l}^{\text{Long}}$, where
%
...
\begin{align*}
    &\| \sum_{\alpha,j,l} T_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^2(M)} \lesssim \left( \sum_{\alpha,j,s,l} \| T_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^2(M)}^2 \right)^{1/2}.
\end{align*}
%
...
and so we have
%
\[ \| \sum_{\alpha,j,l} T_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^2(M)} \lesssim C_p(m) \left( \sum_{\alpha,j,s,l} \| a_{\alpha,j,s,W} \|_{L^2(M)}^2 \right)^{1/2} \lesssim C_p(m) 2^s |\Omega_s|^{1/2}. \]
%
Applying H\"{o}lder's inequality, since our inputs are supported on $\Omega_s$, we have
%
\[ \| \sum_{\alpha,j,l} T_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^p(M)} \lesssim C_p(m) 2^s |\Omega_s|^{1/p}. \]
%
Summing in $s$, we have
%
\[ \| \sum_{\alpha,j,s,l} T_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^p(M)} \lesssim C_p(m) \sum_s 2^s |\Omega_s|^{1/p} \lesssim C_p(m) \| Su \|_{L^p(M)} \lesssim C_p(m) \| u \|_{L^p(M)}. \]
%
...
%
\[ \| \sum_{\alpha,j,l} T_{j,l}^{\text{Long}} \{ a_{\alpha,j,s,W} \} \|_{L^p(M)} \lesssim \sum_{l \geq 0} \left( \sum_j \| \sum_{l(W) = l - j} T_{j,l}^{\text{Long}} \{ A_{j,W} \} \|_{L^p(M)}^p \right)^{1/p}. \]
%
Suppose we can prove that there exists $\varepsilon > 0$ such that
%
\[ \| \sum_{l(W) = l - j} T_{j,l}^{\text{Long}} \{ A_{j,W} \} \|_{L^p(M)} \lesssim C_p(m) 2^{-l \varepsilon} \left( \sum_{l(W) = l - j} |W| \| A_{j,W} \|_{L^\infty(M)}^p \right)^{1/p}. \]
%
Lemma BLAH then implies that this quantity is bounded by $C_p(m) 2^{-l \varepsilon} 2^s |\Omega_s|$, and summing in $j$ gives that
%
\[ \| \sum_{\alpha,j,l} T_{j,l}^{\text{Long}} \{ a_{\alpha,j,s,W} \} \|_{L^p(M)} \lesssim C_p(m) 2^{-l \varepsilon} 2^s |\Omega_s|. \]
%
Summing in $s$ then gives
%
\[ \| \sum_{\alpha,j,l} T_{j,l}^{\text{Long}} \{ a_{\alpha,j,s,W} \} \|_{L^p(M)} \lesssim C_p(m) \| Su \|_{L^p(M)} \lesssim C_p(m) \| u \|_{L^p(M)}, \]
%
