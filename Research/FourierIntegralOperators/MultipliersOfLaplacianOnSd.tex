...
\begin{theorem} \label{maintheoremprime}
    ...
    %
    Here $\alpha(p) = (d-1)|1/p - 1/2|$, and $m_h(\lambda) = m(2^h \lambda) \chi(\lambda)$ for any fixed choice of a smooth, compactly supported function $\chi$ with support on $[1/2,2]$ for which $\sum \chi(2^h \lambda) = 1$. The implicit constants in all the inequalities here depend only on the manifold $M$ and exponent $p$.
\end{theorem}

One consequence of Theorem \ref{maintheoremprime} are new results characterizing the $L^p$ boundedness of \emph{zonal convolution operators} on the unit sphere $S^d$ in $\RR^{d+1}$. We recall that a zonal convolution operator is any Schwartz operator $T: C^\infty(S^d) \to C^\infty(S^d)^*$ which is invariant under the action of the rotation group $SO(d+1)$.
%For any such $T$, there exists a distribution $k$ on $[-1,1]$ such that
%
...
Theorem \ref{maintheoremprime} implies an effective characterization of those functions $m: [0,\infty) \to \RR$ such that the family of zonal convolution operators
%
\[ T_R = \sum_{l = 0}^\infty m(l/R) \mathcal{Q}_l. \]
%
are uniformly bounded in $L^p(S^d)$. This might be expected, given that if $P = \sqrt{-\Delta}$ is defined as above, then for any $f \in \mathcal{H}_k(S^d)$, $Pf = \sqrt{k(k + d)} f$. Moreover, the geodesic flow travels on great circles of $S^d$ at a uniform velocity, and because each great circle has length $2 \pi$, the flow is periodic with period $2 \pi$.
...
Other than the sphere, there are only two known families of compact manifolds with periodic geodesic flow, the compact symmetric spaces of rank one, and the Zoll manifolds. There are three types of compact symmetric spaces of rank one:
%
\begin{itemize}
    \item The real projective space $\RR \PP^n$, with dimension $n$.

    \item The complex projective space $\CC \PP^n$ of dimension $2n$.

    \item The quaternion projective space $\mathbb{H} \PP^n$ of dimension $4n$.
\end{itemize}
%
For each $\mathbb{K} \in \{ \RR, \CC, \mathbb{H} \}$, the projective spaces $\mathbb{K} \PP^n$ all have a natural Riemannian metric obtained by identifying the spaces as a quotient of $\mathbb{K}^{n+1}$. They then all have periodic geodesic flow, and so Theorem \ref{maintheoremprime} applies. The space $\RR \PP^n$ gives us an example of a manifold which is not simply connected to which Theorem \ref{maintheoremprime} applies. The spaces $\RR \PP^n$ and $\mathbb{H} \PP^n$ have constant sectional curvature 1 and 4 respectively, but the manifold $\CC \PP^n$ gives an example of a manifold to which Theorem \ref{maintheoremprime} with non constant sectional curvature, varying on the interval $[1,4]$.

\begin{remark}
    There is one other rank one compact symmetric space, the \emph{Cayley projective plane} $\mathbb{O} \PP^2$, a two dimensional space obtained by taking an associative subalgebra of the octonions. However, since this space is two dimensional, and so not covered by Theorem \ref{maintheoremprime}, which assumes the manifold we are working on has dimension $d \geq 4$.
\end{remark}

We currently only know of one other family of manifolds with periodic geodesic flow, the Zoll manifolds, and unlike the manifolds previously listed, these manifolds provide examples of spaces to which Theorem \ref{maintheoremprime} applies in which there is little additional symmetry to work with. Each Zoll manifold is diffeomorphic to the sphere $S^d$, but is equipped with a metric given by considering a small global pertubation of the metric on the sphere which preserves the periodicity of the geodesic flow. There is an infinite dimensional family of such pertubations, so there are many Zoll manifolds, in particular, many with non constant curvature.
...
\begin{proof}
    ...
    %
    \[ |m_j(\lambda)| \leq \| \langle t \rangle^{\alpha(p)} \widehat{m}_j(t) \|_{L^p(\RR)} \left( \int \langle t \rangle^{- p' \alpha(p)} \right)^{1/p'} \]
    %
    For $d \geq 4$ and for $p < 2d/(d+1)$, we have
    %
    \[ \left( \int \langle t \rangle^{- p' \alpha(p)} \right)^{1/p'} < \infty, \]
    %
    Summing in $j$, and using the almost disjoint support of the functions $\{ m_j \}$ thus gives $\| m \|_{L^\infty(\RR)} \lesssim C_p(m)$.
\end{proof}

\begin{remark}
    It is not important to our proof, but it might be interesting to remark that the H\"{o}lder inequality argument at the end of this proof generalizes to show that for an appropriate quantity $\varepsilon(p) < \infty$,
    %
    ...
    %
    \[ |m(\lambda_1) - m(\lambda_2)| \lesssim \sum\nolimits_j |m_j(\lambda_1 / 2^j) - m_j(\lambda_2 / 2^j)| \lesssim C_p(m) |\lambda_1 - \lambda_2|^{\varepsilon(p)}. \]
\end{remark}
...
%
\[ \left\| \sum_{l,r} \sum_{k = 1}^\infty 2^{k \frac{d-1}{2}} \sum_{(x_0,t_0) \in \mathcal{E}_k} 2^{-(l+r)} f_{x_0,t_0} \right\|_{L^{p,\infty}(M)}^p \lesssim 2^{j[(d-1)p - d]} \sum_{k = 1}^\infty 2^{k(d-1)} \# \mathcal{E}_k. \]%
...
%
\[ \left| \left\{ x: \left|\sum_{k,l,r} 2^{k \frac{d-1}{2}} f_{x_0,t_0}(x)\right| \geq \lambda \right\} \right| \lesssim \lambda^{-p} 2^{j[(d-1)p - d]} \sum_k 2^{k(d-1)} \# \mathcal{E}_k. \]
%
The case $\lambda \lesssim 2^{j(d-1)}$ follows from the $L^1$ boundedness we've already proved. To prove the inequality when $\lambda \gtrsim 2^{j(d-1)}$, we employ the method of density decompositions introduced in \cite{HeoandNazarovandSeeger}. Let
%
...
%
\[ \left\| \sum_k \sum_{(x_0,t_0) \in \widehat{\mathcal{E}}_k} 2^{k \frac{d-1}{2}} {S\!}_{x_0,t_0} \right\|_{L^2(M)}^2 \lesssim_p R^{d-2} \log(A) A^{\frac{2}{d-1}} \sum_k 2^{k(d-1)} \# \mathcal{E}_k. \]
%
...
%
\[ m(P) = \sum T_j u_j = \sum_{\alpha,j,s} \sum_{W \in \mathcal{W}_{\alpha,s}} T_j a_{\alpha,j,s,W}. \]
%
We will find it convenient to reorder this sum as
%
\[ m(P) = \sum_{\alpha,j,s,l} \sum_{W \in \mathcal{W}_{\alpha,s,l}} T_j a_{\alpha,j,s,W}, \]
%
...
The function $T_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \}$ is then concentrated on $W^*$. Since the sets $\{ W^* \}$ have bounded overlap, for each fixed $s$, we have a square root cancellation bound
% l >= 0, and the long range interactions become irrelevant for l >= j
\begin{align*}
    &\left\| \sum_{\alpha,j,l} T_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \right\|_{L^2(M)} \lesssim \left( \sum_{\alpha,j,s,l} \| T_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^2(M)}^2 \right)^{1/2}.
\end{align*}
%
But our calculations in previous sections should already have shown (via interpolation between $L^p(M)$ and $L^{p'}(M)$) that
%
\[ \| T_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \|_{L^2(M)} \lesssim C_p(M) \| a_{\alpha,j,s,W} \|_{L^2(M)}, \]
%
and so we have
%
\begin{align*}
    \left\| \sum_{\alpha,j,l} T_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \right\|_{L^2(M)} &\lesssim C_p(m) \left( \sum_{\alpha,j,s,l} \| a_{\alpha,j,s,W} \|_{L^2(M)}^2 \right)^{1/2}\\
&\lesssim C_p(m) 2^s |\Omega_s|^{1/2}.
\end{align*}
%
Applying H\"{o}lder's inequality, since our inputs are supported on $\Omega_s$, we have
%
\[ \left\| \sum_{\alpha,j,l} T_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \right\|_{L^p(M)} \lesssim C_p(m) 2^s |\Omega_s|^{1/p}. \]
%
Summing in $s$, we have
%
\begin{align*}
    \left\| \sum_{\alpha,j,s,l} T_{j,l}^{\text{Short}} \{ a_{\alpha,j,s,W} \} \right\|_{L^p(M)} &\lesssim C_p(m) \sum_s 2^s |\Omega_s|^{1/p} \lesssim C_p(m) \| u \|_{L^p(M)}.
\end{align*}
%
...
%
\[ \left\| \sum_{\alpha,j,l} T_{j,l}^{\text{Long}} \{ a_{\alpha,j,s,W} \} \right\|_{L^p(M)} \lesssim \sum_{l \geq 0} \left( \sum_j \| \sum_{l(W) = l - j} T_{j,l}^{\text{Long}} \{ A_{j,W} \} \|_{L^p(M)}^p \right)^{1/p}. \]
%
Suppose we can prove that there exists $\varepsilon > 0$ such that
%
...
%
Then Lemma \ref{atomicdecompositionlemma} implies that this quantity is bounded by $C_p(m) 2^{-l \varepsilon} 2^s |\Omega_s|$, and summing in $j$ gives that
%
\[ \left\| \sum_{\alpha,j,l} T_{j,l}^{\text{Long}} \{ a_{\alpha,j,s,W} \} \right\|_{L^p(M)} \lesssim C_p(m) 2^{-l \varepsilon} 2^s |\Omega_s|. \]
%
Summing in $s$ then gives
%
\[ \left\| \sum_{\alpha,j,l} T_{j,l}^{\text{Long}} \{ a_{\alpha,j,s,W} \} \right\|_{L^p(M)} \lesssim C_p(m) \| u \|_{L^p(M)}, \]
%
Putting these two bounds together gives $\| m(P) u \|_{L^p(M)} \lesssim C_p(m) \| u \|_{L^p(M)}$, which completes the proof.
