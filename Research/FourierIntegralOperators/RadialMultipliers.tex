...
To exploit the fact that $C_p(h)$ is finite in the proof, we must employ the Fourier transform of $h$ in some way. A standard method for obtaining an expression for $M_R$ in terms of the Fourier transform of $h$ is to apply the Fourier inversion formula to write
%
...
We can consider a cover
%
\[ \RR = \{ t: |t| < 100/R \} \cup \{ t: 50/R < |t| < 1/100 \} \cup \{ t: 1/200 < |t| < \infty \}, \]
%
 ...
 %   \[ |\nabla_w \{ \theta \cdot (x - w) + \phi(w,y,\xi) + t |\xi|_y \}| \geq |\theta| - |\nabla_w \phi(w,y,\xi)| \gtrsim R, \]
  ...
  %  and in the latter,
   ...
   % \[ |\nabla_w \{ \theta \cdot (x - w) + \phi(w,y,\xi) + t |\xi|_y \}| \geq |\nabla_w \phi(w,y,\xi)| - |\theta| \gtrsim |\xi|. \]
    ...
    \[ s(t,x,y,\xi) \approx c \]
    %
    If we define $a(\xi) = a_1(\xi) a_2(\xi)$, where
    %
    \begin{align*}
        a_1(\xi) &= \int (\eta_\alpha E_{x_1})(w) \sigma(w,\theta) s(x) e^{2 \pi i [\theta \cdot (w - x) + (x - x_1) \cdot \xi ]}\; d\theta\; dw\; dx \quad\text{and}\\
        a_2(\xi) &= \int c(t) s(t,y,\xi) \sigma(y,\zeta) E_{x_0}(z) e^{2 \pi i [ (\phi(t_0 - t_1, x_0, \xi) - \phi(t,y,\xi)) + \zeta \cdot (y - z)]}\; d\zeta\; dt\; dy\; dz,
    \end{align*}
    %
    then we can write the required integral as
    %
    \[ \int a(\xi) e^{2 \pi i [\phi(x_1,x_0,\xi) + t |\xi|_{x_0}]14}\; d\xi. \]
    %
    Then $\text{supp}(a) \subset \{ |\xi| \sim R \}$, and satisfy
    %
    ...
    %
    \[ \int a_1(\xi) a_2(\xi) e^{2 \pi i [x_1 \cdot \xi - \phi(t_0 - t_1,x_0,\xi)]}\; d\xi. \]
    %
