...
%
\[ A_{p,q}(h) = \left( \int |\widehat{h}_t(s)|^q (1 + |s|)^{(d-1)(1 - q/2)}\; ds \right)^{1/q}, \]
...
%
\begin{align*}
    T_R f = \int_{-1/2}^{1/2} [ R \widehat{h}(Rt) + H_R(t) ]  e^{2 \pi i t \sqrt{-\Delta}} f\; dt = I_R f + II_R f,
\end{align*}
%
where
%
\[ H_R(t) = R \left( \sum_{l \neq 0} \widehat{h}(R(t + l)) \right) = \sum_{l \neq 0} H_{R,l}(t). \]
%
Now
%
\begin{align*}
    ...
    &\quad\quad\lesssim R^{1/p^*} A_p(h).
\end{align*}
%
Since the $\{ H_{R,l} : l \neq 0 \}$ could in general be quite correlated with one another, the best thing we can do is apply H\"{o}lder's inequality to conclude that, provided $p < 2d/(d+1)$,
%
\begin{align*}
    \| H_R \|_{L^p[-1/2,1/2]} &\leq \sum_{l \neq 0} \| H_{R,l} \|_{L^p[-1/2,1/2]}\\
    &\lesssim A_p(h) R^{1/p^* - (d-1)(1/p - 1/2)} \left( \sum_{l \neq 0} |l|^{-(d-1)(p^*/p-p^*/2)} \right)^{1/p^*}\\
    &\lesssim A_p(h) R^{1/p^* -(d-1)(1/p - 1/2)}.
\end{align*}
%
A further application of H\"{o}lder's inequality shows that
%
\begin{align*}
    | II_R f | &= \left| \int_{-1/2}^{1/2} H_R(t) e^{2 \pi i t \sqrt{-\Delta}} f\; dt \right|\\
    &\leq A_p(h) R^{1/p^* - (d-1)(1/p - 1/2)} \left( \int_{-1/2}^{1/2} |e^{2 \pi i \sqrt{-\Delta}} f|^{p^*}\; dt \right)^{1/p^*}.
\end{align*}
%
Applying the endpoint local smoothing inequality, we conclude that
%
\begin{align*}
    \| II_R f \|_{L^{p^*}(M)} &\leq A_p(h) R^{1/p^* - (d-1)(1/p - 1/2)} \| e^{2 \pi i \sqrt{-\Delta}} f \|_{L^{p^*}_t L^{p^*}_x}\\
    &\lesssim A_p(h) R^{1/p^* - (d-1)(1/p - 1/2)} \| f \|_{L^{p^*}_{\alpha_{p^*}}(M)},
\end{align*}
%
where $\alpha_{p^*} = (d-1)(1/2 - 1/p^*) - 1/p^*$. But since $f$ is a linear combination of eigenfunctions with eigenvalue $\sim R$, we conclude that
%
\[ \| II_R f \|_{L^{p^*}(M)} \lesssim A_p(h) \| f \|_{L^{p^*}(M)}. \]
%
We have therefore found a bound for $II_R$ which is uniform in $R$, and it remains to analyze $I_R$.

Let us now study the operator
%
\[ I_R = \int_{-1/2}^{1/2} R \widehat{h}(Rt) e^{2 \pi i t \sqrt{-\Delta}}. \]
%
Let us write $I_R = \sum_{|k| \lesssim R} I_{R,k}$, where
%
\[ I_{R,k} = \int \eta(R(t - k)) R \widehat{h}(Rt) e^{2 \pi i t \sqrt{-\Delta}} = \int a_{R,k}(t) e^{2 \pi i t \sqrt{-\Delta}}. \]
%
The fact that $h$ has compact support implies that each of the functions $\{ a_{R,k} \}$ is morally an indicator function supported on a time interval of length $\sim 1/R$, centered at the point $k/R$. Now in this case we also have that for $|k| \lesssim 1$,
%
\[ \| a_{R,k} \|_{L^p} \lesssim R^{1/p^*} A_p(h) \]
%
and
%
\begin{align*}
    &\left( \sum_{1 \lesssim |k| \lesssim R} |k|^{(d-1)(1 - p/2)} \| a_{R,k} \|_{L^p}^p \right)^{1/p}\\
    &\quad\quad \sim R^{-(d-1)(1/p - 1/2)} \left( \int R^p \sum_{0 < |k| \lesssim R} |Rk|^{(d-1)(1/p - 1/2)} |\eta(R(t-k))|^p |\widehat{h}(Rt)|^p \right)^{1/p}\\
    &\quad\quad \sim R^{1/p^* -(d-1)(1/p - 1/2)} \left( \int_{1 \lesssim |t| \lesssim R} |t|^{(d-1)(1/p - 1/2)} |\widehat{h}(Rt)|^p \right)^{1/p}\\
    &\quad\quad \lesssim R^{1/p^* - (d-1)(1/p - 1/2)} A_p(h).
\end{align*}
%
...
%
\[ I_{R,k} = m_k \Big( \sqrt{-\Delta} / R \Big), \]
%
where
% int eta(t) e^{- 2 pi i lambda (t + k)}
\[ m_k(\lambda) = \int h(\alpha) e^{- 2 \pi i (\lambda - \alpha) k} \widehat{\eta}(\lambda - \alpha)\; d\alpha. \]
%
Then
%
\[ |D^N m_k(\lambda)| \lesssim |k|^N \| h \|_{L^\infty(\RR)} \langle \lambda \rangle^{-M} \lesssim \| h \|_{L^\infty(\RR)} \langle \lambda \rangle^{-M}. \]
%
Thus we conclude $m_k$ is a symbol of order $-\infty$, uniformly in $k \lesssim 1$. But this implies that $I_{R,k}$ is a smoothing operator for $|k| \lesssim 1$, uniformly in $R$. Thus $\sum_{|k| \lesssim 1} I_{R,k}$ is a smoothing operator, and thus we immediately obtain $L^{p^*} \to L^{p^*}$ bounds which are uniform in $R$.

On the other hand, for $k \gtrsim 1$, we can apply the fixed time Sobolev estimates for the wave equation to conclude that, for $p \leq r$,
...
\end{align*}
%
Trivially applying the triangle inequality yields that
%
\begin{align*}
    \sum_{1 \lesssim |k| \lesssim R} \| I_{R,k} f \|_{L^{p^*}(M)} &\lesssim R^{(d-1)(1/2 - 1/p^*)-1/p^*} \left( \sum_{1 \lesssim |k| \lesssim R} \| a_{R,k} \|_{L^p} \right) \| f \|_{L^{p^*}}\\
    &\lesssim A_p(h) \left( \sum_{1 \lesssim |k| \lesssim R} |k|^{-(d-1)(p^*/p - p^*/2)} \right)^{1/p^*} \| f \|_{L^{p^*}}\\
    &\lesssim A_p(h) \| f \|_{L^{p^*}}.
\end{align*}
%
We have therefore bound the $L^{p^*}$ mapping properties of $I_R f$ uniformly in $R$.
...
Thus we conclude that
%
\[ |H_R(t)| \lesssim A_p(h) R^{1-(d-1)(1/2 - 1/p^*)}. \]
%
This means that
%
\[ \left| \int H_R(t) e^{2 \pi i t \sqrt{-\Delta}} f \right| \lesssim A_p(h) R^{1-(d-1)(1/2 - 1/p^*)} \int_{-1/2}^{1/2} |e^{2 \pi i t \sqrt{-\Delta}} f |\; dt. \]
...
Thus
%
\[ |H_R(t)| \lesssim A_p(h) \Big( R \langle Rs \rangle^{-(d-1)(1/2 - 1/p^*)} \Big). \]
%
Write
%
...
%
\begin{align*}
    |T_{R,k} f| &\lesssim A_p(h) (R 2^{-k(d-1)(1/2 - 1/p^*)}) (2^k/R)^{1/p} \left( \int_{|t| \sim 2^k/R} |e^{2 \pi i t \sqrt{-\Delta}} f|^{p^*}\; dt \right)^{1/p^*}.
\end{align*}
%
...
%
\begin{align*}
    \| T_{R,k} f \|_{L^{p^*}(M)} &\lesssim A_p(h) (R 2^{-k(d-1)(1/2 - 1/p^*)} ) (2^k/R) R^{\alpha_{p^*}} \| f \|_{L^{p^*}(M)}\\
    &\lesssim A_p(h) R^{\alpha_{p^*}} 2^{k[ 1 - (d-1)(1/2 - 1/p^*) ]} \| f \|_{L^{p^*}(M)}.
...
On the other hand, if we \emph{increase} the exponent in the integrability condition $A_p(h)$ by a $\varepsilon$, i.e. assuming the quantity
%
...
%
We now perform a decomposition of $T_R f$ in both frequency and time, writing
%
\[ T_R f = T_R^{\leq 0} + \sum_{k = 1}^\infty \sum_{n = 1}^\infty T_{R,k}^n \]
%
where $T_{R,k}^{\lambda}$ has kernel
%
\[ \int_{-1/2}^{1/2} \int \beta(Rt/2^k)  H_R(t) a(x,t,y,\xi) \beta(\xi / \lambda) e^{2 \pi i \Phi(t,x,y,\xi)}\; d\xi\; dt, \]
%
...
%
\begin{align*}
    \| T_{R,k}^{\lambda,\nu} f \|_{L^2(\RR^d)} &\lesssim \left( \int_{|t| \sim 2^k / R} H_R(t)\; dt \right) \| f^\lambda_\nu \|_{L^2(\RR^d)} \\
...
%
\[ T^{\lambda,\nu}_{R,k} f = \int_{|t| \sim 2^k / R} H_R(t) T^{\lambda,t}_{R,\nu} f, \]
%
...
%
\[ \| T_{R,k,\theta} f \|_{L^\infty(\RR^d)} \lesssim A_p(m) 2^{-3k(d+1) / 2} \| f \|_{L^\infty} \]
