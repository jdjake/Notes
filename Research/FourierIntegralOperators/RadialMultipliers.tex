...
%
where $\Delta = \sum \partial_i^2$ is the Laplacian on $\RR^d$. Thus the study of radial multipliers is closely connected to interactions between the spherical restriction operators
%
...
There are simple characterizations of the spaces $M^{p,q}(\RR^d)$ for very particular parameters of $p$ and $q$:
%
\begin{itemize}
...
%
Heuristically, $k$ must have $d/q^*$ derivatives in $L^q$. More precisely, if we introduce the homogeneous Besov space $\dot{B}^s_{p,r}(\RR^d)$ to be the family of all distributions $f$ for which the Besov norm
%
...
%
which gives weaker control over the behaviour of the multiplier $m$ for large $\xi$, but stronger control for small $\xi$. But for \emph{unit scale multipliers}, the two conditions are equivalent, i.e. because only $O(1)$ Littlewood-Paley projections are non-zero.
...
%
is finite. The main result of \cite{GarrigosandSeeger} was that if $d > 1$, if $1 < p < 2d/(d+1)$, and if $1 \leq p \leq q < 2$, then $M^{p,q}_{\text{rad}}(\RR^d)$ is a subset of $L^1_{\text{loc}}(\RR^d)$, and for any radial multiplier $m$, if $k$ it's convolution kernel, then
%
...
%
where the implicit constant is independent of $\delta$. We now use the existence of Kakeya sets, ala the proof that the ball multiplier problem is unbounded, to justify that such a bound cannot be possible when $2d/(d+1) \leq p \leq 2$ and where $q \geq p$. It will be more convenient to dualize, and show that for $2 \leq p \leq 2d/(d-1)$ and $q \geq p$, a bound of the form
%
...
\begin{remark}
    The argument above can still be applied for general values $2 \leq p \leq q \leq \infty$. If we use the disjointness of the $\{ T_\theta \}$, we have that
    %
    ...
    We should expect such an analysis, since the multiplier conjecture for a particular value of $p$ would imply the Bochner-Riesz conjecture for the same $p$. But \cite{Tao} shows this implies results for the restriction conjecture, and thus the Kakeya maximal conjecture.
\end{remark}
