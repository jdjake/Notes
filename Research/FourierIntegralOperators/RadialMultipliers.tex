...
Let's being by describing the new ideas introduced in \cite{KimQuasiradial}. Let $h$ and $a$ be as above. If $\beta$ is supported on $\{ 1/4 \leq |\xi| \leq 4 \}$, and equal to one on $\{ 1/2 \leq |\xi| \leq 2 \}$, then
%
\begin{align*}
...
%
where $E_\lambda$ is the projection operator onto the eigenspace corresponding to the eigenvalue $\lambda$. In particular, we wish to characterize the boundedness properties of the operators $M_R$, in terms of appropriate control of the Fourier transform of the function $h$. For a given exponent $1 \leq p \leq \infty$, let $s_p = (d-1)(1/p - 1/2)$. For $1 < p < 2d/(d+1)$, let us assume that the quantity
%
\[ C_p(h) = \left( \int_0^\infty \left[ \langle t \rangle^{s_p} |\widehat{h}(t)| \right]^p\; ds \right)^{1/p} \]
%
is finite. By the result of \cite{HeoandNazarovandSeeger}, we have
%
\[ \| h \|_{L^\infty(\RR)} \lesssim C_p(h), \]
%
so that $h$ is bounded, a fact we use repeatedly in the calculations to come. By Mitjagin's transference principle, the boundedness of $C_p(h)$ is necessary for the operators $\{ M_R \}$ to be uniformly bounded on $L^p(S^d)$. Moreover, the boundedness of $C_p(h)$ has already been proved sufficient provided the inputs are restricted to \emph{zonal} functions \cite{Alladi}. The main result of these notes is to extend this bound for general inputs to the operator.

\begin{theorem} \label{MainSphereTheorem}
    Suppose $1 \leq p < 2d/(d+1)$. If $R > 0$, $h$ is a bounded function, and $C_p(h) < \infty$, then for $f \in L^p(S^d)$, we have
    %
...
The projection operators $\{ E_\lambda \}$ are smoothing, and thus individually satisfy bounds of the form $\| E_\lambda f \|_{L^p(S^d)} \lesssim_\lambda \| f \|_{L^p(M)}$ for all $1 \leq p \leq \infty$. It thus follows trivially from the triangle inequality, and the fact that there are finitely many eigenvalues for $P$ in $[0,200]$, that for any $R \leq 100$,
%
\begin{align*}
...
\end{align*}
%
Thus, in the sequel, we will assume that $R \geq 100$.
...
\begin{proof}[Proof that Theorem \ref{RestrictedMainSphereTheorem} implies Theorem \ref{MainSphereTheorem}]
    Fix a smooth bump function $\beta$ supported on $\{ 1/4 \leq |t| \leq 4 \}$, and equal to one on $\{ 1/2 \leq |t| \leq 2 \}$. Set $\beta_R(\lambda) = \beta(\lambda / R)$, and consider the multiplier operator
    %
...
The main advantage of restricting our analysis to inputs $f \in V_R$ is that, since $P$ is a self-adjoint, elliptic pseudodifferential operator on $S^d$, we can, for heuristic sake, think of $f$ as being localized in frequency at a scale $R$. In particular, for any $s \geq 0$, we have derivative estimates of the form
%
\begin{equation} \label{ManifoldBernsteinInequality}
    \| f \|_{L^p_s(S^d)} \lesssim_s R^s \| f \|_{L^p(S^d)}
\end{equation}
%
which hold uniformly for $R \geq 100$ and $f \in V_R$; this is a manifold analogue of Bernstein's inequality (see Proposition 5.1 of \cite{Wolff} for the Euclidean version of Bernstein's inequality, and Section 3.3 of \cite{Sogge} for the proof of \eqref{ManifoldBernsteinInequality}).

One can obtain uniform $L^p$ bounds on $M_R f$ as in Theorem \ref{RestrictedMainSphereTheorem} given quantitative control over the derivatives of $f$. We give a simple version of this result here, which will be useful in the sequel.

\begin{lemma} \label{SmoothInputBounds}
    If $s > 2d + 1$, then for $1 \leq p \leq \infty$,
    %
...
\end{lemma}
\begin{proof}
    Without loss of generality, since $S^d$ has finite measure, we may assume $p = \infty$. Uniform bounds on $M_R f$ are easy to obtain if we have smoothness conditions on $f$. A simple calculation shows that if $f \in L^2_s(S^d)$ for $s > 2d + 1$, then $M_R f$ lies in $L^\infty(S^d)$, uniformly as we vary $R$. Indeed, using Sobolev embeddings we calculate that
    %
    \begin{align*}
        \| E_\lambda f \|_{L^\infty(S^d)} \lesssim \| E_\lambda f \|_{L^2_{d+1}(S^d)} \lesssim \| P^{d+1} f \|_{L^2(S^d)} \lesssim \lambda^{d+1} \| E_\lambda f \|_{L^2(S^d)} \lesssim \lambda^{d+1-s} \| f \|_{L^2_s(S^d)}.
    \end{align*}
    %
    The Weyl law implies that for $j \geq 1$, there are $O(2^{jd})$ eigenvalues of $P$ in the interval $[2^j,2^{j+1}]$, and so
    %
    \begin{align*}
        \left\| h \Big( \sqrt{-\Delta} / R \Big) f \right\|_{L^\infty(S^d)} &\lesssim \| h \|_{L^\infty(\RR)} \left( \| E_0 f \|_{L^\infty(S^d)} + \sum_{j = 1}^\infty \sum_{2^j \leq \lambda \leq 2^{j+1}} \| E_\lambda f \|_{L^\infty(S^d)} \right)\\
        &\lesssim \| h \|_{L^\infty(\RR)} \Big( 1 + \sum_{j = 1}^\infty 2^{jd} 2^{j(d+1 - s)} \Big) \| f \|_{L^2_s(S^d)}\\
        &\lesssim \| h \|_{L^\infty(\RR)} \| f \|_{L^2_s(S^d)} \lesssim C_p(h) \| f \|_{L^2_s(S^d)}.
    \end{align*}
    %
    We've therefore proved the first inequality. To prove the second inequality, we may assume without loss of generality that $f \in V_R$, using a similar method we used to reduce Theorem \ref{MainSphereTheorem} to Theorem \ref{RestrictedMainSphereTheorem}. We also assume $1 \leq p \leq 2$. Then it follows by Bernstein's inequality \eqref{ManifoldBernsteinInequality} and the fact that $S^d$ has finite mass that
    %
    \[ \| M_R f \|_{L^p(S^d)} \lesssim \| h \|_{L^\infty} \| f \|_{L^2_s(S^d)} \lesssim R^s \| h \|_{L^\infty} \| f \|_{L^2(S^d)} \lesssim R^s \| h \|_{L^\infty} \| f \|_{L^p(S^d)}. \]
    %
    Now that the result has been proved for $1 \leq p \leq 2$, the result for $2 \leq p \leq \infty$ is immediately implied by duality.
\end{proof}
...
To truly exploit the fact that $C_p(h)$ is finite (rather than simply employing $L^\infty$ bounds on $h$ as in Theorem \ref{SmoothInputBounds}), a condition controlling the behaviour of the Fourier transform of $h$, we apply the inversion formula to the sum defining $M_R$, writing
%
...
Before we move on to studying bounds for the operators $\{ II_R \}$, it will be helpful later on to study the behaviour of the wave-equation analysis of the projection operators $Q_R$. We write $Q_R = \sum Q_{R,\Pi}$, where
%
\[ Q_{R,\Pi} = \int \chi_\Pi(t) \widehat{\beta}_R(t) e^{2 \pi i t P}\; dt. \]
%
The method of proof in Theorem \ref{LocalSmoothingLargeTimesTheorem} also implies results for $Q_{R,III}$.

\begin{lemma} \label{LargeTimeQBounds}
    Let $Q_R = \beta \left( \sqrt{-\Delta} / R \right)$ as above, and define
    %
    \[ Q_{R,III} = \int \chi_{III}(t) \left[ R \widehat{\beta}(Rt) \right] e^{2 \pi i t P}\; dt. \]
    %
    Then
    %
    \[ \| Q_{R,III} f \|_{L^p(S^d)} \lesssim_N R^{-N} \| f \|_{L^p(S^d)} \]
\end{lemma}
\begin{proof}
    Define
    %
    \[ H_R(t) = \sum_l \chi_{III}(t) R \widehat{\beta}(R (t + l)). \]
    %
    Unlike $h$, the function $\beta$ is smooth, and thus $\widehat{\beta}$ is rapidly decaying, so that we conclude $|H_R(t)| \lesssim_N R^{-N}$. Employing local smoothing and H\"{o}lder's inequality as in the proof of Theorem \ref{LocalSmoothingLargeTimesTheorem} yields the result.
\end{proof}

If we combine Lemma \ref{LargeTimeQBounds} with Lemma \ref{SmoothInputBounds}, we conclude that
%
\[ \| (M_R \circ Q_{R,III}) f \|_{L^p(S^d)} \lesssim_s R^s \| Q_{R,III} f \|_{L^p(S^d)} \lesssim_N R^{s-N} \| f \|_{L^p(S^d)}. \]
%
Taking $N \geq s$ yields
%
\[ \| (M_R \circ Q_{R,III}) f \|_{L^p(S^d)} \lesssim \| f \|_{L^p(S^d)}. \]
%
To bounding $M_R \circ Q_R$ uniformly in $R$, it thus suffices to study $M_R \circ Q_{R,I}$ and $M_R \circ Q_{R,II}$.
...
It finally remains to analyze the operator $II_R \circ Q_{II,R}$, where
%
...
%
where $\text{supp}(\chi) \subset \{ |t| \leq 1 \}$. Since we are restricting our analysis to functions in the subspace $V_R$, it actually suffices to analyze the operator $II_R \circ Q_R$. This will take the bulk of our analysis, and to prevent notation from becoming too cumbersome later on, let us eschew uses of the subscript $R$ for now, writing the operator $II_R$ as
%
\[ II = \int b(t) e^{2 \pi i t P}\; dt, \]
%
and the operators $Q_R$ and $Q_{R,II}$ as $Q$ and $Q_{II}$. Essentially, we know the following facts about the function $b$:
%
\begin{itemize}
    % b_R(t) = chi(t) R h^(Rt)
...
To better understand the behaviour of the operator $II \circ Q$, we employ the Lax Parametrix $T_W$, an operator whose kernel is a finite sum of Fourier integrals, such that
%
\[ e^{2 \pi i t P} = T_W + T_A, \]
%
where $T_A$ is a smoothing operator. Let us use this decomposition to write $II = II_W + II_A$,
%
\[ II_W = \int b(t) T_W\; dt \quad\text{and}\quad II_A = \int b(t) T_A\; dt. \]
%
Similarily, we can write the operator $Q_{II}$ as $Q_W + Q_A$, where
%
\[ Q_W = \int \chi_{II}(t) R \widehat{\beta}(Rt) T_W\; dt \quad\text{and}\quad Q_A = \int \chi_{II}(t) R \widehat{\beta}(Rt) T_A\; dt. \]
%
...
It follows from this Lemma, together with the uniform boundedness of $Q$ by a constant independent of $R$, that the operator norm of $II_A \circ Q$ is uniformly bounded by a multiple of $C_p(h)$, independently of $R$. A similar analysis gives the boundedness of the operator $II_W \circ Q_A$, so that we need only focus on the operator $II_W \circ Q_W$.

\begin{lemma}
    We have
    %
    \[ \| (II_W \circ Q_A) f \|_{L^{p^*}(M)} \lesssim C_p(h) \| f \|_{L^{p^*}(M)}, \]
    %
    where the implicit constant is uniform in $R$.
\end{lemma}
\begin{proof}
    General properties of distributions, together with the smoothness of $A$, imply that the operator
    %
    \[ K_1(t,s,x,y) = \iint W(t,x,z) A(s,z,y)\; dz \]
    %
    is smooth. The periodicity of the wave operator on $S^d$ implies that $K_1(t,s,x,y)$ is periodic in the $t$ and $s$ variables, so we actually have bounds of the form
    %
    \[ |K_1(t,s,x,y)| \lesssim 1 \]
    %
    which are uniform over all variables. We can then write the kernel of $II_W \circ Q_A$ as
    %
    \[ K(x,y) = \iint \big[ b(t) \big] \cdot \big[ R \widehat{\beta}(Rs) \big] \cdot K_1(t,s,x,y)\; dt\; ds. \]
    %
    Thus. employing the uniform bound on $K_1$, we conclude that
    %
    \[ |K(x,y)| \lesssim \| b \|_{L^\infty} \int_{|t| \leq 1} R \widehat{\beta}(Rt)\; dt\; ds \lesssim \| b \|_{L^\infty} \lesssim C_p(h). \]
    %
    Applying Schur's Lemma as in the last proof gives the claim.
\end{proof}

The operator $II_W \circ Q_W$ is more tricky to analyze. We will obtain bounds for this operator given some restricted bounds. So let us consider an analysis of functions of the form $S = (II_W \circ Q_W) \{ E \}$, for some fixed measurable set $E \subset S^d$. We cover $S^d$ by a family of coordinate charts $\{ \theta \}$, and then perform a decomposition $II_W = \sum II_{W,i}$ and $Q_W = \sum Q_{W,j}$ by applying a partition of unity to the kernels, such that the following property holds: for any two indices $i$ and $j$, such that $II_{W,i} \circ Q_{W,j} \neq 0$, there exists a coordinate system $\theta$ such that the kernels of $II_{W,i}$ and $Q_{W,j}$ both have support on a compact subset of $\theta$ (here we are using the fact that we are only looking at the \emph{small time} behaviour of the operators with respect to the wave equation, so that the kernels of the operators are supported on a small neighborhood of the diagonal). The pushforward operator $T_W^\theta$ has a kernel $K_\theta$ defined by an oscillatory integral distribution
%
...
Consider a maximal $1/2R$ separated subset $\mathcal{X}$ of the ball of radius $O(1)$, which has cardinality $O(R^d)$. Then break $E$ down into a disjoint union of sets $\{ E_{x_0}: x_0 \in \mathcal{X} \}$, where for $x_0 \in \mathcal{X}$, the set $E_{x_0}$ is supported on the geodesic ball of radius $1/R$ about $x_0$. If we now write
%
\[ {S\!}_{x_0,t_0}(x) = \int b_{t_0}(t) (T_W \circ \mathcal{Q}) \{ E_{x_0} \}\; dt, \]
% 
then $S = \sum {S\!}_{x_0,t_0}$. We will begin by understanding the orthogonality properties of these functions in the decomposition of $S$.
