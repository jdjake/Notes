...
%
so that $h$ is bounded. By Mitjagin's transference principle, the boundedness of $C_p(h)$ is necessary for the operators $\{ M_R \}$ to be uniformly bounded on $L^p(S^d)$. Moreover, the boundedness of $C_p(h)$ has already been proved sufficient provided the inputs are restricted to \emph{zonal} functions \cite{Alladi}.
...
\begin{theorem} \label{MainSphereTheorem}
    ...
    %
    where the implicit constant is uniform in $R$, $h$, and $f$.
\end{theorem}

The projection operators $\{ E_\lambda \}$ are each smoothing operators, and thus individually bounded on $L^p(M)$ for all $1 \leq p \leq \infty$. It thus follows trivially from the triangle inequality, and the fact that there are finitely many eigenvalues for $P$ in $[0,200]$, that for any $R \leq 100$,
%
\begin{align*}
...
\begin{theorem} \label{RestrictedMainSphereTheorem}
    ...
    \[ \| M_R f \|_{L^p(S^d)} \lesssim C_p(h) \| f \|_{L^p(S^d)}, \]
    %
    where the implicit constant is uniform in $R$, $f$, and $h$.
\end{theorem}
...
\end{equation}
%
which hold uniformly for $R \geq 100$ and $f \in V_R$; this is a manifold analogue of Bernstein's inequality (see Proposition 5.1 of \cite{Wolff} for the Euclidean version of Bernstein's inequality, and Section 3.3 of \cite{Sogge} for the generalization to manifolds used here).
...
where
%
\[ b_R(t) = \chi(t) R \widehat{h}(Rt), \]
%
where $\text{supp}(\chi) \subset \{ |t| \leq 1 \}$. Since we are restricting our analysis to functions in the subspace $V_R$, it actually suffices to analyze the operator $II_R \circ Q_R$, where we recall that $Q_R = \beta ( \sqrt{-\Delta} / R  )$ for a fixed $\beta \in C_c^\infty(\RR)$.
...
To better understand the behaviour of the operator $II_R$, we employ the Lax Parametrix $T_W$, an operator whose kernel is a finite sum of Fourier integrals, such that
%
...
The operator $II_W$ proves more tricky to analyze. We will obtain bounds for this operator given some restricted bounds for the operator $II_W \circ Q$. So let us consider an analysis of functions of the form $S = (II_W \circ Q_R) \{ E \}$, for some fixed measurable set $E \subset S^d$. We can cover $S^d$ by a finite family of coordinate charts $\{ \theta \}$, and break down $II_W$ into a finite sum $II_{W,\theta}$, where the kernel of $II_{W,\theta}$ has $(x,y)$ support on a compact subset of $\theta$, and moreover, the pushforward operator $II_{W,\theta}^\theta$ on $\RR^d$ has a kernel $K_\theta$ which is defined by an oscillatory integral distribution
%
...
%
\[ {S\!}_{x_0,t_0}(x) = \int b_{t_0}(t) T_W \{ E_{x_0} \}(x,t)\; dt, \]
%
