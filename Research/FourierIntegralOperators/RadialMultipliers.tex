...
%
Littlewood-Paley theory tells us that this is an `almost orthogonal' decomposition as a family of operators from $L^p(\RR^d)$ to $L^q(\RR^d)$ for any $1 < p,q < \infty$, i.e. so that
%
\[ \| m \|_{M^{p,q}(\RR^d)} \sim_{p,q} \sup_{t > 0} t^{d(1/p - 1/q)} \| m_t \|_{M^{p,q}(\RR^d)}. \]
%
In the study of multipliers in the range $1 < p,q < \infty$, it is therefore completely natural to restrict to the study of multipliers with a symbol supported on the annulus $1/2 \leq |\xi| \leq 2$. In the sequel, we will call these \emph{unit scale multipliers}.
...
%
One can get results taking into account the oscillation of $k$ by combining this result with a Littlewood-Paley decomposition. Namely, one form of the result of H\"{o}rmander-Mikhlin states that if $m$ is a unit scale multiplier, then for $1 < p < \infty$,
%
\[ \| m \|_{M^p(\RR^d)} \lesssim_\alpha \| m \|_{L^2_\alpha}  \]
%
where $\alpha > d/2$.
...
%
Integration by parts then gives that $|k(x)| \lesssim |x|^{- \frac{d+1}{2}}$. Since $k$ is bounded near the origin (i.e. by the Paley-Wiener theorem), we conclude that $k \in L^q(\RR^d)$ for $q > 2d/(d+1)$. In particular, we conclude that  the radial multiplier conjecture cannot hold for $q > 2d/(d+1)$. TODO: Is there a counterexample for the endpoint $q = 2d/(d+1)$?
...
\chapter{Distributions on Manifolds}
...
%
Since $T_R$ is a multiplier with symbol supported on $R/2 \leq \lambda \leq 2R$, we may assume that $f$ is also supported on this frequency range, i.e. is in the span of eigenfunctions to $\sqrt{-\Delta}$ with eigenvalue $R/2 \leq \lambda \leq 2R$. In particular, this implies that for $R \geq 1$, we have derivative estimates of the form
%
\[ \| f \|_{L^p_\alpha} \lesssim_\alpha R^\alpha \| f \|_{L^p}, \]
%
for any $\alpha \geq 0$. This is a manifold analogue of Bernstein's inequality.
...
%
Fix a smooth function $\eta(t)$ with $\eta(t) = 1$ for $|t| \leq 1$ and with support on $|t| \leq 10$. Choose a smooth function $\chi_R$ supported on $1/R \leq |t| \leq 1$ such that
%
\[ 1 = \eta(Rt) + \chi_R(t) + \eta((3/4) t^{-1}). \]
%
Then
%
\begin{align*}
    T_R &= \int [\eta(Rt) + \chi_R(t) + \eta((3/4) t^{-1})] R \widehat{h}(Rt) e^{2 \pi i t \sqrt{-\Delta}}\; dt\\
    &= I_R + II_R + III_R.
\end{align*}
%
...
where
%
\[ H_R(t) = \sum_{l \neq 0} \eta((3/4) t^{-1}) R \widehat{h}(R(t + l)) = \sum_{l \neq 0} H_{R,l}(t). \]
%
Now
%
\begin{align*}
...
\begin{align*}
    ...
    &\lesssim C_p(h) R^{1/p^* - (d-1)(1/p - 1/2)} \| f \|_{L^{p^*}_{\alpha_{p^*}}(M)},
\end{align*}
%
where $\alpha_{p^*} = (d-1)(1/2 - 1/p^*) - 1/p^* = (d-1)(1/p - 1/2) - 1/p^*$. But since $f$ is a linear combination of eigenfunctions with eigenvalue $\sim R$, we have some estimates of the form
%
\[ \| f \|_{L^{p^*}_{\alpha_{p^*}}(M)} \lesssim R^{\alpha_{p^*}}. \]
%
Thus we conclude that
%
...
On the other hand, the operators $I_R$ and $II_R$ are obtained by integrating against the wave operators over times $|t| \leq 1$, and so from the speed of propogation of the wave equation, we should expect them to be roughly pseudolocal. The operator $I_R$ is obtained by integrating over a much smaller set of times $|t| \leq 1/R$, and from this it will follow that $I_R$ is actually a pseudodifferential operator of order zero, with symbol uniformly bounded in $R$, and thus satisfies uniform estimates of the form
%
\[ \| I_R f \|_{L^{p^*}(M)} \lesssim \left( \| h \|_{L^\infty(\RR)} + C_p(h) \right) \| f \|_{L^{p^*}(M)}. \]
%
To see this, let us write
%
\[ I_R = \int \eta(Rt) R \widehat{h}(Rt) e^{2 \pi i t \sqrt{-\Delta}}\; dt. \]
%
If $a$ is the inverse Fourier transform of $t \mapsto \eta(t) \widehat{h}(t)$, then
%
\[ I_R = a \left( \frac{\sqrt{-\Delta}}{R} \right). \]
%
If $\beta$ denotes the inverse Fourier transform of $\eta$, then we can write
%
\[ a(\lambda) = \int h(\alpha) \cdot \beta \left( \lambda - \alpha \right)\; d\alpha. \]
%
The fact that $h$ is compactly supported and that $\beta$ is Schwartz implies immediately that $a$ is a Schwartz function. Thus the functions $\{ a_R \}$ given by $a_R(\lambda) = a(\lambda/R)$ are uniformly bounded symbols of order zero, and thus
%
\[ I_R = a_R \Big( \sqrt{-\Delta} \Big) \]
%
are uniformly bounded from $L^p(M)$ to itself for all $1 < p < \infty$.
...
where
%
\[ \tilde{b}_R(t) = \chi_R(t) R \widehat{h}(Rt) = [ 1 - \eta(Rt) - \eta((3/4) t^{-1}) ] R \widehat{h}(Rt). \]
%
Our assumptions on the multiplier $m$ now amount to the fact that
%
\[ \| \tilde{b}_R \|_{L^p(\RR^d)} \leq R^{1/p^*} C_p(h), \]
%
that $\tilde{b}_R$ is supported on $1/R \lesssim |t| \leq 1/2$, and the heuristic fact that $\tilde{b}_R$ is locally constant at a scale $1/R$. Our goal is now to obtain bounds of the form
%
\[ \| II_R f \|_{L^{p^*}(M)} \leq R^{-1/p^*} \| \tilde{b}_R \|_{L^p(\RR)} \| f \|_{L^{p^*}(M)} \]
%
for all functions $f$ which are a linear combination of eigenfunctions for the Laplace-Beltrami operator on $M$ with eigenvalues $\sim R$. In particular, we can heuristically assume $f$ is also locally constant at a scale $1/R$.
...
\end{align*}
%
whereas we should also expect by speed of propogation and local constancy of $f$ that
%
\begin{align*}
...
One thing that will help us is introducing the Lax Parametrix for the wave equation, which is valid because we are analyzing the wave equation over a small set of times. To do this, we can find a finite family of coordinate systems covering $S^d$, such that, modulo a smoothing operator, we can write each operator $e^{2 \pi i t \sqrt{-\Delta}}$, for $|t| \leq 1$, as a finite sum of operators $\{ W \}$, each with an associated kernel $\{ K \}$ supported on a single coordinate system of the family, such that, in these coordinates, the kernel has the form
%
\[ K(x,y) = \int s(x,y,\xi,t) e^{2 \pi i \Phi(x,y,\xi,t)}\; d\xi, \]
%
where
%
\begin{itemize}
...
\end{itemize}
%
Using the Lax parametrix, we can thus write the kernel of $I_R$ as
%
\[ \int \tilde{b}_R(t) s(x,t,y,\xi) e^{2 \pi i \Phi(t,x,y,\xi)}\; d\xi. \]
%
We will attempt to use the method of Lee and Seeger, combined with the methods of Heo, Nazarov, and Seeger, to understand this kernel by discretizing at a scale $1/R$, and apply a variant of the Calderon-Zygmund decomposition.
...
%
\[ b_R(t) = \chi_R(t) \cdot R \cdot (\psi * \widehat{m})(Rt). \]
%
Since $m$ is compactly supported, it is heuristically locally constant, and since $\psi$ is compactly supported, $\psi * \widehat{m}$ should satisfy all the same estimates that $\widehat{m}$ does. Thus we may assume all the previous parts of our argument have been completely for $\psi * \widehat{m}$. Now consider the kernel
%
\[ K_R(x,y) = \int b_R(t) s(x,y,\xi,t) e^{2 \pi i \Phi(t,x,y,\xi)}\; d\xi, \]
%
It suffices to obtain estimates on the operator corresponding to this kernel, uniformly in $R$.
