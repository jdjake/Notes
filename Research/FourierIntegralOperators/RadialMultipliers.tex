...
We now cover $S^d$ by a finite family of coordinate charts $\{ (x_\alpha, U_\alpha) \}$, which contain enough overlap that we can find another two coverings $\{ V_\alpha \}$ and $\{ \tilde{V}_\alpha \}$ such that we have containments
%
\[ N(V_\alpha,1/50) \subset \tilde{V}_\alpha \quad\text{and}\quad N(\tilde{V}_\alpha, 1/50) \subset U_\alpha, \]
%
i.e. $\tilde{V}_\alpha$ contains the $1/50$-thickening of $V_\alpha$, and $U_\alpha$ the $1/50$ thickening of $\tilde{V}_\alpha$. Now consider a partition of unity $\{ \eta_\alpha \}$ subordinate to $\{ V_\alpha \}$, and a family of $C^\infty$ functions $\{ \tilde{\eta}_\alpha \}$ adapted to $\{ \tilde{V}_\alpha \}$, such that $\tilde{\eta}_\alpha(x) = 1$ for $x \in V_\alpha$.
...
%
Our goal will be to understand the orthogonality properties of the functions $\{ {S\!}_{x_0,t_0} \}$.

We calculate, using adjoints, that
%
\begin{align*}
...
%
i.e. the convolution of $b_{t_0}$ with the reflection of $\overline{b_{t_1}}$ about the $y$-axis, and we set $W'(u) = Q \circ W(u) \circ Q$, then
%
\begin{align*}
    & \int b_{t_0}(t) \overline{b_{t_1}(s)} \Big\langle (Q \circ W(t-s) \circ Q) \{ E_{x_0} \}, E_{x_1} \Big\rangle\\
    &\quad \quad = \int c_{t_0,t_1}(u) \langle W'(u) \{ E_{x_0} \}, E_{x_1} \rangle\; du.
\end{align*}
%
...
\begin{lemma}
    For each $\alpha$, there exists a Schwartz operator $Q_\alpha$ supported on $U_\alpha \times \tilde{V}_\alpha$, such that for $f \in C^\infty(\tilde{V}_\alpha)$, $\text{supp}(Q_\alpha f) \subset N(\text{supp}(f), 0.01)$, and the pushforward operator $Q_\alpha^{x_\alpha}$ is a pseudodifferential operator on $x_\alpha(U_\alpha)$ given by an oscillatory integral of the form
    %
    \[ \int \sigma(x,y,\xi) e^{2 \pi i \xi \cdot (x - y)}\; d\xi, \]
    %
    where 
    %
    ...
    %
    \[ |\partial^\alpha_x \partial^\beta_\xi \sigma(x,\xi)| \lesssim_N R^{-|\beta|} \quad\text{for $|\alpha|, |\beta| \leq N$}, \]
    %
...
\begin{proof}
    ...
    %
    Next, we note that for $f \in C^\infty(S^d)$ with $\text{supp}(f) \subset \tilde{V}_\alpha$, $\text{supp}(f) \subset U_\alpha$, and $W(t) f = W_\alpha(t) f$, from which it follows that the pushforward of $T_1$, restricted to inputs supported on $\tilde{V}_\alpha$, has a kernel representation
    %
    \begin{align*}
        T_1^{x_\alpha}(x,y) &= \int \rho(t) R \widehat{\beta}(Rt) W^{x_\alpha}(t,x,y)\; dt\\
        &= \int \int \rho(t) R \widehat{\beta}(Rt) s(t,x,y,\xi) e^{2 \pi i [\phi(x,y,\xi) + t |\xi_y|_g]}\; d\xi,
    \end{align*}
    %
    where $s$ is supported on $\{ d_g(x,y) \leq 0.01 \}$. If we let $s_0(\lambda,x,y,\xi)$ denote the inverse Fourier transform of $\rho(t) R \widehat{\beta}(Rt) a(t,x,y,\xi)$ in the $t$ variable, then we can write this integral as
    %
    \[ \int s_0(|\xi_y|_g,x,y,\xi) e^{2 \pi i \phi(x,y,\xi)}\; d\xi. \]
    %
    The function $s_0$ satisfies
    % Derivative in x does nothing
    % Derivative in y does nothing
    % Derivative in xi reduces decay like a symbol estimate
    % Smooth, rapid decay away from { |t| ~ 1 } , convolved with { |t| ~ R } supported function
    \[ | (1 - \chi(\lambda / R)) (\partial_x^\alpha \partial_y^\beta \partial_\xi^\kappa \partial_\lambda^\gamma s_0)(\lambda,x,y,\xi)| \lesssim_N R^{-N} |\xi|^{- \kappa} \]
    %
    and
    %
    \[ | \chi(\lambda/R) (\partial_x^\alpha \partial_y^\beta \partial_\xi^\kappa \partial_\lambda^\gamma b)(\lambda,x,y,\xi) | \lesssim |\xi|^{-\kappa} \]
\end{proof}
