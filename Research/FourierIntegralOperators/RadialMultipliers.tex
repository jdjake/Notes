...
Here we discuss a method due to Lax, which constructs a \emph{parametrix} for the equation $\partial_t = 2 \pi i P$ for $|t| \lesssim 1$, which is an operator given by \emph{Fourier integrals}. A similar parametrix exists for the wave equation $\partial_t^2 = - 4\pi^2 P$, obtained by using the fact that
%
\[ \cos\left( 2 \pi t \sqrt{-\Delta} \right) = (1/2) e^{2 \pi i \sqrt{-\Delta}} + (1/2) e^{-2 \pi i \sqrt{-\Delta}}. \]
%
The equation $\partial_t = 2 \pi i P$ is a pseudodifferential variant of a first order hyperbolic partial differential equation, which is where the idea first came to Lax. We hope to find a parametrix defined by an oscillatory integral of the form
%
...
\begin{lemma}
    For each $\mu$, each $m \in \mathcal{S}^\mu(\RR)$, and each $R \geq 100$, the operator $m(P/R)$ is a pseudodifferential operator of order $\mu$, and if we let $s_R \in \mathcal{S}^\mu(T^* M)$ denote it's symbol, then the family of symbols $\{ R^\mu s_R \}$ form a bounded subset of $\mathcal{S}^\mu(T^* M)$.
\end{lemma}
\begin{proof}
    Fix $\varepsilon > 0$, such that the Lax parametrix for $\{ e^{2 \pi i t P} \}$ exists up to times $|t| \leq 2 \varepsilon$. Consider a function $\rho \in \mathcal{S}(\RR)$ such that
    %
    ...
    \end{align*}
    %
    where $r_R$ is the inverse Fourier transform of the function $t \mapsto (1 - \widehat{\rho}(t)) [ R \widehat{m}(Rt) ]$. For $|t| \geq \varepsilon/2$, we have bounds of the form
    %
    \[ |D^N \widehat{m}(t)| \lesssim_{N,M} |t|^{-M}. \]
    %
    But this implies that
    %
    \[ |r_R(\lambda)| \lesssim_{N,M} R^{-M} \lambda^{-N}. \]
    %
    Now since we can write
    %
    \[ r_R(P)(x,y) = \sum r_R(\lambda_n) e_n(x) \overline{e_n(y)}, \]
    %
    we can use the Weyl formula, plus some crude estimates on the $L^\infty$ norm of the eigenfunctions $\{ e_n \}$ and their derivatives (e.g. using Sobolev embeddings plus a Bernstein type inequality) to conclude that
    %
    \[ |D^\alpha_x D^\beta_y r_R(P)(x,y)| \lesssim_{\alpha,\beta,N} R^{-N}. \]
    %
    Next, we discuss $(m_R * \rho)(P)$. Applying the Lax parametrix, we write
    %
    \[ (m_R * \rho)(P) = \sum_{W \in \mathcal{W}} T_{R,W} + S_R, \]
    %
    where for each $W \in \mathcal{W}$, there exists $a$ such that
    %
    \[ T_{R,W} f(G(x)) = \int R \widehat{m}(Rt) a(x,y,\eta,t) e^{2 \pi i [ \phi(x,y,\eta) + t p(y,\eta) ]} f(G(y))\; d\eta\; dy, \]
    %
    and
    %
    \begin{align*}
        S_R f(G(x)) &= \int \widehat{\rho}(t) [R \widehat{m}(Rt)] \alpha(t,x,y) f(G(y))\; dt\; dy\\
        &= \int \widehat{\rho}(t/R) \widehat{m}(t) \alpha(t/R,x,y) f(G(y))\; dt.
    \end{align*}
    %
    Buw the fact that $\alpha$ is smooth, and that it is multiplied against a compactly supported function in time, gives that
    %
    \[ |D^\alpha_x D^\beta_y S_R(x,y)| \lesssim_{\alpha,\beta} R. \]
    %
    Finally, it suffices to prove that each of the operators $\{ T_{R,W} \}$ is a pseudodifferential operator. To do this, it suffices to compute the symbol of the operator in the coordinate system given by the diffeomorphism $G$. Set
    %
    \[ c_{R,W}(x,y,\eta) = \int \widehat{\rho}(t) R \widehat{m}(Rt) a(x,y,\eta,t) e^{2 \pi i t p(y,\eta)}\; dt. \]
    %
    Then
    %
    \[ T_{R,W} f(G(x)) = \int c_{R,W}(x,y,\eta) e^{2 \pi i \phi(x,y,\eta)} f(G(y)). \]
    % muth derivative, R^{-mu} |eta|^mu

    \[ |c_{R,W}(x,y,\eta)| \lesssim R + R^{-\mu} |\eta|^\mu. \]
\end{proof}
...
We first argue that we can pretend that $g_{j,y}$ is localized to a $2^j / R$ ball centered at $y$. Then we do a density decomposition at a scale $2^j / R$.
