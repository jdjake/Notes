...
\begin{enumerate}
    ...
    %One can make some of this precise (reducing the bound in Theorem \ref{MainSphereTheorem} to a bound restricted to inputs in $V_R$) but we find it more convenient to use the projection operators $\{ Q_R \}$ explicitly.
...
%
We have some naive bounds for the propogators $\{ e^{2 \pi i t P} \}$, namely
%
\[ \| e^{2 \pi i t P} f \|_{L^2(S^d)} = \| f \|_{L^2(S^d)} \]
%
and
%
\[ \| e^{2 \pi i t P} f \|_{L^p(S^d)} \lesssim \| f \|_{L^2_s(S^d)}, \]
%
for $s > 2d$, which follows from the multiplier representation of the propogators.
...
\begin{proof}
    ...
    For a pair of H\"{o}lder conjugates $p$ and $q$, we have $s_q = - s_p$. Using the periodicity of the wave equation, i.e. that
    %
    ...
    \end{align*}
    %
    Since the family of functions $\{ H_{R,l} \}$ could in general be chosen arbitrarily, they can be quite correlated, and so we should expect H\"{o}lder's inequality should be efficient, in the worst case. Thus we conclude that, provided $p < 2d/(d+1)$, so that $q > 2d/(d-1)$, we have
    %
    \[ s_q q = (d-1)(q/2 - 1) > 1, \]
    %
    and so
...
It finally remains to analyze the operator $II_R \circ Q_R$, where
%
\[ II_R = \int \chi_{II}(t) R \widehat{h}(Rt) e^{2 \pi i t P}\; dt \] % \quad\text{and} \quad Q_R^{\text{Low}} = \int c_R(t) e^{2 \pi i t P}\; dt, \]
%
is obtained by integrating the wave propogators over times $100/R \leq |t| \leq 0.01$ respectively. To prevent notation from growing too cumbersome later on, let us eschew uses of the subscript $R$ in our operators for now, e.g. writing the operator $II_R$ as
%
\[ II = \int b(t) e^{2 \pi i t P}\; dt, \]
%
where $b(t) = \chi_{II}(t) R \widehat{h}(Rt)$.
...
We begin by covering $S^d$ by a finite family of coordinate charts $\{ U_\alpha \}$, which contain enough overlap that we can find $V_\alpha \subset \tilde{V}_\alpha \subset U_\alpha$ for each $\alpha$ such that $\tilde{V}_\alpha$ contains the $0.1$ thickening of $V_\alpha$, $U_\alpha$ the $0.1$ thickening of $\tilde{V}_\alpha$, and $\{ V_\alpha \}$ also covers $S^d$.
%
%\[ N(V_\alpha,0.1) \subset \tilde{V}_\alpha \quad\text{and}\quad N(\tilde{V}_\alpha, 0.1) \subset U_\alpha, \]
%
Now consider a partition of unity $\{ \eta_\alpha \}$ subordinate to $\{ V_\alpha \}$ %, and a family of $C^\infty$ functions $\{ \tilde{\eta}_\alpha \}$ adapted to $\{ \tilde{V}_\alpha \}$, such that $\tilde{\eta}_\alpha(x) = 1$ for $x \in V_\alpha$.

We will employ some restricted bounds, together with interpolation, to understand the operators $II \circ Q$. We thus introduce a set $E \subset S^d$ and try to obtain $L^{p,\infty}$ bounds on the function $S = (II_W \circ Q) \{ E \}$. Given that $Q$ already acts, heuristically, as a mollifier at a scale $1/R$, i.e. such that $Q \{ E \}$ is locally constant at a scale $1/R$, it is natural to discretize $E$ at this scale. Consider a maximal $1/2R$ separated subset $\mathcal{X}$ of $S^d$. Then break $E$ down into a disjoint union of sets $\{ E_{x_0}: x_0 \in \mathcal{X} \}$, where for $x_0 \in \mathcal{X}$, the set $E_{x_0}$ is supported on the geodesic ball of radius $1/R$ about $x_0$. Similarily, let $\mathcal{T}$ be all points in the lattice $\ZZ / 10 R$ lying in the set $\{ 100/R \leq |t| \leq 1 \}$, and for each $t_0 \in \mathcal{T}$, write
%
\[ b(t) = \sum_{t_0 \in \mathcal{T}} \chi_{t_0}(t) b(t) = \sum_{t_0 \in \mathcal{T}} b_{t_0}(t), \]
%
where for each $t_0 \in \mathcal{T}$, the function $\chi_{t_0}$ is smooth and adapted to the sidelength $1/R$ interval centered at $t_0$. We can then write
%
\[ S = \sum {S\!}_{x_0,t_0} \quad\text{where}\quad {S\!}_{x_0,t_0} = \int b_{t_0}(t) (W(t) \circ Q) \{ E_{x_0} \}\; dt. \]
%
Our first goal will be to understand the orthogonality properties of the functions $\{ {S\!}_{x_0,t_0} \}$, i.e. obtaining estimates on the inner products ${S\!}_{x_0,t_0}$.
...
To begin with, we can use adjoints to write
%
\begin{align*}
    ...
    &= \int c_{t_0,t_1}(u) \Big\langle (Q \circ e^{2 \pi i u P} \circ Q) \{ E_{x_0} \}, E_{x_1} \Big\rangle,
\end{align*}
%
\begin{comment}
...
%
is the convolution of $b_{t_0}$ with the reflection of $\overline{b_{t_1}}$ about the $y$-axis. Then $c_{t_0,t_1}$ satisfies estimates of the form
%
\[ \| c_{t_0,t_1} \|_{L^\infty(\RR)} \lesssim \| b_{t_0} \|_{L^1(\RR)} \| b_{t_1} \|_{L^1(\RR)}. \]
%
and is supported on the length $2/R$ interval centered at $t_0 - t_1$.

Next, we perform a partition of unity into the coordinate charts $\{ U_\alpha \}$, i.e. fixing $\alpha$, and consider the term
%
\[ \int c_{t_0,t_1}(u) \Big\langle (Q \circ e^{2 \pi i u P} \circ Q) \{ \eta_\alpha E_{x_0} \}, E_{x_0} \Big\rangle. \]
%
The next Lemma allows us to approximate the operator $Q$ with another operator more amenable to analysis in the coordinate system on $U_\alpha$, modulo an error which is negligible to our analysis.
...
\begin{lemma} \label{pseudodifferentialCoordinateLemma}
    For each $\alpha$, there exists a Schwartz operator $Q_\alpha$, with kernel supported on $U_\alpha \times \tilde{V}_\alpha$, such that for $f \in C^\infty(\tilde{V}_\alpha)$, $\text{supp}(Q_\alpha f) \subset N(\text{supp}(f), 0.01)$, and in the coordinate system of $U_\alpha$, $Q_\alpha$ is a pseudodifferential operator of order zero given by a symbol $\sigma(x,\xi)$, where
    %
    \[ \text{supp}(\sigma) \subset \{ (x,y,\xi) : x,y \in \tilde{V}_\alpha, |\xi| \sim_\alpha R \}, \]
    %
    where $\sigma$ satisfies derivative estimates of the form
    %
    \[ |\partial^\alpha_x \partial^\beta_\xi \sigma(x,\xi)| \lesssim_{\alpha,\beta} R^{-|\beta|}, \]
    %
    and such that for $f \in C^\infty(S^d)$ with $\text{supp}(f) \subset \tilde{V}_\alpha$,
    %
...
It follows from this analysis (TODO: Detail) that
%
\[ \left| \int c_{t_0,t_1}(u) \big\langle (Q \circ e^{2 \pi i u P} \circ (Q - Q_\alpha)) \{ \phi_\alpha E_{x_0} \}, E_{x_1} \big\rangle\; du \right| \lesssim_N C_p(h)^2 R^{-N}. \]
%
Thus, ignoring this negligible error, we proceed to analyzing the weighted averages of the operators $Q \circ e^{2 \pi i u P} \circ Q_\alpha$. Next, we introduce the Lax-H\"{o}rmander Parametrix for the wave propogators $\{ e^{2 \pi i u P} \}$ in the coordinate system on $U_\alpha$. For any $\alpha$, we can write $e^{2 \pi i u P} = W(u) + A(u)$, where $A(u)$ has a kernel $A(u,x,y)$ which is a smooth function of $u$, $x$, and $y$, and $W(u)$ is a Schwartz operator, with kernel
%
\[ \text{supp}(W(u)) \subset \{ (u,x,y) : d(x,y) \leq u + 0.01 \}, \]
%
and such that in the coordinate system of $U_\alpha$, the kernel of $W(u)$ is given by an oscillatory integral of the form
%
\[ W(u,x,y) = \int s(t,x,y,\xi) e^{2 \pi i [ \phi(x,y,\xi) + t |\xi_y|_g ]}, \]
%
where
%
\[ | \partial_\xi^\alpha \{ \phi(x,y,\xi) - \xi \cdot (x - y) \} | \lesssim_\alpha |x - y|^2 |\xi|^{1 - \alpha}, \]
%
and $|\nabla_\xi \phi(x,y,\xi)| \gtrsim |x - y|$ for $(x,y,\xi) \in \text{supp}(s)$. The smoothness of the kernel of $A(u)$ implies that
%
\[ \| \mathcal{P}_\lambda \{ A(u, \cdot, y) \} \|_{L^\infty(S^d)} \lesssim_N \lambda^{-N}, \]
%
...
%
\[ \left| \int c_{t_0,t_1}(u) \big\langle (Q \circ A(u) \circ Q_\alpha) \{ \phi_\alpha E_{x_0} \}, E_{x_1} \big\rangle\; du \right| \lesssim_N C_p(h)^2 R^{-N}, \]
%
another negligilbe error. Finally, using Lemma \ref{pseudodifferentialCoordinateLemma} again, we have
%
\[ \left| \int c_{t_0,t_1}(u) \big\langle (Q - Q_\alpha) \circ W(u) \circ Q_\alpha) \{ \phi_\alpha E_{x_0} \}, E_{x_1} \big\rangle\; du \right| \lesssim_N C_p(h)^2 R^{-N}. \]
%
Thus, ignoring negligible errors, we need only analyze
%
\[ \left| \int c_{t_0,t_1}(u) \big\langle (Q_\alpha \circ W(u) \circ Q_\alpha) \{ \phi_\alpha E_{x_0} \}, E_{x_1} \big\rangle\; du \right|. \]
%
The behaviour of all operations in this expression are now completely localized to $U_\alpha$ for inputs supported on $\tilde{V}_\alpha$ (in particular, the expression is equal to zero unless $E_{x_1}$ is contained in $U_\alpha$), and each operator has a natural expression in the coordinate system $U_\alpha$. So we can now naturally work with the kernels of the operators in coordinates.
...
\begin{proof}
    ...
    %
    \begin{align*}
        U(t,x,y) &= \int \sigma(x,\xi) s(t,z,w,\theta) \sigma(w,\eta)\\
        &\quad\quad\quad\quad\quad e^{2 \pi i [ \xi \cdot (x - z) + \phi(z,w,\theta) + t |\theta_w|_g + \zeta \cdot (w - y) ]}\\
        &\quad\quad\quad\quad\quad\quad\quad\; dz\; dw\; d\xi\; d\theta\; d\zeta.
    \end{align*}
    %
    ...
    %
    \[ \Phi(t,x,z,w,y,\xi,\theta,\zeta) = \xi \cdot (x - z) + \phi(z,w,\theta) + t |\theta_w|_g + \zeta \cdot (w - y), \]
    %
    then, using the fact that $\nabla_z \phi$ is non-vanishing on the support of $s$, and the homogeneity in $\theta$, we have $|(\nabla_z \phi)(z,w,\theta)| \sim |\theta|$. It follows that if $C > 0$ is chosen appropriately large, then on the support of the amplitude function, if $|\theta| \leq C^{-1} R$,
    %
    \[ |(\nabla_z \Phi)(t,x,z,w,y,\xi,\theta,\zeta)| = |(\nabla_z \phi)(z,w,\theta) - \xi| \gtrsim R, \]
    %
    ...
    %
    \begin{align*}
        \tilde{U}(t,x,y) &= \int \sigma(x,\xi) \left\{ \chi(\theta / R) s(t,z,w,\theta) \right\} \sigma(w,\eta)\\
        &\quad\quad\quad\quad\quad e^{2 \pi i [ \xi \cdot (x - z) + \phi(z,w,\theta) + t |\theta_w|_g + \zeta \cdot (w - y) ]}\\
        &\quad\quad\quad\quad\quad\quad\quad\; dz\; dw\; d\xi\; d\theta\; d\zeta,
    \end{align*}
    %
    ...
    %
    and we can rescale the integral defining $\tilde{U}$ in the $\xi$, $\theta$, and $\zeta$ variables, and write
    %
    \[ \tilde{U}(t,x,y) = R^{3d} \int a\; e^{2 \pi i R \Phi}\; dz\; dw\; d\xi\; d\theta\; d\zeta. \]
    %
    For each $w$, consider a diffeomorphism $\exp_w$ that expresses $z$ is geodesic normal coordinates, and consider a polar coordinate system in the $\theta$ variables with respect to the metric $g$, i.e. writing $\theta = \rho \tilde{\theta}$ with $|\theta_w|_g = \rho$. Letting $a'$ be the product of $a$ with the relevant Jacobians and $\rho^{d-1}$, we can write
    %
    \[ \tilde{U}(t,x,y) = R^{3d} \int a'\; e^{2 \pi i R \tilde{\Phi}} \; dz\; dw\; d\xi\; d\rho\; d\tilde{\theta}\; d\zeta. \]
    %
    where
    %
    \[ \tilde{\Phi}(t,x,z,w,y,\xi,\theta,\zeta) = \xi \cdot (x - \exp_w(z)) + \rho (\phi(\exp_w(z),w, \tilde{\theta}) + t) + \zeta \cdot (w - y). \]
    %
    Note that $a'$ satisfies all the same conditions that $a$ does. One nice feature of this coordinate change is that $\phi(\exp_w(z), w, \tilde{\theta}) = z \cdot \tilde{\theta}$ (TODO: DOUBLE CHECK THIS WITH AN EXPERT). In particular, this implies that, in the $\tilde{\theta}$ variable, this oscillatory integral has only two stationary points, i.e. when
    %
    \[ \tilde{\theta} = \pm \frac{z}{|z|}. \]
    %
    The principle of stationary phase thus implies that
    %
    \begin{align*}
...
\begin{remark}
    It might be enlightening to compare this bound to a rescaled version of the analogous bound in the Euclidean setting, obtained in Lemma 3.3 of \cite{HeoandNazarovandSeeger}. Consider a radial multiplier $m: \RR^d \to \CC$. The Fourier multiplier with symbol $m( \cdot / R)$ is then given by a convolution by $R^d\; k( R \cdot )$, where $k$ is the Fourier transform of $m$.
