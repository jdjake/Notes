...
%
\[ M t = \sup_{t \in I} |T_t f| \]
%
maps $L^p_c(Y)$ to $L^p_{\text{loc}}(X)$ if $\mu < -(n-1)(1/2 - 1/p) - (1/p - \delta(p,n))$. If the local smoothing conjecture held, we would conclude that, except at the endpoint $T^*$ has the same $L^p_c(Y)$ to $L^p_{\text{loc}}(X)$ mapping properties as each of the operators $T_t$. For the subsequence proof, we also get mixed norm estimates, such that for any finite interval $I$, the operator
%
\[ Sf(x) = \left( \int_I |T_t f(x)|^2\; dt \right)^{1/2} \]
%
if
%
\[ 2 \leq p \leq \quad\text{and}\quad q = 3p/(p-1) \quad\text{and}\quad \mu < -2(1/2 - 1/q), \]
%
is bounded from $L^p_c(Y)$ to $L^q_{\text{loc}}(X)$.

Our main reason to focus on this paper is the results of the latter half of the paper applying these techniques to radial multipliers on compact manifolds with periodic geodesics. Thus we consider a compact Riemannian manifold $M$, such that the geodesic flow is periodic with minimal period $2 \pi \Pi$. We consider $m \in L^\infty(\RR)$, such that $\sup_{s > 0} \| \beta \text{Dil}_s m \|_{L^2_\alpha(\RR)} = A_\alpha$ is finite for some $\alpha > 1/2$. We define a `radial multiplier' operator
%
\[ Tf = \sum m(\lambda) P_\lambda f \]
%
where $P_\lambda$ is the projection of $f$ onto the space of eigenfunctions for the operator $\sqrt{-\Delta}$ on $M$ with eigenvalue $\lambda$. We can also write this operator as $m(\sqrt{-\Delta})$. Then the wave propogation operator $e^{2 \pi i t \sqrt{-\Delta}}$ is periodic of period $\Pi$. The Weyl formula tells us that the number of eigenvalues of $\sqrt{-\Delta}$ which are smaller than $\lambda$ is equal to $V(M) \lambda^n + O(\lambda^{n-1})$.

\begin{theorem}
    Let $m \in L^2_\alpha(\RR)$ be supported on $(1,2)$, and assume $\alpha > 1/2$, then for $2 \leq p \leq 4$, and $f \in L^p(M)$,
    %
    \[ \left\| \sup_{2^i \leq \tau \leq 2^{i+1}} |\text{Dil}_\tau m(\sqrt{-\Delta}) f| \right\|_{L^p(M)} \lesssim_\alpha \| m \|_{L^2_\alpha(M)} \| f \|_{L^p(M)}. \]
\end{theorem}
\begin{proof}
    ...
    \begin{align*}
        ...
        &\leq \| m \|_{L^2_\alpha(M)} \left( \int_{-\infty}^\infty \frac{2^k}{(1 + |2^k t|^2)^\alpha} |e^{2 \pi i t \sqrt{-\Delta}} L_k f(x)|^2 \right)
    \end{align*}
    %
    Because of periodicity, it suffices to prove that for $\alpha > 1/2$,
    %
    \[ \left\| \left( \int_0^\Pi \frac{2^k}{(1 + |2^k t|^2)^\alpha} |e^{2 \pi i t \sqrt{-\Delta}} L_k f(x)|^2\; dt \right)^{1/2} \right\|_{L^p(M)} \lesssim_{\alpha,p} \| f \|_{L^p(M)}. \]
    %
    This is a weighted combination of the wave propogators, roughly speaking, assigning weight $2^i$ for $t \lesssim 1/2^i$, and assigning weight $1/t$ to values $t \gtrsim 1/2^k$. For a fixed $0 < \delta$, we can split this using a partition of unity into a region where $t \gtrsim \delta$ and a region where $t \lesssim \delta$, where $\delta$ is independent of $i$. The square function estimate above can be applied in the region where $t \gtrsim \delta$, because the weighted square integral above has weight $O_\delta(1)$ uniformly for all $i$. If we localize near $t \lesssim 1/2^k$ by multiplying by $\phi(2^k t)$ for some compactly supported smooth $\phi$ supported on $|t| \lesssim 1$, then the family of functions
    %
    ...
    For $t \lesssim \delta$, we localize around each $x$, so that we can use local coordinates, and apply the approximation to the wave equation to get more detailed information, i.e. finding a symbol $b \in S^0$ such that for $|t| \leq 2\delta$, we have $e^{2 \pi i t \sqrt{-\Delta}} = A_t + R_t$, where $R_t$ has $C^\infty$ kernel, uniformly for $0 \leq t \leq 2\delta$, and such that
    %
    \[ A_t g(x) = \int b(x,t,\xi) e^{2 \pi i (\xi \cdot x + t |\xi| + t^2 r(x,t,\eta))} \widehat{g}(\xi)\; d\xi, \]
    %
    where $r \in S^1$ is homogeneous in $\eta$. We have $\| R_t g \|_{L^p_x(M) L^2_t[0,\delta]} \lesssim 1$ because of the $C^\infty$ kernel. Applying $L_k$ gives a frequency cutoff, i.e. if $b_i(x,t,\xi) = b(x,t,\xi) \beta(\xi/2^i)$, then
    %
    \[ A_t L_k f(x) = \int b_i(x,t,\xi) e^{2 \pi i (\xi \cdot x + t|\xi| + t^2 r(x,t,\eta))} \widehat{f}(\eta)\; d\eta. \]
    %
    ...
    %
    \[ \left\| \left( \int \phi(2^j t) \frac{2^i}{(1 + |2^i t|^2)^\alpha} |A_t L_k f(x)|^2\; dt \right)^{1/2} \right\|_{L^p(M)}, \]
    %
