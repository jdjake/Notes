...
\part{Review of Square Function Techniques}


\chapter{Heo-Nasarov-Seeger: Extending Radial Multiplier Bounds to $\RR^d$}

Let's review how the method of atomic decompositions is used in \cite{HeoandNazarovandSeeger} to extend results for unit scale multipliers to a general result for multipliers. Let us consider a general multiplier $m$, and write
%
\[ m_j(\xi) = \eta(\xi) m(2^j \xi), \]
%
Let $H_j = \widehat{m}_j$, and let $K_j = 2^{jd} H_j(2^j \xi)$. Then our assumptions are that
%
\[ \| K_j \|_{L^p(\RR^d)} \leq A 2^{-jd(1 - 1/p)}. \]
%
Given a general input $f$, let us define $f_j = \eta(D / 2^j) f$. We utilize a non-tangential variant of Peetre's maximal square function
%
\[ Sf(x) = \left( \sum_j \left( 2^{jd} \sup_{|y| \lesssim 2^{-j}} |f_j(x + y)| \right)^2 \right)^{1/2}. \]
%
Then for $1 < p < \infty$,
%
\[ \| Sf \|_{L^p(\RR^d)} \lesssim \| f \|_{L^p(\RR^d)}. \]
%
We will try and show that
%
\[ \left\| \sum_j K_j * f_j \right\|_{L^p(\RR^d)} \lesssim A \| Sf \|_{L^p(\RR^d)}, \]
%
from which the result would follow.

An initial estimate is obtained from the cancellation of the forms $\{ K_j \}$, i.e.
%
\[ \| \sum_j \eta(D/2^j) f_j \|_{L^p(\RR^d)} \lesssim \| f_j \|_{l^p(\NN) L^p(\RR^d)}. \]
%
The inequality is easily obtained using Plancherel for $p = 2$, and the triangle inequality for $p = 1$. We complement this inequality with an estimate obtained from an `atomic decomposition' for $f_j$. We let
%
\[ \Omega_s = \{ x : Sf(x) > 2^s \} \]
%
and then set
%
\[ \Omega_s^* = \{ x : M \chi_{\Omega_s} > 100^{-d} \}. \]
%
The weak $L^1$ boundedness for the maximal function implies $|\Omega_s^*| \lesssim |\Omega_s|$. We also let $\mathcal{Q}_s^j$ be the set of all dyadic cubes of sidelength $2^{-j}$, such that
%
\[ |Q \cap \Omega_s| \geq \frac{|Q|}{2} \quad\text{and}\quad |Q \cap \Omega_{s+1}| < \frac{|Q|}{2}. \]
%
We then perform a Whitney decomposition of $\Omega_s^*$ into dyadic cubes $\mathcal{W}_s$, i.e. a maximal set of cubes $W$ such that the $20$-fold dilate $W^*$ is contained in $\Omega_s^*$. Each cube in $\mathcal{Q}_s^j$ is contained in a unique cube in $\mathcal{W}_s$. This is because if $Q \in \mathcal{Q}_s^j$, then
%
\[ \frac{|Q^* \cap \Omega_s|}{|Q^*|} \geq 20^{-d} \frac{|Q \cap \Omega_s|}{|Q|} \geq \frac{20^{-d}}{2} \geq 100^{-d}.  \]
%
Thus $Q^* \subset \Omega_s^*$, implying $Q$ is contained in some cube in $\mathcal{W}_s$.

We now break our functions down into `atoms'. For $W \in \mathcal{W}_s$, set
%
\[ a_{j,s,W} = \sum_{\substack{Q \in \mathcal{Q}_s^j\\Q \subset W}} f_j \chi_Q. \]
%
We then consider `cumulative atoms'
%
\[ a_{j,W} = \sum_{W \in \mathcal{W}_s} A_{j,s,W}. \]
%
Then we have an atomic decomposition
%
\[ f_j = \sum_W a_{j,W} = \sum_s \sum_{W \in \mathcal{W}_s} a_{j,s,W}. \]
%
We have two important bounds for the atoms. First, we have that
%
\[ \left( \sum_{W \in \mathcal{W}_s} \sum_j \| a_{j,s,W} \|_{L^2}^2 \right) \lesssim 2^j |\Omega_j|^{1/2}. \]
%
To see this, we calculate that
%
\begin{align*}
    \sum_{W \in \mathcal{W}_s} \sum_j \| a_{j,s,W} \|_{L^2(\RR^d)}^2 &= \sum_s \sum_{Q \in \mathcal{Q}_s^j} \| f_j \chi_Q \|_{L^2(\RR^d)}^2\\
    &\leq \sum_s \sum_{Q \in \mathcal{Q}_s^j} |Q| \| f_j \chi_Q \|_{L^\infty(\RR^d)}^2\\
    &\lesssim \sum_s \sum_{Q \in \mathcal{Q}_s^j} |Q - \Omega_{j+1}| \| f_j \chi_Q \|_{L^\infty(\RR^d)}^2\\
    &\leq \int_{\Omega_s^* - \Omega_{s+1}} \sum_j \sup_{|y| \leq 2^{-j}} |f_j(x + y)|^2\; dx\\
    &\lesssim 2^{2s} |\Omega_s^*| \lesssim 2^{2s} |\Omega_s|.
\end{align*}
%
Another useful bound is that, for any assignment $j(W)$ for each $W \in \mathcal{W}_s$, and for each $0\leq p \leq 2$,
%
\[ \left( \sum_{W \in \mathcal{W}_s} |W|\; \| A_{j(W),s,W} \|_{L^\infty}^p \right)^{1/p} \lesssim 2^s |\Omega_j|^{1/p}. \]
%
To see this result, we simply note that for each $W \in \mathcal{W}_s$, and each $Q \in \mathcal{Q}_s^j$, $Q \subset \Omega_s^* - \Omega_{s+1}$,
%
\[ \| A_{j,s,W} \|_{L^\infty(\RR^d)} \lesssim \sup_{\substack{Q \in \mathcal{Q}_s^j \\ Q \subset W}} \| f_j \chi_Q \|_{L^\infty(\RR^d)} \leq \sup_{x \in \Omega_s^* - \Omega_{s+1}} |Sf(x)| \lesssim 2^s, \]
%
and $\sum_{W \in \mathcal{W}_s} |W| \leq |\Omega_j^*| \lesssim |\Omega_j|$.

We must argue that
%
\[ \left\| \sum_{s,j} \sum_{l \geq 0} \sum_{\substack{W \in \mathcal{W}_s \\ l(W) = 2^{l-j}}} K_j * A_{j,s,W} \right\|_{L^p(\RR^d)} \lesssim A \| Sf \|_{L^p(\RR^d)}. \]
%
Let us split the convolution operator $K_j$ into a `short range piece' and a `long range piece', writing $K_j = K_{j,l}^{\text{short}} + K_{j,l}^{\text{long}}$, where
%
\[ K_{j,l}^{\text{short}}(x) = 2^{jd} H_{j,l}^{\text{short}}(2^j x) \quad\text{and}\quad K_{j,l}^{\text{long}}(x) = 2^{jd} H_{j,l}^{\text{long}}(2^j x), \]
%
where 
%
\[ H_{j,l}^{\text{short}}(x) = H_{j,l}(x) \mathbf{I}(|x| \leq 2^l) \quad\text{and}\quad H_{j,l}^{\text{long}}(x) = H_{j,l}(x) \mathbf{I}(|x| > 2^l). \]
%
It suffices to show that for $1 \leq q \leq 2$, and each fixed $s$,
%
\[ \left\| \sum_j \sum_l \sum_{\substack{W \in \mathcal{W}_s\\l(W) = 2^{l-s}}} K_{j,l}^{\text{short}} * A_{j,s,W} \right\|_{L^q(\RR^d)}^q \lesssim B 2^{sq} |\Omega_s|. \]
%
This is because, setting $q = p$,
%
\[ \sum_s 2^{sp} |\Omega_s| \lesssim \| Sf \|_{L^p(\RR^d)}^p. \]
%
It suffices to prove the result for $q = 2$, since the result for $q < 2$ follows by H\"{o}lder's inequality. To do this, we note that for a radial kernel $k$ whose Fourier transform decays rapidly away from the unit annulus, and $p < 2d/(d+1)$,
%
\[ \| \widehat{k} \|_{L^\infty} \lesssim \| k \|_{L^p(\RR^d)}. \]
%
The result follows from Bessel asymptotics. Since the sets $\{ W^* : W \in \mathcal{W}_s \}$ have bounded overlap, and with Parseval,
%
\begin{align*}
    \left\| \sum_{W \in \mathcal{W}_s} \sum_j K_{j,l(W) + s}^{\text{short}} * A_{j,s,W} \right\|^2_{L^2(\RR^d)}\\
    &\lesssim \sum_{W \in \mathcal{W}_s} \left\| \sum_s K_{j,l(W) + s}^{\text{short}} * A_{j,s,W} \right\|_{L^2(\RR^d)}^2\\
    &\leq \sum_{W \in \mathcal{W}_s} \| \widehat{K}_{j,l(W) + s}^{\text{short}} \|_{L^\infty(\RR^d)}^2 \sum_{W \in \mathcal{W}_s} \sum_j \| A_{j,s,W} \|_{L^2(\RR^d)}^2.
\end{align*}
%
But we know this last quantity is bounded by $B 2^{2j} |\Omega_j|$.

Now we look at the long range estimates. We are helped here by an exponential bound on the $L^p$ norm of smoothed convolution kernels supported on large radii. TODO: Finish Off last part of argument on Page 23, and Proposition 6.1.



\chapter{Seeger: Singular Convolution Operators in $L^p$ Spaces}

Let $m: \RR^d \to \CC$ be the symbol for a Fourier multiplier operator $m(D)$. If the operator was bounded from $L^p(\RR^d)$ to $L^p(\RR^d)$, then it would also be bounded `at all frequency scales'. That is, if we consider a Littlewood-Paley decomposition $1 = \sum \eta(\xi / 2^j)$, and define $m_j(\xi) = m(2^j \xi) \eta_j(\xi)$, i.e. so that
%
\[ m(\xi) = \sum_{j \geq 0} m_j(\xi / 2^j), \]
%
then we would have estimates of the form
%
\begin{equation} \label{piecewiseBound}
    \| m_j(D) f \|_{L^p(\RR^d)} \lesssim \| f \|_{L^p(\RR^d)},
\end{equation}
%
where the implicit constant is uniform in $j$. The main focus of the paper in question is to determine whether a uniform bound of the form \eqref{piecewiseBound} implies $m(D)$ is bounded. More precisely, for which values of $p$ is it true that
%
\begin{equation} \label{operatorbound}
    \| m \|_{M^p(\RR^d)} \lesssim \sup\nolimits_{j \geq 0} \| m_j \|_{M^p(\RR^d)}.
\end{equation}
%
It is clear that \eqref{operatorbound} is true for $p = 2$, since in this case \eqref{operatorbound} is equivalent to an inequality of the form
%
\[ \| m \|_{L^\infty(\RR^d)} \lesssim \sup_{j \geq 0} \| m_j \|_{L^\infty(\RR^d)}, \]
%
which holds because the supports of the symbols $\{ m_j \}$ have the bounded finite intersection property.

In $M^1(\RR)$, the result fails. The Hilbert transform $H$ is a Fourier multiplier with symbol $m(\xi) = \text{sgn}(\xi)$. For each $j > 0$, $m_j = \eta(\xi) \text{sgn}(\xi)$ is invariant of $j$, with inverse fourier transform given by $k = H \eta$. In particular,
%
\[ \| m_j \|_{M^1(\RR)} = \| k \|_{L^1(\RR)} = \| H \widehat{\eta} \|_{L^1(\RR)} \lesssim 1. \]
%
The multipliers $\{ m_j(D) \}$ are therefore uniformly bounded on $L^1(\RR)$. It follows that \eqref{operatorbound} cannot hold for $p = 1$, since $H = m(D)$ is not bounded on $L^1(\RR)$. A substitute in this setting is the H\"{o}rmander-Mikhlin multiplier criterion discussed in the introduction, which gives boundedness of $m$ in $L^{1,\infty}(\RR)$ given a slightly stronger assumption on the individual multipliers $\{ m_j \}$, i.e. given that for some $\varepsilon > 0$, the convolution kernels $\{ k_j \}$ associated with the multipliers $\{ m_j \}$ satisfy
%
\[ \sup_j \int \langle x \rangle^\varepsilon |k_j(x)|\; dx \leq B, \]
%
the theorem then guarantees that $\| m(D) f \|_{L^{1,\infty}(\RR)} \lesssim_\varepsilon B \| f \|_{L^1(\RR)}$.

For $1 < p < \infty$, with $p \neq 2$, \eqref{operatorbound} \emph{also fails}, due to a modification of an example of Littman, McCarthy, and Rivi\'{e}re, due to Triebel. For simplicity, let's work in $\RR$, though generalizations exist in higher dimensions. If we fix an even bump function $\phi \in C_c^\infty(\RR)$ supported on a small enough neighborhood of the origin, and set
%
\[ m_N(\xi) = \sum_{j = N}^{2N} e^{2 \pi i (2^j \xi)} \phi(\xi - 2^j), \]
%
then
%
\[ m_{N,j}(\xi) = m_N(2^j \xi) \eta(\xi) = e^{2 \pi i (4^j \xi)} \phi(2^j(\xi - 1)), \]
%
and thus the multiplier operator corresponding to $m_{N,j}$ has convolution kernel
%
\[ k_{N,j}(x) = 2^{-j} e^{2 \pi i (4^j + x)} \widehat{\phi}( 2^j + x / 2^j ) \]
%
Modulation and translation invariance of $L^1(\RR)$ imply that
%
\[ \| m_{N,j} \|_{M^p(\RR)} \leq \| m_{N,j} \|_{M^1(\RR)} = \| k_{N,j} \|_{L^1(\RR)} \lesssim 1 \quad\text{uniformly in $N$ and $j$}. \]
%
On the other hand, we have
%
\[ \| m_N \|_{M^p(\RR)} \gtrsim N^{|1/p - 1/2|}. \]
%
To see this, define a function $f_N$ such that
%
\[ \widehat{f}_N(\xi) = \sum_{j = N}^{2N} e^{- 2 \pi i 2^j \xi} \phi(\xi - 2^j). \]
%
Then by the compactness of $\phi$,
%
\[ m_N(\xi) \widehat{f}_N(\xi) = \sum_{j = N}^{2N} \phi(\xi - 2^j)^2. \]
%
This is a sum of functions supported on different dyadic frequency scales, and so Littlewood-Paley theory implies that square-root cancellation occurs, so that for $1 < p < \infty$,
%
\[ \| m_N(D) \{ f \} \|_{L^p(\RR)} \sim_p N^{1/2}. \]
%
On the other hand, we have, by orthogonality,
%
\[ \| f_N \|_{L^2(\RR)} = \| \widehat{f}_N \|_{L^2(\RR)} = N^{1/2}, \]
%
and by the triangle inequality,
%
\[ \| f_N \|_{L^1(\RR)} \lesssim N \quad\text{and}\quad \| f_N \|_{L^\infty(\RR)} \lesssim N. \]
%
Interpolation implies that
%
\[ \| f_N \|_{L^p(\RR)} \lesssim N^{1/2 + |1/p - 1/2|}. \]
%
But putting this together with the lower bound on the $L^p$ norm of $m_N(D) \{ f_N \}$ we conclude that
%
\[ \| m_N \|_{M^p(\RR)} \gtrsim N^{|1/p - 1/2|}. \]
%
The Baire category theorem can then produce a counterexample to \eqref{operatorbound}.

\begin{comment}
. On the other hand, \eqref{operatorbound} does not hold when $p = 1$ or $p = \infty$, which makes sense, since Littlewood-Paley runs into all kinds of problems for these values of $p$. Arguing more precisely, the condition would be equivalent to showing that for any $K: \RR^d \to \CC$,
%
...
In this paper, it is shown one can combine bounds at each scale in $L^p$, by assuming the H\"{o}rmander-Mikhlin condition, but with a logarithmic improvement to that obtained using the standard Calderon-Zygmund type argument.

\begin{theorem}
    Suppose
    %
    \[ \sup_{j > 0} \| m_j \|_{M^p(\RR^d)} \leq A \]
    %
    and
    %
    \[ \sup_{j > 0} \int \langle x \rangle^\varepsilon |k_j(x)| \leq B, \]
    %
    then
    %
    \[ \| m \|_{M^p(\RR^d)} \lesssim_{\varepsilon,p} A \widetilde{\log}(B/A)^{|1/p - 1/2|}. \]
\end{theorem}

\begin{remark}
    Any multiplier $m$ to which the theorem above applies satisfies the assumptions of the H\"{o}rmander-Mikhlin multiplier theorem, and so that theorem already guarantees that $m \in M^p(\RR^d)$. The gain in this theorem is the logarithmic dependance on the constant in the H\"{o}rmander-Mikhlin criterion.
\end{remark}

Triebel's example shows this result is tight. Indeed, since
%
\[ |D^\alpha m_N| \lesssim_\alpha (2^{2N})^{|\alpha|} \quad\text{for $\alpha \geq 0$}, \]
%
we can apply the theorem above with $B = 2^{2N}$, which gives
%
\[ \| m \|_{M^p(\RR)} \lesssim N^{|1/p - 1/2|}, \]
%
and we have already seen that $\| m \|_{M^p(\RR)} \gtrsim N^{|1/p - 1/2|}$. The usual H\"{o}rmander-Mikhlin multiplier argument would only be able to obtain bounds which were exponential in $N$.

Let us prove this result. Without loss of generality, we may assume $2 \leq p \leq \infty$. To prove the result, we rely on vector-valued variants of  Littlewood-Paley theory and the Fefferman-Stein sharp maximal function. Given a Banach space $X$, and an $X$-valued function $f:\RR^d \to X$, we define the sharp maximal function to be
%
\[ f^{\#}(x) = \sup_Q \fint_Q \| f(y) - f_Q \|_X\; dy \quad\text{where}\quad f_Q = \fint_Q f(y)\; dy. \]
%
For $p_0 \in [1,\infty]$ and $p \in (1,\infty]$ with $p \geq p_0$, the key fact about this function is that if $f \in L^{p_0}(\RR^d,X)$, and $f^\# \in L^p(\RR^d)$, then $Mf \in L^p(\RR^d)$, and
%
\[ \| Mf \|_{L^p(\RR^d)} \lesssim \| f^\# \|_{L^p(\RR^d)}, \]
%
where $M$ is the Hardy-Littlewood Maximal Function. To exploit this, we set $X = l^2(\NN)$, and define a vector valued operator $Sf = \{ m_j(D/2^j) f \}$. If we can show that
%
\[ \| (Sf)^\# \|_{L^p(\RR^d)} \lesssim A \widetilde{log}(B/A)^{1/2 - 1/p} \| f \|_{L^p(\RR^d)}, \]
%
then our proof would be completed, because it follows by Littlewood-Paley theory that, since $m(D) = \sum_j m_j(D/2^j)$,
%
\begin{align*}
    \| m(D) f \|_{L^p(\RR^d)} &\sim \| Sf \|_{L^p(\RR^d) l^2(\NN)}\\
    &\lesssim \| M(Sf) \|_{L^p(\RR^d)}\\
    &\lesssim \| S^\# f \|_{L^p(\RR^d)}\\
    &\lesssim A \widetilde{\log}(B/A)^{1/2 - 1/p} \| f \|_{L^p(\RR^d)}.
\end{align*}

%
Thus our goal is to prove the estimate for
%
\[ S^\# f(x) = \sup_{x \in Q} \fint_Q \left( \sum_{i = 0}^\infty \left| m_j(D/2^j) f(y) - \fint_Q m_j(D/2^j) f(z)\; dz \right|^2 \right)^{1/2}\; dy. \]
%
For the purposes of interpolation, we will help to  linearize $S^\#$ using duality, i.e. picking a dyadic cube $Q_x$ for each $x$, and a family of functions $\{ \chi_j(x,y) \}$ such that
%
\[ \left( \sum |\chi_j(x,y)|^2 \right)^{1/2} \leq 1, \]
%
chosen such that
%
\[ S^\# f(x) \approx \fint_{Q_x} \sum_{j = 0}^\infty \left( m_j(D / 2^j) f(y) - \fint_{Q_x} m_j(D / 2^j) f(z)\; dz \right) \chi_j(x,y)\; dy. \]
%
Also, let $N = \widetilde{\log}(B/A)$. Then we can consider a decomposition $S^\# f = S_1 f + S_2 f$, where if $l: \RR^d \to \ZZ$ is the function such that $Q_x$ has sidelength $2^{l(x)}$,
%
\[ S_1 f(x) = \sum_{|j + l(x)| \leq N} \fint_{Q_x}  \Big( m_j(D/2^j) f(y) - [m_j(D/2^j) f]_{Q_x} \Big) \chi_j(x,y)\; dy \]
%
and
%
\[ S_2 f(x) = \sum_{|j + l(x)| > N} \fint_{Q_x} \Big( m_j(D/2^j) f(y) - [ m_j(D/2^j) f ]_{Q_x} \Big) \chi_j(x,y)\; dy. \]
%
If $|j + l(x)|$ is small, then the uncertainty principle should tell us that $m_j(D/2^j) f$ is roughly constant on squares of radius $Q_x$, up to some small error, so not much cancellation can be expected to be obtained in the analysis of the function $S_1 f$. We will prove the more general fact that for any functions $\{ f_j \}$, if we define
%
\[ \tilde{S}_1 \{ f_j \}(x) = \sum_{|j + l(x)| \leq N} \fint_{Q_x}  \Big( \eta(D/2^j) f_j(y) - [ \eta(D/2^j) f_j ]_{Q_x} \Big) \chi_j(x,y)\; dy, \]
%
then $\| \tilde{S}_1 \{ f_j \} \|_{L^p(\RR^d)} \lesssim N^{1/2 - 1/p} \| f_j \|_{L^p(\RR^d) l^p(\NN)}$, and in our particular case, where $f_j = m_j(D/2^j) f$, we can use the uniform boundedness of the operators $\{ m_j(D/2^j) \}$ to argue that, since $L^p(\RR^d) l^p(\NN) = l^p(\NN) L^p(\RR^d)$,
%
\begin{align*}
    \| S_1 f \|_{L^p(\RR^d)} &\lesssim N^{1/2 - 1/p} \| m_j(D/2^j) f \|_{L^p(\RR^d) l^p(\RR^d)}\\
    &= N^{1/2 - 1/p} \| m_j(D/2^j) \eta(D/2^j) f \|_{L^p(\RR^d) l^p(\NN)}\\
    &\leq A N^{1/2 - 1/p} \| \eta(D/2^j) f \|_{l^p(\NN) L^p(\RR^d)}\\
    &\lesssim A N^{1/2 - 1/p} \| f \|_{L^p(\RR^d)}.
\end{align*}
%
To obtain the bound for $\tilde{S}_1$, we interpolate. We start by applying Cauchy-Schwartz, and then the result of Stein and Fefferman, to obtain a pointwise bound
%
\[ |\tilde{S}_1 \{ f_j \}| \lesssim M \left( \left( \sum_j |\eta(D/2^j) f_j|^2 \right)^{1/2} \right). \]
%
The operator $M$ is $L^2$ bounded, so we obtain
%
\begin{align*}
    \| \tilde{S}_1 \{ f_j \} \|_{L^2(\RR^d)} &\lesssim \Bigg\| \Big( \sum_j |\eta(D/2^j) f_j|^2 \Big)^{1/2} \Bigg\|_{L^2(\RR^d)}\\
    &= \| \eta_j(D/2^j) f_j \|_{L^2(\RR^d) l^2(\NN)}\\
    &\leq \| f_j \|_{L^2(\RR^d) l^2(\NN)}.
\end{align*}
%
For $p = \infty$, we apply the slightly more precise pointwise bound
%
\[ |\tilde{S}_1 \{ f_j \}| \lesssim \sup_l M \left( \left( \sum_{|j - l| \leq N} |\eta(D/2^j) f_j|^2 \right)^{1/2} \right), \]
%
and since $M$ is bounded in $L^\infty$,
%
\begin{align*}
    \| \tilde{S}_1 \{ f_j \} \|_{L^\infty} &\lesssim \sup_l \left\| \left( \sum_{|j - l| \leq N} |\eta(D/2^j) f_j|^2 \right)^{1/2} \right\|_{L^\infty(\RR^d)}\\
    &\lesssim N^{1/2} \sup_j \| \eta(D/2^j) f_j \|_{L^\infty(\RR^d)} \lesssim N^{1/2} \sup_j \| f_j \|_{L^\infty(\RR^d)}.
\end{align*}
%
Interpolating between the $L^2(\RR^d) l^2(\NN) \to L^2(\RR^d)$ and $L^\infty(\RR^d) l^\infty(\NN) \to L^\infty(\RR^d)$ bound gives the required bound.

The function $S_2 f$ is obtained by averaging the functions $m_j(D/2^j) f$ over cubes either much smaller than the frequency scale, or much larger. For small cubes, we should therefore be able to show the function is close to it's average, and for large cubes, we should be able to exploit some cancelation. We will again consider a operator of the form
%
\[ \tilde{S}_2 \{ f_j \}(x) = \sum_{|k + l(x)| > N} \fint_{Q_x} \left[ m_j(D/2^j) f_j(y) - [m_j(D/2^j) f_j]_{Q_x} \right] \chi_j(x,y)\; dy. \]
%
We will prove that
%
\[ \| \tilde{S}_2 \{ f_j \} \|_{L^p(\RR^d)} \lesssim A \| f_j \|_{L^p(\RR^d) l^2(\NN)}, \]
%
and then, with $f_j = \eta(D/2^j) f$, we conclude from the inequality above, and Littlewood-Paley, that
%
\[ \| S_2 f \|_{L^p(\RR^d)} \lesssim A \| \eta(D/2^j) f \|_{L^p(\RR^d) l^2(\NN)} \lesssim A \| f \|_{L^p(\RR^d)}. \]
%
This would then complete the proof of the argument.

Again, we obtain this result by interpolation between an $L^2$ bound, and an $L^\infty$ bound. To get $L^2$ boundedness, we use Cauchy-Schwartz to get rid of the functions $\{ \chi_j \}$, apply the $L^2$ boundedness of the maximal function, and Plancherel to exploit the disjointness of the supports of the symbols $\{ m_j(\cdot/2^j) \}$, to conclude that
%
\begin{align*}
    \| \tilde{S}_2 \{ f_j \} \|_{L^2(\RR^d)} &\lesssim \left\| M \left( \sum | m_j(D/2^j) f_j |^2 \right)^{1/2} \right\|_{L^2(\RR^d)}\\
    &\lesssim \| m_j(D/2^j) f_j \|_{L^2(\RR^d) l^2(\NN)}\\
    &= \left\| m \cdot \left( \sum_j |\widehat{f}_j|^2 \right)^{1/2} \right\|_{L^2(\RR^d)}\\
    &= \left\| m(D) \left\{ \mathcal{F}^{-1} \left( \sum_j |\widehat{f}_j|^2 \right)^{1/2} \right\} \right\|_{L^2(\RR^d)}\\
    &\leq A \left\| \mathcal{F}^{-1} \left( \sum_j |\widehat{f}_j|^2 \right)^{1/2} \right\|_{L^2(\RR^d)}\\
    &= A \| f_j \|_{L^2(\RR^d) l^2(\NN)}.
\end{align*}
%
It now suffices to prove a bound of the form $\| \tilde{S}_2 \{ f_j \} \| \lesssim A \| f_j \|_{L^\infty(\RR^d) l^2(\NN)}$, and we can then interpolate to get the required $L^p$ bound.

To obtain this bound, let's consider an upper bound $|\tilde{S}_2| \lesssim I + II$, where
%
\[ I(x) = \fint_{Q_x} \left( \sum_j |m_j(D/2^j) \{ Q_x^* f_j \}|^2 | \right)^{1/2} \]
%
and
%
\[ II(x) = \fint_{Q_x} \left( \sum_{|j + l(x)| > N} \Big| m_j(D/2^j) \{ (Q_x^*)^c f_j \}(y) - [ m_j(D/2^j) \{ (Q_x^*)^c f_j \} ]_{Q_x} \Big|^2 \right)^{1/2}\; dy. \]
%  
Indeed, both bounds can be obtained by applying Cauchy-Schwartz to get rid of the functions $\{ \chi_j \}$, and for $I$, we can discard the averaging terms $[ m_j(D/2^j) \{ Q_x^* f_j \}]_{Q_x}$ using the triangle inequality.

Applying H\"{o}lder's inequality and then Plancherel as before to exploit the disjointness of the $\{ m_j(D/2^j) \}$, we get that
%
\begin{align*}
    |I(x)| &\leq |Q_x|^{-1/2} \| m_j(D/2^j) \{ Q_x^* f_j \} \|_{l^2(\NN) L^2(\RR^d)}\\
    &\leq A |Q_x|^{-1/2} \| Q_x^* f_j \|_{l^2(\NN) L^2(\RR^d)}\\
    &\lesssim A \left( \fint_{Q_x} \sum |f_j|^2 \right)^{1/2}\\
    &\leq A \| f_j \|_{L^\infty(\RR^d) l^2(\NN)}.
\end{align*}
%
On the other hand, to bound $II$, we now exploit cancellation. Wr write
%
\begin{align*}
    &\Big| m_j(D/2^j) \{ (Q_x^*)^c f_j \}(y) - [ m_j(D/2^j) \{ (Q_x^*)^c f_j \} ]_{Q_x} \Big|^2\\
    &\quad \leq \sup_{y' \in Q_x} \Big| m_j(D/2^j) \{ (Q_x^*)^c f_j \}(y) - m_j(D/2^j) \{ (Q_x^*)^c f_j \}(y') \Big|.
\end{align*}
%
If we write $k_j$ for the convolution kernel of $m_j$, then we can write
%
\begin{align*}
    &\left| m_j(D/2^j) \{ (Q_x^*)^c f_j \}(y) - m_j(D/2^j) \{ (Q_x^*)^c f_j \}(y') \right|\\
    &\quad = \left| 2^{jd} \int_{(Q_x^*)^c} [k_j(2^j (y - z)) - k_j(2^j(y' - z)) ] f_j(z)\; dz \right|\\
    &\quad \leq 2^{jd} \| f_j \|_{L^\infty(\RR^d)} \int_{(Q_x^*)^c} | k_j(2^j(y - z)) - k_j(2^j(y' - z))|.
\end{align*}
%
Let us write
%
\[ E_j(x,y,y') = 2^{jd} \int_{(Q_x^*)^c} | k_j(2^j(y - z)) - k_j(2^j(y' - z))|\; dz. \]
%
Then we have
%
\[ |II(x)| \lesssim \sup_{y,y' \in Q_x} \left( \sum_{|k + l(x)| > N} |E_j(x,y,y')|^2 \right)^{1/2} \| f_j \|_{L^\infty(\RR^d)l^\infty(\NN)}. \]
%
We claim that $|E_j(x,y,y')| \lesssim B \min( 2^{-\varepsilon(j + l(x))}, 2^{j + l(x)} )$, to which we can geometrically sum up, using the fact that $|j+l(x)| > N$, to annihilate the $B$ term, and conclude that
%
\[ |II(x)| \lesssim A \| f_j \|_{L^\infty(\RR^d) l^\infty(\NN)}. \]
%
The bounds on $\{ E_j \}$ follow from our assumptions about the multipliers $\{ m_j \}$. Indeed, rescaling, we have
%
\begin{align*}
    E_j(x,y,y') &= \int_{2^j (Q_x^*)^c} | k_j(2^j y - z) - k_j(2^j y' - z)|\; dz\\
    &\leq 2 \int_{|z| > 2^{j + l(x)}} |k_j(z)|\\
    &\lesssim B 2^{-\varepsilon (j + l(x))}.
\end{align*}
%
On the other hand, if $j \leq - l(x)$, we can utilize the smoothness of $m_j$, which are uniformly supported on unit frequencies. Using Taylor's formula, we can write
%
\begin{align*}
    |k_j(2^j y - z) - k_j(2^j y' - z)| &= \left| \int_0^1 (\nabla k_j)(2^j (y + t (y - y')) - z) \cdot [2^j (y - y')]\; dt \right|\\
    &\lesssim 2^{j + l(x)} \int_0^1 |(\nabla k_j)(2^j (y + t(y - y')) - z)|
\end{align*}
%
and then for each $t$, we can write
%
\begin{align*}
    &\int_{2^j (Q_x^*)^c} | k_j(2^j (y + t(y - y')) - z)|\; dz\\
    &\quad \leq \| \nabla k_j \|_{L^1(\RR^d)} = \| \nabla \{ \eta * k_j \} \|_{L^1(\RR^d)} = \| (\nabla \eta) * k_j \|_{L^1(\RR^d)} \leq \| k_j \|_{L^1(\RR^d)} = B,
\end{align*}
%
which completes the argument for the required bound, and thus the proof of the entire theorem.
...
\chapter{Seeger: Square Function Techniques for Multipliers on Compact Manifolds}

In \cite{SeegerEndpointEstimatesMultipliers}, some square function estimates are used to extend bounds on unit scale multipliers, to more general multipliers.
...
%
% THIS IS THE RIGHT EXPONENT in R for p = 2
% Interpolate with right exponent in R for lower p?
% Can we get the right exponent for p = 1, at least
%       perhaps in the weak Lp sense, i.e. 
%       with an R^{-1} in the exponent?
%
% F^m_j has height O(R^d 2^m r_j) on an annulus of 
% measure (2^k / R)^{d-1} r_j
%           and sum r_j = #(E_k) / R 2^m
% 
...
\part{Papers to Read in More Detail}
