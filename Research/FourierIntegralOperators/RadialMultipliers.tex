...
%
\[ (\phi,\psi) \mapsto \int_M \phi(x) \psi(x)\; dV_g(x). \]
%
...
%
\[ (X,Y) \mapsto \int_M \langle X, Y \rangle_g\; dV_g. \]
%
...
%
\[ \int_M \langle X, \nabla_g f \rangle_g\; dV_g = - \int_M \langle \nabla_g \cdot X, f \rangle_g\; dV_g \]
%
...
%
Consider a new coordinate system $y = Tx$ for some invertible linear operator $T: \RR^d \to \RR^d$. The chain rule implies that if $f = T^* h = h \circ T$, then
%
\[ \nabla_g f = G^{-1} \cdot \nabla_x f = (G^{-1} T^t) (\nabla_x h \circ T), \]
%
and thus
...
%
If we choose $T$ such that $TG^{-1}T^t = I$, then we obtain that
%
\[ \Delta_g f = \Delta h \circ T, \]
%
where $\Delta h$ is the normal Laplacian of $h$. This happens precisely when $G = T^t T$. For functions $u$ on $\RR^{d+1}$, with $u = v \circ T$, we have
%
\[ \Box_g u = \Box v \circ T. \]
%
Now this means that if $E$ is the forward fundamental solution to $\Box$ on $\RR^{d+1}$, and we define $E_+ = (T^{-1})^* E$, then we claim $E_+$ is a fundamental solution to the operator $\Box_g$. Indeed, for $\phi \in \DD(\RR^d)$, if we set, for $\varepsilon > 0$,
%
\[ E_\varepsilon(x,t) = c_d \cdot \frac{H(t)}{\text{Im}(|x|^2 - (t + i \varepsilon)^2)^{\frac{d-1}{2}}}, \]
%
then using our change of variables calculations, we find that if $\phi = T^* \psi$, then
%
\begin{align*}
    \iint E_+(x,t) \Box_g \phi(x,t)\; dV_g(x)\; dt &= \lim_{\varepsilon \to 0} \iint (T^{-1})^* E_\varepsilon(x,t) \cdot \Box_g \phi(x,t)\; dV_g(x)\; dt\\
    &= \lim_{\varepsilon \to 0} \iint E_\varepsilon(y,t) \cdot \Box_g \phi(T^{-1} y,t)\; dy\; dt\\
    &= \lim_{\varepsilon \to 0} \iint E_\varepsilon(y,t) \cdot \Box \psi(y,t)\; dy\; dt\\
    &= \psi(0,0) = \phi(0,0).
\end{align*}
%
Thus $\Box_g E_+ = \delta_{0,0}$.
%
...
%
We also have a Fourier transform representation of this fundamental solution, namely we can write
%
...
%
The injectivity radius $r_0$ is then equal to the largest quantity $\varepsilon$ such that for all $q \in M$, the exponential map on a radius $r_0$ ball gives such a diffeomorphism. For each $q \in M$, the inverse map $x_q: B_{r_0}(q) \to T_q M$ gives a coordinate system on $M$, which we call the \emph{normal coordinates} at $q$.

As the tangent space of a Riemannian manifold, $T_q M$ is naturally an inner product space, and thus naturally comes equipped with a natural normalization of the Lebesgue measure, integration against which we will just denote using notation like $dx$ or $dy$. On the other hand, the normal coordinates equip $B \subset T_q M$ with an alternate Riemannian metric $g_B$, and we denote the induced volume form with respect to this metric by $dV_g$. We have
%
\[ \int_B \phi(a)\; dV_g = \int_B \phi(a) |G(a)|^{1/2}\; da. \]
%
The tangent space $TB$ can be naturally identified with $B \times T_q M$, such that for $a \in B$, $T_a B$ can be identified with $\{ v_a : v \in T_q M \}$. The metric $g_B$ thus induces a family of positive-definite transformations $G(a): T_q M \to T_q M$ for $a \in B$ such that for $v,w \in T_q M$,
%
\[ \langle v_a, w_a \rangle_{g_B} = \langle G(a) v, w \rangle_g. \]
%
The fact that we are using geodesic normal coordinates here gives us two useful properties of these transformations:
%
\begin{itemize}
    \item $G(0)$ is the identity map.

    \item (The Gauss Lemma) For each $a \in B$,
    %
    \[ G(a) a = G(0) a, \]
    %
    i.e. so that for $v \in T_q M$, $\langle a_a, v_a \rangle_{g_B} = \langle a, v \rangle_g$.
\end{itemize}
%
These equations are key to the construction of the Hadamard parametrix, which should be natural to see given that the wave equation propogates radially, and the Gauss Lemma tells us that in normal coordinates, the Riemannian metric is roughly constant when discussing radial tangent vectors, and their interactions with other vectors.

We also have a natural gradient operator $\nabla$ on $T_q M$, such that for a smooth function $f: B \to \CC$, $\nabla f: B \to T_q M$ is defined such that
%
\[ \lim_{a \to 0} \frac{f(a_0 + a) - f(a_0) - \langle \nabla f(a_0), a \rangle_g}{|a|_g} = 0. \]
%
If, abusing notation, we consider the gradient $\nabla_{g_B} f: B \to T_q M$ taken with respect to the Riemannian metric, then we have $(\nabla_{g_B} f)(a) = G(a)^{-1} \cdot \nabla f(a)$. Similarily, for a vector field $v: B \to T_q M$, we can consider the divergence $\nabla \cdot v: B \to \CC$, such that for compactly supported, smooth $f: B \to \CC$,
%
\[ \int_B (\nabla \cdot v)(x) f(x)\; dx = - \int_B \langle v(x), \nabla f(x) \rangle_g\; dx. \]
%
We then have $(\nabla_g \cdot v)(a) = |G(a)|^{-1/2} \left( \nabla \cdot \{ |G|^{1/2} v \} \right) (a)$.

Now let's start constructing the Hadamard parametrix. Applying the product rule, for $f: B \to \CC$, we have
%
\begin{align*}
    (\Delta_{g_B} f) &= (\nabla_{g_B} \cdot \nabla_{g_B} f)(a)\\
    &= \nabla \cdot \nabla_{g_B} f + \langle |G|^{-1/2} \nabla |G|^{1/2}, \nabla_{g_B} f \rangle_g\\
    &= \nabla \cdot \nabla_{g_B} f + \langle v, \nabla_{g_B} f \rangle_g\\
    &= Lf + Rf,
\end{align*}
%
where $v = |G|^{-1/2} \nabla |G|^{1/2}$. We view $Lf = \nabla \cdot \nabla_g f$ as a kind of `constant coefficient' second order equation, and $R f = \langle v, \nabla_g f \rangle_g$ as a kind of a `non-constant coefficient' first order equation.

This second term is precisely the problem with constructing a true fundamental solution. If we could make them disappear, then it would be easy to construct a fundamental solution for small times (TODO: This occurs when $|G|$ is constant on $B$ - what does this tell us about the geodesic flow on $M$). Indeed, set
%
\[ E(a,t) = c_d \cdot \frac{H(t)}{\text{Im} \left\{ (|a|^2_g - (t + i0)^2)^{\frac{d-1}{2}} \right\} }, \]
%
defined by taking weak limits of the associated smooth functions $\{ E_\varepsilon \}$ obtained by replacing the $0$ in the definition with a $\varepsilon$, where we identify the smooth functions with distributions with respect to the volume form $dx$ on $T_q M$. We have already argued that $\Box E = \delta$, where $\Box = \partial_t^2 - \nabla \cdot \nabla$. We claim that in general, we still have that for $|t| < r_0 - r$,
%
...
%
\[ \nabla f(a) = 2 F'(|a|^2_g) a = \nabla_g f(a). \]
%
But then if we take $E$ as the weak limit $E_\varepsilon$ of smooth, radial functions $\{ E_\varepsilon \}$ as we have done previously in this chapter, i.e. with $E_\varepsilon(x,t) = F(|x|^2_{g_0}, t)$, then this identity gives that
%
\begin{align*}
...
Suppose $\alpha_0 \in C^\infty(B)$. Suppose $E_\nu(x,t) = F_\nu(|x|_g^2,t)$. For $\nu = 0$, we can expand out $\Box_{g_B}$ to yield that
% |G|^{-1/2} nabla . nabla_g { alpha_0 E_0 }
% = 2 |G|^{-1/2}  < G(a)^{-1} nabla alpha_0, nabla E_0 >_g
\begin{align*}
    \Box_{g_B} \{ \alpha_0 E_0 \} &= \alpha_0 \partial_t^2 E_0 - \Delta_{g_B} \{ \alpha_0 E_0 \}\\
    &= \alpha_0 \partial_t^2 E_0 - \Delta_{g_B} \{ \alpha_0 \} E_0 - \alpha_0 \Delta_{g_B} \{ E_0 \} - 2 |G|^{-1/2} \langle G(a)^{-1} \nabla \alpha_0, \nabla E_0 \rangle_g\\
    &= \alpha_0 \delta_{0,0} - \Delta_{g_B} \{ \alpha_0 \} E_0 - \alpha_0 R \{ E_0 \} - 2 |G|^{-1/2} \langle G(a)^{-1} \nabla \alpha_0, \nabla E_0 \rangle_g\\
    &= \alpha_0 \delta_{0,0} - \Delta_{g_B} \{ \alpha_0 \} E_0 - |G|^{-1/2} \langle \alpha_0 \nabla |G|^{1/2} - 2 G(a)^{-1} \nabla \alpha_0 , \nabla E_0 \rangle_g\\
    &= \alpha_0 \delta_{0,0} - \Delta_{g_B} \{ \alpha_0 \} E_0 - 2 |G|^{-1/2} F_0' \cdot \langle \alpha_0 \nabla |G|^{1/2} - 2 G^{-1} \nabla \alpha_0 , a \rangle_g.
\end{align*}
%
Similarily, we have that for $a_\nu \in C^\infty(B)$,
%
\begin{align*}
    \Box_{g_B} \{ \alpha_\nu E_\nu \} &= \alpha_\nu \nu E_{\nu-1} - \Delta_{g_B} \{ \alpha_\nu \} E_\nu - 2 |G|^{-1/2} F_\nu' \langle \alpha_\nu \nabla |G|^{1/2} - 2 G^{-1} \nabla \alpha_\nu, a \rangle_g\\
    &= \alpha_\nu \nu E_{\nu-1} - \Delta_{g_B} \{ \alpha_\nu \} E_\nu + |G|^{-1/2} a E_{\nu-1} \langle \alpha_\nu \nabla |G|^{1/2} - 2 G^{-1} \nabla \alpha_\nu, a \rangle_g.
\end{align*}
%
Suppose we can choose $\alpha_0$ such that $\alpha_0(0) = 0$, and $\alpha_0 \nabla |G|^{1/2} = 2 G(a)^{-1} \nabla \alpha_0$. Then
%
\[ \Box_{g_B} \{ \alpha_0 E_0 \} = \delta_{0,0} - \Delta_{g_B} \{ \alpha_0 \} E_0. \]
%
If we successively choose $\{ \alpha_\nu : \nu \geq 1 \}$ such that
%
\[ |G|^{-1/2} a \langle \alpha_\nu \nabla |G|^{1/2} - 2 G^{-1} \nabla \alpha_\nu, a \rangle_g = \Delta_{g_B} \{ \alpha_{\nu-1} \}, \]
%
then if $S_N = \sum_{j = 1}^N \alpha_j E_j$, then
%
\[ \Box_{g_B} S_N = \delta_{0,0} - \Delta_{g_B} \{ \alpha_N \} E_{N+1}, \]
%
and we see that as $N \to \infty$, the difference between $\Box_{g_B} S_N$ and $\delta_{0,0}$ becomes smoother and smoother.
...
We now recall the Hadamard parametrix constructing, which we will make heavy use of in the sequel. Normally, this parametrix is written, modulo elements of $C^\infty(M \times M)$, by a distribution $E(x,y,t)$ given by an integral in normal coordinates about the point $y$ by an equation of the form
%
...
\begin{proof}
    ...
    %
    These derivative bounds are sufficient to conclude that the family of functions $a_R(\lambda) = a(\lambda/R)$ are uniformly bounded in the Fre\'{c}het space $\mathcal{S}^0(\RR)$ of order zero symbols, and so the operators $I_R$ are pseudodifferential operators of order zero, and uniformly bounded on $L^p(S^d)$ for all $1 < p < \infty$.
\end{proof}

\section{Analysis of the Operators $III_R$}

We now show the uniform boundedness of the operators $\{ III_R \}$ are uniformly bounded in $L^p(S^d)$ for the range of $p$ we are considering, by reducing this analysis to the endpoint local smoothing inequality. This might seem unintuitive, since the operators $III_R$ are obtained by averaging the wave equation over large times $|t| \gtrsim 1$, whereas local smoothing gives bounds for averages of the wave equation over times $|t| \lesssim 1$. But we will be able to reduce this analysis to a local set of times by exploiting the \emph{periodicity} of the half-wave equation on the sphere.
