...
%
One can use Bessel function asymptotics to convert this into a condition on the one-dimensional Fourier transform of $h$ (extended to an even function on $\RR$). Indeed, there exists a differential operator $L$ with constant coefficients and order at most $(d-1)/2$ such that we can write
%
\begin{align*}
...
%    \[ \sup_{t > 0} t^{d(1/p - 1/q)} \left( \int |\widehat{h}_t(s)|^q \langle s \rangle^{(d-1)(1 - q/2)} \; ds \right)^{1/q} < \infty. \]
    %
Continuing this calculation shows that if $h$ is supported at a unit scale, then the radial multiplier conjecture is equivalent to the condition that
%
\[ \| m \|_{M^{p,q}(\RR^d)} \sim_{p,q} \left( \int_0^\infty (1 + |s|)^{(d-1)(1 - q/2)} |\widehat{h}(s)|^q\; ds \right)^{1/q}. \]
%
...
Fix a geodesically complete Riemannian manifold $X$. The operator $\sqrt{-\Delta}$ is then essentially self-adjoint, and so we can apply the spectral calculus of unbounded operators to define $h(\sqrt{-\Delta})$ for each locally bounded, Borel measurable function $h$. These operators are analogues to the radial multipliers studied in the Euclidean setting. Just like multiplier operators on $\RR^n$ are crucial to an understanding of the interactions between the functions $e_\xi(x) = e^{2 \pi i \xi \cdot x}$ on $\RR^n$, understanding the operators $h(\sqrt{-\Delta})$ is crucial to understanding the interactions of eigenfunctions of the Laplace-Beltrami operator on $X$. We let $M^{p,q}(X, \sqrt{-\Delta} )$ denote the family of all symbols $h: \RR \to \CC$ such that the operator $T_h = h(\sqrt{-\Delta})$ extends to a bounded operator from $L^p(X)$ to $L^q(X)$, with the analogous operator norm, though, when there is no ambiguity, we will overload notation and write this space as $M^{p,q}(X)$.

% M^{1,q}(X) should be related to the L^q norm of the Fourier transform of h
% h(sqrt{-Delta}) f = int k(t) e^{2 pi i t sqrt(-Delta)} f
% <= |k|_{L^q}

To avoid technicalities, we will focus on the study of this problem on a compact Riemannian manifold $X$. Such manifolds are automatically geodesically complete. On such a manifold, there exists $0 = \lambda_1 < \lambda_2 \leq \dots$ with $\lambda_i \to \infty$, and an orthonormal family of eigenfunctions $\{ e_n \}$ in $C^\infty(X)$, forming a basis for $L^2(X)$, such that
%
\[ \Delta f = \sum - \lambda_n^2 \langle f, e_n \rangle \cdot e_n. \]
%
For any function $h: [0,\infty) \to \CC$, we then have
%
...
\[ \| E_\lambda f \|_{L^2(M)} \lesssim \lambda^{\frac{d-1}{2}} \| f \|_{L^1(M)} \]

\[ \left( \sum \| E_\lambda f \|_{L^2(M)}^2 \right)^{1/2} \lesssim \| f \|_{L^2(M)} \]
...
%
Thus we have found a simple characterization of the space $M^{2,2}_{\text{Dil}}(X)$. Just like on $\RR^d$, it is the space of bounded functions.
...
%
If $h$ is compactly supported, and $1 \leq q \leq 2$, then this condition is equivalent to the condition that
%
...
\end{align*}
%
where $W_t(x,y) = \sum_n e^{2 \pi i t \lambda_n} e_n(x) \overline{e_n(y)}$ is the kernel of the \emph{half-wave propogator} $e^{2 \pi i t \sqrt{-\Delta}}$ on $X$. The connection between radial multipliers on $X$ and the Fourier transform of their symbol is therefore closely related to the study of the half-wave equation $\partial_t = \sqrt{-\Delta}$ on $X$. An analogous characterization of the form
%
\[ \| h \|_{M^{1,q}(X)} \lesssim \left( \int (1 + |t|)^{(d-1)(1 - q/2)} |\widehat{h}(t)|^q\; dt \right)^{1/q} \]
%
would follow if we could prove a general inequality of the form
%
\[ \left\| \int a(t) W_t(x,y)\; dt \right\|_{L^\infty_y L^q_x} \lesssim \left( \int (1 + |t|)^{(d-1)(1 - q/2)} |a(t)|^q\; dt \right)^{1/q} \]
%
to hold. By interpolation, it would suffice to prove that
%
\[ \left\| \int a(t) W_t(x,y)\; dt \right\|_{L^\infty_y L^1_x} \lesssim \int (1 + |t|)^{\frac{d-1}{2}} |a(t)|\; dt \]
%
and
%
\[ \left\| \int a(t) W_t(x,y)\; dt \right\|_{L^\infty_y L^2_x} \lesssim \left( \int |a(t)|^2\; dt \right)^{1/2} \]

TODO: Here is a toy example.
