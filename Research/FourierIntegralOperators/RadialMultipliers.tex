...
\begin{itemize}
    \item For a function $f$, we let $\text{Dil}_t f$ denote the function such that
    %
    \[ \text{Dil}_t f(x) = f(x/t). \]
    %
    Thus if $f$ has support on a set $A$, then $\text{Dil}_t f$ has support on $t A$. We let
    %
    \[ \text{Dil}_j f(x) = f(x/2^j) \]
    %
    denote the dyadic dilations.

    \item It will often be helpful to consider a family of bump functions which we can use to localize problems. We begin by consider a function $\beta \in C_c^\infty(\RR^d)$ with the following properties:
    %
    \begin{itemize}
        \item $\text{supp}(\beta) \subset \{ 0.5 \leq |x| \leq 2 \}$.
    ...
    \item We let
    %
    \[ \alpha = \sum_{j < 0} \text{Dil}_j \beta. \]
    %
    Then $\alpha \in C_c^\infty(\RR^d)$, and has the following properties:
    %
    \begin{itemize}
        ...
        \item For any $x \neq 0$,
        %
        \[ \alpha(x) + \sum_{j = 0}^\infty \text{Dil}_j \beta(x) = 1. \]
    \end{itemize}

    \item We let
    %
    \[ \tilde{\beta} = \sum_{j \in \{ -1, 0, +1 \}} \text{Dil}_j \beta. \]
    %
    Then:
    %
    \begin{itemize}
        \item $\text{supp}(\tilde{\beta}) \subset \{ 0.25 \leq |x| \leq 4 \}$.
        \item $\tilde{\beta}(x) = 1$ for $0.375 \leq |x| \leq 2.5$.
        \item For any function $f$ with $\text{supp}(f) \subset \{ 1/2 \leq |x| \leq 2 \}$,
        %
        \[ f \tilde{\beta} = f. \]
    \end{itemize}
\end{itemize}
...
The question of the regularity of translation-invariant operators on $\RR^d$ has proved central to the development of modern harmonic analysis. For essentially any such operator, we can find a tempered distribution $m: \RR^d \to \CC$, the \emph{symbol} of $T$, such that for any Schwartz function $f \in \mathcal{S}(\RR^d)$,
%
...
%
or equivalently, such that the vector valued integral representation
%
\[ T = \int_{\RR^d} k(x) \cdot \text{Trans}_x\; dx \]
%
holds. If $T$ is radial, we can write $k(x) = a(|x|)$ for some function $a: [0,\infty) \to \CC$, and then we have a representation
%
...
The general study of the boundedness properties of Fourier multipliers in multiple variables was initiated in the 1950s, as the theory became more and more connected to the study of partial differential equations \cite{Hormander1}. It was quickly realized that the most fundamental estimates were $L^p$ bounds of the form
%
...
%
For notational convenience, $M^{p,p}(\RR^d)$ is denoted by $M^p(\RR^d)$. Duality implies that $M^{p,q}(\RR^d)$ is isometric to $M^{q^* \negmedspace ,p^*}(\RR^d)$, where $p^*$ and $q^*$ are the conjugates to $p$ and $q$. And the fact these operators are translation invariant, together with Littlewood's Principle, implies that $M^{p,q}(\RR^d) = 0$ unless $q \geq p$. Combining these two results means we can always reduce our anaysis to the study of the spaces $M^{p,q}(\RR^d)$ when $1 \leq p \leq 2$ and when $q \geq p$.

It was quickly realized that some of the spaces $M^{p,q}(\RR^d)$ are very easy to characterize. For instance, $M^{1,q}(\RR^d)$ was characterized by virtue of the fact that the boundedness of operators with domain $L^1(\RR^d)$ are relatively trivial, i.e. as characterized by Schur's lemma; for any symbol $m$, if $k = \widehat{m}$, then
%
\[ \| m \|_{M^{1,q}(\RR^d)} = \begin{cases} \| k \|_{L^q(\RR^d)} &: q > 1, \\ \| k \|_{M(\RR^d)} &: q = 1, \end{cases} \]
%
where $M(\RR^d)$ is the space of finite signed Borel measures equipped with the total variation norm. The unitary nature of the Fourier transform implies that the spaces $M^{p,2}(\RR^d)$ could also be characterized, for $1 \leq p \leq 2$, by the identity
%
\[ \| m \|_{M^{p,2}(\RR^d)} = \| m \|_{L^q(\RR^d)}, \]
%
% | m f^ |_{L^2} <= | m |_q | f |_p
% If f^ is an extremizer for m
where $q = 2p/(2-p)$ for $1 \leq p < 2$, and $q = \infty$ for $p = 2$. However, finding a simple characterization of the remaining spaces $M^{p,q}(\RR^d)$ for $1 < p \leq 2$ and any other exponent $q \geq p$ with $q \neq 2$ has proved more challenging, if not potentially impossible. Since the problem has been introduced, there has been no tractable characterization of the spaces $M^{p,q}(\RR^d)$ for any other pair of exponents $p$ or $q$. Nonetheless, several tools for analyzing Fourier multipliers in this range have been developed, and we end this section by briefly summarizing some relevant results.

We begin with the major tool of \emph{Littlewood-Paley} theory, which makes it natural to restrict to the study of Fourier multipliers compactly suported on dyadic annuli. We fix a smooth bump function $\phi \in C_c^\infty(\RR^d)$ supported on $\{ 1/2 \leq |\xi| \leq 2 \}$, and such that for any $x \in \RR^d - \{ 0 \}$, the family of functions $\{ \text{Dil}_{2^j} \phi : j \in \ZZ \}$ form a partition of unity of $\RR^d - \{ 0 \}$.
\begin{comment}
...
%
Thus $m_t$ describes the behaviour of the multiplier $m$ restricted to the annulus of frequencies $|\xi| \sim t$, but rescaled so that this behaviour is now lying on the annulus $|\xi| \sim 1$.
%
\[ m(D) = \sum\nolimits_j m_{2^j}(D / 2^j). \]
%
Littlewood-Paley hints that, heuristically, that the operators $\{ m_{2^j}(D/2^j) \}$ are almost orthogonal, and do not interact with one another in a significant way. It is therefore natural to restrict to the study of multipliers supported on the annulus $\{ 1/2 \leq |\xi| \leq 2 \}$, which the hope that we can use some kind of dyadic decomposition technology to put such pieces together to yield a general result. In the sequel, we call multipliers supported on this annulus \emph{unit scale multipliers}.

Some sufficient conditions guaranteeing boundedness have been obtained on the convolution kernel side. Young's convolution inequality, a very crude bound (not taking into account any oscillation in the convolution kernel) implies that if $p < q$ and $1/p - 1/q = 1 - 1/r$, then
%
...
%
One can get more sophisticated results by taking into account the oscillatory nature of $k$. Namely, one form of the result of H\"{o}rmander-Mikhlin states that for $1 < p < \infty$,
%
\[ \| m \|_{M^p(\RR^d)} \lesssim_{p,\alpha} \sup_t \| m_t \|_{L^2_\alpha}  \]
%
where $\alpha > d/2$. Notice that in this situation, we see that the parts of the Littlewood-Paley decompostion of the multiplier \emph{do not} interact with one another, i.e. the bound on the entire operator is obtained by controlling each element of the decomposition separately.
...
Conversely, some control over the mass and oscillatory properties of the multiplier $m$ is necessary in order to have $m \in M^{p,q}(\RR^d)$ for some exponents $p$ and $q$. For instance, if $m$ is a unit scale multipliers and $1 \leq p \leq q \leq 2$,
%
\[ \| m \|_{L^{q^*}(\RR^d)} \lesssim \| m \|_{M^{p,q}(\RR^d)}, \]
%
though if $p < 2 < q$, there are multipliers in $M^{p,q}(\RR^d)$ which are distributions of positive order, i.e. that are not measures. On the convolution kernel side, for any values of $p$ and $q$, if $m$ is a unit scale multiplier, if $\phi \in C_c^\infty(\RR^d)$ has Fourier transform equal to one on the annulus $1/4 \leq |\xi| \leq 4$, then
%
...
Before we move on, let us analyze the reason for the endpoints in the radial multiplier conjecture.
%
...
%
To show why $p < 2d/(d+1)$ is necessary, consider the multiplier
%
...
%
For $|x| \geq 1$, Bessel function asymptotics gives that there is some constant $c$, depending on $d$, such that
%
\[ k(x) = |x|^{-\frac{d-1}{2}} \int_{1/4}^1 r^{\frac{d-1}{2}} h(r) \cos(r |x| + c)\; dr + O \left( |x|^{-\frac{d+1}{2}} \right). \]
%
Integration by parts then gives that $|k(x)| \lesssim |x|^{- \frac{d+1}{2}}$. Since $k$ is bounded near the origin (i.e. by the Paley-Wiener theorem), we conclude that $k \in L^p(\RR^d)$ for $p > 2d/(d+1)$. In particular, we conclude that  the radial multiplier conjecture cannot hold for $2d/(d+1) < p < 2$.
...
\begin{align*}
    ...
    &= \int [R \widehat{h}(Ru)] \chi_{y,u}(x)\; du,
\end{align*}
%
where
%
\begin{align*}
    \chi_{y,u}(x) = \int \int \chi_R(t) [R \psi(R(u - t))] s(x,y,\xi,t) e^{2 \pi i \Phi(t,x,y,\xi)} \; d\xi\; dt.
\end{align*}
%
We are `averaging' (by an oscillatory quantity) over a $O(1/R)$ neighborhood of $u$, and thus we should expect from the wavefront set analysis of $\chi_{y,u}$ that this function has mass concentrated on an annulus of thickness $O(1/R)$ and radius $u$, centered at $y$.

Now our goal is to prove bounds of the form
%
\[ \left\| \int_{1/R \lesssim |u| \lesssim 1} \int_U f(y) [R \widehat{h}(Ru)] \chi_{y,u}(x)\; dy\; du \right\|_{L^p_x} \lesssim \| f \|_{L^p_y} \]
%
...
%
\[ \left\| \int \int a(y,u) \chi_{y,u}\; dy\; du \right\|_{L^p_x} \lesssim R^{s_{p^*}} C_p(h) \left( \int \int |a(y,u)|^p |u|^{(d-1)(1 - p/2)}\; dy\; du \right)^{1/p}. \]
%
...
%
\[ \left\| \sum_{(y,u) \in \mathcal{E}} a(y,u) \chi_{y,u} \right\|_{L^p_x} \lesssim R^{s_{p^*}} \left( \sum_{(y,u) \in \mathcal{E}} |a(y,u)|^p |u|^{(d-1)(1 - p/2)} \right)^{1/p}. \]
%
...
%
\[ \left\| \sum_{(y,u) \in \mathcal{E}_k} \chi_{y,u} \right\|_{L^p_x} \lesssim R^{s_{p^*}} \left( \sum_k 2^{k(d-1)(1 - p/2)} \#(\mathcal{E}_k) \right)^{1/p}. \]
%
The values of $k$ for which $\mathcal{E}_k$ is nonempty occur for $1 \lesssim k \lesssim \log R$.

We can further perform a frequency decomposition in $\xi$, i.e. writing for $j \gtrsim 1$,
%
\begin{align*}
    \chi_{y,u,j}(x) &= \int \int \chi_R(t) [R \psi(R(u - t))] \beta(\xi / 2^j) s(x,y,\xi,t) e^{2 \pi i \Phi(t,x,y,\xi)}\\
...
%
\[ |\chi_{y,u,j}(x)| \lesssim_N 2^{j(d-N)} (|d_g(x,y) - u| - 1/R)^{-N}. \]
%
