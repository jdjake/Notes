...
A major tool to understand multipliers outside the range above is Littlewood-Paley theory. Fix a function $\phi \in C_c^\infty(\RR^d)$ such that $1 = \sum_j \text{Dil}_j \phi$. Given a symbol $m$, define
%
...
% Suppose frequency has units t
% Space has units 1/t
% m^ has units t^d
% m^ * f(x) is unitless
% The L^q norm of m^ * f is t^{d/q}
% The L^p norm of f is t^{-d/p}
% so the M^{p,q} norm of m has units t^{d/p - d/q}
% L^q norm of m^ has units t^{d/q^*}
...
To exploit the assumed properties of $h$, we apply the Fourier transform, writing
%
\[ T_R = R \int w(R s) e^{2 \pi i s \sqrt{-\Delta}}\; ds, \]
%
where $\widehat{w} = h$. In $S^n$, the wave equation is periodic on a sphere, with periodicity $2 \pi$. Thus we can rewrite this equation as
%
\[ R \sum_{k = -\infty}^\infty \int_0^{2\pi} w(R s + (2 \pi k) R) e^{2 \pi i s \sqrt{-\Delta}}\; ds. \] 
%
Applying H\"{o}lder's inequality, we have, for $k \neq 0$,
%
\begin{align*}
    R & \int_0^{2\pi} w(R s + (2 \pi k) R) e^{2 \pi i s \sqrt{-\Delta}}\; ds\\
    &\lesssim A \left( \sum_{k = -\infty}^\infty (1 + (2 \pi k R))^{-(d-1) (1 - q/2)} \int_0^{2\pi} |e^{2 \pi i s \sqrt{-\Delta}}|^{q^*} \; ds \right)^{1/q^*}\\
    &\lesssim A R^{-(d-1)(1-q/2)} \left( \int_0^{2\pi} |e^{2 \pi i s \sqrt{-\Delta}}|^{q^*}\; ds \right)^{1/q^*}
\end{align*}
% q < 2(d-2)/(d-1)
%
Thus, provided the wave equation is local smoothing of order $1/p + 1/q - 1$, Sobolev embedding implies the result (it actually implies the operator is $O(R^{-(d-1)(1-q/2)}$, should I do a sanity check?).
...
%
\[ R \int_0^{2\pi} \int_{\RR^d} \sum_{k = 0}^\infty w(R s + (2 \pi k) R) a(s,x,y,\xi) e^{2 \pi i \Phi(x,y,s,\xi)}\; d\xi\; ds, \]
%
