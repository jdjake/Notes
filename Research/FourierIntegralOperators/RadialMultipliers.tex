...
\begin{align*}
    ...
    &= A_p(h) R^{1/p^* - (d-1)(1/p - 1/2)} \left( \sum_{l \neq 0} |l|^{-(d-1)(p^*/2 - 1)} \right)^{1/p^*} \\
...
\end{align*}
%
where $\alpha_{p^*} = (d-1)(1/2 - 1/p^*) - 1/p^* = (d-1)(1/p - 1/2) - 1/p^*$. But since $f$ is a linear combination of eigenfunctions with eigenvalue $\sim R$, we conclude that
%
...
On the other hand, for $1 \lesssim k \lesssim R$, local smoothing and H\"{o}lder imply that
%
\begin{align*}
...
where
%
\[ T_m = \int \phi(t - m) \widehat{h}(t) e^{2 \pi i t \sqrt{-\Delta}}\; dt. \]
%
...
%
On the other hand, H\"{o}lder implies that we have pointwise bound of the form
%
\[ \left( \sum_{|m| \gtrsim 1} |m|^{(d-1)(1 - p/2)} |T_m f|^p \right)^{1/p} \lesssim \sup_{t_0 \gtrsim 1} \left( \int_{t_0}^{t_0 + 1} |e^{2 \pi i t \sqrt{-\Delta}} f|^{p^*} \right)^{1/p^*}. \]
%
Thus for $p^* > 2d/(d-1)$, another H\"{o}lder's inequality gives us a pointwise bound of the form
%
\[  | II f | \lesssim \sup_{t_0 \gtrsim 1} \left( \int_{t_0}^{t_0 + 1} |e^{2 \pi i t \sqrt{-\Delta}} f|^{p^*} \right)^{1/p^*}. \]
%
If we let $M_{\text{LS}} f$ denote the right hand side of this equation, then we find that
%
\[ \| II f \|_{L^{p^*}(\RR^d)} \leq \| M_{\text{LS}} f \|_{L^{p^*}(\RR^d)}, \]
%
and our result would thus follow if we could show that
%
\[ \| M_{\text{LS}} f \|_{L^{p^*}(\RR^d)} \lesssim \| f \|_{L^{p^*}(\RR^d)}. \]
%
Can we now use the Christ-Kiselev Lemma as in the proof of the Menshov-Paley-Zygmund Theorem (i.e. with a rescaling / splitting argument) to prove the required result?
