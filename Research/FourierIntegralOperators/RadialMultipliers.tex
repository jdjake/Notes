...
%
i.e. such that $\widehat{Tf} = m \widehat{f}$. This is why these operators are also called \emph{Fourier multipliers}. Using the spectral calculus of unbounded operators, one can also write this operator as $m(D)$, where $D = (2 \pi i)^{-1} \nabla$ is a self-adjoint normalization of the gradient. Thus the study of the boundedness of translation invariant operators is closely connected to the study of the interactions of the characters $e_\xi(x) = e^{2 \pi i \xi \cdot x}$, which are the eigenfunctions for the components of $D$ in $L^2(\RR^d)$.
...
\[ m(\xi) = h(|\xi|) \]
%
for some function $h: [0,\infty) \to \CC$. This class of operators is therefore also called the class of \emph{radial Fourier multipliers}. Again, applying the spectral calculus, one can write $m(D) = h(\sqrt{-\Delta})$, where $\Delta$ is the Laplacian on $\RR^n$. Thus the study of radial multipliers is closely connected to interactions between the spherical projection operators
%
\[ E_\lambda f(x) = \int_{|\xi| = 1} \widehat{f}(\xi) e^{2 \pi i \xi \cdot x}, \]
%
for $0 < \lambda < \infty$.

In this setup, one can define the study of radial multipliers on a general \emph{geodesically complete} Riemannian manifold $M$. On such a manifold we have a Laplace-Beltrami operator $\Delta$ which is an essentially self-adjoint unbounded operator on $L^2(M)$, $\sqrt{-\Delta}$ will be a self-adjoint operator, and one can consider the study of operators of the form $h(\sqrt{-\Delta})$ for functions $h: [0,\infty) \to \CC$. Understanding such operators requires an understanding of the interactions between the projection operators onto eigenspaces of $\sqrt{-\Delta}$.

This research projection studies necessary and sufficient conditions to guarantee the $L^p$ boundedness of radial multiplier operators, both in the Euclidean setting, and also in the setting of Riemannian manifolds, stimulated by recent developments which indicate lines of attack for three related problems in the field.
...
A major tool to understand multipliers outside the range is the Littlewood-Paley theory, which allows us to use orthogonality when mapping into $L^q(\RR^d)$ when $1 < q < \infty$. We fix a smooth bump function $\phi \in C_c^\infty(\RR^d)$ such that $1 = \sum_j \text{Dil}_{2^j} \phi$. Then given a symbol $m$, we define
%
\[ m_t = (\text{Dil}_{1/t} m) \cdot \phi. \]
%
Thus $m_t$ describes the behaviour of the multiplier $m$ on the annulus $|\xi| \sim t$, rescaled so that this behaviour is now lying on the annulus $|\xi| \sim 1$. Littlewood-Paley theory implies that for $1 < p,q < \infty$,
%
...
%
Thus $k$ must satisfy some (admittedly weak) regularity assumptions in order to be the convolution kernel of a bounded Fourier multiplier.

For $p = 1$ and $1 < q < \infty$, this result is tight up to the exponent $q$. The result says that
%
...
%
In the sequel, we shall call this the \emph{radial multiplier conjecture} in Euclidean space.
...
Fix a geodesically complete Riemannian manifold $M$. We can then define operators $h(\sqrt{-\Delta})$, which are analogues to the radial multipliers studied in the Euclidean setting. Just like multiplier operators on $\RR^n$ are crucial to an understanding of the interactions between the functions $e_\xi(x) = e^{2 \pi i \xi \cdot x}$ on $\RR^n$, understanding the operators $h(\sqrt{-\Delta})$ is crucial to understanding the interactions of eigenfunctions of the Laplace-Beltrami operator on $X$. We let $M^{p,q}(X)$ denote the family of all symbols $h: \RR \to \CC$ such that the operator $T_h = h(\sqrt{-\Delta})$ is bounded from $L^p(X)$ to $L^q(X)$, with the analogoue norm.

One problem which prevents a direct generalization of the radial multiplier conjecture in this setting comes from studying multipliers on compact Riemannian manifolds. One problem occurs on compact Riemannian manifolds. On such a manifold, $1 / (\varepsilon + \sqrt{-\Delta})$ is a compact, bounded operator from $L^2(M)$ to itself for any $\varepsilon > 0$, and so there exists $0 \leq \lambda_1 \leq \lambda_2 \leq \dots$ with $\lambda_i \to \infty$, and an orthonormal family of eigenfunctions $e_n \in C^\infty(M)$ which diagonalize $L^2(M)$ such that
%
...
%
If $h$ has compact support, this sum will be finite, and thus trivially bounded from $L^p(M)$ to $L^q(M)$ for any choice of exponents $p$ and $q$, regardless of the function $h$. In order to prevent this problem from being trivial, we instead determine what conditions ensure that we have a bound of the form
%
...
\section{Bochner-Riesz on Euclidean space and Riemannian Manifolds}

The goal of this section is to compare and contrast approaches to understanding the Bochner-Riesz conjecture on Euclidean space and on compact Riemannian manifolds. We define the Riesz multipliers $r_{\lambda,\delta}$ for $\lambda > 0$ by setting, for $\tau > 0$,
%
\[ r_{\lambda,\delta}(\tau) = (1 - \tau / \lambda)_+^\delta. \]
%
Here $s_+ = \max(s,0)$. The resulting radial multipliers on $\RR^n$ and on a compact Riemannian manifold will be denoted by $S_{\lambda,\delta} = r_{\lambda,\delta}(\sqrt{-\Delta})$. The goal of the Bochner-Riesz conjecture is to determine the boundedness of the operators $\{ S_{\lambda,\delta} \}$. For $1 \leq p < \infty$, we define
%
\[ \delta(p) = \max ( n |1/2 - 1/p| - 1/2, 0 ). \]
%
If we consider the spherical projection operators
%
\[ \chi_\lambda f(x) = \int_{|\xi| = \lambda} \widehat{f}(\xi) e^{2 \pi i \xi \cdot x}\; d\xi, \]
%
then
%
\[ S_{\lambda,\delta} f = \int_0^\lambda (1 - \tau / \lambda)^\delta \chi_\tau f. \]
%
The Stein-Tomas theorem implies that $\| \chi_\lambda f \|_{L^{p^*}(\RR^d)} \lesssim \lambda^{d(1/p - 1/p^*) - 1} \| f \|_{L^p(\RR^d)}$, and thus that for $1 \leq p < 2$,
%
\[ \| S_{\lambda,\delta} f \|_{L^{p^*}(\RR^d)} \lesssim \int_0^\lambda (1 - \tau / \lambda)^\delta \tau^{d(1/p - 1/p^*) - 1} \| f \|_{L^p(\RR^d)}\; d\tau. \]

On $\RR^d$, we can take Fourier transforms, applying stationary phase to determine that if $K_{\lambda,\delta}$ is the convolution kernel corresponding to $S_{\lambda,\delta}$, then
%
\begin{align*}
    K_\delta(x) &= \int_0^1 \tau^{n-1} (1 - \tau)^\delta e^{2 \pi i \xi \cdot x}\; d\tau\\
...
\begin{lemma}
    ...
    %
    \[ \left\| \int e^{2 \pi i \lambda |x - y|} a(x,y) f(y) \right\|_{L^q(\RR^n)} \lesssim \lambda^{n/q} \| f \|_{L^p(\RR^n)} \]
    %
...
Now let us review the proof on a compact Riemannian manifold.
...
%
for an appropriate constant $C$ (TODO: Determine this more precisely). One can calculate (see Remark in Section 4.1 Sogge's book) that the canonical relation of the family of operators $\{ T_t \}$ is given by
%
