...
%
For later reference, we note that, using the theory of stationary phase, one can obtain an asymptotic expansion of this formula for large $\lambda$, i.e. so that as $\lambda \to \infty$, we may find constants $\{ a_{\alpha,k} \}$ and $\{ b_{\alpha,k} \}$ such that
%
\[ J_\alpha(\lambda) \sim \cos(\lambda - \omega) \sum_{k = 0}^\infty (-1)^k \frac{a_{\alpha,k}}{\lambda^{2k+1/2}} + \sin(\lambda - \omega) \sum_{k = 0}^\infty (-1)^k \frac{b_{\alpha,k}}{\lambda^{2k + 3/2}}. \]
%
Stated more crudely, $J_\alpha(\lambda) = \cos(\lambda - \omega) s_1(\lambda) + \sin(\lambda - \omega) s_2(\lambda)$, where $s_1$ and $s_2$ are symbols of order $-1/2$.
...
\section{Range of Exponents in the Radial-Multiplier Conjecture}
...
%
\begin{align*}
    \int_0^1 r^{d-1} \beta(r) s_+(rx) e^{2 \pi i r |x|}\; dr &= \frac{s_+(x) e^{2 \pi i r |x|}}{2 \pi i |x|}\\
    &\quad - \frac{1}{2 \pi i |x|} \int_0^1 \frac{d}{dr} \left\{ r^{d-1} \beta(r) s_+(rx) \right\} e^{2 \pi i r |x|}\; dr.
\end{align*}
%
Each of these terms is $O( |x|^{- \frac{d+1}{2}} )$. Thus
%
...
TODO: Clean this analysis up. To show the radial multiplier conjecture cannot hold for $p = 2d/(d+1)$, we must be slightly more careful in our analysis. To do this, we will find a family of smooth, unit scale multipliers $\{ m_\delta \}$ which become more and more singular near the boundary of the unit ball as $\delta \to 0$, and such that for $2d/(d+1) \leq p \leq q \leq 2$,
%
...
\chapter{Heo, Nazarov, and Seeger: Initial Radial Multiplier Conjecture Results} \label{HeoNazarovSeeger}
...
\chapter{Lee and Seeger: Decomposition Arguments For Estimating Fourier Integral Operators with Cinematic Curvature}
...
\chapter{Jongchon Kim - Bounds on Quasi-Radial Multipliers}

This chapter discusses the 2017 paper \emph{Endpoint Bounds for Quasiradial Fourier Multipliers}, published in 2017 by Jongchon Kim. Given a homogeneous function $a: \RR^d \to (0,\infty)$ of degree one, smooth away from the origin, the paper discusses the problem of bounding `quasiradial' Fourier multipliers with symbols of the form $m(\xi) = h(a(\xi))$. Under the assumption that the `cosphere'
%
\[ \Sigma = \{ x : a(x) = 1 \} \]
%
has everywhere non-vanishing Gaussian curvature, Kim proves that for $d \geq 4$, and $1 < p < 2(d-1)/(d+1)$, if $h$ is a unit scale multiplier, then
%
\[ \| m \|_{M^p(\RR^d)} \sim C_p(h). \]
%
...
Let's see how this is done. Let $m$ be such a multiplier. If $\beta$ is supported on $\{ 1/4 \leq |\xi| \leq 4 \}$, and equal to one on $\{ 1/2 \leq |\xi| \leq 2 \}$, then
%
\begin{align*}
...
%
By the curvature hypothesis, for each $z \in \RR^d - \{ 0 \}$, there are exactly two points $\xi_+$ and $\xi_-$ on $\Sigma$ such that $x$ is normal to $\Sigma$ at these points. We let $\psi_+(z) = \xi_+ \cdot z$ and $\psi_-(z) = \xi_- \cdot z$. To obtain bounds on this kernel, we consider the coordinate system $(0,\infty) \times \Sigma \to \RR^d$ given by $(\rho,\omega) \mapsto \rho \omega$. In coordinates, we have
%
\[ d\xi = \rho^{d-1} \langle \omega, n(\omega) \rangle d\rho d\sigma(\omega). \]
%
Thus we can write
%
\begin{align*}
...
\end{align*}
%
where $b_+$ and $b_-$ are symbols of order $-(d-1)/2$, where $b_+$ and $b_-$ have principal symbols which are scalar multiples of
%
\[ z \mapsto |z|^{- \frac{d-1}{2}} \langle \xi_+, n(\xi_+) \rangle\; \eta(a(\xi_+)) \quad\text{and}\quad z \mapsto |z|^{-\frac{d-1}{2}} \langle \xi_-, n(\xi_-) \rangle\; \eta(a(\xi_-)) \]
%
respectively. Integrating by parts gives that $K_t(z)$ decays very fast away from the annulus $|\psi_+(z) + t| \lesssim 1$ and $|\psi_-(z) - t| \lesssim 1$.
...
It finally remains to analyze the operators
%
\[ II_R = \int b_R(t) e^{2 \pi i t P}\; dt, \]
%
obtained by integrating the wave propogators over times $50/R \leq |t| \leq 2/5$, with
%
...
\begin{itemize}
    ...
    %
    Uncertainty principle heuristics thus imply that we should expect $b$ to be locally constant at a scale $1/R$. If we let $\mathcal{T}$ be all points in the lattice $(1/R) \ZZ$ lying in the set $\{ 48/R \leq |t| \leq 2/5 + 2/R \}$, and for each $t_0 \in \mathcal{T}$, write
    %
    \[ \chi_{II} = \sum_{t_0 \in \mathcal{T}} \chi_{t_0}, \]
    %
    where for each $t_0 \in \mathcal{T}$, $\chi_{t_0}$ is supported on the sidelength $2/R$ interval centered at $t_0$, and satisfies estimates of the form
    %
    ...
    %
    so we should still think of $\widehat{b}_{t_0}$ as being Fourier localized to frequencies lying in the ball of radius $O(R)$ about the origin. In particular, we have Bernstein's inequalities
    %
    \[ \| b_{t_0} \|_{L^p_s} \lesssim_s R^s \| b_{t_0} \|_{L^p} \]
    %
    So by the Sobolev embedding theorem, for $r > s$,
    %
    \[ \| b_{t_0} \|_{L^r} \lesssim R^{1/s - 1/r} \| b_{t_0} \|_{L^s}. \]

    % L^p_s is contained in C^{r + a}
    % 1/p - 1/q = s
...
%
for all functions $f \in V_R$. Given the presence of the $L^p$ norm of $b_R$ and the $L^{p^*}$ norm of $f$, a natural first choice might be to try and prove this inequality using H\"{o}lder's inequality.
%
% writing
% R^{-s_p} R^{1/p-s_p}
%\begin{align*}
...
Given the Lax Parametrix, which decomposes the half-wave operator into an expression of the form
%
\[ e^{2 \pi i t P} = T_{A,t} + \sum_W T_{W,t}, \]
%
we write $II = II_A + \sum_W II_W$, where
%
\[ II_A = \int b(t) T_{A,t}\; dt \quad\text{and}\quad II_W = \int b(t) T_{W,t}\; dt. \]
%
The sum in the decomposition of $II$ is finite, independent of $R$, and so it suffices to bound each term individually.

Let's start by bounding $II_A$ uniformly in $R$. Let $K$ denote the kernel of $II_A$, i.e. so we have
%
...
\begin{align*}
    ...
    &\lesssim \left[ (1/R)^{s_p - 1/p^*} C_p(h) \right] \left[ R^{s_p - 1/p^*} \right]\\
    &\lesssim C_p(h).
\end{align*}
%
Applying Schur's Lemma, we conclude that $II_A$ is bounded from $L^1(M)$ to $L^\infty(M)$, and $L^\infty(M)$ to $L^1(M)$, with operator norm bounded by a constant multiple of $C_p(h)$, independent of $R$. Interpolating these bounds gives that $II_A$ is bounded from $L^r(M)$ to $L^s(M)$ for any $1 \leq r,s \leq \infty$, with operator norm bounded by a constant multiple of $C_p(h)$. In particular, it is bounded from $L^{p^*}(M)$ to $L^{p^*}(M)$.

We next analyze the operators $II_W$, for $W \in \mathcal{W}$. Our analysis will again apply our simplifying assumption that our inputs $f$ lie in the space $V_R$. In particular, for such $f$, we have $II_W \{ f \} = (II_W \circ Q_R) \{ f \}$. One consequence of this is that we can localize the amplitude in the oscillatory integral representation of the operator $II_W$ to be supported near where the phase-variable has magnitude $\Theta(R)$. More precisely, let $\upsilon \in C_c^\infty(\RR^d)$ be supported on $\{ C^{-1} \leq |\eta| \leq C \}$, and equal to one on a slightly smaller neighborhood. Set $\upsilon_R(\eta) = \upsilon( \eta / R)$. Define the operators $T'_{W,t}$ to be the Schwartz operator with kernel $W'(t,\cdot,\cdot)$, where
%
...
%
where $a$ is supported on $|\eta| \sim R$. There is no issue in this, since, these two operators have comparable operator norms. Similarily, by an element of $V_R$ we now mean the restriction of a linear combination of eigenfunctions to the coordinate system $\Omega$, rather than a linear combination of the eigenfunctions themselves.
