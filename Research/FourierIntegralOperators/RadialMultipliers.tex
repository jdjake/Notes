...
%
\[ \| f \|_{L^p_\alpha} \lesssim (1 + R)^\alpha \| f \|_{L^p}, \]
%
...
%
Using the periodicty of the wave equation, we write
%
\[ T_R f = \int_0^1 \left( \sum_{l = -\infty}^\infty R \cdot \widehat{h}(R(t + l)) \right) e^{2 \pi i t \sqrt{-\Delta}} f\; dt \]
%
Now since $1 < p < 2d/(d+1)$,
%
\begin{align*}
    |T_R f| &\lesssim \int_0^1 R \left( \sum_{l = -\infty}^\infty |\widehat{h}(R(t + l))|^p \langle R(t + l) \rangle^{(d-1)(1 - p/2)} \right)^{1/p} e^{2 \pi i t \sqrt{-\Delta}} f\; dt\\
    &\lesssim R \left( \int_0^1 \left( \sum_{l = -\infty}^\infty |\widehat{h}(R(t + l))|^p \langle R(t + l) \rangle^{(d-1)(1 - p/2)} \right)\; dt \right)^{1/p}\\
    &\quad \quad \quad \left( \int_0^1 |e^{2 \pi i t \sqrt{-\Delta}} f|^{p^*} \right)^{1/p^*}\\
    &\lesssim R \left( \int_{-\infty}^\infty |\widehat{h}(Rt)|^p \langle Rt \rangle^{(d-1)(1 - p/2)} \right)^{1/p}\\
    &\quad \quad \quad \left( \int_0^1 |e^{2 \pi i t \sqrt{-\Delta}} f|^{p^*} \right)^{1/p^*}\\
    &\lesssim A_p(h) R^{1/p^*} \left( \int_0^1 |e^{2 \pi i t \sqrt{-\Delta}} f|^{p^*} \right)^{1/p^*}.
\end{align*}
%
If the endpoint local smoothing conjecture held, we would find that for $p \leq 2d/(d+1)$,
%
\[ \left\| \left( \int_{|t| \lesssim 1} |e^{2 \pi i t \sqrt{-\Delta}} f|^{p^*} \right)^{1/p^*} \right\|_{L^p(M)} \lesssim \| f \|_{L^{p^*}_{\alpha_{p^*}}(M)}, \]
%
where
%
\[ \alpha_{p^*} = (d-1)(1/2 - 1/p^*) - 1/p^*. \]
%
...
%
\[ \| f \|_{L^{p^*}_{\alpha_p}} \lesssim R^{\alpha_{p^*}} \| f \|_{L^{p^*}(M)} \]
%
Thus we conclude that
%
\[ \| T_R f \|_{L^{p^*}(M)} \lesssim A_p(h) R^{1/p^* + \alpha_{p^*}} \|_{L^{p^*}(M)} = A_p(h) R^{(d-1)(1/2 - 1/p^*)}. \]
%
Thus we get uniform bounds in $R \geq 1$ for the $L^{p^*}$ norm of $T_R$. By a duality argument, this implies the required results in the $L^p$ norm as well. In this situation, we do not even need endpoint smoothing though, we just need smoothing of order $\sigma$ such that $1/p^* + (d-1)(1/2 - 1/p^*) - \sigma \leq 0$, i.e. when
%
\[ \sigma \geq \frac{d-1}{2} - \frac{d-2}{p^*}. \]
%
