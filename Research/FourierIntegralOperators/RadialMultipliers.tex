...
\begin{remarks}
    \ 
\begin{enumerate}
    \item If $1 \leq p < 2d/(d+1)$ and $C_p(h) < \infty$, then \cite{HeoandNazarovandSeeger} implies that the operator $h(|\cdot|)$ is bounded as a Fourier multiplier operator on $L^p(\RR^d)$ with $\| h(|\cdot|) \|_{M^p(\RR^d)} \lesssim C_p(h)$. Interpolation and duality (see Section 2.5.5 of \cite{Grafakos} for more details) implies that the operator is also a Fourier multiplier operator on $L^2(\RR^d)$, and the characterization of the norm of such operators gives a bound
    %
...
and
%
\[ \| e^{2 \pi i t P} f \|_{L^p(S^d)} \lesssim_s \| f \|_{L^2_s(S^d)}, \]
%
...
\begin{proof}
    ...
    We begin by noting that for a pair of H\"{o}lder conjugates $p$ and $q$, we have
    %
    \[ s_q = (d - 1)(1/q - 1/2) = - (d-1)(1/p - 1/2) = - s_p. \]
    %
    Using the periodicity of the wave equation on $S^d$, i.e. that
    %
    ...
    \[ s_q q = (d-1)(q/2 - 1) > 1, \]
    %
    we can use H\"{o}lder's inequality to conclude that
    % (d-1)(q/2 - 1) > 1
    % q > 2d/(d-1)
    \begin{align*}
...
%
is obtained by integrating the wave propogators over times $100/R \leq |t| \leq 0.01$ respectively. To prevent notation from growing too cumbersome later on, let us eschew uses of the subscript $R$ in our operators for now, e.g. writing $II_R$ as
%
\[ II = \int b(t) e^{2 \pi i t P}\; dt, \]
%
where $b(t) = \chi_{II}(t) R \widehat{h}(Rt)$. This argument will involve a more subtle analysis of this operator, and we will begin by converting our problem in coordinates on $S^d$, where our operators have more explicit representations via oscillatory integrals.
\begin{comment}
...
We begin by covering $S^d$ by a finite family of coordinate charts $\{ U_\alpha \}$, which contain enough overlap that we can find smaller open sets $V_\alpha \subset V^*_\alpha \subset U_\alpha$ for each $\alpha$, such that:
%
\begin{itemize}
    \item $V^*_\alpha$ contains $N(V_\alpha, 1/10)$, the $1/10$ thickening of $V_\alpha$.

    \item $U_\alpha$ contains $N(V_\alpha^*, 1/10)$.

    \item $\{ V_\alpha \}$ also covers $S^d$.
\end{itemize}
%
...
We will employ some restricted bounds, together with interpolation, to understand the operators $II \circ Q$. We thus introduce a set $E \subset S^d$ and try to obtain $L^{p,\infty}$ bounds on the function $S = (II_W \circ Q) \{ E \}$. Given that $Q$ already acts, heuristically, by localizing the behaviour of it's inputs to the frequency $R$, and so, by the uncertainty principle, $Q \{ E \}$ will behave like a $1/R$ discretized set, it is natural to discretize at this scale. Consider a maximal $1/2R$ separated subset $\mathcal{X}$ of $S^d$. Then break $E$ down into a disjoint union of sets $\{ E_{x_0}: x_0 \in \mathcal{X} \}$, where for $x_0 \in \mathcal{X}$, the set $E_{x_0}$ is supported on the geodesic ball of radius $1/R$ about $x_0$. Similarily, let $\mathcal{T}$ be all points in the lattice $\ZZ / 10 R$ lying in the set $\{ 100/R \leq |t| \leq 1 \}$, and write
%
...
%
Our first goal will be to understand how orthogonal the functions $\{ {S\!}_{x_0,t_0} \}$ are to one another. The rest of this section will be devoted to proving the following estimate, which, together with a density decomposition argument, introduced in \cite{HeoandNazarovandSeeger}, can be used to obtain the required $L^p$ estimates on the function $S$.
...
Let us proceed with the proof. To begin with, we can use the self-adjointness of the operators $Q$, and the semigroup structure of $\{ e^{2 \pi i t P} \}$, to write
%
\begin{align*}
...
Next, perform a partition of unity, writing
%
...
The next Lemma allows us to approximate the operator $Q$ with another operator more amenable to analysis in the coordinate system $U_\alpha$, modulo an error which is negligible to our analysis.
...
\begin{lemma} \label{pseudodifferentialCoordinateLemma}
    For each $\alpha$, there exists a Schwartz operator $Q_\alpha$, with kernel supported on $U_\alpha \times V^*_\alpha$, such that the following properties hold:
    %
    \begin{itemize}
        \item For $f \in C^\infty(S^d)$ with $\text{supp}(f) \subset V^*_\alpha$,
        %
        \[ \text{supp}(Q_\alpha f) \subset N(\text{supp}(f), 0.01), \]
        %
        and
        %
...
\begin{lemma}
    For any $\alpha$, we can find a Schwartz operator $W_t$ such that:
    %
    \begin{itemize}
        % \item For $f \in C^\infty(\tilde{V}_\alpha)$, $e^{2 \pi i t P} f = W_t f + A_t f$.

        \item For $f \in C^\infty(V^*_\alpha)$, $\text{supp}(W_t f) \subset N( \text{supp}(f), t + 0.01 )$.
        ...
        %
        \[ W_t(x,y) = \int s_t(x,y,\xi) e^{2 \pi i [ \phi(x,y,\xi) + t |\xi|_y ]}\; d\xi, \]
        %
        where
        %
        \[ \text{supp}(s_t) \subset \{ |\xi| \sim R \}, \]
        %
...
\begin{lemma}
    ...
    % 1/r = (2 - p)/p
    % r = p/(2-p)
    % int |b(t)|\; dt << |b|_{L^p} R^{-1/p*}
    \[ \langle S_{x_0,t_0}, S_{x_1,t_1} \rangle \lesssim_N R^{- \frac{3d-1}{2}} \frac{\| b_{t_0} \|_{L^p(\RR)} \| b_{t_1} \|_{L^p(\RR)}}{\big\langle R (d(x_0,x_1) - |t_0 - t_1|) \big\rangle^{N}}. \]
\end{lemma}
\begin{proof}
  ...
  %  and in the latter,
   ...
   % \[ |\nabla_w \{ \theta \cdot (x - w) + \phi(w,y,\xi) + t |\xi|_y \}| \geq |\nabla_w \phi(w,y,\xi)| - |\theta| \gtrsim |\xi|. \]
    ...
    \begin{align*}
        ...
        &\quad\quad\quad s_t(x,y,\xi) e^{2 \pi i [ \phi(x,y,\xi) + t |\xi|_y ]} \sigma(y,\eta) e^{2 \pi i \eta \cdot (y - z)} E_{x_0}(z)\\
        &\quad\quad\quad\quad\quad \; dt\; dx\; dy\; dz\; dw\; d\theta\; d\xi\; d\eta.
    \end{align*}
    %
    The integral looks complicated, but can be simplified considerably by noticing that all the spatial variables are highly localized. To begin with, we use the fact that $s$ is compactly supported in all it's variables, so that we may write
    %
    \[ s_t(x,y,\xi) \approx \]
    %
    If we define
    %
    \[ f(\xi) = \int (\eta_\alpha E_{x_1})(w) \sigma(w,\theta) e^{2 \pi i [\theta \cdot (w - x) + (x - x_1) \cdot \xi ]} s(x)\; d\theta\; dw\; dx  \]
    % f(xi) = int a(R(x - x_1)) e^{2 pi i (x - x_1) * xi} dx
    %       = int a(Rx) e^{2 pi i x * xi} dx
    %       = R^{-d} a^(xi / R)
    %
    and set
    %
    \[ g(\xi) = \int c_{t_0,t_1}(t) s(t,y,\xi) e^{2 \pi i [\phi(t_0 - t_1,x_0,\xi) - \phi(t,y,\xi)]} \sigma(y,\zeta) e^{2 \pi i \zeta \cdot (y - z)} E_{x_0}(z)\; d\xi\; d\zeta\; dt\; dy\; dz. \]
    %
    Then, modulo negligible errors, $f$ and $g$ are compactly supported on $|\xi| \sim R$, and satisfy
    %
    \[ |(\nabla_\xi^\kappa f)(\xi)| \lesssim_{\kappa} R^{-d-|\kappa|}, \]
    %
    and
    %
    \[ |(\nabla_\xi^\kappa g)(\xi)| \lesssim_{\kappa} \| c \|_{L^1(\RR)} R^{-d-|\kappa|}, \]
    % g(xi) = int c(t) s(t,y,xi) e^{2 pi i [ phi(t_0 - t_1, x_0, xi) - phi(t,y,xi) ]} a(R(y - x_0)) dt dy
    and it suffices to consider the integral
    %
    \[ \int f(\xi) g(\xi) e^{2 \pi i [x_1 \cdot \xi - \phi(t_0 - t_1,x_0,\xi)]}\; d\xi. \]
    %
    We rescale, i.e. letting $a(\xi) = \| c \|_{L^1(\RR)}^{-1} R^{2d} f(R\xi) g(R\xi)$. Then $v$ has uniformly bounded derivatives of all orders in $R$, is compactly supported on $|\xi| \sim 1$, and the integral above is
    %
    \[ \| c \|_{L^1(\RR)} R^{-d} \int a(\xi) e^{2 \pi i R [ x_1 \cdot \xi - \phi(t_0 - t_1, x_0, \xi) ]}\; d\xi. \]
    %
