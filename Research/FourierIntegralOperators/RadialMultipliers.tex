...
where $q = 2p/(2-p)$. However, characterizing the remaining spaces $M^{p,q}$ for $1 < p < \infty$ and $p \in (1,2]$ and $q \in [p, \infty) - \{ 2 \}$, proved more challenging. In the past 60 years there has been no tractable characterization of the spaces $M^{p,q}(\RR^d)$ for any other pair of exponents $p$ or $q$.
...
\end{align*}
%
TODO: Finish this calculation, found in Theorem 1.2 of \cite{GarrigosandSeeger}. Continuing this calculation shows that the condition in the radial multiplier condition is equivalent to 
    %
...
%
% Since $h$ is compactly supported, $\widehat{h}$ is compactly supported at a scale $1$, but for large R this is irrelevant.
%
...
where
%
\[ T_{R,0} f = \int \left( \tilde{\eta}(Rt) \sum_{l = -\infty}^\infty R \cdot \widehat{h}(R(t + l)) \right) e^{2 \pi i t \sqrt{-\Delta}} f\; dt \]
%
and
%
\[ T_{R,k} f = \int \eta(R t / 2^k) \left( \sum_{l = -\infty}^\infty R \cdot \widehat{h}(R(t + l)) \right) e^{2 \pi i t \sqrt{-\Delta}} f\; dt. \]
%
% (R^{-1} alpha(t/R)) * (Sum_n h(n/R) delta_n)
% sum_n h(n/R) R^{-1} alpha((L - n)/R)
% Sum of O(R) terms,
% Bound by |h|_{l^1}
If $\alpha$ is the inverse Fourier transform of $\tilde{\eta}$, then $T_{R,0}$ corresponds to a multiplier operator with symbol
%
\[ m_{0,R}(\lambda) = R^{-1} \sum_n h(n/R) \alpha \Big( \frac{\lambda - n}{R} \Big). \]
%
But
%
\[ D^\beta m_{0,R}(\lambda) = R^{-1-\beta} \sum_n h(n/R) D^\beta \alpha \Big( \frac{\lambda - n}{R} \Big), \]
%
so that
%
\[ |D^\beta m_{0,R}(\lambda)| \lesssim_N R^{-\beta} \| h \|_{L^\infty} \langle \lambda/R \rangle^{-N} \lesssim \lambda^{-\beta} \| h \|_{L^\infty}. \]
%
Thus $m_{0,R}$ is a symbol of order zero, uniformly bounded by the $L^\infty$ norm of $h$, and so for $1 < p < \infty$,
%
...
%
so we could reasonably use this bound for $k \lesssim 1$. On the other hand, H\"{o}lder's inequality implies that
%
...
\begin{align*}
    ...
    &\lesssim A_p(h) R^{\alpha_{p^*}} 2^{-k \alpha_{p^*}} \| f \|_{L^{p^*}(M)}.
...
% k >= log_2 R - O(1/a)
% 2^k / R >= 1 / 100000
This bound is summable in $k$, resulting in a bound uniform in $R$, provided that
%
\[ k \geq \log_2(R) - O(1). \]
%
...
%
is finite, then the strategy above leads us to study the wave equation over a very very small set of times, i.e. $100/R \leq |t| \leq R^{-O(\varepsilon)}$.
...
\begin{align*}
    ...
    &= \int H_R(t) a_{\lambda,\nu}(x,t,y,\xi) e^{2 \pi i \Phi(t,x,y,\xi)}.
\end{align*}
%
Let us suppose that, using the techniques of their paper, we can show that
%
\[ \| T_R^\lambda f \|_{L^{p^*}(\RR^d)} \lesssim_{p,\varepsilon} \lambda^{\alpha(p^*) + \varepsilon} \left( \sum_\nu \| T^\lambda_{R,\nu} f \|_{L^{p^*}(\RR^d)}^{p^*} \right)^{1/p^*}. \]
%
Thus it suffices to analyze the behaviour of each of the operators $T^\lambda_{R,\nu}$ separately. For each fixed $t$, energy conservation implies that
%
\begin{align*}
    \| T_R^\lambda f \|_{L^2(\RR^d)} &\lesssim \left( \int_{-1/2}^{1/2} H_R(t)\; dt \right) \| f^\lambda_\nu \|_{L^2(\RR^d)} \\
    &\lesssim \int_{-1/2}^{1/2} R \langle Rs \rangle^{-(d-1)(1/2 - 1/p)} \| f^\lambda_\nu \|_{L^2(\RR^d)}\\
    &\lesssim R \| f^\lambda_\nu \|_{L^2(\RR^d)}.
\end{align*}
%
Thus $L^2$ orthogonality implies that
%
\[ \left( \sum_\nu \| T^\lambda_{R,\nu} f \|_{L^2(\RR^d)}^2 \right)^{1/2} \lesssim R \| f \|_{L^2(\RR^d)}. \]
%
...
%
\[ T^\lambda_{R,\nu} f = \int_{-1/2}^{1/2} H_R(t) T^{\lambda,t}_{R,\nu} f, \]
%
then the kernel $K^{\lambda,t}_{R,\nu}$ of $T^{\lambda,t}_{R,\nu}$ satisfies estimates of the form
%
\[ |K^{\lambda,t}_{R,\nu}(x,t;y)| \lesssim_N \frac{\lambda^{(d+1)/2}}{\Big\langle \lambda | \pi_\nu \nabla_\xi \Phi(x,y,t,\nu) | + \lambda^{1/2} | \pi_\nu^\perp \nabla_\xi \Phi(x,y,t,\nu) | \Big\rangle^N}. \]
%
Here we have $\nabla_\xi \Phi(x,y,t,\nu) = (x - y) + t \nu + O(|x - y|)$. Thus we conclude that, for a fixed $x$, this kernel has the majority of it's support on a cap centered at the point $x + t \nu$, with thickness $1/\lambda$ in the direction $\nu$, and thickness $1/\lambda^{1/2}$ in directions tangential to $\nu$. But this implies that the kernel of $K^\lambda_{R,\nu}$ is essentially supported on a $1/\lambda^{1/2}$ neighborhood of the line $\{ t \nu : -1/2 \leq t \leq 1/2 \}$, and moreover, on that line we have
%
\[ |K^\lambda_{R,\nu}(x;t\nu)| \lesssim \int_{t - 1/\lambda}^{t + 1/\lambda} R \langle Rs \rangle^{-(d-1)(1/2 - 1/p^*)} \lambda^{(d+1)/2} \; ds \]
%
For $\lambda \geq R$, and $|t| \leq 10/R$, we have
%
\[ |K^\lambda_{R,\nu}(x;t\nu)| \lesssim R \lambda^{(d-1)/2}. \]
%
For $\lambda \geq R$, and $|t| \geq 10/R$, we get that
%
...
%
\[ \left( \sum \| T^\lambda_{R,\nu} f \|_{L^{p^*}}^{p^*} \right)^{1/p^*} \lesssim R^{2/p^*} \| f \|_{L^{p^*}}, \]
%
and thus that
%
\[ \| T^\lambda_R f \|_{L^{p^*}} \lesssim_\varepsilon \lambda^{\alpha(p^*) + \varepsilon} R^{2/p^*} \| f \|_{L^{p^*}}. \]
