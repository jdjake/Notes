...
\begin{fact}[Bang-Jensen]
    Let $x$ be a preflow on a directed graph. Then there exists a random $r$-rooted arboresence $A$ such that $\mathbf{P}(e \in A) \leq x(e)$, and $\mathbf{P}(v \in A)$ is equal to the max $r-v$ flow with respect to $x$.
\end{fact}
