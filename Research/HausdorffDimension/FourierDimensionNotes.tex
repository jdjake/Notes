...
%
It will be useful to note that if $\mu_\varepsilon$ is compactly supported, then it is sufficient to prove
%
\[ \sup_{m \in \RR^d} |m|^{s - \varepsilon} |\widehat{\mu_\varepsilon}(m)| < \infty. \]
%
...
\[ \frac{nd - s}{n}, \]
%
such that for each set of $n$ distinct elements $x_1, \dots, x_n \in X$, $(x_1, \dots, x_n) \not \in Z$. We rely on a random selection approach, like in our paper on rough configurations, to obtain such a result.
...
\begin{lemma} \label{discreteLemma}
    ...
    \begin{enumerate}
        ...
        \item[(B)] If $L = \sup_{m \in \ZZ^d} |m|^{\frac{dn-s}{2n} - a\varepsilon} |\widehat{\mu_T}(m)|$, then
        %
        \begin{align*}
            &\sup_{m \in \ZZ^d} |m|^{a\varepsilon - \frac{dn-s}{2n}} |\widehat{\mu_S}(m)|\\
            &\ \ \ \ \ \ \leq (1 + 1/2^m) \left( \max\left( 1, L \right) + 10^d M_{m+1}^{-\varepsilon} \right),
        \end{align*}
        %
        where
        %
...
\begin{remark}
    To make the statement of \eqref{discreteLemma} more clean, we have hidden the explicit constant $C(\mu_T,n,d,s,\varepsilon)$. But this constant can be made explicit by determining which value $C(\mu_T,n,d,s,\varepsilon)$ must be chosen to ensure that \eqref{equation1095121284102} implies \eqref{equation10491249012}, \eqref{equation194012904129009}, \eqref{equation1940129041}, \eqref{equation129041902412901290129}, \eqref{equation190512919204912901}, \eqref{equation68943893493849}, \eqref{equation1290412904129049102}, and \eqref{equation129301923109} are all satisfied.
\end{remark}
...
\begin{proof}[Proof of Lemma \ref{discreteLemma}]
    ...
    %
    then $S$ is $\DQ_{m+1}$ discretized, $\mu_S$ is supported on $S$, and $S \subset T$. Our goal is to show that, with non-zero probability, some choice of $\{ j_i \}$ yields a set $S$ satisfying Properties (A) and (B) of the Lemma.
...
\begin{proof}
    ...
    \end{equation}
    %
    For a fixed $m$, the family of random variables $\{ X_{im} \}$ are independent. Furthermore, $\sum X_{im} = \widehat{\nu_S}(m) - \widehat{\EE(\nu_S)}(m)$. Equations \eqref{equation990249012409129041290} and \eqref{equation129412904912090} imply that
    %
...
\begin{proof}
    ...
    \begin{split}
        ...
        &\ \ \ \ \ \ \ \leq \frac{A(d + 1 + s/2) 2^{1 + s/2} |\eta|^{-1}}{|\eta|^{s/2}}\\
        &\ \ \ \ \ \ \ \leq \frac{A(d + 1 + s/2) 2^{1 + s/2} l_{m+1}}{|\eta|^{s/2}}\\
        &\ \ \ \ \ \ \ \leq \frac{L}{10 \cdot 2^d \cdot |\eta|^{s/2}}.
    \end{split}
    \end{equation}
    %
    ...
    \begin{equation} \label{equation2194129}
        \int_{|\eta|/2 \leq |\xi| \leq 2|\eta} |\widehat{\mu_T}(\eta - \xi)| |\widehat{\psi_{m+1}}(\xi)|\; d\xi \leq \frac{L}{10 \cdot 2^d \cdot |\eta|^{s/2}}.
    \end{equation}
    %
...
\begin{proof}[Proof of Lemma \ref{discreteLemma}, Continued]
    ...
    \end{align*}
    %
    Applying Lemma \ref{deviationLemma}, we conclude that if $|m| \leq 10 l_{m+1}^{-1}$,
    %
    \begin{align*}
        |\widehat{\nu_{S}}(m)| &\leq |\widehat{\nu_{S}}(m) - \widehat{\mu_T}(m)| + |\widehat{\mu_T}(m)|\\
        &\leq r_{m+1}^{d/2} \log(M_{m+1}) + \left( L \right) |m|^{c\varepsilon - \frac{dn-s}{2n}}\\
        &\leq l_{m+1}^{-\frac{dn-s}{2n}-c\varepsilon + \varepsilon} + \left( L \right)|m|^{c\varepsilon - \frac{dn-s}{2n}}\\
        &\leq \left[ L + 10^d M_{m+1}^{-\varepsilon} \right] |m|^{c\varepsilon - \frac{dn-s}{2n}}.
    \end{align*}
    %
    Lemma \ref{largeFrequencyLemma} with $r = 2c\varepsilon - (dn - s)/n$ implies that for $|m| \geq 10l_{m+1}^{-1}$,
    %
    \[ |\widehat{\nu_{S}}(m)| \leq L |m|^{c\varepsilon-\frac{dn-s}{2n}} \leq |m|^{c\varepsilon - \frac{dn-s}{2n}}, \]
    %
    Thus we conclude that for all $m \in \ZZ^d$,
    %
    \[ |\widehat{\nu_S}(m)| \leq (L + 10^d M_{m+1}^{-\varepsilon}) |m|^{c\varepsilon - \frac{dn-s}{2n}}. \]
    %
    ...
    \end{equation}
    %
    we conclude that for all $m \in \ZZ^d$,
    %
    \begin{align*}
        |\widehat{\mu_S}(m)| &\leq \frac{L + 10^d M_{m+1}^{-\varepsilon}}{1 - M_{m+1}^{-1/2}} |m|^{c\varepsilon - \frac{dn-s}{2n}}\\
        &\leq (1 + M_{m+1}^{-1/2}) [L + 10^d M_{m+1}^{-\varepsilon}] |m|^{c\varepsilon - \frac{dn-s}{2n}}\\
        &\leq (1 + 1/2^m) [L + 10^d M_{m+1}^{-\varepsilon}] |m|^{c\varepsilon - \frac{dn-s}{2n}}. \qedhere
    \end{align*}
\end{proof}
...
%
\[ A_{m+1,\varepsilon} \leq (1 + 2^{-m})(A_{m,\varepsilon} + 10^d M_{m+1}^{-\varepsilon_{m+1}}). \]
%
Since $\prod_{m = 1}^\infty (1 + 2^{-m}) < \infty$, we conclude that
%
\[ \sup_{m \to \infty} A_{m,\varepsilon} \lesssim 10^d \sum_{m = 0}^\infty M_{m+1}^{-\varepsilon_{m+1}}. \]
%
If, we assume \eqref{equation12901290}, then the right hand side of the inequality is summable. And so we conclude $\sup_{m \to \infty} A_{m,\varepsilon} < \infty$ for each $\varepsilon > 0$. Thus $X$ has Fourier dimension $(dn - s) / n$.
