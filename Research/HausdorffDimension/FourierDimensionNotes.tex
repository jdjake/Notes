...
\begin{itemize}
	...
	\item (Bounded Variables are Subgaussian): If $|X| \leq A$ almost surely, then $\psitwo{X} \leq 10 A$.
	...
	\item (Hoeffding's Inequality): If $X_1, \dots, X_N$ are \emph{independent}, then
	%
...
%
Since $\varepsilon$ was arbitrary, we conclude $X$ has Fourier dimension $(dn - s) / n$. Because $X_m$ is the union of $(M_1 \dots M_m)^d$ sidelength $l_m$ cubes, one can easily show using \eqref{equation5890129048128941891} that the lower Minkowski dimension of $X$ is upper bounded by $(dn - s)/n$. Thus $X$ has Hausdorff dimension $(dn - s)/n$ as well, and so $X$ is Salem. This concludes the proof of Theorem \ref{maintheorem}.
