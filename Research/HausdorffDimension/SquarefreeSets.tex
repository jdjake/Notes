...
%
so by induction, if $J$ is a length $1/M_{N+1}$ interval considered in the process, then
%
\begin{align*}
    \mu(J) \leq \prod_{n = 1}^N \frac{M_n}{(C_n M_{n+1})^{\frac{1}{d-1}}} = \left( \prod_{n = 1}^N \frac{M_{n+1}^{1-\frac{1}{d-1} }}{C_n^{\frac{1}{d-1}}} \right) \frac{1}{M_{N+1}} = \frac{A_{N+1}}{M_{N+1}}
\end{align*}
%
Now if $1/M_{N+1} \leq |I| \leq 1/M_N$, then $I$ can be covered by $|I|M_{N+1}$ cubes of length $1/M_{N+1}$, so
%
\begin{align*}
    \mu(I) &\leq |I|M_{N+1} \frac{A_{N+1}}{M_{N+1}} = A_{N+1}|I| = \frac{A_N M_{N+1}^{1 - \frac{1}{d-1}}}{C_N^{\frac{1}{d-1}}} |I| \lesssim_\varepsilon |I|M_{N+1}^{1 - \frac{1}{d-1} - \varepsilon} \leq |I|^{\frac{1}{d-1} - \varepsilon}
\end{align*}
%
Provided that we can choose $M_N$ such that $A_N/C_N \lesssim_\varepsilon M_{N+1}^\varepsilon$ for all $\varepsilon$. This is true, for instance, if we define $M_{N+1} = \exp(A_N/C_N) + O(1)$.
...
Now we discuss a third method of interpolating the Hausdorff dimension bounds, which requires us to choose the lengths $l_n$ in advance as a function of the values beforehand.
