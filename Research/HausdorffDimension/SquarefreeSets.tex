...
%
Playing around with these inequalities shows the most elegant choices of parameters guaranteeing all such constraints are
%
\[ N_n = n\ \ \ l_n = 1/8^{n-1}n!\ \ \ \Delta_n = l_n\ \ \ \varepsilon_n = 6l_n \]
%
We have avoided giving these constants until the end to emphasize {\it why} Keleti needed to choose these particular constants. Once they are chosen, we end up with a Hausdorff dimension one set. What's more, these parameters also give us a set $X$ with Hausdorff dimension one. The intuitive reason for this is that at the $N$'th scale of the process, we have $N!$ intervals of length $1/8^{N-1}N!$, and these scales are roughly comparable. The fact that $N!/8^{N-1}N! = o(1)$ implies that our set won't have positive Lebesgue measure, but is sufficiently slowly decaying compared to the number of intervals that we can show the sets have Hausdorff dimension $\geq 1 - \varepsilon$ for every $\varepsilon$. For completeness, and to introduce readers to the methods of calculating Hausdorff dimension, we will prove this explicitly.
...
Since each $X_N$ consists of $\prod_{n \leq N} N_n$ intervals, any one such interval $I$ at this scale has
%
...
\section{A Note on Calculating Hausdorff Dimension}
...
\begin{example}
    Fraser and Pramanik construct a $1/(d-1)$ dimensional set avoiding nontrivial solutions to a countable family of $d$ variate functions $f(x_1, \dots, x_d)$. Does our method apply to extend from the discrete scale bound?
\end{example}

\begin{remark}
    If this probability measure fails to satisfy the required bounds, does it follow that the set itself doesn't have the required Hausdorff dimension? Is there a counterexample to this statement?
\end{remark}
