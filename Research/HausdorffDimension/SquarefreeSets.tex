...
Katz and Tao have also found a way to work at scales at exponential rather than dyadic differences. We fix a positive integer $N$, and consider {\bf hyperdyadic scales}, which are of the form $2^{- N^k}$. For any $n$, and $\delta > 0$, the number of hyperdyadic scales between $\delta$ and $\delta^n$ is $O(\log n / \log N)$, most importantly, independant of $\delta$. which makes things very easy to work with, rather than $O(n \log(1/\delta)/\log N)$, as in the dyadic scales. Unfortunately, a naive covering lemma does not give us good results about the Hausdorff measure with respect to hyperdyadic covers. Here choosing $2^{-N^{k+1}} \leq r_n \leq 2^{-N^k}$, and the multiplicative gap between the interior and exterior values is still on the order of $2^{N^{k+1}}$, which isn't even independant of $k$. However, Katz and Tao found a way to work with such bounds.

We let $A \preccurlyeq B$ means $A \leq (1/\delta)^{CN} C_N B$ for some constants $C_N, C$. We let $A \approx B$ mean $A \preccurlyeq B$ and $B \preccurlyeq A$ simultaneously hold. We say a set $E$ is $\delta$ discretized if it is the union of balls $\approx \delta$.
...
\begin{proof}
    ...
    %
    \[ \sum_n r_{\delta n}^{\alpha + CN} \lesssim 1 \]
    %
    Choose $m_n$ such that $2^{-N^{m_n+1}} \leq r_{\delta n} \leq 2^{-N^{m_n}}$. Now
    %
    \[ \frac{2^{-(\alpha + CN) N^{m_n}}}{r_{\delta n}^{\alpha + tCN}} = \frac{2^{- (\alpha + CN) N^{m_n}}}{2^{- (\alpha + tCN) N^{m_n + 1}}} = 2^{- \alpha N^{m_n} + (\alpha - C)N^{m_n + 1} + tCN^{m_n+ 2}} \]
    %
    \[ \sum_n 2^{-(\alpha + CN)N^{m_n}} = \sum_n 2^{- \alpha N^{m_n}} 2^{- CN^{m_n + 1}} = \sum_n 2^{- \alpha N^{m_n}} r_{\delta n}^{-C} \]

    Can we make $2^{-\alpha N^{m_n}} \leq r_{\delta n}^{\alpha + CN}$
\end{proof}
...
Josh suggested an alternate idea. Given $Y$, we can consider a hyperdyadic cover $Y$ by $A_1$ sidelength $2^{-N}$ cubes, $A_2$ sidelength $2^{-N^2}$ cubes, and so on and so forth, with $\sum A_n/2^{\alpha N^n} \lesssim \varepsilon$.
