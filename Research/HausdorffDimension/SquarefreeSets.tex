...
% TODO: SOME EXAMPLES OF BOUNDED OPERATORS ON SUCH A SPACE?
...
% TODO: INTUITIONS FOR HAUSDORFF DIMENSION
...
\begin{algorithm}
    \begin{algorithmic}%[1]
        ...
        \MRepeat
            ...
            \MForAll{\ $J \in \mathcal{B}(L_n)$ completely contained in $X_N$}
                \State{Order the intervals in $\mathcal{B}(L_{n+1})$ contained in $J$ as $J_0, J_1, \dots, J_M$}

                \State{{\bf If} $J \subset I$, add all odd indexed intervals $J_1, J_3, \dots$ to $X_{N+1}$}
                \State{{\bf Else} Add all even indexed intervals $J_0, J_2, \dots$}
            \EndForAll
