...
%
By approximating the indicator function of an interval $[a,b] \subset [0,1]$ from above and below by continuous functions to an arbitrary degree of error, we conclude that
%
\[ \frac{\# \{ n \in [1,N]: [n \alpha] \in [a,b] \}}{N} \to b - a \]
%
where for a general point $x \in \mathbf{R}$, we let $x]$ denote the coset containing $x$ in $\mathbf{T}$. Later on we try and apply equidistribution theory in our Cantor dissection techniques. Here we discuss some of the higher dimensional results we will apply. The most famous proof of Weyl's equidistribution theorem is obtained by approximating $f$ by a Fourier series, reducing the problem to proving that the theorem is true when $f(x)$ is an exponential $e(nx) = e^{2 \pi i n x}$. In higher dimensions, essentially the same approximation techniques give an analogous result.
...
In the higher dimensional case, sequences can equidistribute themselves on subsets of the space. Ratner's theorem provides the precise version of this theorem.

\begin{theorem}[Ratner]
    If $x_n = n \alpha + \beta$, then we can write $\alpha = \alpha_0 + \alpha_1$, where the sequence $n \alpha_0$ is equidistributed on a subtorus of $\mathbf{T}^d$, and $n \alpha_1 + \beta$ is periodic.
\end{theorem}
\begin{proof}
    We induct on the dimension. For $d = 1$, the theorem is obvious, since either $\alpha$ is rational, and therefore $n \alpha + \beta$ is periodic, or $\alpha$ is irrational, and $n \alpha + \beta$ is equidistributed on $\mathbf{T}$. So now let us consider a sequence on the torus $\mathbf{T}^{d+1}$. If $\alpha$ is irrational, then $x_n$ is equidistributed, and the theorem is obvious. Otherwise, there exists $\xi \in \mathbf{Z}^{d+1}$ such that $\xi \cdot \alpha \in \mathbf{Z}$. Then we may find $\alpha_1 \in \mathbf{Q}^{d+1}$ such that $\xi \cdot \alpha_1 = \xi \cdot \alpha$, so that $\alpha_0 = \alpha - \alpha_1$ is orthogonal to $\xi$. The sequence $n \alpha_1 + \beta$ is periodic, whereas $n \alpha_0$ takes values in the subspace of points $x$ with $\xi \cdot x = 0$. If $\xi_{n+1} \neq 0$ (for simplicity, changing indices if necessary if some other $\xi_i \neq 0$), then this subspace is spanned by the vectors $v_i = \xi_{n+1} e_i - \xi_i e_{n+1}$, for $i \leq n$. The map $x \mapsto \sum x_i v_i$ is then an isomorphism between $\mathbf{R}^d$ and $\xi^\perp$, which descends to a homomorphism from $\mathbf{T}^d$ to $\xi^\perp/\mathbf{Z}^d$. 
\end{proof}
...
\begin{remark}
    Since $\xi_0$ is rational, the perpendicular subspace to $\xi_0$ contains a rational lattice?
\end{remark}

\begin{remark}
    If we consider just $x_n = n \alpha$, then Ratner's theorem says there is $M$ such that if $y_n = X_{Mn}$, such that $y_n$ is equidistributed on a subtorus containing the origin.
\end{remark}
...
If $T$ is a non-invertible matrix containing irrational coefficients, then the values $Tx$, for $x \in \mathbf{Z}^n$, do not form a lattice, and therefore we cannot use the direct combinatorial arguments of the past section to obtain the decomposition lemma. However, without loss of generality, we can write $T(x) = S(x_1) + U(x_2)$, where $x = (x_1,x_2)$, $x_1 \in \mathbf{R}^k$, $x_2 \in \mathbf{R}^{n-k}$, and $S$ has full rank $k$. Then $S$ is an embedding of $\mathbf{R}^k$ into $\mathbf{R}^n$, so $\Gamma = S((\mathbf{Z}/N)^k)$ forms a lattice with points spaced apart by a minimum distance $\Omega_T(1/N)$. Since $T$ has rank $k$, the image of $U$ is contained within the image of $S$. We let $\mathbf{T}$ denote the torus obtained by quotienting the $k$ dimensional subplane forming the image of $T$ by $\Gamma$. If $\pi: \mathbf{R}^k \to \mathbf{T}$ is the homomorphism obtained by composing the quotient map onto the torus with the linear map $T$, then $\pi(x_1,0) = 0$ for all $x_1 \in (\mathbf{Z}/N)^k$. If we consider $\pi(0, nx_2) = n \pi(0,x_2)$, then this sequence is equidistributed on $\mathbf{T}$ if and only if $\xi \cdot U(x_2) \not \in \mathbf{Z}$ for all nonzero $\xi \in \mathbf{Z} \langle S(\mathbf{Z}/N)^k \rangle$. In particular, unless $\xi \cdot U(x_2) = 0$ for one of the discrete $\xi$, because there are only countably many $\xi$ we can find $\alpha$ arbitrarily close to one such that $\xi \cdot U(\alpha x_2) \not \in \mathbf{Z}$, so we get an equidistribution result. Regardless, the sequence $n \pi(0,x_2)$ is equidistributed on a subtorus on $\mathbf{T}$, which should enable us to concentrate a particular subsequence concentrated at certain parts of the torus.
