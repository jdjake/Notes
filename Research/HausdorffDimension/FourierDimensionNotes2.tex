...
\begin{proof}
    ...
    %
    is precisely the family of $(E,\mu) \in X$ such that $E$ has Hausdorff dimension $\beta$. Certainly any $E$ in this family must have $H^\alpha(E) = 0$ for all $\alpha > \beta$, so $\hausdim(E) \leq \beta$. But the condition that $\mu \in M(A)$ implies $\fordim(\mu) \geq \beta$. Thus
    %
    \[ \beta \geq \hausdim(E) \geq \fordim(E) \geq \fordim(\mu) \geq \beta, \]
    %
    hence all these quantities are equal to $\beta$. Thus it suffices to show that $A(\alpha,\varepsilon)$ is dense in $X$ for $\alpha > \beta$ and $\varepsilon > 0$.

    Fix $(E,\mu) \in X$ and $\varepsilon > 0$. Without loss of generality, we may assume $\mu \in C^\infty(\TT^d)$. Consider large integers $N$ and $M$, and write $\varepsilon = 1/NM$. Then construct a family of independant, uniformly distributed random variables $X_a \in [M]^d$ for each $a \in [2N]^d$. Define a measure $\nu$ such that for each $x \in \TT^d$,
    %
    \[ d\nu(x) = \left( \sum_{a \in [2N]^d} \phi_\varepsilon \left( x - \frac{a}{2N} - \frac{X_a}{2NM} \right) \right) d\mu(x). \]
    %
...
\section{Appendix: Random Segments of Smooth Functions}
...
\begin{proof}
    ...
    %
    \begin{align*}
        |Y(i,j_1) - Y(i,j_2)| &= \int_{\TT^d} \phi_\varepsilon(x) \left( d\mu_0 \left( x + \frac{i}{2N} + \frac{j_1}{2NM} \right) - d\mu_0 \left( x + \frac{i}{2N} + \frac{j_2}{2NM} \right) \right)\\
        &\lesssim_d N^{-1} \| \phi_\varepsilon \|_{L^1(\TT^d)} \| \nabla \mu_0 \|_{L^\infty(\TT^d)} \lesssim_{\mu_0} N^{-1}.
    \end{align*}
    %
    In particular, this implies $|Y(i,X_i) - \EE(Y(i,X_i))| \lesssim_{d,\mu_0} N^{-1}$. Summing up this inequality over all $i \in [2N]^d$ and applying \eqref{equation555326} gives
    %
    \[ |\nu(\TT^d) - \EE(\nu(\TT^d))| \lesssim_{d,\mu_0} N^{d-1}. \]
    %
    ...
    %
    \[ \nu(\TT^d) = (2N)^d \left( 1 + O_{d,\mu_0}(1/N) \right). \qedhere \]
\end{proof}
...
\begin{proof}
    ...
    %
    Then for each $k \in \ZZ^d$, define $X_{ik} = \widehat{\nu_i}(k)$. For each $i$ and $k$, the standard $(L^1,L^\infty)$ bound on the Fourier transform, combined with a mean value-theorem type bound, imply that
    %
    \begin{equation} \label{equation123102}
    \begin{split}
        |X_{ik}| \leq \nu_i(\TT^d) \lesssim_{\mu_0,d} \EE(\nu_i(\TT^d)) + N^{-1}.
    \end{split}
    \end{equation}
    %
    We also calculate using H\"{o}lder's inequality that
    %
    \begin{equation} \label{equation923521}
        |\EE \nu_i(\TT^d)| \lesssim \| \mu \|_{L^\infty(\TT^d)}.
    \end{equation}
    %
    Combining \eqref{equation123102} and \eqref{equation923521}, we conclude that
    %
    \begin{equation} \label{equation654321}
    \begin{split}
        \sum_i \left( \EE(\nu_i(\TT^d)) + N^{-1} \right)^2 &\lesssim \sum_i \EE(\nu_i(\TT^d))^2 + N^{-2} \lesssim_{\mu_0} N^d.
    \end{split}
    \end{equation}
    %
    For a fixed $k$, the family of random variables $\{ X_{ik} : i \in [2N]^d \}$ are independant and moreover, $\sum X_{ik} = \widehat{\nu}(k)$. Thus Hoeffding's inequality together with \eqref{equation654321} implies that there exists a constant $C$, depending on $\mu$ and $d$, such that for each $t \geq 0$,
    %
    \[ \PP \left( \left| \widehat{\nu}(k) - \EE(\widehat{\nu}(k)) \right| \geq t \right) \leq 2 \exp \left( - Ct^2 / N^d \right). \]
    %
    In particular, applying a union bound over all $|k| \leq 10/N$
\end{proof}
