...
\begin{remarks}
    ...
    \begin{enumerate}
        ...
        \item Unlike in \cite{OurPaper}, the case of Theorem \ref{maintheorem} when $0 \leq \alpha < d$ is still interesting, since it is non-obvious that the trivial construction $[0,1]^d - \pi_i(Z)$, which has Hausdorff dimension $d$, is necessarily a Salem set, where $\pi_i(x_1,\dots,x_n) = x_i$ is projection onto a particular coordinate. That this set might not necessarily be Salem is hinted at in Example 8 of \cite{Ekstrom2014}, where it is shown that there exists a set $E \subset [0,1]$ which is the countable union of a family of compact sets $\{ E_k \}$ with $\sup_k \minkdim(E_k) \leq 3/4$ and $\fordim([0,1] - E) \leq 3/4$. For each $n \geq 2$, set
        %
        \[ Z = \bigcup_{i = 0}^{n-1} \left[ \{ 0 \}^i \times  \left( \bigcup_{t \in \mathbf{Q} / \mathbf{Z}} (E + t) \right) \times \{ 0 \}^{n - 1 - i} \right]. \]
        %
        Then for each $i$, $\pi_i(Z)$ is a dense subset of $\mathbf{T}^d$. Theorem \ref{maintheorem} constructs a Salem set $X$ avoiding $Z$ with $\fordim(X) > 3/4$, whereas for each $i \in \{ 1, \dots, n \}$, $\fordim([0,1] - \pi_i(Z)) \leq 3/4$.
    \end{enumerate}
\end{remarks}
...
%    \[ \EE( e^{2 \pi i \xi \cdot X_1(k)} \mathbf{I}(k \in S) ) = \PP(k \in S) \EE \left( e^{2 \pi i X_1(k)} | k \in S \right) \]
    ...
    %
    which completes the proof of this lemma in the case $n = 2$.
    ...
    %
    but the analysis of this equation is more complicated now that we cannot come up for an explicit formula for $\PP(1 \in S|X_n(1) = x)$. For a set $E \subset \TT^{d(n-1)}$, let $A(E)$ denote the event that there exists $k_1,\dots,k_{n-1}$ such that $(X_1(k_1),\dots,X_{n-1}(k_{n-1})) \in E$. Then
    %
