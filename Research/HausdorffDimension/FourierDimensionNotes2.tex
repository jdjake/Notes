...
\begin{remarks}
    ...
    \begin{enumerate}
        ...
        %
        Then $Z$ is a dense subset of $\mathbf{T}^d$, and $\fordim([0,1] - \pi_i(Z)) \leq 3/4$ for each $i \in \{ 1, \dots, n \}$. Theorem \ref{maintheorem} constructs a Salem set $X$ avoiding $Z$ with
        %
        \[ \fordim(X) = (n - 3/4)/(n-1/2) > 3/4. \]
        %
        Thus the trivial solution does not necessarily give a pattern avoiding set with optimal Fourier dimension in this setting.
    \end{enumerate}
\end{remarks}
...
\begin{theorem} \label{theoremJOICVIOJVI122}
    Consider a countable union of smooth functions of the form $f^\alpha: V_\alpha \to U_\alpha$, where $V_\alpha$ is an open subset of $\TT^{d(n-1)}$, $U_\alpha$ is an open subset of $\TT^d$, and for each $k \in \{ 1, \dots, n-1 \}$, the matrix
    %
    \[ D_{x_k} f^\alpha(x_1,\dots,x_{n-1}) = \begin{pmatrix} \frac{\partial f^\alpha_i}{\partial x_{kj}} \end{pmatrix} \]
    %
    ...
    %
    \[ x_n \neq f^\alpha(x_1,\dots,x_{n-1}). \]
    %
    Moreover, if $\beta \leq \beta_0$, then the family of pairs $(E,\mu) \in \mathcal{X}_\beta$ such that $E$ is Salem and avoids solutions to the equation $x_n = f^\alpha(x_1,\dots,x_{n-1})$ for distinct points $x_1,\dots,x_n \in E$ is the complement of a set of first category in $\mathcal{X}_\beta$.
\end{theorem}
...
An important problem in current research on pattern avoidance is to construct sets $E$ which avoid \emph{linear patterns}, that is avoiding nontrivial solutions to equations of the form
%
...
\[ \fordim(X) = \frac{d}{n-1}. \]
%
The problem here is that if $n \geq 3$ and $\beta > d/(n-3/4)$, there is too much `overlap' between the various cubes we use in our covering argument; thus the inclusion-exclusion argument found in this proof cannot be used to control $\EE(\widehat{\sigma}(\xi))$. We believe our method can construct Salem sets with Fourier dimension $d/(n-1)$, but other tools are required to improve the calculation of Lemma \ref{lemmaOIJIOCJSOIJSIOJ123}.
