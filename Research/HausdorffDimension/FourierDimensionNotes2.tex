...
\begin{lemma} \label{LemmaGISCICS1}
    ...
    %
    Then there exists a constant $C$ such that% depending on $\beta$ and $d$, such that
    %
...
\begin{proof}[Proof of Lemma \ref{LemmaGISCICS1}]
    ...
    %
    then there exists a constant $C \geq 1$ such that %depending on $d$ and $\beta$ such that
    %
...
%    \end{equation}
    ...
    \end{equation}
    %
    Since $K \geq r^{\varepsilon_1/2 - \beta}$, we can apply Lemma \ref{remarkGGIVJIS} to $\eta_1$, and then apply Corollary \ref{lemmaIOJDD23124} to the result of Lemma \ref{remarkGGIVJIS} to conclude that for any $\delta > 0$ to find $r_0 > 0$ % depending only on $d$, $q$, $n$, $\beta$, $\mu_0$, $\varepsilon$, $\delta$, and $\varepsilon_1$
