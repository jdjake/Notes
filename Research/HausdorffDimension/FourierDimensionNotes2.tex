...
\end{equation}
%
then \eqref{discretefordim}, together with the Poisson summation formula, implies $\fordim(\mu^*) = \fordim(\mu)$. Since $\mu$ is compactly supported, it is also simple to see that $\hausdim(\mu^*) = \hausdim(\mu)$. Thus Theorem \ref{maintheorem} is clearly equivalent to it's periodic variant, introduced below.
...
%
Then the following standard properties hold:
%
\begin{enumerate}
    \item[(1)] For each $\varepsilon \in (0,1)$, $\phi_\varepsilon$ is a smooth probability density, and $\phi_\varepsilon(x) = 0$ for $|x| \geq \varepsilon$.

    \item[(2)] For any $\varepsilon \in (0,1)$,
    %
...
%    \end{equation}
    ...
    \item[(4)] For each $T > 0$, and for all $\varepsilon > 0$ and non-zero $k \in \ZZ^d$,
    %
...
\end{equation}
%
which follows from the fact that $\| \mu \|_{M(\beta/2 - \varepsilon/2)}$ is finite.
\begin{comment}
\begin{theorem}
...
\end{proof}
\end{comment}
A density approach following from this remark allows us to work with smooth measures for the remainder of this paper.

\begin{lemma} \label{smoothdensitylemma}
    The set of all $(E,\mu)$ with $\mu \in C^\infty(\TT^d)$ is dense in $\mathcal{X}$.
\end{lemma}
\begin{proof}
    Consider $(E,\mu) \in \mathcal{X}$. For each $\delta > 0$, consider the convolved measure $\mu_\delta = \mu * \phi_\delta$. Then $\mu_\delta \in C^\infty(\TT^d)$. If we set $E_\delta = E \cup \text{supp}(\mu_\delta)$, then we claim
    %
    \begin{equation} \label{equationFFSPCOS}
        \lim_{\delta \to 0}\; (E_\delta, \mu_\delta) = (E,\mu).
    \end{equation}
    %
    Since $\text{supp}(\mu_\delta) \subset E_\delta$, we conclude that
    %
    \begin{equation} \label{equationFFSICSI}
        d_H(E,E_\delta) \leq \delta.
    \end{equation}
    %
    Thus $E_\delta \to E$ as $\delta \to 0$ under the Hausdorff distance. Now fix $\varepsilon_1 \in (0,\beta/2]$ and $\varepsilon > 0$. For each $\xi \in \ZZ^d$, $|\widehat{\mu_\delta}(\xi)| = |\widehat{\phi_\delta}(\xi)| |\widehat{\mu}(\xi)|$, so
    %
    ...
    \end{equation}
    %
    Since $(E,\mu) \in \mathcal{X}$, we can apply \eqref{equationGFSCSC4} to conclude that there exists $R$ such that for $|\xi| \geq R$,
    %
    ...
    \end{equation}
    %
    By \eqref{approximationtoidentitypointwiseconvergence}, we conclude that there exists $\delta_0 > 0$ such that for $\delta \leq \delta_0$ and $|\xi| \leq R$,
    %
    \begin{equation} \label{equationDISCIIS}
        |\xi|^{\beta/2 - \varepsilon} |\widehat{\phi_\delta}(\xi) - 1| \leq \varepsilon.
    \end{equation}
    %
    ...
    \end{equation}
    %
    But from \eqref{equationDISCIIS} and \eqref{equationFSCISIXX} applied to \eqref{equationFFSCI}, we find that for $\delta \leq \delta_0$ and $|\xi| \leq R$,
    %
    ...
    \end{equation}
    %
    Since $\varepsilon$ and $\varepsilon_1$ were arbitrary, we conclude from \eqref{equationDAIWOIC} and \eqref{equationFFSICSI} that $(E_\delta,\mu_\delta) \to (E,\mu)$, completing the proof.
\end{proof}

\begin{remark} \label{remarkDOIWJDIOWJ2}
    We note that if $(E_0,\mu_0)$ are such that $\text{supp}(\mu_0) = E_0$, then in the proof above, one may let $E_\delta$ be equal to $\text{supp}(\mu_\delta)$, since then $d_H(E_0,E_\delta) \leq \delta$. It then follows that such pairs $(E_0,\mu_0)$ are approximated by pairs $(E,\mu)$, where $\mu$ is smooth and $\text{supp}(\mu) = E$.
\end{remark}

The reader might not be wondering why we don't work with the smaller space $\tilde{\mathcal{X}} \subset \mathcal{X}$ of pairs $(E,\mu) \in \mathcal{X}$ with $\text{supp}(\mu) = E$. The reason is that $\tilde{\mathcal{X}}$ is not a closed subset of $\mathcal{E} \times M(\beta)$, and so is not a complete metrizable space under the product topology. However, as a consolation, we note that one can prove that quasi-all elements of $\mathcal{X}$ belong to $\tilde{\mathcal{X}}$, so that one can think of $\mathcal{X}$ and $\tilde{\mathcal{X}}$ as being equal `generically'.
...
\begin{proof}
    ...
    \begin{split}
        |\xi|^{\beta/2 - \varepsilon} \left| \sum_{0 \leq |\eta| \leq |\xi|/2} \widehat{f}(\eta) \widehat{\mu_0}(\xi - \eta) \right| &\lesssim_{\mu_0,d} \left( 1 + \sum_{0 < |\eta| \leq |\xi|/2} \frac{1}{|\xi|^{d+1}} \right) \| f \|_{M(\beta/2 - \varepsilon)}\\
        &\lesssim_d \| f \|_{M(\beta/2 - \varepsilon)}.
    \end{split}
    \end{equation}
    %
...
The next lemma shows that provided that, for $K$ points $x_1, \dots, x_K \in \TT^d$, that there is enough square root cancellation in the exponential sum
%
...
\begin{lemma} \label{Lemma65493}
    ...
    \end{equation}
    %
    Then there exists a large integer $K_0(C,\beta,\varepsilon_1,\varepsilon_2,\varepsilon)$, such that if $K \geq K_0$, $\varepsilon_0 \geq C^{-1} K^{-1/\beta}$, and we define
    %
...
\begin{lemma} \label{lemmaIOJDD23124}
    ...
    \end{equation}
    %
    Then there exists a large integer $K_0(C,d,\beta,\mu_0,\varepsilon_1,\varepsilon_2,\varepsilon)$, such that if $K \geq K_0$ and $\varepsilon_0 \geq C^{-1} K^{-1/\beta}$, and we define
    %
    \[ f(x) = \frac{1}{K} \sum_{i = 1}^K \phi_{\varepsilon_0}(x - x_i), \]
    %
...
\begin{lemma} \label{LemmaGISCICS1}
    ...
    %
    Then there exists a constant $C(\beta,d)$, such that
    %
...
\begin{proof}
    ...
    \[ B = \{ \xi \in \ZZ^d : |\xi| \leq K^{1/\beta + 1} \}, \]
    %
    then there exists a constant $C(d,\beta) \geq 1$ such that
    %
...
%
    ...
    Fix a large integer $K$. Lemma \ref{LemmaGISCICS1} shows that there exists a constant $C(\beta,d)$, as well as $K$ points $x_1, \dots, x_K \in \TT^d$ such that if
    %
    \[ D(x) = \sum_{k = 1}^K \delta(x - x_k), \]
    %
    ...
    \end{equation}
    %
    Using \eqref{equationDDVVIXXSX23} to apply Lemma \ref{lemmaIOJDD23124}, we conclude that there exists $K_1(d,\beta,\mu_0,\varepsilon_1,\varepsilon)$, such that if $K \geq K_1$, if $\varepsilon_0 = K^{-1/\beta}$, if
    %
    ...
    \end{equation}
    %
    Note that $\mu$ is supported on $K$ balls of radius $\varepsilon_0$. Thus if we assume $K \geq \delta^{-1/\beta}$, so that $\varepsilon_0 \leq \delta$, we find that
    %
    \begin{equation} \label{equationGGSCPXX22}
        H^\alpha_\delta(\text{supp}(\mu)) \leq K \varepsilon_0^\alpha = K^{1 - \alpha/\beta}.
    \end{equation}
    %
    Since $\alpha > \beta$, \eqref{equationGGSCPXX22} implies that there is $K_2(\alpha,\beta)$ such that for $K \geq K_2$,
    %
    ...
    %
    where $\{ y_1, \dots, y_N \} \subset E_0$ is a $\varepsilon$ net of $E_0$. Set $K_0(d,\alpha,\beta,\delta,\mu_0,\varepsilon_1,\varepsilon) = \max(K_1,K_2, \delta^{-1/\beta})$, and suppose $K \geq K_0$. Equation \eqref{equationGGSXSOF9923} implies that $H^\alpha_\delta(E) \leq s$, so $(E,\mu) \in A(\alpha,\delta,s)$. Since $\text{supp}(\mu) \subset E$,
    %
...
\begin{lemma}
    For quasi-all $(E,\mu) \in \mathcal{X}$, for any distinct points $x_1, \dots, x_n \in E$, $(x_1, \dots, x_n) \not \in Z$.
\end{lemma}
\begin{proof}
...
%    \end{equation}
    ...
    and the result of Lemma \ref{LemmaGISCICS1}, we conclude that there exists $K$ points $x_1, \dots, x_K \in \TT^d$ and a constant $C(d,\beta)$ such that the following two statements hold:
    %
    \begin{itemize}
    ...
    \end{equation}
    %
    Since $\varepsilon_0 \geq (1/10) K^{-1/\beta_0}$, we can apply Lemma \ref{lemmaIOJDD23124} in conjunction with \eqref{equationGGSCSIAXAXXXSFGG} for any $\delta > 0$ to find an integer $K_0(d,\beta,\mu_0,\delta,\varepsilon_1)$, such that if $K \geq K_0$, if we define
    %
    ...
    \begin{equation} \label{equationvVVV323285853S}
        \| \mu - \mu_0 \|_{M(\beta_0/2-\varepsilon_1/2)} = \| \mu - \mu_0 \|_{M(\beta/2 - \varepsilon_1)} \leq \delta.
    \end{equation}
    %
    The topology induced by the norm $\| \cdot \|_{M(\beta/2 - \varepsilon_1)}$ is stronger than the weak $*$ topology on $M(\beta)$. We note that if $\{ \mu_k \}$ is a sequence of measures converging weakly to a measure $\mu$, with $\text{supp}(\mu_k) \subset \text{supp}(\mu)$ for each $k$, then $\text{supp}(\mu_k)$ converges in the Hausdorff metric to $\text{supp}(\mu)$. Thus if $\delta$ is suitably small relative to $\varepsilon$, then we conclude $d_H(\text{supp}(\mu), E_0) \leq \varepsilon$. If we also choose $\delta \leq \varepsilon$ then from \eqref{equationvVVV323285853S} we also conclude that $\| \mu - \mu_0 \|_{M(\beta/2 - \varepsilon_1)} \leq \varepsilon$. Thus all that remains is to show that if $E = \text{supp}(\mu_k)$, then for sufficiently large $K$, $(E,\mu) \in B(W,s)$. If fact, we will show this is the case for $\varepsilon_0 \leq s$.

    Consider $n$ points $y_1, \dots, y_n \in \text{supp}(\mu)$, with $|y_i - y_j| \geq \varepsilon_0$ for any two indices $i \neq j$. We can therefore find distinct indices $k_1, \dots, k_n \in \{ 1, \dots, K \}$ such that for each $i \in \{ 1, \dots, n \}$,
    %
    \begin{equation} \label{equationFFISICSIC223}
        |x_{k_i} - y_i| \leq (d^{-1/2}/2) \cdot \varepsilon_0.
    \end{equation}
    %
    ...
    \end{equation}
    %
    Thus in particular we conclude $y \not \in W$, which shows $(E,\mu) \in B(W,s)$ if $\varepsilon_0 \leq s$.
\end{proof}

\begin{concludingremarks}
    Let us end this Section by discussing where the loss in Theorem \ref{maintheorem} occurs in our proof, as compared to \cite{OurPaper}. In the last calculation, in order to obtain the bound \eqref{equationGGSCSIAXAXXXSFGG} using the $(L^1,L^\infty)$ bound using the Fourier transform, we were forced to choose parameters in such that $\#(S) \leq K^{1/2}$. However, if we were able to justify that square root cancellation occured in the sum
    %
