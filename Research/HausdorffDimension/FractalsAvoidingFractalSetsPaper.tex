...
\begin{proof}
	...
	%
	is the sum of $4^k$ independant, $\text{Bin}(4,p^k)$ random variables, each with probability $p^k$ chance of succeeding. Theorem A.1.14 of %TODO: PROBABILISTIC METHOD ALON AND SPENCER
...
%	We now apply the sub-Gaussian norm (see definition H). We calculate
	%
%	\[ \mathbf{E}(e^{\mathbf{I}(I_j \subset C_k)^2 / t^2}) = p^k e^{1/t^2} + (1 - p^k) e^{0/t^2} \]
	%
%	This is upper bounded by two exactly when $1/t^2 \leq \log(1 + 1/p^k)$. Now
	%
%	\[ \log(1 + 1/p^k) \gtrsim k \log(1/p) \gtrsim \alpha k \] %\geq \frac{2p^{-k}}{2 + p^{-k}} \geq \frac{1}{\sqrt{2} \cdot p^{k/2}} \]
	%
%	so $\mathbf{E}(e^{\mathbf{I}(I_j \subset C_k)^2/t^2}) \leq 2$ when $t \gtrsim \sqrt{\alpha k}$, so $\| \mathbf{I}(I_j \subset C_k) \|_{\psi_2} \lesssim \sqrt{\alpha k}$. Equation (\ref{squareindependence}) implies
	%
%	\[ \| Y_k \|_{\psi_2} \leq \sqrt{4^k} \cdot \sqrt{\alpha k} \lesssim 2^k \sqrt{\alpha k} \]
	%
%	Then (\ref{subgaussian}) shows there is a small constant $c$ such that for any $t > 0$,
	%
%	\[ \mathbf{P} \left( Y_k \geq t \right) \leq 2 \exp \left(- c t^2 / 4^k \alpha k \right). \]
	%
%	In particular, we conclude
	%
%	\[ \mathbf{P} \left( Y_k \geq 2^{(2-\alpha + \varepsilon)(k/2)} \right) \leq 2 \exp( - c \cdot 2^{k \varepsilon}). \]
	%
%	Most importantly,
	%
%	\[ \sum_{k = 1}^\infty \mathbf{P} \left( Y_k \geq 2^{(2-\alpha + \varepsilon)(k/2)} \right) < \infty. \]
	%
%	The Borel Cantelli lemma implies that almost surely, $Y_k \leq 2^{(2 - \alpha + \varepsilon)(k/2)}$ for sufficiently large $k$. But this certainly implies that
	%
%	\[ (2 - \alpha + \varepsilon)(1/2) \]
%	$\dim_{\mathbf{M}}(C) \leq (2 - \alpha + \varepsilon)(k/2)$
\end{proof}
...
%	 Theorem A.1.12 shows that
	%
%	\[ \mathbf{P} \left[ \tilde{X}_L \geq 2 p^k M \right] < [e^2/9]^{p^k M} \leq [e^2/9]^{c(2p)^k} = \exp(-C (2p)^k). \]
	%
%	Here we have set $C = c \cdot \log(9/e^2) > 0$. On the other hand, suppose $M \leq c 2^k$.


%	Thus Theorem A.1.14 shows that if $M > 1/p^{k(1 + \varepsilon)}$, there exists a small constant $c$ such that
	%
%	\[ \mathbf{P}\left[ \tilde{X}_L > 2Mp^k \right] < 2 \exp( -c M p^k) < 2 \exp(- c p^{- \varepsilon k}) \]
	%
%	On the other hand, if $M < 1/p^{k(1 + \varepsilon)}$, then Theorem A.1.16 implies that
	%
%	\[ \mathbf{P} \left[ \tilde{X}_L \geq Mp^k + t \right] \leq \exp(-t^2/2M) \leq \exp(-t^2 p^{k(2 + \varepsilon)}/2) \]
	%
%	Setting $t = \sqrt{M} p^{-k \varepsilon}$, we find
	%
%	\[ \mathbf{P} \left[ \tilde{X}_L \geq Mp^k + \sqrt{M} p^{-k \varepsilon} \right] \leq \exp(-p^{-2k \varepsilon}/2) \]
	%
%	Setting $t = p^{-k(1 + \varepsilon)}$ gives
	%
%	\[ \mathbf{P} \left[ \tilde{X}_L \geq Mp^k + p^{-k(2 + 2\varepsilon)} \leq \exp(- p^{-\varepsilon k}/2) \right] \]

%	This completes the proof. % TODO: FIXXXXXX!

%	$2Mp^k \lesssim (2p)^k$
%	$Mp^k + p^{-k(\varepsilon + 1/2)} \lesssim p^{-k(\varepsilon + 1/2)}$


%	We now apply concentration of measure of results, summarized using the subgaussian norm (see definition H). We calculate
	%
%	\[ \mathbf{E}(e^{\mathbf{I}(I_j \subset C_k)^2 / t^2}) = p^k e^{1/t^2} + (1 - p^k) e^{0/t^2} \]
	%
%	This is upper bounded by two exactly when $1/t^2 \leq \log(1 + 1/p^k)$. Now by concavity we know $\log(1 + 1/p^k) \leq 1/p^k$, so the expectation above is bounded by two when $t^2 \geq p^k$. Thus we have shown that $\| \mathbf{I}(I_j \subset C_k) \|_{\psi_2} \leq p^{k/2}$. Equation (\ref{squareindependence}) implies
	%
%	\[ \| \tilde{X}_L \|_{\psi_2} \leq \sqrt{M} \cdot p^{k/2} \lesssim (2p)^{k/2} = 2^{(k/2)(1 - \alpha)} \]
	%
%	Then (\ref{subgaussian}) shows there is a small constant $c$ such that for any $t > 0$,
	%
%	\[ \mathbf{P} \left(\tilde{X}_L \geq t \right) \leq 2 \exp \left(- c t^2 / 2^{(1 - \alpha) (k/2)} \right) \]
	%
%	In particular, we conclude
	%
%	\[ \mathbf{P} \left(\tilde{X}_L \geq 2^{(1-\alpha + \varepsilon)(k/2)} \right) \leq 2 \exp( - c \cdot 2^{k \varepsilon}). \tag*{\qedhere} \]
\end{proof}
...
%	Consider an arbitrary line $L$, and let $M$ denote the number of sidelength $1/2^k$ dyadic squares intersecting $L_\delta$, where $\delta = 1/2^k$. Label them $I_1, \dots, I_M$. We know that $M \lesssim 2^k$. Let $X_k$ denote the number of sidelength $1/2^k$ dyadic cubes in $C_k$ intersecting $L_{1/2^k}$. Since each sidelength $1/2^k$ cube is contained in $C_k$ with probability $p^k$, we know that $\mathbf{E}(X_k) = p^k M \lesssim (2p)^k$. If $Y_{I_j} = \mathbf{I}(I_j \subset C_k)$, then $X_k = Y_{I_1} + \dots + Y_{I_M}$

%	Since $C_k$ contains each sidelength $1/2^k$ square with probability $p^k$.

%	 such that the $1/2^k$ thickened line $L_{1/2^k}$ intersects $M$ sidelength $1/2^k$ dyadic squares $I_1, \dots, I_M$ in $[0,1]^2$. We know that $M \lesssim 2^k$, and on average, intersects $p^k M \lesssim (2p)^k$ dyadic cubes in $C_k$. We let $Y = \sum_{m = 1}^M X_{I_m}$ count the number of dyadic cubes in $C_k$ intersecting $L_{1/2^k}$. We wish to show $Y \lesssim 2^{k(1+\varepsilon)} p^k$ with overwhelming probability, so we may assume without loss of generality that $M \geq 2^{k(1 + \varepsilon)} p^k$. Hoeffding's inequality then implies that
	%
%	\begin{align*}
...
%	\end{align*}
	%
%	There are $O(4^k)$ lines which when thickened by $1/2^k$ are such that all lines are contained in one of the $1/2^k$ thickened line. Thus we can upper bound the probability that any line intersects more than $2^{k(1+\varepsilon)} p^k$ by
	%
...
%	\[ Z_0 = \{ (y_1, y_2, y_3) \in C^3: d(y_1,y_2) = d(y_1,y_3) \} \]
	%
%	Then almost surely, we may for sufficiently large $k$, cover $Z_0$ by $32^{k+\varepsilon} p^{3k}$ dyadic cubes. This implies that the Minkowski dimension of $Z_0$ is almost surely upper bounded by $5 - 3 \log_2(1/p)$. In particular, if $p = 1/2$, this shows $Z_0$ is almost surely two dimensional.
\end{proof}
