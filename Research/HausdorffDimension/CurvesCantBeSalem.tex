...
% phi(x) = xi_0 * A(x) - xi * x
% Stationary when xi(x) = xi
% Second derivative is
% sum_{j,k,l} xi_0^j partial A^j / partial x^k partial x^l
%   sum_j xi_0^j H(A^j)
% is invertible as a matrix in the k and l variables.

% Then at least we can conclude that, in a neighborhood of the stationary points, we have

% int e^{2 pi i xi * gamma(x + 1)} dmu(t)
% = int c(xi',xi) mu^(xi') dxi'
%
% where c(xi',xi) = int phi(x) e^{2 pi i [xi * A(x) - xi' * x]} dx
% The phase here vanishes when xi' = DA(x)^T xi for some x.
% If |xi'| = R, then xi is also proportional to R in order for this
% to be true, so equal to
% int phi(x) e^{2 pi i R [ xi^ * A(x) - xi'^ * x ]}
% This is an isolated critical point provided that xi(x) is injective
% defined as above. Perhaps under additional assumptions, we can
% make the critical point nondegenerate, and thus ensure that, near the
% stationary points, the magnitude of the quantity is O( R^{-d/2} )
% But mu^ has magnitude O( R^{-s/2} ) near these critical points
% The family of critical points takes up a region of mass O( R^d )
% So we conclude the integral over this region has magnitude O( R^{(d-s)/2} )
% |xi|^{d/2 - s/2} which is not good enough to obtain the required conclusion.
%
% OTHER IDEAS:
%
% The L2 norm of phi(x) e^{2 pi i [ xi * A(x) ]}
% is O(1), so the same should be true of it's Fourier
% transform, which is another way to see that the
% function can have at most magnitude O( R^{-d/2} ) over
% it's mass O(R^d) of stationary points.
%
% Is applying the multiplication formula doomed here?
%
% What if we work in the opposite direction, and
% consider a function psi whose Fourier transform is localized
% in a region of mass O(1) about a frequency xi_0 with magnitude R.
% Then
%
% int psi(x) dmu(x) = R^{-s/2}
%
% We can write psi(x) = e^{2 pi i xi_0 * x} phi(x)
%
% BONUS IDEA:
%
% WHAT ABOUT NONTRANSLATIONAL AVERAGES THAT MIGHT
...
% (x',y',z') = (x, y - sin(x), z - e^x)
% dx' = dx
% dy' = dy - cos(x) dx
% dz' = dz - e^x dx
% dz = dz' + e^x dx = dz' + e^x dx'
...
% dx' = { RESULT WHEN y = z = 0 } + y dv_1 + z dw_1
% dv_1
% dv_1 = -z (dx,dy,dz) dw_1 = -y

$ a + b v_1 + cw_1 = e^x$

% If we set $dz = dz' - e^x\; dx$, then $dz' = dz + e^x dx$
