...
\begin{abstract}
    We construct Salem sets with large dimension that avoid patterns described by the zero sets of a family of smooth functions, as well as rough families of patterns, complementing previous results constructing sets with large Hausdorff dimension, with results of varying generality depending on the regularity of the patterns involved. For (possibly uncountable) families of uniformly Lipschitz functions $\{ f_i : (\TT^d)^{n-2} \to \TT^d \}$, we obtain a Salem subset of $\TT^d$ with dimension $d/(n-1)$ avoiding nontrivial solutions to the equation $x_n - x_{n-1} = f_i(x_1,\dots,x_{n-2})$. For a countable family of smooth functions $\{ f_i : (\TT^d)^{n-1} \to \TT^d \}$ satisfying a modest geometric condition, we obtain a Salem subset of $\TT^d$ with dimension $d/(n-3/4)$ avoiding nontrivial solutions to the equation $x_n = f(x_1,\dots,x_{n-1})$. For a set $Z \subset \TT^{dn}$ which is the countable union of a family of sets, each with lower Minkowski dimension $s$, we obtain a Salem subset of $\TT^d$ of dimension $(dn - s)/(n - 1/2)$ which avoids the `rough pattern' specified by $Z$, i.e. whose Cartesian product does not intersect $Z$ except at points with non-distinct coordinates.
\end{abstract}
...
\begin{theorem} \label{thirdTheorem}
    Fix $0 \leq \alpha < dn$, $a \in \QQ - \{ 0 \}$, and a locally Lipschitz function $T: V \to \mathcal{E}$, where $V$ is an open subset of $\TT^{d(n-2)}$ and $\mathcal{E}$ is the family of all compact subsets of $\TT^d$, equipped with the Hausdorff distance metric. Suppose that the sets $T(x_1,\dots,x_{n-2})$ \emph{locally uniformly} have lower Minkowski dimension at most $\alpha - (n-1)d$, in the sense that for any $\gamma > \alpha$, and any closed set $W \subset V$, there exists a decreasing sequence $\{ r_i \}$ with $\lim_{i \to \infty} r_i = 0$ such that for $x \in W$, $|T(x)_{r_i}| \leq r_i^{nd-\gamma}$ for all $x \in V$. Set
    % dn 
...
%
This is the scenario in which we have robust \emph{upper bounds} on the dimension of pattern avoiding sets. It is simple to prove that if $E \subset \TT^d$ supports a measure $\mu$ such that $\fordim(E) > 2d/n$, then there exists some integers $m_1,\dots,m_n \in \ZZ$ and distinct points $x_1,\dots,x_n \in E$ such that $m_1x_1 + \dots + m_nx_n = 0$ (see Lemma 1.2 of \cite{Korner1}). Recently, under the same assumptions, Liang and Pramanik have shown (Lemma 1.5 of \cite{LiangPramanik}) that for $d = 1$, one can choose these integers $m_1,\dots,m_n$ to satisfy $m_1 + \dots + m_n = 0$. 
...
\end{theorem}
\begin{proof}
    Without loss of generality, we may assume $S$ is compact, and it will suffice to show that a generic Salem set $(E,\mu) \in \mathcal{X}_\beta$ avoids solutions to equations of the form
    %
    \begin{equation}
        x_n - a_{n-1} x_{n-1} = s + a_3x_3 + \dots + a_nx_n,
    \end{equation}
    %
    with $a_2,\dots,a_n \in \QQ$, $s \in S$, and where either $a_2 \neq 0$, or $a_2 = a_3 = \dots = a_n = 0$ (this reduction is possible because the countable intersection of sets of first category is also of first category). If $a_2 \neq 0$, then Theorem \ref{thirdTheorem} applies directly to the equation
    %
    \begin{equation} \label{FirstKindofEquation}
        x_1 + a_2 x_2 \in S(x_3,\dots,x_n),
    \end{equation}
    %
    where $T(x_3,\dots,x_n) = S - a_3x_3 - \dots - a_nx_n$. Thus we conclude that the set of $(E,\mu) \in \mathcal{X}_\beta$ such that $E$ is Salem and avoids solutions to \eqref{FirstKindofEquation} is comeager. On the other hand, if $a_2 = a_3 = \dots = a_n = 0$, then the equation is precisely $x_1 \in S$,
    %
    \begin{equation} \label{SecondKindofEquation}
        x_1 \in S,
    \end{equation}
    %
    and it follows from Theorem \ref{maintheorem} with $Z = S$ and $n = 1$ that the set of $(E,\mu) \in \mathcal{X}_\beta$ such that $E$ is Salem and avoids solutions to \eqref{SecondKindofEquation} is comeager.
...
\begin{proof}[Proof of Lemma \ref{LemmaFIOAJFOIWJ}]
    ...
    %
    \[ \eta = \frac{1}{N} \sum_{k = 1}^N a(x_k) \delta(x_k), \]
    %
    where $\delta(x_k)$ is a Dirac delta mass at $x_k$, then $\eta$ is a probability measure, and Property (2) implies that for $|\xi| \leq (1/r)^{1 + \kappa}$,
    %
...
%    \end{equation}
    ...
    \end{equation}
    %
    As long as we can show that $S_r \in \mathcal{B}(Z,s)$, \eqref{equationFIOJIOJOIadawda} and \label{equationFIOJIOJOIadawda} allows us to apply Lemma \ref{LemmaFIOAJFOIWJ}. To check this, consider $n$ points $y_1,\dots,y_n \in S_r$, with $|y_i - y_j| \geq r$ for any two indices $i \neq j$. We can therefore find distinct indices $k_1, \dots, k_n \not \in I$ such that for each $i \in \{ 1, \dots, n \}$, $|x_{k_i} - y_i| \leq r$, which means if we set $x = (x_{k_1}, \dots, x_{k_n})$ and $y = (y_1, \dots, y_n)$, then
    %
    ...
    \end{equation}
    %
    Thus in particular we conclude $y \not \in Z$. Thus $S_r \in \mathcal{B}(Z,s)$. Thus Lemma \ref{LemmaFIOAJFOIWJ} implies that $B(Z,s)$ is dense in $\mathcal{X}_\beta$ for each $s > 0$, completing the proof.
\end{proof}
...
\end{equation}
%
obtained by the triangle inequality, was viable enough to control the Fourier dimension of the resulting pair $(E,\mu)$. On the other hand, if we were able to show that one could instead obtain the \emph{square root cancellation bound}
%
...
\begin{proof}
    ...
    \end{equation}
    %
    which gives a contradiction, proving that $S_r \in \mathcal{H}(S_r \in \mathcal{H}(R_1,\dots,R_n))$. The remainder of the proof focuses on bounding exponential sums associated with $S$, so that we may apply Lemma \ref{LemmaFIOAJFOIWJ}.
    ...
    \end{equation}
    %
    Since $\sum_{x \in S} a(x) \gtrsim N$, so \eqref{equationDOIJAOIDJWOIJ} is a kind of exponential sum bound that allows us to apply Lemma \ref{LemmaFIOAJFOIWJ}. Thus together with \eqref{equationCOIJCOIJVIOJOIJIOJ4234141} and the fact that $S_r \in \mathcal{H}(R_1,\dots,R_n)$, we find that that $H(R_1,\dots,R_n)$ is dense in $\mathcal{X}_\beta$.
\end{proof}
...
\begin{remark}
    Before we begin the proof of this lemma, let us describe the idea of the proof. As a random quantity, $H(\xi)$ is a function of the independent random quantities $\{ X_i(k) \}$, and so McDiarmid's inequality presents itself as a useful concentration bound. However, a naive application of McDiarmid's inequality fails here, because changing a single random variable $X_i(k)$ for $1 \leq i \leq n-1$ while fixing all other random variables can change $H(\xi)$ by as much as $O(M)$ (see Figure \ref{thefigure}), which is far too much to obtain the square root cancellation bounds like we obtained in \eqref{equationCOIACOIAJCPPPPP}. On the other hand, it seems that a single variable $X_i(k)$ only changes $\widehat{\sigma}(\xi)$ by $O(M)$ if the random variables $\{ X_1(k) \}$ are configured in a very particular way, which is unlikely to happen. Thus we should expect that adjusting a single random variable $X_i(k)$ does not influence the value of $H(\xi)$ \emph{averaged} over the possible choices of $\{ X_1(k) \}$, and then we can apply McDiarmid's inequality.
\end{remark}
...
\caption{The two diagrams displayed indicate two instances of the process of defining the set $S$ for the function $f(x_1,x_2) = x_1^2 + x_2^2$. Here $M = 3$, $n = 3$, the values on the $x$-axis represent the values $X_1(1), X_1(2),$ and $X_1(3)$, the values on the $y$-axis represent the values $X_2(1), X_2(2)$, and $X_2(3)$, the dark points represent the family of all pairs $(X_1(k_1),X_2(k_2))$, and the annuli represent the $r$-neighborhoods of $f^{-1}(X_3(1)), f^{-1}(X_3(2))$, and $f^{-1}(X_3(3))$. In the left diagram, $S = \{ 1,2,3 \}$, whereas in the right diagram $S = \emptyset$. Thus $S$ can be completely changed simply by adjusting the position of the shaded point on the $y$-axis (adjusting one of the variables $X_2(k)$), which completely changes the value of the exponential sums associated with $S$. Nonetheless, this only occurs if the other values are arranged in a highly particular manner, which is why we are able to show that the \emph{average} value of the exponential sums associated with $S$ over all possible choices of the variables $\{ X_1(1), X_1(2), X_1(3) \}$ is not changed drastically when we vary the other variables, which enables us to apply McDiarmid's inequality after this averaging process.}
\label{thefigure}
\end{center}
\end{figure}
...
\begin{proof}
    ...
    \begin{equation} \label{equationCOIJCOIJX1232312}
        \PP \left( \left| H(\xi) - \EE(H(\xi)|\Sigma) \right| \geq t \right) \leq 4 \exp \left( \frac{-M t^2}{2} \right).
    \end{equation}
    %
    It is simple to see that
    %
    \begin{equation}
        \EE(H(\xi) | \Sigma) = \int_\Omega \psi_n(x) e^{2 \pi i \xi \cdot x}\; dx.
    \end{equation}
    %
    Since
    %
    \begin{equation}
        \Omega = \bigcup \left\{ T(X_1(k_1),\dots,X_{n-1}(k_{n-1}))_r : 1 \leq k_1,\dots,k_{n-1} \leq N \right\}.
    \end{equation}
    % K^{n-1} = r^{(n-1) \varepsilon_1/2 - d}
    we see that varying each random variable $X_i(k)$, for $1 \leq i \leq n-1$ while fixing the other random variables adjusts at most $M^{n-2}$ of the radius $r$ balls forming $\Omega$, and thus varying $X_i(k)$ independently of the other random variables changes $\EE(H(\xi)|\Sigma)$ by at most
    %
    \begin{equation}
        2 \cdot (2r)^d \cdot M^{n-2} = 2^{d+1} \cdot r^d \cdot M^{n-2} \leq \frac{2^{d+1}}{M} \leq \frac{2^{d+1}}{M}.
    \end{equation}
    % K^{-(n-1)} = r^d
    % 
    ...
    \begin{equation} \label{equationCIJCIJIVJIO}
        \PP \left( |\EE(H(\xi)|\Sigma) - \EE(H(\xi))| \geq t \right) \leq 4 \exp \left( \frac{-M t^2}{2^{2d+1}} \right).
    \end{equation}
    %
    ...
    \begin{equation} \label{equationCNCIJIJOJOPPPPOPODAW}
        \PP \left( | H(\xi) - \EE(H(\xi)) | \geq t  \right) \leq 8 \exp \left( \frac{-M t^2}{2^{2d + 1}} \right).
    \end{equation}
    %
...
%    \[ \EE( e^{2 \pi i \xi \cdot X_1(k)} \mathbb{I}(k \in S) ) = \PP(k \in S) \EE \left( e^{2 \pi i X_1(k)} | k \in S \right) \]
    ...
    \begin{equation} \label{equationGGIJICJIjjpopwaowarr}
        \EE(H(\xi)) = A_n \int_{\TT^d} \psi_n(x) \PP(1 \in I| X_n(1) = x_n) e^{2 \pi i \xi \cdot x_n}\; dx_n.
    \end{equation}
    %
    However, the analysis of this equation is made more complicated by the lack of an explicit formula for $\PP(1 \in I|X_n(1) = x)$. For a set $E \subset \TT^{d(n-1)}$, let $A(E)$ denote the event that there exists $k_1,\dots,k_{n-1}$ such that $(X_1(k_1),\dots,X_{n-1}(k_{n-1})) \in E$. Then
    %
    \begin{equation}
...
%    Then $|B_i(Q_i)| \lesssim K r^d$ and $|C_i(Q_i)| \lesssim K^2 r^{2d}$. If $\#(I) = m$, then we would conclude that $\PP(A(Q) \cap A(Q')) \lesssim K^{2n-m-2} r^{d(2n - m-2)}$.
    ...
    %
    \begin{equation}
        |\EE(H(\xi)) | \lesssim M^n r^d |\xi|^{-\beta/2} \lesssim M^{3/4} |\xi|^{-\beta/2} + M^{1/2}.
    \end{equation}
    %
    Thus for any $\delta > 0$, if $N_0 > 0$ there exists $N_0 > 0$ such that for $r \leq r_1$, and any nonzero $\xi \in \ZZ^d$,
    %
...
\begin{proof}
    ...
    %
    \[ \tilde{S}(x_1,\dots,x_{n-2}) = \bigcup_{0 \leq b_1,\dots,b_d < m a_2} S(x_1,\dots,x_{n-2}) + b/m \]
    %
    satisfies the same continuity and Minkowski dimension bounds that were required for $T$ in the assumptions of Theorem \ref{thirdTheorem}, and proving density arguments for $\tilde{S}$ would imply the same density results for $S$.
...
%    then $\tilde{H}(R_1,\dots,R_n)$ is an open subset of $H(R_1,\dots,R_n)$. Showing $\tilde{H}(R_1,\dots,R_n)$ is dense thus implies that $H(R_1,\dots,R_n)$ is dense. Thus we need only concentrate on the proof that $\tilde{H}(R_1,\dots,R_n)$ is dense, which is why the periodicity assumptions above are harmless.
    ...
    %
    depends only on the value of $x - a X_{n-1}(k_{n-1})$ in $\TT^d / (1/am) \cdot \ZZ^d$, and the distribution of the random variable $x - a X_{n-1}(k_{n-1})$ is independant of $x$ modulo $\TT^d / (1/am) \cdot \ZZ^d$, and is uniformly distributed in $\TT^d / (1/am) \cdot \ZZ^d$. Let $P$ denote the common quantity of the values $P(x)$ for each $x \in R_n$. A union bound shows that
    %
    \begin{equation} \label{equatojaciojwaofij231o41412414}
    \begin{split}
        P &= P(x)\\
        &= \PP \left( \bigcup_{k_1,\dots,k_{n-1}} \Big\{ d_{\mathbb{H}}(x - a X_{n-1}(k_{n-1}), T(X_1(k_1),\dots, X_{n-2}(k_{n-2}))) \leq 2 n^{1/2} \cdot (L+1) \cdot r \Big\} \right)\\
        &\leq \sum_{k_1,\dots,k_{n-1}} \PP \left( \Big\{ d_{\mathbb{H}}(x - a X_{n-1}(k_{n-1}), T(X_1(k_1),\dots, X_{n-2}(k_{n-2}))) \leq 2 n^{1/2} \cdot (L+1) \cdot r \Big\} \right) \\
    ...
    %
    \begin{align*}
        \PP & \left( \sup_{|\xi| \leq M^{1 + \kappa}} | F(\xi) - \EE(F(\xi)) | \geq C M^{1/2} \log(M)^{1/2} \right)\\
        &= \PP \left( \sup_{|\xi| \leq M^{1 + \kappa}} | F(\xi) | \geq C M^{1/2} \log(M)^{1/2} \right) \leq 1/10.
    \end{align*}
    %
    Set $N = \#(S)$. Then
    %
    ...
    \end{equation}
    %
    In particular, there exists some instantiation of the random variables $\{ X_i(k) \}$ such that if $a(X_i(k)) = A_i$, then
    %
    \begin{equation} \label{equationdIOJAWOidjawodij1243123}
        \left| \frac{1}{N} \sum_{x \in S} a(x) e^{2 \pi i \xi \cdot x} \right| \leq C N^{1/2} \log(N)^{1/2} \leq C N^{1/2} \log(N)^{1/2}.
    \end{equation}
    %
    Since $\sum_{x \in S} a(x) \gtrsim N$, \eqref{equationdIOJAWOidjawodij1243123} is an exponential sum bound we can use to apply Lemma \ref{LemmaFIOAJFOIWJ}, together with \eqref{equationDIOWAJDOIWAJIO} and the fact that $S_r \in \mathcal{H}(R_1,\dots,R_n)$, implies that $H(R_1,\dots,R_n)$ is dense in $\mathcal{X}_\beta$, completing the proof.
\end{proof}

\begin{lemma} \label{lastconcentrationbound}
    For any $\kappa > 0$, there exists $C > 0$ such that
    %
    \[ \PP \left( \sup_{|\xi| \leq M^{1 + \kappa}} | F(\xi) - \EE(F(\xi)) | \geq C M^{1/2} \log(M)^{1/2} \right) \leq 1/10 \]
\end{lemma}
\begin{proof}
    Consider the random set $\Omega$ of values $x_n \in Q_n$ such that there are $k_1,\dots,k_{n-1} \in \{ 1,\dots,M \}$ with
    %
    ...
    %
    \begin{equation}
        \Omega = \bigcup \left\{ T(X_1(k_1),\dots,X_{n-1}(k_{n-1}))_r : 1 \leq k_1,\dots,k_{n-1} \leq K \right\}.
    \end{equation}
    % K^{n-1} = r^{(n-1) \varepsilon_1/2 - d}
    we see that varying each random variable $X_i(k)$, for $1 \leq i \leq n-1$ while fixing the other random variables adjusts at most $M^{n-2}$ of the sets of dimension $\alpha$ forming $\Omega$, and thus varying $X_i(k)$ independently of the other random variables changes $\EE(H(\xi)|\Sigma)$ by at most
    %
    \begin{equation}
        O(r^{dn - }) \cdot M^{n-2} = 2^{d+1} \cdot r^d \cdot M^{n-2} \leq \frac{2^{d+1}}{M} \leq \frac{2^{d+1}}{M}.
    \end{equation}
    % K^{-(n-1)} = r^d
    % 
    Thus McDiarmid's inequality shows that for any $t \geq 0$,
