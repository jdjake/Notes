...
\begin{proof}[Proof of Lemma \ref{LemmaFIOAJFOIWJ}]
    ...
    %
    where $\delta(x_k)$ is a Dirac delta mass at $x_k$, then Property (2) implies that for $|\xi| \leq (1/r)^{1 + \kappa}$,
    %
...
%    \end{equation}
    ...
    \end{equation}
    %
    Thus in particular we conclude $y \not \in Z$. Thus $S_r \in \mathcal{B}(Z,s)$. Since $\lambda$ and $r$ were arbitrary, Lemma \ref{LemmaFIOAJFOIWJ} implies that $B(Z,s)$ is dense in $\mathcal{X}_\beta$ for each $s > 0$, completing the proof.
\end{proof}
...
\begin{proof}
    ...
    %
    then the Lipschitz condition on $f$ implies that $S_r \in \mathcal{H}(R_1,\dots,R_n)$. The remainder of the proof is on bounding exponential sums associated with $S$, so that we may apply Lemma \ref{LemmaFIOAJFOIWJ}.
    ...
    %
    We will show that with high probability, we have enough control on $|F(\xi)|$ with high probability to justify an application of Lemma \ref{LemmaFIOAJFOIWJ}. To analyze $F$, introduce the auxillary exponential sums
    %
    ...
    \end{equation}
    %
    \eqref{equationDOIJAOIDJWOIJ} is a sufficent exponential sum bound to apply Lemma \ref{LemmaFIOAJFOIWJ}, so together with \eqref{equationCOIJCOIJVIOJOIJIOJ4234141} and the fact that $S_r \in \mathcal{H}(R_1,\dots,R_n)$ we obtain that $H(R_1,\dots,R_n)$ is dense in $\mathcal{X}_\beta$, completing the proof.
\end{proof}
...
\begin{proof}
    ...
    %
    then $\tilde{H}(W;R_1,\dots,R_n)$ is an open subset of $H(W;R_1,\dots,R_n)$. Showing $\tilde{H}(W;R_1,\dots,R_n)$ is dense thus implies that $H(W;R_1,\dots,R_n)$ is dense. Thus we need only concentrate on the proof that $\tilde{H}(W;R_1,\dots,R_n)$ is dense, hence why the periodicity assumptions above are harmless.

    Fix a large integer $N > 0$, Since $T$ is a Lipschitz map, we may fix $L > 0$ such that for $x_1,x_2 \in R_1 \times \dots \times R_{n-2}$,
    %
    \[ d_{\mathbb{H}}(T(x_1), T(x_2)) \leq L |x_1 - x_2|. \]
    %
    For $1 \leq i \leq n$, consider a family of independent random variables $\{ X_i(k): 1 \leq k \leq N \}$ uniformly distributed on $R_i$, as well as another independant family of random variables $\{ X_0(k) : 1 \leq k \leq N \}$ uniformly distributed on $\TT^d - (R_1 \cup \dots \cup R_n)$. Let $I$ be the set of indices $k_n \in \{ 1, \dots, N \}$ such that there are indices $k_1,\dots,k_{n-1} \in \{ 1, \dots, N \}$ with the property that
    %
    \begin{equation}
    ...
    \[ S = \{ X_i(k) : 0 \leq i \leq n-1, 1 \leq k \leq N \} \cup \{ X_n(k): k \not \in I \}. \]
    %
    then the Lipschitz condition on $T$ implies that $S_r \in \mathcal{H}(W;R_1,\dots,R_n)$. Lemma \ref{lastconcentrationbound}, which follows from a very similar argument to Lemma \ref{lemma24901401921209} in the last section, .
    %
    \[ s \]
\end{proof}
